...
module Java::Io
  ...
  class FileDescriptor 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:In) { standard_stream(0) }
      ...
      const_set_lazy(:Out) { standard_stream(1) }
      ...
      const_set_lazy(:Err) { standard_stream(2) }
    ...
    def valid
      return ((!(@handle).equal?(-1)) || (!(@fd).equal?(-1)))
    end
    ...
    class_module.module_eval {
      ...
      JNI.native_method :Java_java_io_FileDescriptor_set, [:pointer, :long, :int32], :int64
      typesig { [::Java::Int] }
      def set(d)
        JNI.__send__(:Java_java_io_FileDescriptor_set, JNI.env, self.jni_id, d.to_int)
      end
      
      typesig { [::Java::Int] }
      def standard_stream(fd)
        desc = FileDescriptor.new
        desc.attr_handle = set(fd)
        return desc
      end
    }
