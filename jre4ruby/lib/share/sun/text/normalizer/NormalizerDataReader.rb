...
module Sun::Text::Normalizer
  module NormalizerDataReaderImports #:nodoc:
  ...
  class NormalizerDataReader 
    include_class_members NormalizerDataReaderImports
    ...
    typesig { [Array.typed(::Java::Byte), Array.typed(::Java::Byte), Array.typed(::Java::Byte), Array.typed(::Java::Char), Array.typed(::Java::Char)] }
    ...
    # @draft 2.1
    def read(norm_bytes, fcd_bytes, aux_bytes, extra_data, combining_table)
      # Read the bytes that make up the normTrie
      @data_input_stream.read(norm_bytes)
      # normTrieStream= new ByteArrayInputStream(normBytes);
      # Read the extra data
      i = 0
      while i < extra_data.attr_length
        extra_data[i] = @data_input_stream.read_char
        i += 1
      end
      # Read the combining class table
      i_ = 0
      while i_ < combining_table.attr_length
        combining_table[i_] = @data_input_stream.read_char
        i_ += 1
      end
      # Read the fcdTrie
      @data_input_stream.read(fcd_bytes)
      # Read the AuxTrie
      @data_input_stream.read(aux_bytes)
    end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:DATA_FORMAT_ID) { Array.typed(::Java::Byte).new([0x4e, 0x6f, 0x72, 0x6d]) }
      const_attr_reader  :DATA_FORMAT_ID
      
      const_set_lazy(:DATA_FORMAT_VERSION) { Array.typed(::Java::Byte).new([0x2, 0x2, 0x5, 0x2]) }
      const_attr_reader  :DATA_FORMAT_VERSION
    }
    
    private
    alias_method :initialize__normalizer_data_reader, :initialize
  end
