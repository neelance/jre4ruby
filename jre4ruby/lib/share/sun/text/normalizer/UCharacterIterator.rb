...
module Sun::Text::Normalizer
  module UCharacterIteratorImports #:nodoc:
  ...
  class UCharacterIterator 
    include_class_members UCharacterIteratorImports
    include Cloneable
    ...
    class_module.module_eval {
      ...
      def get_instance(source)
        return ReplaceableUCharacterIterator.new(source)
      end
      ...
      # @stable ICU 2.4
      def get_instance(source)
        return CharacterIteratorWrapper.new(source)
      end
    }
    ...
    def next_code_point
      ch1 = next_
      if (UTF16.is_lead_surrogate(RJava.cast_to_char(ch1)))
        ch2 = next_
        if (UTF16.is_trail_surrogate(RJava.cast_to_char(ch2)))
          return UCharacterProperty.get_raw_supplementary(RJava.cast_to_char(ch1), RJava.cast_to_char(ch2))
        else
          if (!(ch2).equal?(DONE))
    ...
    private
    alias_method :initialize__ucharacter_iterator, :initialize
  end
