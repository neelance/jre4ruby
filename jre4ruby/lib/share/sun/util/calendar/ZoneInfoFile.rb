...
module Sun::Util::Calendar
  module ZoneInfoFileImports #:nodoc:
    ...
    class_module.module_eval {
      ...
      def create_zone_info(id)
        ...
        if (buf[((index += 1) - 1)] > JAVAZI_VERSION)
          System.err.println("ZoneInfo: incompatible version (" + RJava.cast_to_string(buf[index - 1]) + "): " + id)
          return nil
        end
        ...
        begin
          while (index < filesize)
            ...
            when TAG_GMTOffsetWillChange
              ...
              System.err.println("ZoneInfo: unknown tag < " + RJava.cast_to_string(tag) + ">. ignored.")
              index += len
            end
          end
      ...
      def get_zone_info_mappings
        ...
        if (data[((index += 1) - 1)] > JAVAZM_VERSION)
          System.err.println("ZoneInfo: incompatible version (" + RJava.cast_to_string(data[index - 1]) + "): " + JAVAZM_FILE_NAME)
          return nil
        end
      ...
      def read_zone_info_file(file_name)
        ...
        begin
          ...
          if (!(ex.is_a?(FileNotFoundException)) || (JAVAZM_FILE_NAME == file_name))
            System.err.println("ZoneInfo: " + RJava.cast_to_string(ex.get_message))
          end
        end
        return buffer
      end
