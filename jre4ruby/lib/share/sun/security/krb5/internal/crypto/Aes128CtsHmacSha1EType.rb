...
module Sun::Security::Krb5::Internal::Crypto
  module Aes128CtsHmacSha1ETypeImports #:nodoc:
  ...
  class Aes128CtsHmacSha1EType < Aes128CtsHmacSha1ETypeImports.const_get :EType
    include_class_members Aes128CtsHmacSha1ETypeImports
    ...
    def encrypt(data, key, ivec, usage)
      begin
        return Aes128.encrypt(key, usage, ivec, data, 0, data.attr_length)
    ...
    def decrypt(cipher, key, ivec, usage)
      begin
        return Aes128.decrypt(key, usage, ivec, cipher, 0, cipher.attr_length)
      rescue GeneralSecurityException => e
        ke = KrbCryptoException.new(e.get_message)
        ke.init_cause(e)
        raise ke
    ...
    private
    alias_method :initialize__aes128cts_hmac_sha1etype, :initialize
  end
