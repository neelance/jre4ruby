...
module Sun::Security::Krb5::Internal::Crypto
  module HmacSha1Aes256CksumTypeImports #:nodoc:
  ...
  class HmacSha1Aes256CksumType < HmacSha1Aes256CksumTypeImports.const_get :CksumType
    include_class_members HmacSha1Aes256CksumTypeImports
    ...
    def calculate_keyed_checksum(data, size, key, usage)
      begin
        return Aes256.calculate_checksum(key, usage, data, 0, size)
    ...
    def verify_keyed_checksum(data, size, key, checksum, usage)
      begin
        new_cksum = Aes256.calculate_checksum(key, usage, data, 0, size)
    ...
    private
    alias_method :initialize__hmac_sha1aes256cksum_type, :initialize
  end
