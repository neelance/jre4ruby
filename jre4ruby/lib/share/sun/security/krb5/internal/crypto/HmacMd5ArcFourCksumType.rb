...
module Sun::Security::Krb5::Internal::Crypto
  module HmacMd5ArcFourCksumTypeImports #:nodoc:
  ...
  class HmacMd5ArcFourCksumType < HmacMd5ArcFourCksumTypeImports.const_get :CksumType
    include_class_members HmacMd5ArcFourCksumTypeImports
    ...
    def calculate_keyed_checksum(data, size, key, usage)
      begin
        return ArcFourHmac.calculate_checksum(key, usage, data, 0, size)
    ...
    def verify_keyed_checksum(data, size, key, checksum, usage)
      begin
        new_cksum = ArcFourHmac.calculate_checksum(key, usage, data, 0, size)
    ...
    private
    alias_method :initialize__hmac_md5arc_four_cksum_type, :initialize
  end
