...
module Sun::Security::Krb5::Internal::Ktab
  ...
  class KeyTab 
    ...
    class_module.module_eval {
      typesig { [String] }
      def get_instance(s)
        self.attr_name = RJava.cast_to_string(parse(s))
      ...
      def get_instance(file)
        begin
          ...
          self.attr_singleton = nil
          if (DEBUG)
            System.out.println("Could not obtain an instance of KeyTab" + RJava.cast_to_string(e.get_message))
          end
        end
          ...
          begin
            ...
            if (!(keytab_names).nil?)
              ...
              while (st.has_more_tokens)
                kname = RJava.cast_to_string(parse(st.next_token))
                if (!(kname).nil?)
                  break
                end
              end
            ...
            kname = RJava.cast_to_string(nil)
          end
          if ((kname).nil?)
            user_home = Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("user.home"))
            if ((user_home).nil?)
              user_home = RJava.cast_to_string(Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("user.dir")))
            end
            if (!(user_home).nil?)
              kname = user_home + RJava.cast_to_string(JavaFile.attr_separator) + "krb5.keytab"
            end
          end
          return kname
        end
    ...
    def load(kis)
      ...
      while (kis.available > 0)
        ...
        if (DEBUG)
          System.out.println(">>> KeyTab: load() entry length: " + RJava.cast_to_string(entry_length) + "; type: " + RJava.cast_to_string((!(entry).nil? ? entry.attr_key_type : 0)))
        end
        if (!(entry).nil?)
        ...
        while i >= 0
          ...
          if (entry.attr_service.match(service))
            if (EType.is_supported(entry.attr_key_type))
              ...
              if (DEBUG)
                System.out.println("Found unsupported keytype (" + RJava.cast_to_string(entry.attr_key_type) + ") for " + RJava.cast_to_string(service))
              end
            end
          end
          i -= 1
    ...
    def read_service_keys(service)
        ...
        while i >= 0
          ...
          if (entry.attr_service.match(service))
            if (EType.is_supported(entry.attr_key_type))
              ...
              keys.add(key)
              if (DEBUG)
                System.out.println("Added key: " + RJava.cast_to_string(entry.attr_key_type) + "version: " + RJava.cast_to_string(entry.attr_key_version))
              end
            else
              if (DEBUG)
                System.out.println("Found unsupported keytype (" + RJava.cast_to_string(entry.attr_key_type) + ") for " + RJava.cast_to_string(service))
        ...
        while j < etypes.attr_length && pos < size_
          ...
          while i < size_ && pos < size_
            ...
            if (!(k).nil? && (k.get_etype).equal?(target))
              if (DEBUG)
                System.out.println(RJava.cast_to_string(pos) + ": " + RJava.cast_to_string(k))
              end
              ret_val[((pos += 1) - 1)] = k
      ...
      if (!(pos).equal?(size_))
        raise RuntimeException.new("Internal Error: did not copy all keys;expecting " + RJava.cast_to_string(size_) + "; got " + RJava.cast_to_string(pos))
      end
      return ret_val
    end
