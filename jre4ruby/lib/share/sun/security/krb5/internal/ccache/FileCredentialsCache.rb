  ...
  class FileCredentialsCache < FileCredentialsCacheImports.const_get :CredentialsCache
    ...
    class_module.module_eval {
      ...
      def get_default_cache_name
        ...
        if (!(osname).nil?)
          ...
          if (osname.starts_with("SunOS") || (osname.starts_with("Linux")))
            begin
              ...
              name = RJava.cast_to_string(JavaFile.attr_separator) + "tmp" + RJava.cast_to_string(JavaFile.attr_separator) + std_cache_name_component + "_" + RJava.cast_to_string(uid)
              if (self.attr_debug)
        ...
        if (!(user_name).nil?)
          name = user_home + RJava.cast_to_string(JavaFile.attr_separator) + std_cache_name_component + "_" + user_name
        else
          name = user_home + RJava.cast_to_string(JavaFile.attr_separator) + std_cache_name_component
        end
        if (self.attr_debug)
          System.out.println(">>>KinitOptions cache name is " + name)
        end
      ...
      def check_validation(name)
        ...
        begin
          # get full path name
          fullname = RJava.cast_to_string((JavaFile.new(name)).get_canonical_path)
          ...
          if (!(f_check.exists))
            ...
            if (!(temp.is_directory))
              fullname = RJava.cast_to_string(nil)
            end
            temp = nil
          end
          f_check = nil
        rescue IOException => e
          fullname = RJava.cast_to_string(nil) # invalid name
        end
        return fullname
      end
      ...
      def exec(c)
        ...
        begin
          ...
          if (((command.attr_length).equal?(1)) && ((command[0] == "/usr/bin/env")))
            while (!((s1 = RJava.cast_to_string(command_result.read_line))).nil?)
              if (s1.length >= 11)
                if ((s1.substring(0, 11)).equals_ignore_case("KRB5CCNAME="))
                  s1 = RJava.cast_to_string(s1.substring(11))
                  break
                end
              end
            end
          else
            s1 = RJava.cast_to_string(command_result.read_line)
          end
