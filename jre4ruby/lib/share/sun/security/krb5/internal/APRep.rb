...
module Sun::Security::Krb5::Internal
  ...
  class APRep 
    include_class_members APRepImports
    ...
    def init(encoding)
      ...
      if (!(@msg_type).equal?(Krb5::KRB_AP_REP))
        raise KrbApErrException.new(Krb5::KRB_AP_ERR_MSG_TYPE)
      end
      @enc_part = EncryptedData.parse(der.get_data, 0x2, false)
    ...
    def asn1_encode
      ...
      bytes.write(DerValue.create_tag(DerValue::TAG_CONTEXT, true, 0x2), @enc_part.asn1_encode)
      temp = DerOutputStream.new
      temp.write(DerValue.attr_tag_sequence, bytes)
      aprep = DerOutputStream.new
      aprep.write(DerValue.create_tag(DerValue::TAG_APPLICATION, true, 0xf), temp)
      return aprep.to_byte_array
    end
    
    private
    alias_method :initialize__aprep, :initialize
  end
