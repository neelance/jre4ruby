...
module Sun::Security::Krb5::Internal
  ...
  class TCPClient 
    ...
    def receive
      ...
      if (Krb5::DEBUG)
        System.out.println(">>>DEBUG: TCPClient reading " + RJava.cast_to_string(len) + " bytes")
      end
      if (len <= 0)
        if (Krb5::DEBUG)
          System.out.println(">>>DEBUG: TCPClient zero or negative length field: " + RJava.cast_to_string(len))
        end
        return nil
      end
      ...
      if (!(count).equal?(len))
        if (Krb5::DEBUG)
          System.out.println(">>>DEBUG: TCPClient could not read complete packet (" + RJava.cast_to_string(len) + "/" + RJava.cast_to_string(count) + ")")
        end
        return nil
      else
        return data
