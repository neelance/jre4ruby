...
module Sun::Security::Krb5::Internal
  module CredentialsUtilImports #:nodoc:
      ...
      def acquire_service_creds(service, ccreds)
        ...
        if ((local_realm).nil?)
          ...
          if (!((temp = ccreds.get_server)).nil?)
            local_realm = RJava.cast_to_string(temp.get_realm_string)
          end
        end
        if ((local_realm).nil?)
        ...
        while i < realms.attr_length
          ...
          if (self.attr_debug)
            System.out.println(">>> Credentials acquireServiceCreds: main loop: [" + RJava.cast_to_string(i) + "] tempService=" + RJava.cast_to_string(temp_service))
          end
          begin
          ...
          if ((new_tgt).nil?)
            ...
            while (new_tgt).nil? && k > i
              ...
              if (self.attr_debug)
                System.out.println(">>> Credentials acquireServiceCreds: inner loop: [" + RJava.cast_to_string(k) + "] tempService=" + RJava.cast_to_string(temp_service))
              end
              begin
                new_tgt = service_creds(temp_service, c_tgt)
              rescue JavaException => exc
          ...
          new_tgt_realm = RJava.cast_to_string(new_tgt.get_server.get_instance_component)
          if (self.attr_debug)
          ...
          if (k < realms.attr_length)
            ...
            c_tgt = new_tgt
            if (self.attr_debug)
              System.out.println(">>> Credentials acquireServiceCreds: continuing with main loop counter reset to " + RJava.cast_to_string(i))
            end
            next
          else
        ...
        if (!(the_tgt).nil?)
          ...
          if (self.attr_debug)
            ...
            # printDebug(theTgt);
            System.out.println(">>> Credentials acquireServiceCreds: obtaining service creds for " + RJava.cast_to_string(sname))
          end
          begin
