...
module Sun::Security::Krb5::Internal
  ...
  class KRBError 
    ...
    def parse_edata(data)
      ...
      if ((@error_code).equal?(Krb5::KDC_ERR_PREAUTH_REQUIRED) || (@error_code).equal?(Krb5::KDC_ERR_PREAUTH_FAILED))
        begin
          ...
          if (self.attr_debug)
            System.out.println("Unable to parse eData field of KRB-ERROR:\n" + RJava.cast_to_string(Sun::Misc::HexDumpEncoder.new.encode_buffer(data)))
          end
    ...
    def parse_padata(data)
      ...
      while (der_pa.attr_data.available > 0)
        ...
        if (self.attr_debug)
          ...
          System.out.println("\t PA-DATA type = " + RJava.cast_to_string(pa_type))
        end
        case (pa_type)
        ...
        when Krb5::PA_ETYPE_INFO
          if (!(pa_value).nil?)
            ...
            @salt = info.get_salt
            if (self.attr_debug)
              System.out.println("\t PA-ETYPE-INFO etype = " + RJava.cast_to_string(@etype))
            end
          end
        when Krb5::PA_ETYPE_INFO2
          if (!(pa_value).nil?)
            ...
            if (self.attr_debug)
              System.out.println("\t PA-ETYPE-INFO2 etype = " + RJava.cast_to_string(@etype))
            end
          end
        else
    ...
    def init(encoding)
      ...
      @e_text = RJava.cast_to_string(nil)
      @e_data = nil
      @e_cksum = nil
      if (der.get_data.available > 0)
        if (((der.get_data.peek_byte & 0x1f)).equal?(0xb))
          sub_der = der.get_data.get_der_value
          @e_text = RJava.cast_to_string(sub_der.get_data.get_general_string)
        end
      end
      if (der.get_data.available > 0)
    ...
    def show_debug
      if (self.attr_debug)
        ...
        if (!(@c_time).nil?)
          System.out.println("\t cTime is " + RJava.cast_to_string(@c_time.to_date.to_s) + " " + RJava.cast_to_string(@c_time.to_date.get_time))
        end
        if (!(@cu_sec).nil?)
          System.out.println("\t cuSec is " + RJava.cast_to_string(@cu_sec.int_value))
        end
        System.out.println("\t sTime is " + RJava.cast_to_string(@s_time.to_date.to_s) + " " + RJava.cast_to_string(@s_time.to_date.get_time))
        System.out.println("\t suSec is " + RJava.cast_to_string(@su_sec))
        System.out.println("\t error code is " + RJava.cast_to_string(@error_code))
        System.out.println("\t error Message is " + RJava.cast_to_string(Krb5.get_error_message(@error_code)))
        if (!(@crealm).nil?)
          System.out.println("\t crealm is " + RJava.cast_to_string(@crealm.to_s))
        end
        if (!(@cname).nil?)
          System.out.println("\t cname is " + RJava.cast_to_string(@cname.to_s))
        end
        if (!(@realm).nil?)
          System.out.println("\t realm is " + RJava.cast_to_string(@realm.to_s))
        end
        if (!(@sname).nil?)
          System.out.println("\t sname is " + RJava.cast_to_string(@sname.to_s))
        end
        if (!(@e_data).nil?)
        ...
        System.out.println("\t msgType is " + RJava.cast_to_string(@msg_type))
      end
    end
