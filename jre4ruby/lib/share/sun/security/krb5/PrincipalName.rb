...
module Sun::Security::Krb5
  module PrincipalNameImports #:nodoc:
        ...
        while (i < temp.length)
          if ((temp.char_at(i)).equal?(NAME_COMPONENT_SEPARATOR))
            ...
            if (i > 0 && (temp.char_at(i - 1)).equal?(Character.new(?\\.ord)))
              temp = RJava.cast_to_string(temp.substring(0, i - 1) + temp.substring(i, temp.length))
              next
            else
              if (component_start < i)
                component = RJava.cast_to_string(temp.substring(component_start, i))
    ...
    def initialize(name, type)
      ...
      if ((realm_string).nil?)
        begin
          ...
          realm_string = RJava.cast_to_string(config.get_default_realm)
    ...
    class_module.module_eval {
      ...
      def map_host_to_realm(name)
        ...
        begin
          ...
          if (!((result = RJava.cast_to_string(c.get_default(name, "domain_realm")))).nil?)
            return result
          else
            i = 1
            while i < name.length
              if (((name.char_at(i)).equal?(Character.new(?..ord))) && (!(i).equal?(name.length - 1)))
                ...
                subname = RJava.cast_to_string(name.substring(i))
                result = RJava.cast_to_string(c.get_default(subname, "domain_realm"))
                if (!(result).nil?)
                  break
                else
                  subname = RJava.cast_to_string(name.substring(i + 1)) # or mapping could be ibm.com = AUSTIN.IBM.COM
                  result = RJava.cast_to_string(c.get_default(subname, "domain_realm"))
                  if (!(result).nil?)
                    break
                  end
