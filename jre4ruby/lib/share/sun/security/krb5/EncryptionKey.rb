...
module Sun::Security::Krb5
  module EncryptionKeyImports #:nodoc:
  ...
  class EncryptionKey 
    ...
    class_module.module_eval {
      ...
      def acquire_secret_keys(password, salt, pa_exists, pa_etype, pa_s2kparams)
        ...
        if ((pa_exists) && (!(pa_etype).equal?(EncryptedData::ETYPE_NULL)))
          if (DEBUG)
            System.out.println("Pre-Authentication: " + "Set preferred etype = " + RJava.cast_to_string(pa_etype))
          end
        ...
        while i < etypes.attr_length
          if (EType.is_supported(etypes[i]))
            ...
            if (DEBUG)
              System.out.println("Encryption Type " + RJava.cast_to_string(EType.to_s(etypes[i])) + " is not supported/enabled")
            end
          end
          i += 1
        end
      ...
      def string_to_key(password, salt, s2kparams, key_type)
        ...
        begin
          ...
          when EncryptedData::ETYPE_AES256_CTS_HMAC_SHA1_96
            ...
            raise IllegalArgumentException.new("encryption type " + RJava.cast_to_string(EType.to_s(key_type)) + " not supported")
          end
    ...
    typesig { [] }
    def to_s
      return String.new("EncryptionKey: keyType=" + RJava.cast_to_string(@key_type) + " kvno=" + RJava.cast_to_string(@kvno) + " keyValue (hex dump)=" + RJava.cast_to_string(((@key_value).nil? || (@key_value.attr_length).equal?(0) ? " Empty Key" : Character.new(?\n.ord) + Krb5.attr_hex_dumper.encode(@key_value) + Character.new(?\n.ord))))
    end
    ...
    class_module.module_eval {
      ...
      def find_key(etype, keys)
        ...
        if (!EType.is_supported(etype))
          raise KrbException.new("Encryption type " + RJava.cast_to_string(EType.to_s(etype)) + " is not supported/enabled")
        end
        ktype = 0
        i = 0
