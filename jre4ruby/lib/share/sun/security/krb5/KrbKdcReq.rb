...
module Sun::Security::Krb5
  ...
  class KrbKdcReq 
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        ...
        begin
          ...
          temp = RJava.cast_to_string(cfg.get_default("udp_preference_limit", "libdefaults"))
          ...
          if (DEBUG)
            System.out.println("Exception in getting kdc_timeout value, " + "using default value " + RJava.cast_to_string(exc.get_message))
          end
        end
        if (timeout > 0)
    ...
    def send(realm, use_tcp)
      ...
      cfg = Config.get_instance
      if ((realm).nil?)
        realm = RJava.cast_to_string(cfg.get_default_realm)
        if ((realm).nil?)
      ...
      while (st.has_more_tokens)
        temp_kdc = RJava.cast_to_string(st.next_token)
        begin
    ...
    def send(realm, temp_kdc, use_tcp)
      ...
      if (DEBUG)
        System.out.println(">>> KrbKdcReq send: kdc=" + kdc + RJava.cast_to_string((use_tcp ? " TCP:" : " UDP:")) + RJava.cast_to_string(port) + ", timeout=" + RJava.cast_to_string(timeout) + ", number of retries =" + RJava.cast_to_string(DEFAULT_KDC_RETRY_LIMIT) + ", #bytes=" + RJava.cast_to_string(@obuf.attr_length))
      end
      ...
      begin
        ...
        if (DEBUG)
          System.out.println(">>> KrbKdcReq send: #bytes read=" + RJava.cast_to_string((!(@ibuf).nil? ? @ibuf.attr_length : 0)))
        end
