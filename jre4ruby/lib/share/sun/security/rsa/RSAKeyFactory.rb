...
module Sun::Security::Rsa
  module RSAKeyFactoryImports #:nodoc:
  ...
  class RSAKeyFactory < RSAKeyFactoryImports.const_get :KeyFactorySpi
    ...
    class_module.module_eval {
      const_set_lazy(:RsaPublicKeySpecClass) { RSAPublicKeySpec }
      const_attr_reader  :RsaPublicKeySpecClass
      
      const_set_lazy(:RsaPrivateKeySpecClass) { RSAPrivateKeySpec }
      const_attr_reader  :RsaPrivateKeySpecClass
      
      const_set_lazy(:RsaPrivateCrtKeySpecClass) { RSAPrivateCrtKeySpec }
      const_attr_reader  :RsaPrivateCrtKeySpecClass
      
      const_set_lazy(:X509KeySpecClass) { X509EncodedKeySpec }
      const_attr_reader  :X509KeySpecClass
      
      const_set_lazy(:Pkcs8KeySpecClass) { PKCS8EncodedKeySpec }
    ...
    class_module.module_eval {
      ...
      def check_key_lengths(modulus_len, exponent, min_modulus_len, max_modulus_len)
        if ((min_modulus_len > 0) && (modulus_len < (min_modulus_len)))
          raise InvalidKeyException.new("RSA keys must be at least " + RJava.cast_to_string(min_modulus_len) + " bits long")
        end
        ...
        if (modulus_len > max_len)
          raise InvalidKeyException.new("RSA keys must be no longer than " + RJava.cast_to_string(max_len) + " bits")
        end
        # If a RSAPublicKey, make sure the exponent isn't too big.
        if (RestrictExpLen && (!(exponent).nil?) && (modulus_len > MAX_MODLEN_RESTRICT_EXP) && (exponent.bit_length > MAX_RESTRICTED_EXPLEN))
          raise InvalidKeyException.new("RSA exponents can be no longer than " + RJava.cast_to_string(MAX_RESTRICTED_EXPLEN) + " bits " + " if modulus is greater than " + RJava.cast_to_string(MAX_MODLEN_RESTRICT_EXP) + " bits")
        end
      end
    }
