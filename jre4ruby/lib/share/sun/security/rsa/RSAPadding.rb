...
module Sun::Security::Rsa
  module RSAPaddingImports #:nodoc:
  ...
  class RSAPadding 
    ...
    def initialize(type, padded_size, random, spec)
      ...
      when PAD_OAEP_MGF1
        ...
        begin
          if (!(spec).nil?)
            md_name = RJava.cast_to_string(spec.get_digest_algorithm)
            ...
            mgf_md_name = RJava.cast_to_string((spec.get_mgfparameters).get_digest_algorithm)
        ...
        raise InvalidKeyException.new("Invalid padding: " + RJava.cast_to_string(type))
      end
    end
    ...
    def pad(data)
      if (data.attr_length > @max_data_size)
        raise BadPaddingException.new("Data must be shorter than " + RJava.cast_to_string((@max_data_size + 1)) + " bytes")
      end
      case (@type)
      when PAD_NONE
        return data
    ...
    def unpad(padded)
      if (!(padded.attr_length).equal?(@padded_size))
        raise BadPaddingException.new("Padded length must be " + RJava.cast_to_string(@padded_size))
      end
      case (@type)
      when PAD_NONE
        return padded
    ...
    def unpad_v15(padded)
      ...
      if (!(padded[((k += 1) - 1)]).equal?(@type))
        raise BadPaddingException.new("Blocktype mismatch: " + RJava.cast_to_string(padded[1]))
      end
      while (true)
        b = padded[((k += 1) - 1)] & 0xff
        if ((b).equal?(0))
        ...
        if (((@type).equal?(PAD_BLOCKTYPE_1)) && (!(b).equal?(0xff)))
          raise BadPaddingException.new("Padding byte not 0xff: " + RJava.cast_to_string(b))
        end
      end
