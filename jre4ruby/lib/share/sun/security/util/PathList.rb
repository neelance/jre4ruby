    ...
    class_module.module_eval {
      ...
      def append_path(path_to, path_from)
        if ((path_to).nil? || (path_to.length).equal?(0))
          ...
          if ((path_from).nil? || (path_from.length).equal?(0))
            ...
            return path_to + RJava.cast_to_string(JavaFile.attr_path_separator) + path_from
          end
        end
      end
      ...
      def file_to_url(file)
        ...
        begin
          ...
          name = RJava.cast_to_string(file.get_absolute_path)
        end
        name = RJava.cast_to_string(name.replace(JavaFile.attr_separator_char, Character.new(?/.ord)))
        if (!name.starts_with("/"))
          name = "/" + name
        end
