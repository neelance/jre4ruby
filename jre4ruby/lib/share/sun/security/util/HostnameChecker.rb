...
module Sun::Security::Util
  module HostnameCheckerImports #:nodoc:
  ...
  class HostnameChecker 
    ...
    class_module.module_eval {
      ...
      def get_instance(check_type)
        ...
        raise IllegalArgumentException.new("Unknown check type: " + RJava.cast_to_string(check_type))
      end
    }
      ...
      def get_server_name(principal)
        ...
        begin
          ...
          if (name_parts.attr_length >= 2)
            host_name = RJava.cast_to_string(name_parts[1])
          end
    ...
    class_module.module_eval {
      ...
      def match_all_wildcards(name, template)
        name = RJava.cast_to_string(name.to_lower_case)
        template = RJava.cast_to_string(template.to_lower_case)
      ...
      def match_wild_cards(name, template)
        ...
        while (!(wildcard_idx).equal?(-1))
          ...
          before_wildcard = RJava.cast_to_string(after_wildcard.substring(0, wildcard_idx))
          after_wildcard = RJava.cast_to_string(after_wildcard.substring(wildcard_idx + 1))
          ...
          name = RJava.cast_to_string(name.substring(before_start_idx + before_wildcard.length))
