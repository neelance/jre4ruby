  ...
  class Debug 
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        self.attr_args = RJava.cast_to_string(Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("java.security.debug")))
        ...
        if (!(self.attr_args).nil?)
          self.attr_args = RJava.cast_to_string(marshal(self.attr_args))
    ...
    class_module.module_eval {
      ...
      def to_hex_string(b)
        ...
        if (hex_value.starts_with("-"))
          buf.append("   -")
          hex_value = RJava.cast_to_string(hex_value.substring(1))
        else
