...
module Sun::Security::Util
  module ObjectIdentifierImports #:nodoc:
  ...
  class ObjectIdentifier 
    ...
    def initialize(oid)
      ...
      begin
        while (!((end_ = oid.index_of(ch, start))).equal?(-1))
          comp = RJava.cast_to_string(oid.substring(start, end_))
          @components[((i += 1) - 1)] = JavaInteger.value_of(comp).int_value
          start = end_ + 1
        end
        comp = RJava.cast_to_string(oid.substring(start))
        @components[i] = JavaInteger.value_of(comp).int_value
      rescue JavaException => e
        raise IOException.new("ObjectIdentifier() -- Invalid format: " + RJava.cast_to_string(e.to_s), e)
      end
    ...
    def check_valid_oid(values, len)
      ...
      while i < len
        if (values[i] < 0)
          raise IOException.new("ObjectIdentifier() -- " + "oid component #" + RJava.cast_to_string((i + 1)) + " must be non-negative ")
        end
        i += 1
      end
    ...
    def initialize(in_)
      ...
      if (!(type_id).equal?(DerValue.attr_tag_object_id))
        raise IOException.new("ObjectIdentifier() -- data isn't an object ID" + " (tag = " + RJava.cast_to_string(type_id) + ")")
      end
    ...
    def to_s
      ...
      if ((s).nil?)
        ...
        s = RJava.cast_to_string(sb.to_s)
        @string_form = s
      end
      return s
    end
