...
module Sun::Security::Util
  module PolicyUtilImports #:nodoc:
      ...
      def get_key_store(policy_url, key_store_name, key_store_type, key_store_provider, store_pass_url, debug)
        ...
        begin
          ...
          if ((key_store_type).nil?)
            key_store_type = RJava.cast_to_string(KeyStore.get_default_type)
          end
          ...
          if (!(store_pass_url).nil?)
            ...
            if (!(debug).nil?)
              debug.println("reading password" + RJava.cast_to_string(pass_url))
            end
            in_ = nil
            begin
          ...
          if ((NONE == key_store_name))
            ...
            if (!(debug).nil?)
              debug.println("reading keystore" + RJava.cast_to_string(key_store_url))
            end
            in_stream = nil
            begin
