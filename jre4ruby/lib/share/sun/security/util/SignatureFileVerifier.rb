...
module Sun::Security::Util
  module SignatureFileVerifierImports #:nodoc:
  ...
  class SignatureFileVerifier 
    ...
    class_module.module_eval {
      const_set_lazy(:ATTR_DIGEST) { ("-DIGEST-" + RJava.cast_to_string(ManifestDigester::MF_MAIN_ATTRS)).to_upper_case(Locale::ENGLISH) }
      ...
      mattr.entry_set.each do |se|
        ...
        if (key.to_upper_case(Locale::ENGLISH).ends_with("-DIGEST-MANIFEST"))
          ...
          if (!(digest).nil?)
            ...
            if (!(Debug).nil?)
              Debug.println("Signature File: Manifest digest " + RJava.cast_to_string(digest.get_algorithm))
              Debug.println("  sigfile  " + RJava.cast_to_string(to_hex(expected_hash)))
              Debug.println("  computed " + RJava.cast_to_string(to_hex(computed_hash)))
              Debug.println
            end
      ...
      mattr.entry_set.each do |se|
        ...
        if (key.to_upper_case(Locale::ENGLISH).ends_with(ATTR_DIGEST))
          ...
          if (!(digest).nil?)
            ...
            expected_hash = decoder.decode_buffer(se.get_value)
            if (!(Debug).nil?)
              Debug.println("Signature File: " + "Manifest Main Attributes digest " + RJava.cast_to_string(digest.get_algorithm))
              Debug.println("  sigfile  " + RJava.cast_to_string(to_hex(expected_hash)))
              Debug.println("  computed " + RJava.cast_to_string(to_hex(computed_hash)))
              Debug.println
            end
        ...
        sf_attr.entry_set.each do |se|
          ...
          if (key.to_upper_case(Locale::ENGLISH).ends_with("-DIGEST"))
            ...
            if (!(digest_).nil?)
              ...
              if (!(Debug).nil?)
                Debug.println("Signature Block File: " + name + " digest=" + RJava.cast_to_string(digest_.get_algorithm))
                Debug.println("  expected " + RJava.cast_to_string(to_hex(expected)))
                Debug.println("  computed " + RJava.cast_to_string(to_hex(computed)))
                Debug.println
              end
              if (MessageDigest.is_equal(computed, expected))
                ...
                if (!@workaround)
                  ...
                  if (MessageDigest.is_equal(computed, expected))
                    if (!(Debug).nil?)
                      Debug.println("  re-computed " + RJava.cast_to_string(to_hex(computed)))
                      Debug.println
                    end
                    @workaround = true
                    one_digest_verified = true
              ...
              if (!ok)
                raise SecurityException.new("invalid " + RJava.cast_to_string(digest_.get_algorithm) + " signature file digest for " + name)
              end
            end
          end
        end
    ...
    def get_signers(infos, block)
      ...
      while i < infos.attr_length
        ...
        if (!(Debug).nil?)
          Debug.println("Signature Block Certificate: " + RJava.cast_to_string(chain.get(0)))
        end
        i += 1
      end
      if (!(signers).nil?)
