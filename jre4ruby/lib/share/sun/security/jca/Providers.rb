      ...
      def begin_thread_provider_list(list)
        synchronized(self) do
          if (!(ProviderList.attr_debug).nil?)
            ProviderList.attr_debug.println("ThreadLocal providers: " + RJava.cast_to_string(list))
          end
      ...
      def end_thread_provider_list(list)
        synchronized(self) do
          if ((list).nil?)
            ...
            if (!(ProviderList.attr_debug).nil?)
              ProviderList.attr_debug.println("Restoring previous ThreadLocal providers: " + RJava.cast_to_string(list))
            end
            ThreadLists.set(list)
          end
          self.attr_thread_lists_used -= 1
