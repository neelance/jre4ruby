...
module Sun::Security::Jgss::Krb5
  module Krb5InitCredentialImports #:nodoc:
  ...
  class Krb5InitCredential < Krb5InitCredentialImports.const_get :KerberosTicket
    ...
    def dispose
      begin
        ...
        gss_exception = GSSException.new(GSSException::FAILURE, -1, "Could not destroy credentials - " + RJava.cast_to_string(e.get_message))
    ...
    class_module.module_eval {
      ...
      def get_tgt(caller, name, init_lifetime)
        ...
        if (!(name).nil?)
          client_principal = RJava.cast_to_string((name.get_krb5principal_name).get_name)
          realm = RJava.cast_to_string((name.get_krb5principal_name).get_realm_as_string)
        else
          client_principal = RJava.cast_to_string(nil)
          begin
            config = Config.get_instance
            realm = RJava.cast_to_string(config.get_default_realm)
          rescue KrbException => e
            ge = GSSException.new(GSSException::NO_CRED, -1, "Attempt to obtain INITIATE credentials failed!" + " (" + RJava.cast_to_string(e.get_message) + ")")
            ge.init_cause(e)
            raise ge
          end
        end
        ...
        begin
          ...
          ge = GSSException.new(GSSException::NO_CRED, -1, "Attempt to obtain new INITIATE credentials failed!" + " (" + RJava.cast_to_string(e.get_message) + ")")
