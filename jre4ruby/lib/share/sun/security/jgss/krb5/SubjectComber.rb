...
module Sun::Security::Jgss::Krb5
  module SubjectComberImports #:nodoc:
      ...
      def find_aux(subject, server_principal, client_principal, cred_class, one_only)
        if ((subject).nil?)
          ...
          if ((cred_class).equal?(KerberosKey))
              ...
              if ((server_principal).nil? || (server_principal == key.get_principal.get_name))
                ...
                if (one_only)
                  ...
                  if ((server_principal).nil?)
                    ...
                    server_principal = RJava.cast_to_string(key.get_principal.get_name)
                  end
                  answer.add(key)
                end
              end
            ...
            if ((cred_class).equal?(KerberosTicket))
              ...
              synchronized((pcs)) do
                ...
                while (iterator_.has_next)
                  ...
                  if (obj.is_a?(KerberosTicket))
                    ticket = obj
                    if (DEBUG)
                      System.out.println("Found ticket for " + RJava.cast_to_string(ticket.get_client) + " to go to " + RJava.cast_to_string(ticket.get_server) + " expiring on " + RJava.cast_to_string(ticket.get_end_time))
                    end
                    if (!ticket.is_current)
                      ...
                      if (!subject.is_read_only)
                        ...
                        begin
                          ticket.destroy
                          if (DEBUG)
                            System.out.println("Removed and destroyed " + "the expired Ticket \n" + RJava.cast_to_string(ticket))
                          end
                        rescue DestroyFailedException => dfe
                          if (DEBUG)
                            System.out.println("Expired ticket not" + " detroyed successfully. " + RJava.cast_to_string(dfe))
                          end
                        end
                      end
                    else
                        ...
                        if ((client_principal).nil? || (client_principal == ticket.get_client.get_name))
                          if (one_only)
                            ...
                            if ((client_principal).nil?)
                              client_principal = RJava.cast_to_string(ticket.get_client.get_name)
                            end
                            if ((server_principal).nil?)
                              server_principal = RJava.cast_to_string(ticket.get_server.get_name)
                            end
                            answer.add(ticket)
                          end
                        end
