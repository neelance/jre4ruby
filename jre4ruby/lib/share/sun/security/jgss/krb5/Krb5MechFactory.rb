...
module Sun::Security::Jgss::Krb5
  module Krb5MechFactoryImports #:nodoc:
  ...
  class Krb5MechFactory 
    ...
    class_module.module_eval {
      ...
      def get_cred_from_subject(name, initiate)
        creds = GSSUtil.search_subject(name, GSS_KRB5_MECH_OID, initiate, (initiate ? Krb5InitCredential : Krb5AcceptCredential))
    ...
    class_module.module_eval {
      ...
      def check_init_cred_permission(name)
        ...
        if (!(sm).nil?)
          ...
          tgs_principal = String.new("krbtgt/" + realm + RJava.cast_to_string(Character.new(?@.ord)) + realm)
          perm = ServicePermission.new(tgs_principal, "initiate")
          begin
            sm.check_permission(perm)
          rescue SecurityException => e
            if (DEBUG)
              System.out.println("Permission to initiate" + "kerberos init credential" + RJava.cast_to_string(e.get_message))
            end
            raise e
          end
        end
