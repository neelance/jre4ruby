...
module Sun::Security::Jgss::Spnego
  module SpNegoContextImports #:nodoc:
  ...
  class SpNegoContext 
    ...
    def init_sec_context(is, mech_token_size)
      ...
      if (DEBUG)
        System.out.println("Entered SpNego.initSecContext with " + "state=" + RJava.cast_to_string(print_state(@state)))
      end
      if (!is_initiator)
      ...
      begin
        if ((@state).equal?(STATE_NEW))
          ...
          if (DEBUG)
            System.out.println("SpNegoContext.initSecContext: " + "sending token of type = " + RJava.cast_to_string(SpNegoToken.get_token_name(init_token.get_type)))
          end
          ...
          if ((@state).equal?(STATE_IN_PROCESS))
            ...
            if (DEBUG)
              System.out.println("SpNegoContext.initSecContext: " + "process received token = " + RJava.cast_to_string(SpNegoToken.get_hex_bytes(server_token)))
            end
            # read the SPNEGO token
            # token will be validated when parsing
            targ_token = NegTokenTarg.new(server_token)
            if (DEBUG)
              System.out.println("SpNegoContext.initSecContext: " + "received token of type = " + RJava.cast_to_string(SpNegoToken.get_token_name(targ_token.get_type)))
            end
            # pull out mechanism
            ...
            if (((nego_result).equal?(SpNegoToken::NegoResult::ACCEPT_COMPLETE)) || ((nego_result).equal?(SpNegoToken::NegoResult::ACCEPT_INCOMPLETE)))
              ...
              if (is_mech_context_established)
                ...
                ret_val = mech_token
                if (DEBUG)
                  System.out.println("SPNEGO Negotiated Mechanism = " + RJava.cast_to_string(@internal_mech) + " " + RJava.cast_to_string(GSSUtil.get_mech_str(@internal_mech)))
                end
              else
                # generate SPNEGO token
                init_token = NegTokenInit.new(nil, nil, mech_token, nil)
                if (DEBUG)
                  System.out.println("SpNegoContext.initSecContext:" + " continue sending token of type = " + RJava.cast_to_string(SpNegoToken.get_token_name(init_token.get_type)))
                end
                # get the encoded token
                ret_val = init_token.get_encoded
              end
        ...
        if (DEBUG)
          if (!(ret_val).nil?)
            System.out.println("SNegoContext.initSecContext: " + "sending token = " + RJava.cast_to_string(SpNegoToken.get_hex_bytes(ret_val)))
          end
        end
    ...
    def accept_sec_context(is, mech_token_size)
      ...
      valid = true
      if (DEBUG)
        System.out.println("Entered SpNegoContext.acceptSecContext with " + "state=" + RJava.cast_to_string(print_state(@state)))
      end
      if (is_initiator)
      ...
      begin
        if ((@state).equal?(STATE_NEW))
          ...
          if (DEBUG)
            System.out.println("SpNegoContext.acceptSecContext: " + "receiving token = " + RJava.cast_to_string(SpNegoToken.get_hex_bytes(token)))
          end
          # read the SPNEGO token
          # token will be validated when parsing
          init_token = NegTokenInit.new(token)
          if (DEBUG)
            System.out.println("SpNegoContext.acceptSecContext: " + "received token of type = " + RJava.cast_to_string(SpNegoToken.get_token_name(init_token.get_type)))
          end
          ...
          if (DEBUG)
            System.out.println("SpNegoContext.acceptSecContext: " + "mechanism wanted = " + RJava.cast_to_string(mech_wanted))
            System.out.println("SpNegoContext.acceptSecContext: " + "negotiated result = " + RJava.cast_to_string(nego_result))
          end
          # calculate MIC only in normal mode
          mic_token = nil
          if (!GSSUtil.use_msinterop && valid)
          ...
          if (DEBUG)
            System.out.println("SpNegoContext.acceptSecContext: " + "sending token of type = " + RJava.cast_to_string(SpNegoToken.get_token_name(targ_token.get_type)))
          end
          # get the encoded token
          ...
          if ((@state).equal?(STATE_IN_PROCESS))
            ...
            # XXX Use logging API
            if (DEBUG)
              System.out.println("AcceptSecContext: state = " + RJava.cast_to_string(@state))
            end
          end
        end
        if (DEBUG)
          System.out.println("SpNegoContext.acceptSecContext: " + "sending token = " + RJava.cast_to_string(SpNegoToken.get_hex_bytes(ret_val)))
        end
      rescue IOException => e
        gss_exception = GSSException.new(GSSException::FAILURE, -1, e.get_message)
        gss_exception.init_cause(e)
    ...
    def generate_mech_list_mic(mech_types)
      ...
      begin
        ...
        if (DEBUG)
          System.out.println("SpNegoContext: getMIC = " + RJava.cast_to_string(SpNegoToken.get_hex_bytes(mic)))
        end
      rescue GSSException => e
        mic = nil
        if (DEBUG)
          System.out.println("SpNegoContext: no MIC token included" + " - getMIC failed : " + RJava.cast_to_string(e.get_message))
        end
      end
      return mic
    end
    ...
    def verify_mech_list_mic(mech_types, token)
      ...
      begin
        ...
        valid = false
        if (DEBUG)
          System.out.println("SpNegoContext: MIC validation failed! " + RJava.cast_to_string(e.get_message))
        end
      end
      return valid
    end
        ...
        while i < supported_mech_set.attr_length
          ...
          while j < mech_set.attr_length
            if ((mech_set[j] == supported_mech_set[i]))
              if (DEBUG)
                System.out.println("SpNegoContext: " + "negotiated mechanism = " + RJava.cast_to_string(mech_set[j]))
              end
              return (mech_set[j])
            end
            j += 1
