...
module Sun::Security::Jgss::Spnego
  module NegTokenTargImports #:nodoc:
  ...
  class NegTokenTarg < NegTokenTargImports.const_get :SpNegoToken
    ...
    def encode
      begin
        ...
        raise GSSException.new(GSSException::DEFECTIVE_TOKEN, -1, "Invalid SPNEGO NegTokenTarg token : " + RJava.cast_to_string(e.get_message))
      end
    end
    ...
    def parse_token(in_)
      begin
        ...
        if (tmp1.attr_data.available > 0)
          ...
          if (DEBUG)
            System.out.println("SpNegoToken NegTokenTarg: negotiated" + " result = " + RJava.cast_to_string(get_nego_result_string(@neg_result)))
          end
        end
        ...
        if (tmp1.attr_data.available > 0)
          ...
          if (DEBUG)
            System.out.println("SpNegoToken NegTokenTarg: " + "supported mechanism = " + RJava.cast_to_string(@supported_mech))
          end
        end
        ...
        if (!GSSUtil.use_msinterop && (tmp1.attr_data.available > 0))
          ...
          if (DEBUG)
            System.out.println("SpNegoToken NegTokenTarg: " + "MechListMIC Token = " + RJava.cast_to_string(get_hex_bytes(@mech_list_mic)))
          end
        else
          if (DEBUG)
