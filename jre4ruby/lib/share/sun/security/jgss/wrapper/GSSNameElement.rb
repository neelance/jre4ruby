...
module Sun::Security::Jgss::Wrapper
  module GSSNameElementImports #:nodoc:
  ...
  class GSSNameElement 
        ...
        if ((GSSUtil::NT_GSS_KRB5_PRINCIPAL == name_type) || (GSSName::NT_HOSTBASED_SERVICE == name_type))
          ...
          begin
            ...
            if ((ge.get_major).equal?(GSSException::BAD_MECH) && GSSUtil.is_sp_nego_mech(stub.get_mech))
              ...
              begin
                ...
                # Should never happen
                SunNativeProvider.debug("Name type list unavailable: " + RJava.cast_to_string(ge2.get_major_string))
              end
            else
              SunNativeProvider.debug("Name type list unavailable: " + RJava.cast_to_string(ge.get_major_string))
            end
          end
          if (!(supported_nts).nil?)
            ...
            if ((GSSUtil::NT_GSS_KRB5_PRINCIPAL == name_type))
              SunNativeProvider.debug("Override " + RJava.cast_to_string(name_type) + " with mechanism default(null)")
              return nil # Use mechanism specific default
            else
              SunNativeProvider.debug("Override " + RJava.cast_to_string(name_type) + " with " + RJava.cast_to_string(GSSUtil::NT_HOSTBASED_SERVICE2))
    ...
    def initialize(name_bytes, name_type, stub)
      ...
      set_printables
      SunNativeProvider.debug("Imported " + @printable_name + " w/ type " + RJava.cast_to_string(@printable_type))
    end
    ...
    def set_printables
      ...
      @printable_name = RJava.cast_to_string(printables[0])
    ...
    def get_krb_name
      ...
      SunNativeProvider.debug("Got kerberized name: " + RJava.cast_to_string(printables2[0]))
