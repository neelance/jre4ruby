...
module Sun::Security::Jgss::Wrapper
  module NativeGSSContextImports #:nodoc:
  ...
  class NativeGSSContext 
    ...
    def retrieve_token(is, mech_token_len)
      begin
        ...
        if (!(mech_token_len).equal?(-1))
          ...
          SunNativeProvider.debug("Precomputed mechToken length: " + RJava.cast_to_string(mech_token_len))
        ...
        SunNativeProvider.debug("Complete Token length: " + RJava.cast_to_string(result.attr_length))
        return result
      rescue IOException => ioe
        raise GSSExceptionImpl.new(GSSException::FAILURE, ioe)
      end
    ...
    def init_sec_context(is, mech_token_len)
      ...
      if ((!@is_established) && (@is_initiator))
        ...
        if (!(@p_context).equal?(0))
          ...
          SunNativeProvider.debug("initSecContext=> inToken len=" + RJava.cast_to_string(in_token.attr_length))
        end
        ...
        SunNativeProvider.debug("initSecContext=> outToken len=" + RJava.cast_to_string(((out_token).nil? ? 0 : out_token.attr_length)))
    ...
    def accept_sec_context(is, mech_token_len)
      ...
      if ((!@is_established) && (!@is_initiator))
        in_token = retrieve_token(is, mech_token_len)
        SunNativeProvider.debug("acceptSecContext=> inToken len=" + RJava.cast_to_string(in_token.attr_length))
        p_cred = ((@cred).nil? ? 0 : @cred.attr_p_cred)
        out_token = @c_stub.accept_context(p_cred, @cb, in_token, self)
        SunNativeProvider.debug("acceptSecContext=> outToken len=" + RJava.cast_to_string(((out_token).nil? ? 0 : out_token.attr_length)))
        if ((@target_name).nil?)
