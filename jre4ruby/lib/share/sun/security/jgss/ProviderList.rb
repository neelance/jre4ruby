...
module Sun::Security::Jgss
  module ProviderListImports #:nodoc:
  ...
  class ProviderList 
    ...
    def initialize(caller, use_native)
      ...
      while i < prov_list.attr_length
        ...
        begin
          ...
          GSSUtil.debug("Error in adding provider " + RJava.cast_to_string(prov.get_name) + ": " + RJava.cast_to_string(ge))
        end
        i += 1
      end # End of for loop
    end
    ...
    def get_mech_factory(e, mech_oid)
      ...
      if ((ret_val).nil?)
        ...
        # the provider.
        prop = PROV_PROP_PREFIX + RJava.cast_to_string(mech_oid.to_s)
        ...
        if (!(class_name).nil?)
          ...
          # throw an exception
          if (!(e.get_oid).nil?)
            raise GSSExceptionImpl.new(GSSException::BAD_MECH, "Provider " + RJava.cast_to_string(p.get_name) + " does not support mechanism " + RJava.cast_to_string(mech_oid))
          end
        end
      end
      return ret_val
    ...
    class_module.module_eval {
      ...
      typesig { [Provider, String, String, JavaException] }
      # Only used by getMechFactoryImpl
      def create_gssexception(p, class_name, trailing_msg, cause)
        err_class_info = class_name + " configured by " + RJava.cast_to_string(p.get_name) + " for GSS-API Mechanism Factory "
      ...
      synchronized(self) do
        ...
        if ((mech_oid).nil?)
          ...
          if ((p.get_property(PROV_PROP_PREFIX + oid_str)).nil?)
            raise GSSExceptionImpl.new(GSSException::BAD_MECH, "Provider " + RJava.cast_to_string(p.get_name) + " does not support " + oid_str)
          end
          @mechs.add(mech_oid)
    ...
    def add_all_mechs_from_provider(p)
      ...
      while (props.has_more_elements)
        prop = RJava.cast_to_string(props.next_element)
        if (is_mech_factory_property(prop))
          ...
          begin
            ...
            GSSUtil.debug("Ignore the invalid property " + prop + " from provider " + RJava.cast_to_string(p.get_name))
          end
