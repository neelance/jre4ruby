...
module Sun::Security::Pkcs
  module SignerInfoImports #:nodoc:
  ...
  class SignerInfo 
    ...
    def verify(block, data)
      begin
        ...
        raise SignatureException.new("InvalidKey: " + RJava.cast_to_string(e.get_message))
      end
      return nil
    end
    ...
    def to_s
      ...
      out = ""
      out += "Signer Info for (issuer): " + RJava.cast_to_string(@issuer_name) + "\n"
      out += "\tversion: " + RJava.cast_to_string(Debug.to_hex_string(@version)) + "\n"
      out += "\tcertificateSerialNumber: " + RJava.cast_to_string(Debug.to_hex_string(@certificate_serial_number)) + "\n"
      out += "\tdigestAlgorithmId: " + RJava.cast_to_string(@digest_algorithm_id) + "\n"
      if (!(@authenticated_attributes).nil?)
        out += "\tauthenticatedAttributes: " + RJava.cast_to_string(@authenticated_attributes) + "\n"
      end
      out += "\tdigestEncryptionAlgorithmId: " + RJava.cast_to_string(@digest_encryption_algorithm_id) + "\n"
      out += "\tencryptedDigest: " + "\n" + RJava.cast_to_string(hex_dump.encode_buffer(@encrypted_digest)) + "\n"
      if (!(@unauthenticated_attributes).nil?)
        out += "\tunauthenticatedAttributes: " + RJava.cast_to_string(@unauthenticated_attributes) + "\n"
      end
      return out
    end
