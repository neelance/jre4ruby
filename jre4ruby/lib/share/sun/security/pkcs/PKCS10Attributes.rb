...
module Sun::Security::Pkcs
  module PKCS10AttributesImports #:nodoc:
  ...
  class PKCS10Attributes 
    ...
    def ==(other)
      ...
      while i < len
        other_attr = attrs[i]
        key = RJava.cast_to_string(other_attr.get_attribute_id.to_s)
        if ((key).nil?)
          return false
        end
    ...
    def to_s
      s = RJava.cast_to_string(@map.size) + "\n" + RJava.cast_to_string(@map.to_s)
      return s
    end
