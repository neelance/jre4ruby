...
module Sun::Security::Provider
  module DSAParametersImports #:nodoc:
  ...
  class DSAParameters < DSAParametersImports.const_get :AlgorithmParametersSpi
    ...
    def engine_init(params)
      ...
      if (!(encoded_params.attr_data.available).equal?(0))
        raise IOException.new("encoded params have " + RJava.cast_to_string(encoded_params.attr_data.available) + " extra bytes")
      end
    end
    ...
    def engine_get_parameter_spec(param_spec)
      begin
        ...
        raise InvalidParameterSpecException.new("Unsupported parameter specification: " + RJava.cast_to_string(e.get_message))
      end
    end
    ...
    def engine_to_string
      return "\n\tp: " + RJava.cast_to_string(Debug.to_hex_string(@p)) + "\n\tq: " + RJava.cast_to_string(Debug.to_hex_string(@q)) + "\n\tg: " + RJava.cast_to_string(Debug.to_hex_string(@g)) + "\n"
    end
