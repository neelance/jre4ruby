...
module Sun::Security::Provider::Certpath
  module BasicCheckerImports #:nodoc:
  ...
  class BasicChecker < BasicCheckerImports.const_get :PKIXCertPathChecker
    ...
    def verify_timestamp(cert, date)
      msg = "timestamp"
      if (!(Debug).nil?)
        Debug.println("---checking " + msg + ":" + RJava.cast_to_string(date.to_s) + "...")
      end
      begin
    ...
    def update_state(curr_cert)
      ...
      if (!(Debug).nil?)
        Debug.println("BasicChecker.updateState issuer: " + RJava.cast_to_string(curr_cert.get_issuer_x500principal.to_s) + "; subject: " + RJava.cast_to_string(curr_cert.get_subject_x500principal) + "; serial#: " + RJava.cast_to_string(curr_cert.get_serial_number.to_s))
      end
    ...
    class_module.module_eval {
      ...
      def make_inherited_params_key(key_value_key, key_params_key)
        ...
        begin
          ...
          raise CertPathValidatorException.new("Unable to generate key with" + " inherited parameters: " + RJava.cast_to_string(e.get_message), e)
        end
        return usable_key
      end
    }
