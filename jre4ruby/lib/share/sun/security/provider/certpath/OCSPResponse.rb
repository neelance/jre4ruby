...
module Sun::Security::Provider::Certpath
  module OCSPResponseImports #:nodoc:
  ...
  class OCSPResponse 
    ...
    def initialize(bytes, params, responder_cert)
      ...
      begin
        ...
        if (!(DEBUG).nil?)
          DEBUG.println("OCSP response: " + RJava.cast_to_string(response_to_text(response_status)))
        end
        if (!(response_status).equal?(OCSP_RESPONSE_OK))
          raise CertPathValidatorException.new("OCSP Response Failure: " + RJava.cast_to_string(response_to_text(response_status)))
        end
        # responseBytes
        ...
        if ((response_type == OCSP_BASIC_RESPONSE_OID))
          ...
          if (!(DEBUG).nil?)
            DEBUG.println("OCSP response type: " + RJava.cast_to_string(response_type))
          end
          raise IOException.new("Unsupported OCSP response type: " + RJava.cast_to_string(response_type))
        end
        # BasicOCSPResponse
        ...
        if ((tag).equal?(NAME_TAG))
          ...
          if (!(DEBUG).nil?)
            DEBUG.println("OCSP Responder name: " + RJava.cast_to_string(responder_name))
          end
        else
        ...
        if (seq_der_in.available > 0)
          ...
          if (seq.is_context_specific(1))
            ...
            while i < response_ext_der.attr_length
              ...
              if (!(DEBUG).nil?)
                DEBUG.println("OCSP extension: " + RJava.cast_to_string(response_extension[i]))
              end
              if (((response_extension[i].get_extension_id) == OCSP_NONCE_EXTENSION_OID))
                ...
                if (response_extension[i].is_critical)
                  raise IOException.new("Unsupported OCSP critical extension: " + RJava.cast_to_string(response_extension[i].get_extension_id))
                end
              end
              i += 1
            end
    ...
    class_module.module_eval {
      ...
      def cert_status_to_text(cert_status)
        ...
        when 2
          ...
          return ("Unknown certificate status code: " + RJava.cast_to_string(cert_status))
        end
      end
      ...
      const_set_lazy(:SingleResponse) { Class.new do
        ...
        def initialize(der)
          ...
          if ((tag).equal?(CERT_STATUS_GOOD))
            ...
            if ((tag).equal?(CERT_STATUS_REVOKED))
              ...
              if (!(DEBUG).nil?)
                ...
                DEBUG.println("Revocation time: " + RJava.cast_to_string(revocation_time))
              end
            else
          ...
          if (!(DEBUG).nil?)
            until_ = ""
            if (!(@next_update).nil?)
              until_ = " until " + RJava.cast_to_string(@next_update)
            end
            DEBUG.println("Response's validity interval is from " + RJava.cast_to_string(@this_update) + until_)
          end
        ...
        def to_s
          ...
          sb.append("thisUpdate is " + RJava.cast_to_string(@this_update) + "\n")
          if (!(@next_update).nil?)
            sb.append("nextUpdate is " + RJava.cast_to_string(@next_update) + "\n")
          end
          return sb.to_s
        end
