...
module Sun::Security::Provider::Certpath
  module URICertStoreImports #:nodoc:
  ...
  class URICertStore < URICertStoreImports.const_get :CertStoreSpi
    ...
    def initialize(params)
      ...
      if ((@uri.get_scheme.to_lower_case == "ldap"))
        ...
        @ldap_path = RJava.cast_to_string(@uri.get_path)
        # strip off leading '/'
        if ((@ldap_path.char_at(0)).equal?(Character.new(?/.ord)))
          @ldap_path = RJava.cast_to_string(@ldap_path.substring(1))
        end
      end
      begin
    ...
    class_module.module_eval {
      ...
      def get_instance(params)
        synchronized(self) do
          if (!(Debug).nil?)
            Debug.println("CertStore URI:" + RJava.cast_to_string(params.attr_uri))
          end
      ...
      def get_instance(ad)
        ...
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("exception creating CertStore: " + RJava.cast_to_string(ex))
