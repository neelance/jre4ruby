  ...
  class Builder 
    ...
    class_module.module_eval {
      ...
      def target_distance(constraints, cert, target)
        ...
        if (!(Debug).nil?)
          Debug.println("Builder.targetDistance() merged constraints: " + RJava.cast_to_string(String.value_of(constraints)))
        end
        ...
        if (!(Debug).nil?)
          Debug.println("Builder.targetDistance() reduced constraints: " + RJava.cast_to_string(permitted))
        end
    ...
    def add_matching_certs(selector, cert_stores, result_certs, check_all)
      ...
      cert_stores.each do |store|
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("Builder.addMatchingCerts, non-fatal " + "exception retrieving certs: " + RJava.cast_to_string(cse))
