...
module Sun::Security::Provider::Certpath
  module X509CertPathImports #:nodoc:
  ...
  class X509CertPath < X509CertPathImports.const_get :CertPath
    ...
    def initialize(certs)
      ...
      certs.each do |obj|
        if ((obj.is_a?(X509Certificate)).equal?(false))
          raise CertificateException.new("List is not all X509Certificates: " + RJava.cast_to_string(obj.get_class.get_name))
        end
      end
    ...
    class_module.module_eval {
      ...
      def parse_pkipath(is)
        ...
        begin
          ...
          ce = CertificateException.new("IOException" + " parsing PkiPath data: " + RJava.cast_to_string(ioe))
          ce.init_cause(ioe)
          raise ce
        end
      end
      ...
      def parse_pkcs7(is)
        ...
        begin
          ...
          raise CertificateException.new("IOException parsing PKCS7 data: " + RJava.cast_to_string(ioe))
        end
        # Assumes that the resulting List is thread-safe. This is true
        # because we ensure that it cannot be modified after construction
        # and the methods in the Sun JDK 1.4 implementation of ArrayList that
    ...
    def encode_pkipath
      ...
      begin
        ...
        ce = CertificateEncodingException.new("IOException encoding PkiPath data: " + RJava.cast_to_string(ioe))
        ce.init_cause(ioe)
        raise ce
      end
    end
