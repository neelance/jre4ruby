...
module Sun::Security::Provider::Certpath
  module LDAPCertStoreImports #:nodoc:
  ...
  class LDAPCertStore < LDAPCertStoreImports.const_get :CertStoreSpi
    ...
    def create_initial_dir_context(server, port)
      url = "ldap://" + server + ":" + RJava.cast_to_string(port)
      env = Hashtable.new
      ...
      const_set_lazy(:LDAPRequest) { Class.new do
        ...
        def get_values(attr_id)
          if (DEBUG && (((self.attr_cache_hits + self.attr_cache_misses) % 50).equal?(0)))
            System.out.println("Cache hits: " + RJava.cast_to_string(self.attr_cache_hits) + "; misses: " + RJava.cast_to_string(self.attr_cache_misses))
          end
        ...
        def get_value_map
          ...
          if (DEBUG)
            System.out.println("Request: " + @name + ":" + RJava.cast_to_string(@requested_attributes))
      ...
      while i < n
        ...
        begin
          ...
          if (!(Debug).nil?)
            ...
            Debug.println("[ " + RJava.cast_to_string(encoder.encode_buffer(encoded_cert[i])) + " ]")
          end
        end
        i += 1
      end
      ...
      while i < n
        begin
          ...
          if (!(Debug).nil?)
            ...
            encoder = HexDumpEncoder.new
            Debug.println("[ " + RJava.cast_to_string(encoder.encode_buffer(encoded_cert_pair[i])) + " ]")
          end
        end
        i += 1
      end
    ...
    def engine_get_certificates(selector)
      synchronized(self) do
        if (!(Debug).nil?)
          Debug.println("LDAPCertStore.engineGetCertificates() selector: " + RJava.cast_to_string(String.value_of(selector)))
        end
        if ((selector).nil?)
        ...
        certs = HashSet.new
        if (!(Debug).nil?)
          Debug.println("LDAPCertStore.engineGetCertificates() basicConstraints: " + RJava.cast_to_string(basic_constraints))
        end
        # basicConstraints:
        ...
        if (!(subject).nil?)
          ...
          if (basic_constraints > -2)
            ...
            if (!(Debug).nil?)
              Debug.println("LDAPCertStore.engineGetCertificates() after " + "getMatchingCrossCerts(subject,xsel,null),certs.size(): " + RJava.cast_to_string(certs.size))
            end
            certs.add_all(get_certificates(request, CA_CERT, xsel))
            if (!(Debug).nil?)
              Debug.println("LDAPCertStore.engineGetCertificates() after " + "getCertificates(subject,CA_CERT,xsel),certs.size(): " + RJava.cast_to_string(certs.size))
            end
          end
          if (basic_constraints < 0)
            certs.add_all(get_certificates(request, USER_CERT, xsel))
            if (!(Debug).nil?)
              Debug.println("LDAPCertStore.engineGetCertificates() after " + "getCertificates(subject,USER_CERT, xsel),certs.size(): " + RJava.cast_to_string(certs.size))
            end
          end
        else
          if (!(Debug).nil?)
        ...
        if ((!(issuer).nil?) && (basic_constraints > -2))
          ...
          if (!(Debug).nil?)
            Debug.println("LDAPCertStore.engineGetCertificates() after " + "getMatchingCrossCerts(issuer,null,xsel),certs.size(): " + RJava.cast_to_string(certs.size))
          end
          certs.add_all(get_certificates(request, CA_CERT, xsel))
          if (!(Debug).nil?)
            Debug.println("LDAPCertStore.engineGetCertificates() after " + "getCertificates(issuer,CA_CERT,xsel),certs.size(): " + RJava.cast_to_string(certs.size))
          end
        end
        if (!(Debug).nil?)
      ...
      while i < n
        begin
          ...
          if (!(Debug).nil?)
            ...
            encoder = HexDumpEncoder.new
            Debug.println("[ " + RJava.cast_to_string(encoder.encode_buffer(encoded_crl[i])) + " ]")
          end
        end
        i += 1
      end
    ...
    def engine_get_crls(selector)
      synchronized(self) do
        if (!(Debug).nil?)
          Debug.println("LDAPCertStore.engineGetCRLs() selector: " + RJava.cast_to_string(selector))
        end
        ...
        issuer_names.each do |nameObject|
          ...
          if (name_object.is_a?(Array.typed(::Java::Byte)))
            begin
              ...
              issuer_name = RJava.cast_to_string(issuer.get_name(X500Principal::RFC2253))
            rescue IllegalArgumentException => e
              next
            end
          else
            issuer_name = RJava.cast_to_string(name_object)
          end
          ...
          if ((cert_checking).nil? || !(cert_checking.get_basic_constraints).equal?(-1))
            ...
            begin
              ...
              if (!(Debug).nil?)
                Debug.println("LDAPCertStore.engineGetCRLs non-fatal error " + "retrieving ARLs:" + RJava.cast_to_string(e))
                e.print_stack_trace
              end
            end
          end
