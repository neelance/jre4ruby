...
module Sun::Security::Provider
  module PolicyFileImports #:nodoc:
  ...
  class PolicyFile < Java::Security::Policy
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:PARAMS1) { Array.typed(Class).new([String]) }
      const_attr_reader  :PARAMS1
      
      const_set_lazy(:PARAMS2) { Array.typed(Class).new([String, String]) }
    ...
    def init_policy_file(propname, urlname, new_info)
      loaded_policy = AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
        ...
        define_method :run do
          ...
          if (self.attr_allow_system_properties)
            ...
            if (!(extra_policy).nil?)
              ...
              if (extra_policy.starts_with("="))
                override_all = true
                extra_policy = RJava.cast_to_string(extra_policy.substring(1))
              end
              begin
          ...
          n = 1
          policy_uri = nil
          while (!((policy_uri = RJava.cast_to_string(Security.get_property(urlname + RJava.cast_to_string(n))))).nil?)
            begin
              policy_url = nil
    ...
    def init(policy, new_info)
      ...
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("error parsing " + RJava.cast_to_string(policy))
    ...
    def get_code_source(ge, key_store, new_info)
      ...
      if (!(ge.attr_signed_by).nil?)
        ...
        if ((certs).nil?)
          ...
          # just return
          if (!(Debug).nil?)
            Debug.println("  -- No certs for alias '" + RJava.cast_to_string(ge.attr_signed_by) + "' - ignoring entry")
          end
          return nil
        end
      end
    ...
    def add_grant_entry(ge, key_store, new_info)
      if (!(Debug).nil?)
        ...
        Debug.println("  signedBy " + RJava.cast_to_string(ge.attr_signed_by))
        Debug.println("  codeBase " + RJava.cast_to_string(ge.attr_code_base))
        if (!(ge.attr_principals).nil? && ge.attr_principals.size > 0)
          li = ge.attr_principals.list_iterator
          while (li.has_next)
            pppe = li.next_
            Debug.println("  " + RJava.cast_to_string(pppe.to_s))
          end
        end
      end
      begin
        ...
        while (enum_.has_more_elements)
          ...
          begin
            ...
            if ((pe.attr_permission == "javax.security.auth.PrivateCredentialPermission") && pe.attr_name.ends_with(" self"))
              pe.attr_name = RJava.cast_to_string(pe.attr_name.substring(0, pe.attr_name.index_of("self"))) + SELF
            end
            # check for self
            ...
            entry.add(perm)
            if (!(Debug).nil?)
              Debug.println("  " + RJava.cast_to_string(perm))
            end
            ...
            if (!(certs).nil? || (pe.attr_signed_by).nil?)
              ...
              entry.add(perm_)
              if (!(Debug).nil?)
                Debug.println("  " + RJava.cast_to_string(perm_))
              end
            end
    ...
    def get_permissions(perms, pd)
      if (!(Debug).nil?)
        Debug.println("getPermissions:\n\t" + RJava.cast_to_string(print_pd(pd)))
      end
    ...
    def add_permissions(perms, cs, principals, entry)
      if (!(Debug).nil?)
        Debug.println("evaluate codesources:\n" + "\tPolicy CodeSource: " + RJava.cast_to_string(entry.get_code_source) + "\n" + "\tActive CodeSource: " + RJava.cast_to_string(cs))
      end
      ...
      if (!(Debug).nil?)
        ...
        Debug.println("evaluate principals:\n" + "\tPolicy Principals: " + RJava.cast_to_string(entry_ps) + "\n" + "\tActive Principals: " + RJava.cast_to_string(acc_ps))
      end
      ...
      while i < entry_ps.size
        ...
        begin
          ...
          if (!PrincipalComparator.is_assignable_from(p_class))
            ...
            if (!(Debug).nil?)
              Debug.println("found PrincipalComparator " + RJava.cast_to_string(pc.get_class.get_name))
            end
    ...
    def add_perms(perms, acc_ps, entry)
      ...
      while i < entry.attr_permissions.size
        ...
        if (!(Debug).nil?)
          Debug.println("  granting " + RJava.cast_to_string(p))
        end
    ...
    def expand_self(sp, entry_ps, pdp, perms)
      if ((entry_ps).nil? || (entry_ps.size).equal?(0))
        ...
        if (!(Debug).nil?)
          Debug.println("Ignoring permission " + RJava.cast_to_string(sp.get_self_type) + " with target name (" + RJava.cast_to_string(sp.get_self_name) + ").  " + "No Principal(s) specified " + "in the grant clause.  " + "SELF-based target names are " + "only valid in the context " + "of a Principal-based grant entry.")
        end
        return
      end
      start_index = 0
      ...
      while (!((v = sp.get_self_name.index_of(SELF, start_index))).equal?(-1))
        ...
        while (pli.has_next)
          ...
          while i < principal_info.attr_length
            ...
            sb.append(RJava.cast_to_string(principal_info[i][0]) + " " + "\"" + RJava.cast_to_string(principal_info[i][1]) + "\"")
            i += 1
          end
          if (pli.has_next)
            sb.append(", ")
      ...
      if (!(Debug).nil?)
        Debug.println("  expanded:\n\t" + RJava.cast_to_string(sp.get_self_name) + "\n  into:\n\t" + RJava.cast_to_string(sb.to_s))
      end
      begin
        ...
        if ((pc).nil?)
          ...
          rescue JavaException => nme
            if (!(Debug).nil?)
              Debug.println("self entry expansion " + " instantiation failed: " + RJava.cast_to_string(nme.to_s))
            end
          end
        end
      rescue JavaException => e
    ...
    def canonicalize_codebase(cs, extract_signer_certs)
      ...
      if (!(u).nil? && (u.get_protocol == "file"))
        ...
        if (is_local_file)
          path = RJava.cast_to_string(u.get_file.replace(Character.new(?/.ord), JavaFile.attr_separator_char))
      ...
      if (!(path).nil?)
        begin
          cs_url = nil
          path = RJava.cast_to_string(canon_path(path))
    ...
    class_module.module_eval {
      ...
      def canon_path(path)
        if (path.ends_with("*"))
          path = RJava.cast_to_string(path.substring(0, path.length - 1)) + "-"
          path = RJava.cast_to_string(JavaFile.new(path).get_canonical_path)
          return RJava.cast_to_string(path.substring(0, path.length - 1)) + "*"
        else
    ...
    def replace_principals(principals, keystore)
      ...
      while (i.has_next)
        ...
        if ((pppe.attr_principal_class == PolicyParser::REPLACE_NAME))
          ...
          name = nil
          if (((name = RJava.cast_to_string(get_dn(pppe.attr_principal_name, keystore)))).nil?)
            return false
          end
          if (!(Debug).nil?)
            Debug.println("  Replacing \"" + RJava.cast_to_string(pppe.attr_principal_name) + "\" with " + X500PRINCIPAL + "/\"" + name + "\"")
          end
    ...
    def expand_permission_name(pe, keystore)
      ...
      while (!((b = pe.attr_name.index_of("${{", start_index))).equal?(-1))
        ...
        if (!((colon_index = value.index_of(":"))).equal?(-1))
          prefix = RJava.cast_to_string(value.substring(0, colon_index))
        end
        ...
        if (prefix.equals_ignore_case("self"))
          ...
          if (prefix.equals_ignore_case("alias"))
            ...
            suffix = RJava.cast_to_string(value.substring(colon_index + 1))
            if (((suffix = RJava.cast_to_string(get_dn(suffix, keystore)))).nil?)
    ...
    def get_dn(alias_, keystore)
      ...
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("  Error retrieving certificate for '" + alias_ + "': " + RJava.cast_to_string(e.to_s))
        end
        return nil
      end
      ...
      if (is_trusted(id))
        if (!(Debug).nil?)
          ...
          Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
            ...
            typesig { [] }
            define_method :run do
              Debug.println("  identity = " + RJava.cast_to_string(id))
              return nil
            end
