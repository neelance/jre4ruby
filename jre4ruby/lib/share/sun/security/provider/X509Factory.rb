...
module Sun::Security::Provider
  module X509FactoryImports #:nodoc:
  ...
  class X509Factory < X509FactoryImports.const_get :CertificateFactorySpi
    ...
    def engine_generate_certificate(is)
      ...
      begin
        ...
        raise CertificateException.new("Could not parse certificate: " + RJava.cast_to_string(ioe.to_s)).init_cause(ioe)
      end
    end
    ...
    def base64_to_binary(is)
      ...
      temp = nil
      if (((temp = RJava.cast_to_string(read_line(br)))).nil? || !temp.starts_with("-----BEGIN"))
        raise IOException.new("Unsupported encoding")
      else
        len += temp.length
      end
      str_buf = StringBuffer.new
      while (!((temp = RJava.cast_to_string(read_line(br)))).nil? && !temp.starts_with("-----END"))
        str_buf.append(temp)
      end
      if ((temp).nil?)
        raise IOException.new("Unsupported encoding")
