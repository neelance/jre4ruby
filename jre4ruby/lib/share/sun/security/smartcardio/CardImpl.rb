  ...
  class CardImpl < CardImplImports.const_get :Card
    ...
    def get_protocol
      ...
      when SCARD_PROTOCOL_T1
        ...
        # should never occur
        return "Unknown protocol " + RJava.cast_to_string(@protocol)
      end
    end
    ...
    def open_logical_channel
      ...
      begin
        ...
        if ((!(response.attr_length).equal?(3)) || (!(get_sw(response)).equal?(0x9000)))
          raise CardException.new("openLogicalChannel() failed, card response: " + RJava.cast_to_string(PCSC.to_s(response)))
        end
    ...
    def begin_exclusive
      synchronized(self) do
        ...
        if (!(@exclusive_thread).nil?)
          raise CardException.new("Exclusive access has already been assigned to Thread " + RJava.cast_to_string(@exclusive_thread.get_name))
        end
        begin
    ...
    typesig { [] }
    def to_s
      return "PC/SC card in " + RJava.cast_to_string(@terminal.get_name) + ", protocol " + RJava.cast_to_string(get_protocol) + ", state " + RJava.cast_to_string(@state)
    end
