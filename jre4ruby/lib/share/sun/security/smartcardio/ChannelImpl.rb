...
module Sun::Security::Smartcardio
  module ChannelImplImports #:nodoc:
  ...
  class ChannelImpl < ChannelImplImports.const_get :CardChannel
    ...
    class_module.module_eval {
      ...
      def get_sw(res)
        if (res.attr_length < 2)
          raise CardException.new("Invalid response length: " + RJava.cast_to_string(res.attr_length))
        end
    ...
    def set_channel(com)
      ...
      if (@channel <= 3)
        ...
        if (@channel <= 19)
          ...
          raise RuntimeException.new("Unsupported channel number: " + RJava.cast_to_string(@channel))
        end
      end
    end
    ...
    def close
      ...
      begin
        ...
        if ((is_ok(res)).equal?(false))
          raise CardException.new("close() failed: " + RJava.cast_to_string(PCSC.to_s(res)))
        end
      rescue PCSCException => e
        @card.handle_error(e)
    ...
    typesig { [] }
    def to_s
      return "PC/SC channel " + RJava.cast_to_string(@channel)
    end
