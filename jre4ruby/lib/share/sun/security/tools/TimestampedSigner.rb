...
module Sun::Security::Tools
  module TimestampedSignerImports #:nodoc:
  ...
  class TimestampedSigner < TimestampedSignerImports.const_get :ContentSigner
    ...
    def generate_signed_data(parameters, omit_content, apply_timestamp)
      ...
      if (with > 0)
        digest_algorithm = RJava.cast_to_string(signature_algorithm.substring(0, with))
        and_ = signature_algorithm.index_of("and", with + 4)
        if (and_ > 0)
          key_algorithm = RJava.cast_to_string(signature_algorithm.substring(with + 4, and_))
        else
          key_algorithm = RJava.cast_to_string(signature_algorithm.substring(with + 4))
        end
      end
      ...
      if (apply_timestamp)
        ...
        if (!(tsa_uri).nil?)
          @tsa_url = RJava.cast_to_string(tsa_uri.to_s)
        else
          # Examine TSA cert
    ...
    def generate_timestamp_token(to_be_timestamped)
      ...
      if (!(status).equal?(0) && !(status).equal?(1))
        ...
        if ((failure_code).equal?(-1))
          raise IOException.new("Error generating timestamp: " + RJava.cast_to_string(ts_reply.get_status_code_as_text))
        else
          raise IOException.new("Error generating timestamp: " + RJava.cast_to_string(ts_reply.get_status_code_as_text) + " " + RJava.cast_to_string(ts_reply.get_failure_code_as_text))
        end
      end
