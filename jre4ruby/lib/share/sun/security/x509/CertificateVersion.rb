...
module Sun::Security::X509
  module CertificateVersionImports #:nodoc:
  ...
  class CertificateVersion 
    ...
    def initialize(version)
      ...
      if ((version).equal?(V1) || (version).equal?(V2) || (version).equal?(V3))
        ...
        raise IOException.new("X.509 Certificate version " + RJava.cast_to_string(version) + " not supported.\n")
      end
    end
    ...
    def to_s
      return ("Version: V" + RJava.cast_to_string((@version + 1)))
    end
