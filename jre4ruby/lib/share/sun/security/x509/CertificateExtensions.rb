...
module Sun::Security::X509
  module CertificateExtensionsImports #:nodoc:
  ...
  class CertificateExtensions 
    ...
    class_module.module_eval {
      ...
      def params
        defined?(@@params) ? @@params : @@params= Array.typed(Class).new([Boolean, Object])
      end
    ...
    def parse_extension(ext)
      begin
        ...
        if ((ext.is_critical).equal?(false))
          ...
          if (!(Debug).nil?)
            Debug.println("Error parsing extension: " + RJava.cast_to_string(ext))
            e.print_stack_trace
  ...
  class UnparseableExtension < CertificateExtensionsImports.const_get :Extension
    ...
    typesig { [Extension, JavaThrowable] }
    def initialize(ext, why)
      ...
      begin
        ...
        if (!(ext_class).nil?)
          ...
          @name = RJava.cast_to_string((field.get(nil))) + " "
        end
      rescue JavaException => e
    ...
    typesig { [] }
    def to_s
      return RJava.cast_to_string(super) + "Unparseable " + @name + "extension due to\n" + RJava.cast_to_string(@why) + "\n\n" + RJava.cast_to_string(Sun::Misc::HexDumpEncoder.new.encode_buffer(get_extension_value))
    end
