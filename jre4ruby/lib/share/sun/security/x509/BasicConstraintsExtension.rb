...
module Sun::Security::X509
  module BasicConstraintsExtensionImports #:nodoc:
  ...
  class BasicConstraintsExtension < BasicConstraintsExtensionImports.const_get :Extension
    ...
    def to_s
      s = RJava.cast_to_string(super) + "BasicConstraints:[\n"
      s += RJava.cast_to_string(((@ca) ? ("  CA:true") : ("  CA:false"))) + "\n"
      if (@path_len >= 0)
        s += "  PathLen:" + RJava.cast_to_string(@path_len) + "\n"
      else
        s += "  PathLen: undefined\n"
      end
      return (s + "]\n")
