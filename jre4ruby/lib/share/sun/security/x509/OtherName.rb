...
module Sun::Security::X509
  ...
  class OtherName 
    ...
    def initialize(oid, value)
      ...
      if (!(@gni).nil?)
        @name = RJava.cast_to_string(@gni.to_s)
      else
        @name = "Unrecognized ObjectIdentifier: " + RJava.cast_to_string(oid.to_s)
      end
    end
    ...
    def get_gni(oid, name_value)
      begin
        ...
        params = Array.typed(Class).new([Object])
        cons = (ext_class).get_constructor(params)
        passed = Array.typed(Object).new([name_value])
        gni = cons.new_instance(passed)
        return gni
      rescue JavaException => e
        raise IOException.new("Instantiation error: " + RJava.cast_to_string(e)).init_cause(e)
      end
    end
