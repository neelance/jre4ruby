  ...
  class AVA 
    ...
    def get_value_string
      begin
        ...
        # should not occur
        raise RuntimeException.new("AVA error: " + RJava.cast_to_string(e), e)
      end
    end
    ...
    class_module.module_eval {
      ...
      def parse_hex_string(in_, format)
        ...
        while (true)
          ...
          if ((c_val).equal?(-1))
            raise IOException.new("AVA parse, invalid hex " + "digit: " + RJava.cast_to_string(RJava.cast_to_char(c)))
          end
          if (((c_ndx % 2)).equal?(1))
            b = ((b * 16) + (c_val))
            baos.write(b)
      ...
      while (!(c).equal?(Character.new(?".ord)))
        if ((c).equal?(Character.new(?\\.ord)))
          ...
          if (!(c).equal?(Character.new(?\\.ord)) && !(c).equal?(Character.new(?".ord)) && SpecialChars.index_of(RJava.cast_to_char(c)) < 0)
            raise IOException.new("Invalid escaped character in AVA: " + RJava.cast_to_string(RJava.cast_to_char(c)))
          end
        end
    ...
    def parse_string(in_, c, format, temp)
      ...
      begin
        ...
        if ((c).equal?(Character.new(?\\.ord)))
          ...
          if ((format).equal?(RFC2253))
            if (!(SpecialChars2253.index_of(RJava.cast_to_char(c))).equal?(-1))
              raise IOException.new("Character '" + RJava.cast_to_string(RJava.cast_to_char(c)) + "' in AVA appears without escape")
            end
          end
        end
        # add embedded hex bytes before next char
    ...
    def initialize(derval)
      ...
      if (!(derval.attr_data.available).equal?(0))
        raise IOException.new("AVA, extra bytes = " + RJava.cast_to_string(derval.attr_data.available))
      end
    end
    ...
    def to_rfc2253string(oid_map)
      ...
      if ((type_and_value.char_at(0) >= Character.new(?0.ord) && type_and_value.char_at(0) <= Character.new(?9.ord)) || !is_der_string(@value, false))
        ...
        val_str = nil
        begin
          val_str = RJava.cast_to_string(String.new(@value.get_data_bytes, "UTF8"))
        rescue IOException => ie
          raise IllegalArgumentException.new("DER Value conversion")
        end
    ...
    def to_rfc2253canonical_string
      ...
      canon = RJava.cast_to_string(canon.to_upper_case(Locale::US).to_lower_case(Locale::US))
    ...
    def to_keyword_value_string(keyword)
      ...
      begin
        ...
        if ((val_str).nil?)
          ...
          if (quote_needed)
            retval.append("\"" + RJava.cast_to_string(sbuffer.to_s) + "\"")
          else
  ...
  class AVAKeyword 
    ...
    def is_compliant(standard)
      ...
      when AVA::DEFAULT
        ...
        raise IllegalArgumentException.new("Invalid standard " + RJava.cast_to_string(standard))
      end
    end
    ...
    class_module.module_eval {
      ...
      def get_oid(keyword, standard, extra_keyword_map)
        ...
        if ((standard).equal?(AVA::RFC2253))
          ...
          keyword = RJava.cast_to_string(keyword.trim)
        end
        ...
        if ((standard).equal?(AVA::RFC1779))
          ...
          keyword = RJava.cast_to_string(keyword.substring(4))
        else
      ...
      def get_keyword(oid, standard, extra_oid_map)
        ...
        if ((keyword_string).nil?)
          ...
          keyword_string = RJava.cast_to_string(keyword_string.trim)
