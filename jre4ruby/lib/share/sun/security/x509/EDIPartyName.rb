...
module Sun::Security::X509
  module EDIPartyNameImports #:nodoc:
  ...
  class EDIPartyName 
    ...
    def initialize(der_value)
      ...
      while i < len
        ...
        if (opt.is_context_specific(TAG_ASSIGNER) && !opt.is_constructed)
          ...
          @assigner = RJava.cast_to_string(opt.get_as_string)
        end
        if (opt.is_context_specific(TAG_PARTYNAME) && !opt.is_constructed)
          ...
          opt = opt.attr_data.get_der_value
          @party = RJava.cast_to_string(opt.get_as_string)
        end
        i += 1
      end
    end
    ...
    def to_s
      return ("EDIPartyName: " + RJava.cast_to_string((((@assigner).nil?) ? "" : ("  nameAssigner = " + @assigner + ","))) + "  partyName = " + @party)
    end
