...
module Sun::Security::X509
  module PolicyConstraintsExtensionImports #:nodoc:
  ...
  class PolicyConstraintsExtension < PolicyConstraintsExtensionImports.const_get :Extension
    ...
    def to_s
      s = nil
      s = RJava.cast_to_string(super) + "PolicyConstraints: [" + "  Require: "
      if ((@require).equal?(-1))
        s += "unspecified;"
      else
        s += RJava.cast_to_string(@require) + ";"
      end
      s += "\tInhibit: "
      if ((@inhibit).equal?(-1))
        s += "unspecified"
      else
        s += RJava.cast_to_string(@inhibit)
      end
      s += " ]\n"
      return s
    end
