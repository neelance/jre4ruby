...
module Sun::Security::X509
  ...
  class X509Cert 
    ...
    def verify(issuer_public_key)
      ...
      begin
        sig_verf = nil
        alg_name = RJava.cast_to_string(@issuer_sig_alg.get_name)
        ...
        if (!sig_verf.verify(@signature))
          raise CertException.new(CertException.attr_verf_invalid_sig, "Signature ... by <" + RJava.cast_to_string(@issuer) + "> for <" + RJava.cast_to_string(@subject) + ">")
        end
    ...
    def get_signer(algorithm_id, private_key)
      ...
      if (!(@pubkey.get_algorithm == algorithm))
        raise InvalidKeyException.new("Private key algorithm " + algorithm + " incompatible with certificate " + RJava.cast_to_string(@pubkey.get_algorithm))
      end
    ...
    def to_s
      ...
      s = "  X.509v" + RJava.cast_to_string((@version + 1)) + " certificate,\n"
      s += "  Subject is " + RJava.cast_to_string(@subject) + "\n"
      s += "  Key:  " + RJava.cast_to_string(@pubkey)
      s += "  Validity <" + RJava.cast_to_string(@notbefore) + "> until <" + RJava.cast_to_string(@notafter) + ">\n"
      s += "  Issuer is " + RJava.cast_to_string(@issuer) + "\n"
      s += "  Issuer signature used " + RJava.cast_to_string(@issuer_sig_alg.to_s) + "\n"
      s += "  Serial number = " + RJava.cast_to_string(Debug.to_hex_string(@serialnum)) + "\n"
    ...
    def parse(val)
      ...
      if (!(val.attr_data.available).equal?(0))
        raise CertParseError.new("signed overrun, bytes = " + RJava.cast_to_string(val.attr_data.available))
      end
