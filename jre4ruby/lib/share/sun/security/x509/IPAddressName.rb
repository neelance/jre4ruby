...
module Sun::Security::X509
  module IPAddressNameImports #:nodoc:
  ...
  class IPAddressName 
    ...
    def to_s
      begin
        return "IPAddress: " + RJava.cast_to_string(get_name)
      rescue IOException => ioe
        # dump out hex rep for debugging purposes
        enc = HexDumpEncoder.new
        return "IPAddress: " + RJava.cast_to_string(enc.encode_buffer(@address))
      end
    end
    ...
    def get_name
      ...
      if (@is_ipv4)
        ...
        @name = RJava.cast_to_string(InetAddress.get_by_address(host).get_host_address)
        if ((@address.attr_length).equal?(8))
          mask = Array.typed(::Java::Byte).new(4) { 0 }
          System.arraycopy(@address, 4, mask, 0, 4)
          @name = @name + "/" + RJava.cast_to_string(InetAddress.get_by_address(mask).get_host_address)
        end
      else
        ...
        if ((@address.attr_length).equal?(32))
          ...
          @name = @name + "/" + RJava.cast_to_string(i_)
          # Verify remaining bits 0
          while i_ < 16 * 8
            if (ba.get(i_))
              raise IOException.new("Invalid IPv6 subdomain - set " + "bit " + RJava.cast_to_string(i_) + " not contiguous")
            end
            i_ += 1
          end
        end
