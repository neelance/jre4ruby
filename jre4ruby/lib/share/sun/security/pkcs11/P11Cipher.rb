...
module Sun::Security::Pkcs11
  ...
  class P11Cipher < P11CipherImports.const_get :CipherSpi
    ...
    def initialize(token, algorithm, mechanism)
      ...
      @algorithm = algorithm
      @mechanism = mechanism
      @key_algorithm = RJava.cast_to_string(algorithm.split(Regexp.new("/"))[0])
    ...
    def engine_set_mode(mode)
      mode = RJava.cast_to_string(mode.to_upper_case)
      if ((mode == "ECB"))
        @block_mode = MODE_ECB
      else
        if ((mode == "CBC"))
    ...
    def engine_init(opmode, key, params, random)
      ...
      if (!(params).nil?)
        begin
          iv_spec = params.get_parameter_spec(IvParameterSpec)
          iv = iv_spec.get_iv
    ...
    def impl_init(opmode, key, iv, random)
      ...
      when Cipher::DECRYPT_MODE
        ...
        raise InvalidAlgorithmParameterException.new("Unsupported mode: " + RJava.cast_to_string(opmode))
      end
