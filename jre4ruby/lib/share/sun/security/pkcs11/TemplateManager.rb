...
module Sun::Security::Pkcs11
  module TemplateManagerImports #:nodoc:
  ...
  class TemplateManager 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:KeyAndTemplate) { Class.new do
        ...
        typesig { [class_self::TemplateKey, class_self::Template] }
    ...
    def add_template(op, object_class, key_algorithm, attrs)
      ...
      if (DEBUG)
        System.out.println("Adding " + RJava.cast_to_string(key) + " -> " + RJava.cast_to_string(template))
      end
    ...
    def get_attributes(op, type, alg, attrs)
      ...
      if (DEBUG)
        System.out.println(RJava.cast_to_string(key) + " -> " + RJava.cast_to_string(Arrays.as_list(new_attrs)))
      end
      return new_attrs
    end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:TemplateKey) { Class.new do
        ...
        typesig { [] }
        def to_s
          return "(" + @operation + "," + RJava.cast_to_string(Functions.get_object_class_name(@key_type)) + "," + RJava.cast_to_string(Functions.get_key_name(@key_algorithm)) + ")"
        end
      ...
      const_set_lazy(:Template) { Class.new do
        ...
        class_module.module_eval {
          const_set_lazy(:A0) { Array.typed(class_self::CK_ATTRIBUTE).new(0) { nil } }
        ...
        typesig { [Array.typed(class_self::CK_ATTRIBUTE)] }
        def initialize(attributes)
          @attributes = nil
          @attributes = attributes
        end
        
        typesig { [class_self::Template] }
        def add(template)
        ...
        class_module.module_eval {
          typesig { [Array.typed(class_self::CK_ATTRIBUTE), Array.typed(class_self::CK_ATTRIBUTE)] }
          ...
          def combine(attrs1, attrs2)
            attrs = class_self::ArrayList.new
            attrs1.each do |attr|
