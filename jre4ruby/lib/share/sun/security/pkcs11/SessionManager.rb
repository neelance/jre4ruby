...
module Sun::Security::Pkcs11
  module SessionManagerImports #:nodoc:
  ...
  class SessionManager 
    ...
    def kill_session(session)
      synchronized(self) do
        ...
        if (!(Debug).nil?)
          location = JavaException.new.get_stack_trace[2].to_s
          System.out.println("Killing session (" + location + ") active: " + RJava.cast_to_string(@active_sessions))
        end
        begin
    ...
    def demote_obj_session(session)
      synchronized(self) do
        ...
        if (!(Debug).nil?)
          System.out.println("Demoting session, active: " + RJava.cast_to_string(@active_sessions))
        end
      ...
      if (!(Debug).nil?)
        if (@active_sessions > @max_active_sessions)
          ...
          if ((@max_active_sessions % 10).equal?(0))
            System.out.println("Open sessions: " + RJava.cast_to_string(@max_active_sessions))
          end
        end
      end
      return session
    ...
    class_module.module_eval {
      const_set_lazy(:Pool) { Class.new do
        ...
        def release(session)
          ...
          if (!(Debug).nil?)
            System.out.println("Closing " + RJava.cast_to_string(i) + " idle sessions, active: " + RJava.cast_to_string(@mgr.attr_active_sessions))
          end
          sub_list_ = @pool.sub_list(0, i)
          sub_list_.clear
          if (!(exc).nil?)
            raise self.class::ProviderException.new(exc)
          end
        end
