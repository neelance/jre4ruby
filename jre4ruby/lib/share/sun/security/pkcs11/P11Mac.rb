...
module Sun::Security::Pkcs11
  ...
  class P11Mac < P11MacImports.const_get :MacSpi
    ...
    def initialize(token, algorithm, mechanism)
      ...
      when (CKM_MD5_HMAC).to_int
        @mac_length = 16
      when (CKM_SHA_1_HMAC).to_int
        @mac_length = 20
      when (CKM_SHA256_HMAC).to_int
        @mac_length = 32
      when (CKM_SHA384_HMAC).to_int
        @mac_length = 48
      when (CKM_SHA512_HMAC).to_int
        @mac_length = 64
      when (CKM_SSL3_MD5_MAC).to_int
        @mac_length = 16
        params = Long.value_of(16)
      when (CKM_SSL3_SHA1_MAC).to_int
        @mac_length = 20
