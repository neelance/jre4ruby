  ...
  class P11Digest < P11DigestImports.const_get :MessageDigestSpi
    ...
    def engine_digest(digest, ofs, len)
      if (len < @digest_length)
        raise DigestException.new("Length must be at least " + RJava.cast_to_string(@digest_length))
      end
      fetch_session
      begin
        n = 0
    ...
    def impl_update(key)
      ...
      if ((key.is_a?(P11Key)).equal?(false))
        raise InvalidKeyException.new("Not a P11Key: " + RJava.cast_to_string(key))
      end
      p11key = key
      if (!(p11key.attr_token).equal?(@token))
        raise InvalidKeyException.new("Not a P11Key of this provider: " + RJava.cast_to_string(key))
      end
      begin
        if ((@state).equal?(S_BUFFERED))
