...
module Sun::Security::Ssl
  module ServerHandshakerImports #:nodoc:
  ...
  class ServerHandshaker < ServerHandshakerImports.const_get :Handshaker
    ...
    def client_hello(mesg)
      ...
      if (@client_requested_version.attr_v < self.attr_enabled_protocols.attr_min.attr_v)
        fatal_se(Alerts.attr_alert_handshake_failure, "Client requested protocol " + RJava.cast_to_string(@client_requested_version) + " not enabled or not supported")
      end
        ...
        if (!(previous).nil?)
          ...
          if (self.attr_resuming_session)
            ...
            if (!(self.attr_debug).nil? && (Debug.is_on("handshake") || Debug.is_on("session")))
              System.out.println("%% Resuming " + RJava.cast_to_string(self.attr_session))
            end
          end
        end
      ...
      when K_ECDH_RSA, K_ECDH_ECDSA
        ...
        raise RuntimeException.new("internal error: " + RJava.cast_to_string(self.attr_key_exchange))
      end
      if (!(m3).nil?)
        if (!(self.attr_debug).nil? && Debug.is_on("handshake"))
          m3.print(System.out)
    ...
    def try_set_cipher_suite(suite)
      ...
      when K_ECDH_ANON
        ...
        raise RuntimeException.new("Unrecognized cipherSuite: " + RJava.cast_to_string(suite))
      end
      set_cipher_suite(suite)
      return true
    end
    ...
    def setup_private_key_and_chain(algorithm)
      ...
      if (!(self.attr_conn).nil?)
        alias_ = RJava.cast_to_string(km.choose_server_alias(algorithm, nil, self.attr_conn))
      else
        alias_ = RJava.cast_to_string(km.choose_engine_server_alias(algorithm, nil, self.attr_engine))
      end
      if ((alias_).nil?)
        return false
      end
    ...
    def setup_kerberos_keys
      ...
      begin
        ...
        if (!(@kerberos_keys).nil?)
          if (!(self.attr_debug).nil? && Debug.is_on("handshake"))
            System.out.println("Using Kerberos key: " + RJava.cast_to_string(@kerberos_keys[0]))
          end
        ...
        if (!(self.attr_debug).nil? && Debug.is_on("handshake"))
          System.out.println("Attempt to obtain Kerberos key failed: " + RJava.cast_to_string(e.to_s))
        end
        return false
      end
    end
    ...
    def client_finished(mesg)
      ...
      if (!self.attr_resuming_session && self.attr_session.is_rejoinable)
        ...
        if (!(self.attr_debug).nil? && Debug.is_on("session"))
          System.out.println("%% Cached server session: " + RJava.cast_to_string(self.attr_session))
        end
      else
        if (!self.attr_resuming_session && !(self.attr_debug).nil? && Debug.is_on("session"))
          System.out.println("%% Didn't cache non-resumable server session: " + RJava.cast_to_string(self.attr_session))
        end
      end
    end
