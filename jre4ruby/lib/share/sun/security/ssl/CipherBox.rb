  ...
  class CipherBox 
    ...
    def initialize(protocol_version, bulk_cipher, key, iv, encrypt)
      ...
      begin
        ...
        raise NoSuchAlgorithmException.new("Could not create cipher " + RJava.cast_to_string(bulk_cipher), e)
    ...
    class_module.module_eval {
      ...
      def new_cipher_box(version, cipher, key, iv, encrypt)
        if ((cipher.attr_allowed).equal?(false))
          raise NoSuchAlgorithmException.new("Unsupported cipher " + RJava.cast_to_string(cipher))
        end
      ...
      begin
        ...
        if (!(Debug).nil? && Debug.is_on("plaintext"))
          begin
            ...
            System.out.println("Padded plaintext before ENCRYPTION:  len = " + RJava.cast_to_string(len))
        ...
        if (!(new_len).equal?(len))
          ...
          raise RuntimeException.new("Cipher buffering error " + "in JCE provider " + RJava.cast_to_string(@cipher.get_provider.get_name))
        end
        return new_len
    ...
    def decrypt(buf, offset, len)
      ...
      begin
        ...
        if (!(Debug).nil? && Debug.is_on("plaintext"))
          begin
            hd = HexDumpEncoder.new
            System.out.println("Padded plaintext after DECRYPTION:  len = " + RJava.cast_to_string(new_len))
    ...
    class_module.module_eval {
      ...
      def remove_padding(buf, offset, len, block_size, protocol_version)
        ...
        newlen = len - (pad + 1)
        if (newlen < 0)
          raise BadPaddingException.new("Padding length invalid: " + RJava.cast_to_string(pad))
        end
        if (protocol_version.attr_v >= ProtocolVersion::TLS10.attr_v)
          ...
          while i <= pad
            ...
            if (!(val).equal?(pad))
              raise BadPaddingException.new("Invalid TLS padding: " + RJava.cast_to_string(val))
            end
            i += 1
          end
        else
          ...
          if (pad > block_size)
            raise BadPaddingException.new("Invalid SSLv3 padding: " + RJava.cast_to_string(pad))
          end
        end
        return newlen
      end
