...
module Sun::Security::Ssl
  module OutputRecordImports #:nodoc:
  ...
  class OutputRecord < OutputRecordImports.const_get :ByteArrayOutputStream
    ...
    def hash_internal(buf, offset, len)
      if (!(Debug).nil? && Debug.is_on("data"))
        begin
          ...
          System.out.println("[write] MD5 and SHA1 hashes:  len = " + RJava.cast_to_string(len))
    ...
    def write(s)
      ...
      if (length < 0)
        raise SSLException.new("output record size too small: " + RJava.cast_to_string(length))
      end
      if (!(Debug).nil? && (Debug.is_on("record") || Debug.is_on("handshake")))
        if ((!(Debug).nil? && Debug.is_on("record")) || (content_type).equal?(self.attr_ct_change_cipher_spec))
          # v3.0/v3.1 ...
          System.out.println(RJava.cast_to_string(JavaThread.current_thread.get_name) + ", WRITE: " + RJava.cast_to_string(@protocol_version) + " " + RJava.cast_to_string(InputRecord.content_name(content_type)) + ", length = " + RJava.cast_to_string(length))
        end
      end
      ...
      if (@first_message && use_v2hello)
        ...
        if (!(Debug).nil? && Debug.is_on("record"))
          System.out.println(RJava.cast_to_string(JavaThread.current_thread.get_name) + ", WRITE: SSLv2 client hello message" + ", length = " + RJava.cast_to_string((self.attr_count - 2))) # 2 byte SSLv2 header
        end
      else
    ...
    def write_buffer(s, buf, off, len)
      ...
      if (!(Debug).nil? && Debug.is_on("packet"))
        begin
          ...
          System.out.println("[Raw write]: length = " + RJava.cast_to_string(bb.remaining))
