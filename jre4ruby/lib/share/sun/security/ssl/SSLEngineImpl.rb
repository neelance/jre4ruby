...
module Sun::Security::Ssl
  ...
  class SSLEngineImpl < SSLEngineImplImports.const_get :SSLEngine
    ...
    def read_net_record(ea)
      ...
      if (packet_len > @sess.get_packet_buffer_size)
        if (packet_len > Record.attr_max_large_record_size)
          raise SSLProtocolException.new("Input SSL/TLS record too big: max = " + RJava.cast_to_string(Record.attr_max_large_record_size) + " len = " + RJava.cast_to_string(packet_len))
        else
    ...
    def close_outbound_internal
      if ((!(Debug).nil?) && Debug.is_on("ssl"))
        System.out.println(RJava.cast_to_string(thread_name) + ", closeOutboundInternal()")
      end
    ...
    def close_outbound
      synchronized(self) do
        ...
        if ((!(Debug).nil?) && Debug.is_on("ssl"))
          System.out.println(RJava.cast_to_string(thread_name) + ", called closeOutbound()")
        end
    ...
    def close_inbound_internal
      if ((!(Debug).nil?) && Debug.is_on("ssl"))
        System.out.println(RJava.cast_to_string(thread_name) + ", closeInboundInternal()")
      end
      # Already closed, ignore
      if (@inbound_done)
        return
    ...
    def close_inbound
      synchronized(self) do
        ...
        if ((!(Debug).nil?) && Debug.is_on("ssl"))
          System.out.println(RJava.cast_to_string(thread_name) + ", called closeInbound()")
        end
    ...
    def fatal(description, diagnostic, cause)
      synchronized(self) do
        ...
        if (!(@close_reason).nil?)
          if ((!(Debug).nil?) && Debug.is_on("ssl"))
            System.out.println(RJava.cast_to_string(thread_name) + ", fatal: engine already closed.  Rethrowing " + RJava.cast_to_string(cause.to_s))
          end
        ...
        if ((!(Debug).nil?) && Debug.is_on("ssl"))
          System.out.println(RJava.cast_to_string(thread_name) + ", fatal error: " + RJava.cast_to_string(description) + ": " + diagnostic + "\n" + RJava.cast_to_string(cause.to_s))
        end
