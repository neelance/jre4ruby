...
module Sun::Security::Ssl
  module ClientHandshakerImports #:nodoc:
  ...
  class ClientHandshaker < ClientHandshakerImports.const_get :Handshaker
    ...
    def process_message(type, message_len)
      if (self.attr_state > type && (!(type).equal?(HandshakeMessage.attr_ht_hello_request) && !(self.attr_state).equal?(HandshakeMessage.attr_ht_client_hello)))
        raise SSLProtocolException.new("Handshake message sequence violation, " + RJava.cast_to_string(type))
      end
      case (type)
      ...
      when HandshakeMessage.attr_ht_server_key_exchange
        ...
        when K_ECDH_ECDSA, K_ECDH_RSA
          raise SSLProtocolException.new("Protocol violation: server sent" + " a server key exchange message for key exchange " + RJava.cast_to_string(self.attr_key_exchange))
        when K_KRB5, K_KRB5_EXPORT
          raise SSLProtocolException.new("unexpected receipt of server key exchange algorithm")
        else
          raise SSLProtocolException.new("unsupported key exchange algorithm = " + RJava.cast_to_string(self.attr_key_exchange))
        end
    ...
    def server_hello(mesg)
      ...
      if ((self.attr_enabled_protocols.contains(mesg_version)).equal?(false))
        raise SSLHandshakeException.new("Server chose unsupported or disabled protocol: " + RJava.cast_to_string(mesg_version))
      end
      ...
      if (!(mesg.attr_compression_method).equal?(0))
        fatal_se(Alerts.attr_alert_illegal_parameter, "compression type not supported, " + RJava.cast_to_string(mesg.attr_compression_method))
        # NOTREACHED
      end
      ...
      if (!(self.attr_session).nil?)
        ...
        if ((self.attr_session.get_session_id == mesg.attr_session_id))
          ...
          if ((session_suite.attr_key_exchange).equal?(K_KRB5) || (session_suite.attr_key_exchange).equal?(K_KRB5_EXPORT))
            ...
            if (!(subject).nil?)
              principals = subject.get_principals(KerberosPrincipal)
          ...
          if (!(self.attr_debug).nil? && Debug.is_on("session"))
            System.out.println("%% Server resumed " + RJava.cast_to_string(self.attr_session))
          end
          return
        else
      ...
      mesg.attr_extensions.list.each do |ext|
        ...
        if ((!(type).equal?(ExtensionType::EXT_ELLIPTIC_CURVES)) && (!(type).equal?(ExtensionType::EXT_EC_POINT_FORMATS)))
          fatal_se(Alerts.attr_alert_unsupported_extension, "Server sent an unsupported extension: " + RJava.cast_to_string(type))
        end
      end
      ...
      if (!(self.attr_debug).nil? && Debug.is_on("handshake"))
        System.out.println("** " + RJava.cast_to_string(self.attr_cipher_suite))
      end
    end
    ...
    def server_hello_done(mesg)
      ...
      if (!(@cert_request).nil?)
        ...
        if (!(keytypes_tmp_size).equal?(0))
          ...
          if (!(self.attr_conn).nil?)
            alias_ = RJava.cast_to_string(km.choose_client_alias(keytypes, @cert_request.get_authorities, self.attr_conn))
          else
            alias_ = RJava.cast_to_string(km.choose_engine_client_alias(keytypes, @cert_request.get_authorities, self.attr_engine))
          end
        end
        m1 = nil
        if (!(alias_).nil?)
      ...
      when K_KRB5, K_KRB5_EXPORT
        ...
        raise RuntimeException.new("Unsupported key exchange: " + RJava.cast_to_string(self.attr_key_exchange))
      end
      if (!(self.attr_debug).nil? && Debug.is_on("handshake"))
        m2.print(System.out)
      end
      ...
      when K_ECDH_RSA, K_ECDH_ECDSA
        ...
        raise IOException.new("Internal error: unknown key exchange " + RJava.cast_to_string(self.attr_key_exchange))
      end
    ...
    def server_finished(mesg)
      ...
      if (!self.attr_resuming_session)
        if (self.attr_session.is_rejoinable)
          ...
          if (!(self.attr_debug).nil? && Debug.is_on("session"))
            System.out.println("%% Cached client session: " + RJava.cast_to_string(self.attr_session))
          end
        else
          if (!(self.attr_debug).nil? && Debug.is_on("session"))
            System.out.println("%% Didn't cache non-resumable client session: " + RJava.cast_to_string(self.attr_session))
          end
        end
      end
    end
    ...
    def get_kickstart_message
      ...
      if (!(self.attr_debug).nil? && Debug.is_on("session"))
        if (!(self.attr_session).nil?)
          System.out.println("%% Client cached " + RJava.cast_to_string(self.attr_session) + RJava.cast_to_string((self.attr_session.is_rejoinable ? "" : " (not rejoinable)")))
        else
        ...
        if (!(self.attr_session).nil?)
          if (!(self.attr_debug).nil?)
            if (Debug.is_on("handshake") || Debug.is_on("session"))
              System.out.println("%% Try resuming " + RJava.cast_to_string(self.attr_session) + " from port " + RJava.cast_to_string(get_local_port_se))
            end
          end
    ...
    def server_certificate(mesg)
      ...
      begin
        ...
        if ((self.attr_key_exchange).equal?(K_RSA_EXPORT) && !@server_key_exchange_received)
          key_exchange_string = RJava.cast_to_string(K_RSA.attr_name)
        else
          key_exchange_string = RJava.cast_to_string(self.attr_key_exchange.attr_name)
        end
