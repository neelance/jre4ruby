...
module Sun::Security::Ssl
  module SSLContextImplImports #:nodoc:
    ...
    def engine_init(km, tm, sr)
      ...
      if ((sr).nil?)
        ...
        if (SunJSSE.is_fips && (!(sr.get_provider).equal?(SunJSSE.attr_crypto_provider)))
          raise KeyManagementException.new("FIPS mode: SecureRandom must be from provider " + RJava.cast_to_string(SunJSSE.attr_crypto_provider.get_name))
        end
        @secure_random = sr
      end
