    ...
    class_module.module_eval {
      ...
      def alert_description(code)
        ...
        when Alert_bad_certificate_hash_value
          ...
          return "<UNKNOWN ALERT: " + RJava.cast_to_string((code & 0xff)) + ">"
        end
      end
      ...
      typesig { [::Java::Byte, JavaThrowable, String] }
      ...
      def get_sslexception(description, cause, reason)
        ...
        if ((reason).nil?)
          if (!(cause).nil?)
            reason = RJava.cast_to_string(cause.to_s)
          else
            reason = ""
          end
        end
