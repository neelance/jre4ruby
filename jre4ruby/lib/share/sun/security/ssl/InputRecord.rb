...
module Sun::Security::Ssl
  module InputRecordImports #:nodoc:
  ...
  class InputRecord < InputRecordImports.const_get :ByteArrayInputStream
    ...
    def hash_internal(databuf, offset, len)
      if (!(Debug).nil? && Debug.is_on("data"))
        begin
          ...
          System.out.println("[read] MD5 and SHA1 hashes:  len = " + RJava.cast_to_string(len))
      ...
      while (n < len)
        ...
        if (!(Debug).nil? && Debug.is_on("packet"))
          begin
            ...
            System.out.println("[Raw read]: length = " + RJava.cast_to_string(bb.remaining))
    ...
    def read_v3record(s, o)
      ...
      if ((record_version.attr_v < ProtocolVersion::MIN.attr_v) || (record_version.attr_major > ProtocolVersion::MAX.attr_major))
        raise SSLException.new("Unsupported record version " + RJava.cast_to_string(record_version))
      end
      ...
      if (content_len < 0 || content_len > self.attr_max_large_record_size - self.attr_header_size)
        raise SSLProtocolException.new("Bad InputRecord size" + ", count = " + RJava.cast_to_string(content_len) + ", buf.length = " + RJava.cast_to_string(self.attr_buf.attr_length))
      end
      ...
      if (!(Debug).nil? && Debug.is_on("record"))
        if (self.attr_count < 0 || self.attr_count > (self.attr_max_record_size - self.attr_header_size))
          System.out.println(RJava.cast_to_string(JavaThread.current_thread.get_name) + ", Bad InputRecord size" + ", count = " + RJava.cast_to_string(self.attr_count))
        end
        System.out.println(RJava.cast_to_string(JavaThread.current_thread.get_name) + ", READ: " + RJava.cast_to_string(record_version) + " " + RJava.cast_to_string(content_name(content_type)) + ", length = " + RJava.cast_to_string(available))
      end
    ...
    def handle_unknown_record(s, o)
      ...
      if ((!((self.attr_buf[0] & 0x80)).equal?(0)) && (self.attr_buf[2]).equal?(1))
        ...
        @last_hashed = self.attr_count
        if (!(Debug).nil? && Debug.is_on("record"))
          System.out.println(RJava.cast_to_string(JavaThread.current_thread.get_name) + ", READ:  SSL v2, contentType = " + RJava.cast_to_string(content_name(content_type)) + ", translated length = " + RJava.cast_to_string(available))
        end
        return
      else
    ...
    class_module.module_eval {
      ...
      def content_name(content_type_)
        ...
        when self.attr_ct_application_data
          ...
          return "contentType = " + RJava.cast_to_string(content_type_)
        end
      end
    }
