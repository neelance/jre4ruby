...
module Sun::Security::Ssl
  module SSLSessionImplImports #:nodoc:
  ...
  class SSLSessionImpl 
    ...
    def initialize(protocol_version, cipher_suite, id, host, port)
      ...
      if (!(Debug).nil? && Debug.is_on("session"))
        System.out.println("%% Created:  " + RJava.cast_to_string(self))
      end
    end
    ...
    def invalidate
      synchronized(self) do
        ...
        @invalidated = true
        if (!(Debug).nil? && Debug.is_on("session"))
          System.out.println("%% Invalidated:  " + RJava.cast_to_string(self))
        end
        if (!(@context).nil?)
    ...
    def to_s
      return "[Session-" + RJava.cast_to_string(@session_count) + ", " + RJava.cast_to_string(get_cipher_suite) + "]"
    end
