...
module Sun::Security::Ssl
  module TrustManagerFactoryImplImports #:nodoc:
      ...
      if ((ks).nil?)
        begin
          ...
          if (!(Debug).nil? && Debug.is_on("trustmanager"))
            System.out.println("SunX509: skip default keystore: " + RJava.cast_to_string(se))
          end
        rescue JavaError => err
          if (!(Debug).nil? && Debug.is_on("trustmanager"))
            System.out.println("SunX509: skip default keystore: " + RJava.cast_to_string(err))
          end
          raise err
        rescue RuntimeException => re
          if (!(Debug).nil? && Debug.is_on("trustmanager"))
            System.out.println("SunX509: skip default keystore: " + RJava.cast_to_string(re))
          end
          raise re
        rescue JavaException => e
          if (!(Debug).nil? && Debug.is_on("trustmanager"))
            System.out.println("SunX509: skip default keystore: " + RJava.cast_to_string(e))
          end
          raise KeyStoreException.new("problem accessing trust store" + RJava.cast_to_string(e))
        end
      end
    ...
    class_module.module_eval {
      ...
      def get_cacerts_key_store(dbgname)
        ...
        if (!("NONE" == store_file_name))
          ...
          if (!(fis).nil?)
            store_file_name = RJava.cast_to_string(store_file.get_path)
          else
            store_file_name = "No File Available, using empty keystore."
          end
        end
        default_trust_store_type = RJava.cast_to_string(props.get("trustStoreType"))
        default_trust_store_provider = RJava.cast_to_string(props.get("trustStoreProvider"))
