    ...
    def map(iter, options)
      ...
      while (!((ch = iter.next_code_point)).equal?(UCharacterIterator::DONE))
        ...
        if ((val.attr_type).equal?(UNASSIGNED) && (allow_unassigned).equal?(false))
          raise ParseException.new("An unassigned code point was found in the input " + RJava.cast_to_string(iter.get_text), iter.get_index)
        else
    ...
    def prepare(src, options)
      ...
      while (!((ch = iter.next_code_point)).equal?(UCharacterIterator::DONE))
        ...
        if ((val.attr_type).equal?(PROHIBITED))
          raise ParseException.new("A prohibited code point was found in the input" + RJava.cast_to_string(iter.get_text), val.attr_value)
        end
      ...
      if ((@check_bi_di).equal?(true))
        ...
        if ((left_to_right).equal?(true) && (right_to_left).equal?(true))
          raise ParseException.new("The input does not conform to the rules for BiDi code points." + RJava.cast_to_string(iter.get_text), (rtl_pos > ltr_pos) ? rtl_pos : ltr_pos)
        end
        # satisfy 3
