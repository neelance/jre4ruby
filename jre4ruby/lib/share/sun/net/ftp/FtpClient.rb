  ...
  class FtpClient < FtpClientImports.const_get :TransferProtocolClient
      ...
      def get_ftp_proxy_host
        return Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
          ...
          define_method :run do
            ...
            if ((result).nil?)
              result = RJava.cast_to_string(System.get_property("ftpProxyHost"))
            end
            if ((result).nil?)
              # as a last resort we use the general one if ftp.useProxy
              # is true
              if (Boolean.get_boolean("ftp.useProxy"))
                result = RJava.cast_to_string(System.get_property("proxyHost"))
              end
            end
            return result
          end
        ...
        Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
          ...
          define_method :run do
            ...
            if ((tmp).nil?)
              ...
              tmp = RJava.cast_to_string(System.get_property("ftpProxyPort"))
            end
            if ((tmp).nil?)
              # as a last resort we use the general one if ftp.useProxy
              # is true
              if (Boolean.get_boolean("ftp.useProxy"))
                tmp = RJava.cast_to_string(System.get_property("proxyPort"))
              end
            end
            if (!(tmp).nil?)
          ...
          if ((raw_list).nil?)
            ...
            if (!(raw_list == self.attr_non_proxy_hosts_source))
              ...
              begin
                ...
                System.err.println("Error in http.nonProxyHosts system property: " + RJava.cast_to_string(ex))
              end
    ...
    def issue_command_check(cmd)
      if (!(issue_command(cmd)).equal?(self.attr_ftp_success))
        raise FtpProtocolException.new(cmd + ":" + RJava.cast_to_string(get_response_string))
      end
    end
    ...
    def read_reply
      ...
      when 5
        ...
        if ((@last_reply_code).equal?(550))
          raise FileNotFoundException.new(@command + ": " + RJava.cast_to_string(get_response_string))
        end
      end
    ...
    def open_passive_data_connection
      ...
      if ((issue_command("EPSV ALL")).equal?(self.attr_ftp_success))
        ...
        if ((issue_command("EPSV")).equal?(self.attr_ftp_error))
          raise FtpProtocolException.new("EPSV Failed: " + RJava.cast_to_string(get_response_string))
        end
        ...
        if ((issue_command("PASV")).equal?(self.attr_ftp_error))
          raise FtpProtocolException.new("PASV failed: " + RJava.cast_to_string(get_response_string))
        end
        server_answer = RJava.cast_to_string(get_response_string)
    ...
    def open_data_connection(cmd)
      ...
      begin
        ...
        port_cmd = "EPRT |" + RJava.cast_to_string(((my_address.is_a?(Inet6Address)) ? "2" : "1")) + "|" + RJava.cast_to_string(my_address.get_host_address) + "|" + RJava.cast_to_string(port_socket.get_local_port) + "|"
        if ((issue_command(port_cmd)).equal?(self.attr_ftp_error) || (issue_command(cmd)).equal?(self.attr_ftp_error))
          ...
          while i < addr.attr_length
            port_cmd = port_cmd + RJava.cast_to_string((addr[i] & 0xff)) + ","
            i += 1
          end
          # append port number
          port_cmd = port_cmd + RJava.cast_to_string(((port_socket.get_local_port >> 8) & 0xff)) + "," + RJava.cast_to_string((port_socket.get_local_port & 0xff))
          if ((issue_command(port_cmd)).equal?(self.attr_ftp_error))
            e = FtpProtocolException.new("PORT :" + RJava.cast_to_string(get_response_string))
            raise e
          end
          if ((issue_command(cmd)).equal?(self.attr_ftp_error))
            e = FtpProtocolException.new(cmd + ":" + RJava.cast_to_string(get_response_string))
            raise e
          end
        end
    ...
    def open_server(host, port)
      ...
      super(host, port)
      if ((read_reply).equal?(self.attr_ftp_error))
        raise FtpProtocolException.new("Welcome message: " + RJava.cast_to_string(get_response_string))
      end
    end
    ...
    def login(user, password)
      ...
      if ((issue_command("USER " + user)).equal?(self.attr_ftp_error))
        raise FtpLoginException.new("user " + user + " : " + RJava.cast_to_string(get_response_string))
      end
      ...
      if ((@last_reply_code).equal?(331))
        if (((password).nil?) || ((password.length).equal?(0)) || ((issue_command("PASS " + password)).equal?(self.attr_ftp_error)))
          raise FtpLoginException.new("password: " + RJava.cast_to_string(get_response_string))
        end
      end
      ...
      while i < self.attr_server_response.size
        l = RJava.cast_to_string(self.attr_server_response.element_at(i))
        if (!(l).nil?)
          if (l.length >= 4 && l.starts_with("230"))
            # get rid of the "230-" prefix
            l = RJava.cast_to_string(l.substring(4))
          end
          sb.append(l)
        end
        i += 1
      end
      @welcome_msg = RJava.cast_to_string(sb.to_s)
      @logged_in = true
    end
    ...
    def get(filename)
      ...
      begin
        ...
        while (t.has_more_elements)
          path_element = RJava.cast_to_string(t.next_token)
    ...
    def pwd
      ...
      answ = RJava.cast_to_string(get_response_string)
