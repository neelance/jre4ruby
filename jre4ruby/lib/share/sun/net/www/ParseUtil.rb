  ...
  class ParseUtil 
    ...
    def canonize_string(file)
      ...
      while ((i = file.index_of("/../")) >= 0)
        if ((lim = file.last_index_of(Character.new(?/.ord), i - 1)) >= 0)
          file = RJava.cast_to_string(file.substring(0, lim) + file.substring(i + 3))
        else
          file = RJava.cast_to_string(file.substring(i + 3))
        end
      end
      # Remove embedded /./
      while ((i = file.index_of("/./")) >= 0)
        file = RJava.cast_to_string(file.substring(0, i) + file.substring(i + 2))
      end
      # Remove trailing ..
      while (file.ends_with("/.."))
        i = file.index_of("/..")
        if ((lim = file.last_index_of(Character.new(?/.ord), i - 1)) >= 0)
          file = RJava.cast_to_string(file.substring(0, lim + 1))
        else
          file = RJava.cast_to_string(file.substring(0, i))
        end
      end
      # Remove trailing .
      if (file.ends_with("/."))
        file = RJava.cast_to_string(file.substring(0, file.length - 1))
      end
      return file
    end
    ...
    class_module.module_eval {
      ...
      def file_to_encoded_url(file)
        ...
        path = RJava.cast_to_string(ParseUtil.encode_path(path))
        if (!path.starts_with("/"))
          path = "/" + path
        end
          ...
          if (opaque_part.starts_with("//["))
            ...
            if (!(end_).equal?(-1) && !(opaque_part.index_of(":")).equal?(-1))
              ...
              if ((end_).equal?(opaque_part.length))
                ...
                dontquote = RJava.cast_to_string(opaque_part.substring(0, end_ + 1))
                doquote = RJava.cast_to_string(opaque_part.substring(end_ + 1))
              end
              sb.append(dontquote)
            ...
            if (authority.starts_with("["))
              ...
              if (!(end_).equal?(-1) && !(authority.index_of(":")).equal?(-1))
                ...
                if ((end_).equal?(authority.length))
                  ...
                  dontquote = RJava.cast_to_string(authority.substring(0, end_ + 1))
                  doquote = RJava.cast_to_string(authority.substring(end_ + 1))
                end
                sb.append(dontquote)
      ...
      def append_encoded(sb, c)
        bb = nil
        begin
          bb = ThreadLocalCoders.encoder_for("UTF-8").encode(CharBuffer.wrap("" + RJava.cast_to_string(c)))
