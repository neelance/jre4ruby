  ...
  class MessageHeader 
    ...
    def print(p)
      synchronized(self) do
        ...
        while i < @nkeys
          if (!(@keys[i]).nil?)
            p.print(RJava.cast_to_string(@keys[i] + (!(@values[i]).nil? ? ": " + RJava.cast_to_string(@values[i]) : "")) + "\r\n")
          end
          i += 1
        end
        p.print("\r\n")
    ...
    def to_s
      synchronized(self) do
        result = RJava.cast_to_string(super + @nkeys) + " pairs: "
        i = 0
        while i < @keys.attr_length && i < @nkeys
          result += "{" + RJava.cast_to_string(@keys[i]) + ": " + RJava.cast_to_string(@values[i]) + "}"
          i += 1
        end
        return result
      end
