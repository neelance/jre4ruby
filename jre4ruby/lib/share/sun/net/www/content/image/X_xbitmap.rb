...
module Sun::Net::Www::Content::Image
  ...
  class X_xbitmap < X_xbitmapImports.const_get :ContentHandler
    include_class_members X_xbitmapImports
    ...
    def get_content(urlc, classes)
      ...
      while i < classes.attr_length
        if (classes[i].is_assignable_from(URLImageSource))
          return URLImageSource.new(urlc)
        end
        if (classes[i].is_assignable_from(Image))
    ...
    private
    alias_method :initialize_x_xbitmap, :initialize
  end
