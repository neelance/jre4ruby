...
module Sun::Net::Www::Protocol::Jar
  ...
  class Handler < Java::Net::URLStreamHandler
    ...
    def parse_url(url, spec, start, limit)
      ...
      if (ref_pos > -1)
        ref = RJava.cast_to_string(spec.substring(ref_pos + 1, spec.length))
        if (ref_only)
          file = RJava.cast_to_string(url.get_file)
        end
      end
      ...
      spec = RJava.cast_to_string(spec.substring(start, limit))
      if (absolute_spec)
        file = RJava.cast_to_string(parse_absolute_spec(spec))
      else
        if (!ref_only)
          file = RJava.cast_to_string(parse_context_spec(url, spec))
          ...
          after_bang_slash = RJava.cast_to_string(canonizer.canonize_string(after_bang_slash))
    ...
    def parse_absolute_spec(spec)
      ...
      begin
        ...
        raise NullPointerException.new("invalid url: " + spec + " (" + RJava.cast_to_string(e) + ")")
      end
      return spec
    end
    ...
    def parse_context_spec(url, spec)
      ...
      if (spec.starts_with("/"))
        ...
        if ((bang_slash).equal?(-1))
          raise NullPointerException.new("malformed " + "context url:" + RJava.cast_to_string(url) + ": no !/")
        end
        ctx_file = RJava.cast_to_string(ctx_file.substring(0, bang_slash))
      end
      if (!ctx_file.ends_with("/") && (!spec.starts_with("/")))
        ...
        if ((last_slash).equal?(-1))
          raise NullPointerException.new("malformed " + "context url:" + RJava.cast_to_string(url))
        end
        ctx_file = RJava.cast_to_string(ctx_file.substring(0, last_slash + 1))
      end
