...
module Sun::Net::Www::Protocol::Jar
  module URLJarFileImports #:nodoc:
    ...
    def get_entry(name)
      ...
      if (!(ze).nil?)
        if (ze.is_a?(JarEntry))
          ...
          raise InternalError.new(RJava.cast_to_string(JarFile.instance_method(:get_class).bind(self).call) + " returned unexpected entry type " + RJava.cast_to_string(ze.get_class))
        end
      end
      return nil
    end
