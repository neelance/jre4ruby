...
module Sun::Net::Www::Protocol::Http
  module NegotiateAuthenticationImports #:nodoc:
    ...
    def set_headers(conn, p, raw)
      synchronized(self) do
        begin
          ...
          response = @scheme + " " + RJava.cast_to_string(B64Encoder.new_local(self).encode((incoming).nil? ? first_token : next_token(incoming)))
  ...
  class Negotiator 
    ...
    class_module.module_eval {
      ...
      def get_supported(hostname, scheme)
        ...
        c = clazz.get_constructor(String, String)
