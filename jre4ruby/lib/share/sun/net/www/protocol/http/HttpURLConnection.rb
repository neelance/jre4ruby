...
module Sun::Net::Www::Protocol::Http
  module HttpURLConnectionImports #:nodoc:
  ...
  class HttpURLConnection < Java::Net::HttpURLConnection
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        ...
        const_set :Version, RJava.cast_to_string(Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("java.version")))
    ...
    def write_requests
      ...
      if (!@set_requests)
        ...
        # request.
        if (!@failed_once)
          @requests.prepend(RJava.cast_to_string(self.attr_method) + " " + RJava.cast_to_string(@http.get_urlfile) + " " + HttpVersion, nil)
        end
        if (!get_use_caches)
        ...
        if (!(port).equal?(-1) && !(port).equal?(self.attr_url.get_default_port))
          host += ":" + RJava.cast_to_string(String.value_of(port))
        end
      ...
      synchronized(self) do
        begin
          ...
          if (!("POST" == self.attr_method) && !("PUT" == self.attr_method) && ("http" == self.attr_url.get_protocol))
            raise ProtocolException.new("HTTP method " + RJava.cast_to_string(self.attr_method) + " doesn't support output")
          end
    ...
    def set_cookie_header
      if (!(@cookie_handler).nil?)
        ...
        if (@set_user_cookies)
          ...
          if (!(k).equal?(-1))
            @user_cookies = RJava.cast_to_string(@requests.get_value(k))
          end
          @set_user_cookies = false
        end
        ...
        if (!(@user_cookies).nil?)
          ...
          if (!((k = @requests.get_key("Cookie"))).equal?(-1))
            @requests.set("Cookie", RJava.cast_to_string(@requests.get_value(k)) + ";" + @user_cookies)
          else
    ...
    def get_input_stream
      synchronized(self) do
        ...
        begin
          begin
              ...
              if (!(server_authentication.is_a?(DigestAuthentication)) || ((@domain).nil?))
                if (server_authentication.is_a?(BasicAuthentication))
                  ...
                  if (!opath.starts_with(npath) || npath.length >= opath.length)
                    ...
                    npath = RJava.cast_to_string(BasicAuthentication.get_root_path(opath, npath))
                  end
            ...
            if (resp_code >= 400)
              if ((resp_code).equal?(404) || (resp_code).equal?(410))
                ...
                raise Java::Io::IOException.new("Server returned HTTP" + " response code: " + RJava.cast_to_string(resp_code) + " for URL: " + RJava.cast_to_string(self.attr_url.to_s))
              end
            end
            @poster = nil
            @str_output_stream = nil
          ...
          raise ProtocolException.new("Server redirected too many " + " times (" + RJava.cast_to_string(redirects) + ")")
        rescue RuntimeException => e
          disconnect_internal
      ...
      synchronized(self) do
        ...
        begin
          begin
            ...
            status_line = RJava.cast_to_string(@responses.get_value(0))
            ...
            if ((resp_code).equal?(HTTP_PROXY_AUTH))
              ...
              if (!@doing_ntlmp2nd_stage)
                proxy_authentication = reset_proxy_authentication(proxy_authentication, authhdr)
                if (!(proxy_authentication).nil?)
                  proxy_host = RJava.cast_to_string(@http.get_proxy_host_used)
                  proxy_port = @http.get_proxy_port_used
                  disconnect_internal
                  retry_tunnel += 1
                  next
    ...
    def send_connectrequest
      ...
      @requests.prepend("CONNECT " + RJava.cast_to_string(self.attr_url.get_host) + ":" + RJava.cast_to_string((!(port).equal?(-1) ? port : self.attr_url.get_default_port)) + " " + HttpVersion, nil)
      @requests.set_if_not_set("User-Agent", UserAgent)
      host = self.attr_url.get_host
      if (!(port).equal?(-1) && !(port).equal?(self.attr_url.get_default_port))
        host += ":" + RJava.cast_to_string(String.value_of(port))
    ...
    def get_server_authentication(authhdr)
      ...
      if (authhdr.is_present)
        ...
        @domain = RJava.cast_to_string(p.find_value("domain"))
        if ((realm).nil?)
          realm = ""
        end
    ...
    def follow_redirect
      ...
      if ((stat).equal?(HTTP_USE_PROXY))
        ...
        set_proxied_client(self.attr_url, proxy_host, proxy_port)
        @requests.set(0, RJava.cast_to_string(self.attr_method) + " " + RJava.cast_to_string(@http.get_urlfile) + " " + HttpVersion, nil)
        self.attr_connected = true
      else
