...
module Sun::Net::Www::Protocol::Http
  module BasicAuthenticationImports #:nodoc:
  ...
  class BasicAuthentication < BasicAuthenticationImports.const_get :AuthenticationInfo
    ...
    def initialize(is_proxy, host, port, realm, pw)
      ...
      plain = RJava.cast_to_string(pw.get_user_name) + ":"
      name_bytes = nil
      begin
      ...
      @auth = "Basic " + RJava.cast_to_string((Sun::Misc::BASE64Encoder.new).encode(concat))
      self.attr_pw = pw
    end
    ...
    class_module.module_eval {
      ...
      def get_root_path(npath, opath)
        ...
        begin
          npath = RJava.cast_to_string(URI.new(npath).normalize.get_path)
          opath = RJava.cast_to_string(URI.new(opath).normalize.get_path)
