...
module Sun::Net::Www::Protocol::Ftp
  ...
  class FtpURLConnection < FtpURLConnectionImports.const_get :URLConnection
    ...
    def initialize(url, p)
      ...
      @read_timeout = -1
      @inst_proxy = p
      @host = RJava.cast_to_string(url.get_host)
      @port = url.get_port
      ...
      if (!(user_info).nil?)
        ...
        if ((delimiter).equal?(-1))
          @user = RJava.cast_to_string(ParseUtil.decode(user_info))
          @password = RJava.cast_to_string(nil)
        else
          @user = RJava.cast_to_string(ParseUtil.decode(user_info.substring(0, ((delimiter += 1) - 1))))
          @password = RJava.cast_to_string(ParseUtil.decode(user_info.substring(delimiter)))
        end
      end
    end
    ...
    def connect
      synchronized(self) do
        ...
        if ((@user).nil?)
          ...
          @password = RJava.cast_to_string(Java::Security::AccessController.do_privileged(GetPropertyAction.new("ftp.protocol.user", "Java" + vers + "@")))
        end
        begin
          if (!(p).nil?)
            @ftp = FtpClient.new(p)
    ...
    def decode_path(path)
      ...
      if (i >= 0)
        ...
        path = RJava.cast_to_string(path.substring(0, i))
      end
      if (!(path).nil? && path.length > 1 && (path.char_at(0)).equal?(Character.new(?/.ord)))
        path = RJava.cast_to_string(path.substring(1))
      end
      ...
      if (!path.ends_with("/"))
        ...
        if (i > 0)
          @filename = RJava.cast_to_string(path.substring(i + 1, path.length))
          @filename = RJava.cast_to_string(ParseUtil.decode(@filename))
          @pathname = RJava.cast_to_string(path.substring(0, i))
        else
          @filename = RJava.cast_to_string(ParseUtil.decode(path))
          @pathname = RJava.cast_to_string(nil)
        end
      else
        @pathname = RJava.cast_to_string(path.substring(0, path.length - 1))
        @filename = RJava.cast_to_string(nil)
      end
      if (!(@pathname).nil?)
        @fullpath = @pathname + "/" + RJava.cast_to_string((!(@filename).nil? ? @filename : ""))
      else
        @fullpath = @filename
      end
    end
      ...
      begin
        ...
        begin
          ...
          if (!((offset = response.index_of(" bytes)"))).equal?(-1))
            ...
            msgh.add("content-length", "" + RJava.cast_to_string(i))
            if (i > 0)
        ...
        if ((type_).nil? && @is.mark_supported)
          type_ = RJava.cast_to_string(guess_content_type_from_stream(@is))
        end
        if (!(type_).nil?)
    ...
    def get_permission
      if ((@permission).nil?)
        ...
        host = RJava.cast_to_string(@host) + ":" + RJava.cast_to_string(port)
    ...
    def get_request_property(key)
      ...
      if ((value).nil?)
        if (("type" == key))
          value = RJava.cast_to_string(((@type).equal?(ASCII) ? "a" : (@type).equal?(DIR) ? "d" : "i"))
        end
      end
      return value
    end
