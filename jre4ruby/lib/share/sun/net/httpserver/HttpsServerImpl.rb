...
module Sun::Net::Httpserver
  module HttpsServerImplImports #:nodoc:
  ...
  class HttpsServerImpl < HttpsServerImplImports.const_get :HttpsServer
    include_class_members HttpsServerImplImports
    ...
    typesig { [] }
    def initialize
      initialize__https_server_impl(InetSocketAddress.new(443), 0)
    end
    ...
    def initialize(addr, backlog)
      @server = nil
      super()
      @server = ServerImpl.new(self, "https", addr, backlog)
    end
    
    typesig { [HttpsConfigurator] }
    def set_https_configurator(config)
      @server.set_https_configurator(config)
    end
    
    typesig { [] }
    def get_https_configurator
      return @server.get_https_configurator
    end
    ...
    private
    alias_method :initialize__https_server_impl, :initialize
  end
