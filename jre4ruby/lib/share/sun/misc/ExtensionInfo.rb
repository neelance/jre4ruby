...
# have any questions.
module Sun::Misc
  module ExtensionInfoImports #:nodoc:
  ...
  class ExtensionInfo 
    ...
    def initialize(extension_key, attr)
      ...
      attr_key = s + RJava.cast_to_string(Name::EXTENSION_NAME.to_s)
      @name = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@name).nil?)
        @name = RJava.cast_to_string(@name.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::SPECIFICATION_TITLE.to_s)
      @title = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@title).nil?)
        @title = RJava.cast_to_string(@title.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::SPECIFICATION_VERSION.to_s)
      @spec_version = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@spec_version).nil?)
        @spec_version = RJava.cast_to_string(@spec_version.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::SPECIFICATION_VENDOR.to_s)
      @spec_vendor = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@spec_vendor).nil?)
        @spec_vendor = RJava.cast_to_string(@spec_vendor.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::IMPLEMENTATION_VERSION.to_s)
      @implementation_version = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@implementation_version).nil?)
        @implementation_version = RJava.cast_to_string(@implementation_version.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::IMPLEMENTATION_VENDOR.to_s)
      @vendor = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@vendor).nil?)
        @vendor = RJava.cast_to_string(@vendor.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::IMPLEMENTATION_VENDOR_ID.to_s)
      @vendor_id = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@vendor_id).nil?)
        @vendor_id = RJava.cast_to_string(@vendor_id.trim)
      end
      attr_key = s + RJava.cast_to_string(Name::IMPLEMENTATION_URL.to_s)
      @url = RJava.cast_to_string(attr.get_value(attr_key))
      if (!(@url).nil?)
        @url = RJava.cast_to_string(@url.trim)
      end
    end
      ...
      if ((pr_index).equal?(-1) && (patch_index).equal?(-1))
        ...
        if (!(patch_index).equal?(-1))
          ...
          if (!(pr_string.index_of("ea")).equal?(-1))
            ms_version = RJava.cast_to_string(pr_string.substring(2))
            delta = 50
          else
            if (!(pr_string.index_of("alpha")).equal?(-1))
              ms_version = RJava.cast_to_string(pr_string.substring(5))
              delta = 40
            else
              if (!(pr_string.index_of("beta")).equal?(-1))
                ms_version = RJava.cast_to_string(pr_string.substring(4))
                delta = 30
              else
