        ...
        synchronized(self) do
          ...
          if (index_file.exists)
            begin
              ...
              while (!((line = RJava.cast_to_string(reader.read_line))).nil?)
                case (line.char_at(0))
                when Character.new(?!.ord), Character.new(?#.ord), Character.new(?@.ord)
                  ...
                  cur_jar_name = RJava.cast_to_string(line.substring(2))
