...
module Sun::Misc
  ...
  class JarIndex 
    ...
    def add(file_name, jar_name)
      ...
      pos = 0
      if (!((pos = file_name.last_index_of("/"))).equal?(-1))
        package_name = RJava.cast_to_string(file_name.substring(0, pos))
      else
    ...
    def parse_jars(files)
      ...
      while i < files.attr_length
        current_jar = RJava.cast_to_string(files[i])
        ...
        while i < @jar_files.attr_length
          ...
          if (!(jarlist).nil?)
            ...
            while (listitr.has_next)
              bw.write(RJava.cast_to_string((listitr.next_)) + "\n")
            end
          end
          bw.write("\n")
          i += 1
    ...
    def read(is)
      ...
      while (!((line = RJava.cast_to_string(br.read_line))).nil? && !line.ends_with(".jar"))
      end
      while !(line).nil?
        if ((line.length).equal?(0))
          line = RJava.cast_to_string(br.read_line)
          next
        end
    ...
    def merge(to_index, path)
      ...
      while (itr.has_next)
        ...
        while (list_itr.has_next)
          jar_name = list_itr.next_
          if (!(path).nil?)
            jar_name = RJava.cast_to_string(path.concat(jar_name))
          end
