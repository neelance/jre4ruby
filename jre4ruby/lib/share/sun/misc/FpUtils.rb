...
module Sun::Misc
  ...
  class FpUtils 
    ...
    class_module.module_eval {
      ...
      def two_to_the_double_scale_up
        defined?(@@two_to_the_double_scale_up) ? @@two_to_the_double_scale_up : @@two_to_the_double_scale_up= power_of_two_d(512)
      end
      alias_method :attr_two_to_the_double_scale_up, :two_to_the_double_scale_up
      
      def two_to_the_double_scale_up=(value)
        @@two_to_the_double_scale_up = value
      end
      alias_method :attr_two_to_the_double_scale_up=, :two_to_the_double_scale_up=
      
      
      def two_to_the_double_scale_down
        defined?(@@two_to_the_double_scale_down) ? @@two_to_the_double_scale_down : @@two_to_the_double_scale_down= power_of_two_d(-512)
      end
      alias_method :attr_two_to_the_double_scale_down, :two_to_the_double_scale_down
      
      def two_to_the_double_scale_down=(value)
        @@two_to_the_double_scale_down = value
      end
      alias_method :attr_two_to_the_double_scale_down=, :two_to_the_double_scale_down=
      ...
      def get_exponent(f)
        ...
        # get true exponent value
        return ((Float.float_to_raw_int_bits(f) & FloatConsts::EXP_BIT_MASK) >> (FloatConsts::SIGNIFICAND_WIDTH - 1)) - FloatConsts::EXP_BIAS
      end
      ...
      def power_of_two_d(n)
        ...
        return Double.long_bits_to_double(((n + DoubleConsts::EXP_BIAS) << (DoubleConsts::SIGNIFICAND_WIDTH - 1)) & DoubleConsts::EXP_BIT_MASK)
      end
      ...
      def power_of_two_f(n)
        ...
        return Float.int_bits_to_float(((n + FloatConsts::EXP_BIAS) << (FloatConsts::SIGNIFICAND_WIDTH - 1)) & FloatConsts::EXP_BIT_MASK)
      end
      ...
      # @author Joseph D. Darcy
      def raw_copy_sign(magnitude, sign)
        return Double.long_bits_to_double((Double.double_to_raw_long_bits(sign) & (DoubleConsts::SIGN_BIT_MASK)) | (Double.double_to_raw_long_bits(magnitude) & (DoubleConsts::EXP_BIT_MASK | DoubleConsts::SIGNIF_BIT_MASK)))
      end
      ...
      # @author Joseph D. Darcy
      def raw_copy_sign(magnitude, sign)
        return Float.int_bits_to_float((Float.float_to_raw_int_bits(sign) & (FloatConsts::SIGN_BIT_MASK)) | (Float.float_to_raw_int_bits(magnitude) & (FloatConsts::EXP_BIT_MASK | FloatConsts::SIGNIF_BIT_MASK)))
      end
      ...
      # <code>false</code> otherwise.
      def is_unordered(arg1, arg2)
        return is_na_n(arg1) || is_na_n(arg2)
      end
      ...
      def scalb(d, scale_factor)
        ...
        max_scale = DoubleConsts::MAX_EXPONENT + -DoubleConsts::MIN_EXPONENT + DoubleConsts::SIGNIFICAND_WIDTH + 1
        exp_adjust = 0
        scale_increment = 0
      ...
      def scalb(f, scale_factor)
        ...
        return (((f).to_f * power_of_two_d(scale_factor))).to_f
      end
        ...
        if (is_na_n(start) || is_na_n(direction))
          ...
          if ((start).equal?(direction))
            ...
            if (direction > start)
              ...
              raise AssertError if not (direction < start)
              if (transducer > 0)
                (transducer -= 1)
              else
                if (transducer < 0)
            ...
            return Double.long_bits_to_double(transducer)
          end
        end
      end
      ...
      def next_after(start, direction)
        ...
        if (is_na_n(start) || is_na_n(direction))
          # return a NaN derived from the input NaN(s)
          return start + (direction).to_f
        else
          if ((start).equal?(direction))
            return (direction).to_f
          else
            # start > direction or start < direction
            # Add +0.0 to get rid of a -0.0 (+0.0 + -0.0 => +0.0)
            # then bitwise convert start to integer.
            ...
            return Float.int_bits_to_float(transducer)
          end
        end
      end
      ...
      # @author Joseph D. Darcy
      def next_up(d)
        if (is_na_n(d) || (d).equal?(Double::POSITIVE_INFINITY))
          return d
        else
          d += 0.0
          return Double.long_bits_to_double(Double.double_to_raw_long_bits(d) + ((d >= 0.0) ? +1 : -1))
        end
      end
      ...
      # @author Joseph D. Darcy
      def next_up(f)
        if (is_na_n(f) || (f).equal?(FloatConsts::POSITIVE_INFINITY))
          return f
        else
          f += 0.0
          return Float.int_bits_to_float(Float.float_to_raw_int_bits(f) + ((f >= 0.0) ? +1 : -1))
        end
      end
      ...
      # @author Joseph D. Darcy
      def next_down(d)
        if (is_na_n(d) || (d).equal?(Double::NEGATIVE_INFINITY))
          return d
        else
          if ((d).equal?(0.0))
      ...
      # @author Joseph D. Darcy
      def next_down(f)
        if (is_na_n(f) || (f).equal?(Float::NEGATIVE_INFINITY))
          return f
        else
          if ((f).equal?(0.0))
      ...
      def copy_sign(magnitude, sign)
        return raw_copy_sign(magnitude, (is_na_n(sign) ? 1.0 : sign))
      end
      ...
      # @since 1.5
      def signum(d)
        return ((d).equal?(0.0) || is_na_n(d)) ? d : copy_sign(1.0, d)
      end
      ...
      # @since 1.5
      def signum(f)
        return ((f).equal?(0.0) || is_na_n(f)) ? f : copy_sign(1.0, f)
      end
    }
    
    private
    alias_method :initialize__fp_utils, :initialize
  end
