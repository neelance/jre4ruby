...
module Sun::Nio::Cs::Ext
  module ISO2022_CN_GBImports #:nodoc:
  ...
  class ISO2022_CN_GB < ISO2022_CN_GBImports.const_get :ISO2022
    include_class_members ISO2022_CN_GBImports
    overload_protected {
    ...
    typesig { [] }
    def initialize
      super("x-ISO-2022-CN-GB", ExtendedCharsets.aliases_for("x-ISO-2022-CN-GB"))
    end
    
    typesig { [Charset] }
    def contains(cs)
      # overlapping repertoire of EUC_CN, GB2312
      return ((cs.is_a?(EUC_CN)) || ((cs.name == "US-ASCII")) || (cs.is_a?(ISO2022_CN_GB)))
    end
    ...
    class_module.module_eval {
      const_set_lazy(:Encoder) { Class.new(ISO2022::Encoder) do
        include_class_members ISO2022_CN_GB
        ...
        def initialize(cs)
          ...
          SODesig = "$)A"
          begin
            cset = Charset.for_name("EUC_CN") # GB2312
    ...
    private
    alias_method :initialize__iso2022_cn_gb, :initialize
  end
