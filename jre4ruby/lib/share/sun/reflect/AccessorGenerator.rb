...
# have any questions.
module Sun::Reflect
  module AccessorGeneratorImports #:nodoc:
  ...
  class AccessorGenerator 
    ...
    class_module.module_eval {
      ...
      def get_class_name(c, add_prefix_and_suffix_for_non_primitive_types)
        if (c.is_primitive)
          ...
          if (c.is_array)
            return "[" + RJava.cast_to_string(get_class_name(c.get_component_type, true))
          else
            if (add_prefix_and_suffix_for_non_primitive_types)
              return internalize("L" + RJava.cast_to_string(c.get_name) + ";")
            else
    ...
    def unboxing_method_for_primitive_type(prim_type)
      ...
      raise InternalError.new("Illegal primitive type " + RJava.cast_to_string(prim_type.get_name))
    end
