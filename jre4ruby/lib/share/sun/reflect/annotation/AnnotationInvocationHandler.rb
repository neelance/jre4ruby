...
module Sun::Reflect::Annotation
  module AnnotationInvocationHandlerImports #:nodoc:
  ...
  class AnnotationInvocationHandler 
    ...
    def invoke(proxy, method, args)
      ...
      if ((member == "equals") && (param_types.attr_length).equal?(1) && (param_types[0]).equal?(Object))
      ...
      @member_values.entry_set.each do |memberValue|
        ...
        if (!(member_type).nil?)
          ...
          if (!(member_type.is_instance(value) || value.is_a?(ExceptionProxy)))
            member_value.set_value(AnnotationTypeMismatchExceptionProxy.new(RJava.cast_to_string(value.get_class) + "[" + RJava.cast_to_string(value) + "]").set_member(annotation_type.members.get(name)))
          end
        end
      end
    end
