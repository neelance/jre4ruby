...
module Sun::Reflect::Annotation
  module AnnotationTypeImports #:nodoc:
  ...
  class AnnotationType 
    ...
    def initialize(annotation_class)
      ...
      methods.each do |method|
        if (!(method.get_parameter_types.attr_length).equal?(0))
          raise IllegalArgumentException.new(RJava.cast_to_string(method) + " has params")
        end
      ...
      if (!(annotation_class).equal?(Retention) && !(annotation_class).equal?(Inherited))
        ret = annotation_class.get_annotation(Retention)
        @retention = ((ret).nil? ? RetentionPolicy::CLASS : ret.value)
        @inherited = annotation_class.is_annotation_present(Inherited)
      end
    end
    ...
    def to_s
      ...
      s.append("   Member types: " + RJava.cast_to_string(@member_types) + "\n")
      s.append("   Member defaults: " + RJava.cast_to_string(@member_defaults) + "\n")
      s.append("   Retention policy: " + RJava.cast_to_string(@retention) + "\n")
      s.append("   Inherited: " + RJava.cast_to_string(@inherited))
      return s.to_s
    end
