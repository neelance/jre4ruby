...
module Sun::Reflect::Generics::Tree
  module IntSignatureImports #:nodoc:
  ...
  class IntSignature 
    include_class_members IntSignatureImports
    include BaseType
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= IntSignature.new
      end
    ...
    def accept(v)
      v.visit_int_signature(self)
    end
    
    private
    alias_method :initialize__int_signature, :initialize
  end
