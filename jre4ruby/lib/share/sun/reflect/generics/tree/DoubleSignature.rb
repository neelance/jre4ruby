...
module Sun::Reflect::Generics::Tree
  module DoubleSignatureImports #:nodoc:
  ...
  class DoubleSignature 
    include_class_members DoubleSignatureImports
    include BaseType
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= DoubleSignature.new
      end
    ...
    def accept(v)
      v.visit_double_signature(self)
    end
    
    private
    alias_method :initialize__double_signature, :initialize
  end
