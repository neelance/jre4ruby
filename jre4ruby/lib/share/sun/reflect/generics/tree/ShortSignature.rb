...
module Sun::Reflect::Generics::Tree
  module ShortSignatureImports #:nodoc:
  ...
  class ShortSignature 
    include_class_members ShortSignatureImports
    include BaseType
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= ShortSignature.new
      end
    ...
    def accept(v)
      v.visit_short_signature(self)
    end
    
    private
    alias_method :initialize__short_signature, :initialize
  end
