...
module Sun::Reflect::Generics::Tree
  module FloatSignatureImports #:nodoc:
  ...
  class FloatSignature 
    include_class_members FloatSignatureImports
    include BaseType
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= FloatSignature.new
      end
    ...
    def accept(v)
      v.visit_float_signature(self)
    end
    
    private
    alias_method :initialize__float_signature, :initialize
  end
