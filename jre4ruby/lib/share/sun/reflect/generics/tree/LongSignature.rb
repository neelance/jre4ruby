...
module Sun::Reflect::Generics::Tree
  module LongSignatureImports #:nodoc:
  ...
  class LongSignature 
    include_class_members LongSignatureImports
    include BaseType
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= LongSignature.new
      end
    ...
    def accept(v)
      v.visit_long_signature(self)
    end
    
    private
    alias_method :initialize__long_signature, :initialize
  end
