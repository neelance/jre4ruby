...
module Sun::Reflect::Generics::Tree
  module BottomSignatureImports #:nodoc:
  ...
  class BottomSignature 
    include_class_members BottomSignatureImports
    include FieldTypeSignature
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= BottomSignature.new
      end
    ...
    def accept(v)
      v.visit_bottom_signature(self)
    end
    
    private
    alias_method :initialize__bottom_signature, :initialize
  end
