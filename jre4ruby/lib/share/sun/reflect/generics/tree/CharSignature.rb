...
module Sun::Reflect::Generics::Tree
  module CharSignatureImports #:nodoc:
  ...
  class CharSignature 
    include_class_members CharSignatureImports
    include BaseType
    
    class_module.module_eval {
      
      def singleton
        defined?(@@singleton) ? @@singleton : @@singleton= CharSignature.new
      end
    ...
    def accept(v)
      v.visit_char_signature(self)
    end
    
    private
    alias_method :initialize__char_signature, :initialize
  end
