...
module Sun::Reflect::Generics::Parser
  module SignatureParserImports #:nodoc:
  ...
  class SignatureParser 
    ...
    def parse_class_type_signature
      ...
      if (!(current).equal?(Character.new(?;.ord)))
        raise error("expected ';' got '" + RJava.cast_to_string(current) + "'")
      end
      advance
    ...
    def parse_type_variable_signature
      ...
      if (!(current).equal?(Character.new(?;.ord)))
        raise error("; expected in signature of type variable named" + RJava.cast_to_string(ts.get_identifier))
      end
      advance
      return ts
    end
