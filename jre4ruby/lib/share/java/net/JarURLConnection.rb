...
# have any questions.
module Java::Net
  module JarURLConnectionImports #:nodoc:
  ...
  class JarURLConnection < JarURLConnectionImports.const_get :URLConnection
    ...
    def parse_specs(url)
      ...
      @entry_name = RJava.cast_to_string(nil)
      # if ! is the last letter of the innerURL, entryName is null
      if (!((separator += 1)).equal?(spec.length))
        @entry_name = RJava.cast_to_string(spec.substring(separator, spec.length))
        @entry_name = RJava.cast_to_string(ParseUtil.decode(@entry_name))
      end
    end
