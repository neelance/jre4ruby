...
# have any questions.
module Java::Net
  module SocksSocketImplImports #:nodoc:
  ...
  class SocksSocketImpl < SocksSocketImplImports.const_get :PlainSocketImpl
    ...
    def initialize(proxy)
      ...
      if (a.is_a?(InetSocketAddress))
        ...
        @server = RJava.cast_to_string(ad.get_host_string)
        @port = ad.get_port
      end
    end
    ...
    def authenticate(method, in_, out)
      ...
      if ((method).equal?(USER_PASSW))
        ...
        if (!(pw).nil?)
          user_name = RJava.cast_to_string(pw.get_user_name)
          ...
          if (!(user_name).nil?)
            ...
            user_name = RJava.cast_to_string(Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("user.name")))
          end
        end
        if ((user_name).nil?)
          return false
    ...
    def connect_v4(in_, out, endpoint)
      ...
      if (!(n).equal?(8))
        raise SocketException.new("Reply from SOCKS server has bad length: " + RJava.cast_to_string(n))
      end
    ...
    def connect(endpoint, timeout)
      ...
      if ((@server).nil?)
        ...
        begin
          uri = URI.new("socket://" + RJava.cast_to_string(ParseUtil.encode_path(host)) + ":" + RJava.cast_to_string(epoint.get_port))
        ...
        while (i_proxy.has_next)
          ...
          if (!(p.type).equal?(Proxy::Type::SOCKS))
            raise SocketException.new("Unknown proxy type : " + RJava.cast_to_string(p.type))
          end
          if (!(p.address.is_a?(InetSocketAddress)))
            raise SocketException.new("Unknow address type for proxy: " + RJava.cast_to_string(p))
          end
          # Use getHostString() to avoid reverse lookups
          @server = RJava.cast_to_string((p.address).get_host_string)
          ...
          begin
            ...
            @server = RJava.cast_to_string(nil)
            @port = -1
            saved_exc = e
        ...
        # worked
        if ((@server).nil?)
          raise SocketException.new("Can't connect to SOCKS proxy:" + RJava.cast_to_string(saved_exc.get_message))
        end
      else
        # Connects to the SOCKS server
        begin
    ...
    def socks_bind(saddr)
      synchronized(self) do
        ...
        if ((@server).nil?)
          ...
          begin
            uri = URI.new("serversocket://" + RJava.cast_to_string(ParseUtil.encode_path(host)) + ":" + RJava.cast_to_string(saddr.get_port))
          rescue URISyntaxException => e
            # This shouldn't happen
            raise AssertError, RJava.cast_to_string(e) if not (false)
          end
        ...
        if (saddr.is_unresolved)
          ...
          if (saddr.get_address.is_a?(Inet4Address))
            ...
            if (saddr.get_address.is_a?(Inet6Address))
              ...
              @cmdsock.close
              raise SocketException.new("unsupported address type : " + RJava.cast_to_string(saddr))
            end
          end
        end
        data = Array.typed(::Java::Byte).new(4) { 0 }
