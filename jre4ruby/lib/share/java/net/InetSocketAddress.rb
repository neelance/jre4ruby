...
# have any questions.
module Java::Net
  module InetSocketAddressImports #:nodoc:
  ...
  class InetSocketAddress < InetSocketAddressImports.const_get :SocketAddress
    ...
    def initialize(addr, port)
      ...
      if (port < 0 || port > 0xffff)
        raise IllegalArgumentException.new("port out of range:" + RJava.cast_to_string(port))
      end
      @port = port
      if ((addr).nil?)
    ...
    def read_object(s)
      ...
      if (@port < 0 || @port > 0xffff)
        raise InvalidObjectException.new("port out of range:" + RJava.cast_to_string(@port))
      end
    ...
    def to_s
      if (is_unresolved)
        return @hostname + ":" + RJava.cast_to_string(@port)
      else
        return RJava.cast_to_string(@addr.to_s) + ":" + RJava.cast_to_string(@port)
      end
    end
