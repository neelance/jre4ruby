...
# have any questions.
module Java::Net
  module Inet6AddressImports #:nodoc:
  ...
  class Inet6Address < Inet6AddressImports.const_get :InetAddress
    ...
    def read_object(s)
      ...
      if (!(@ipaddress.attr_length).equal?(INADDRSZ))
        raise InvalidObjectException.new("invalid address length: " + RJava.cast_to_string(@ipaddress.attr_length))
      end
    ...
    def get_host_address
      ...
      if (@scope_ifname_set)
        ...
        s = s + "%" + RJava.cast_to_string(@scope_ifname.get_name)
      else
        if (@scope_id_set)
          s = s + "%" + RJava.cast_to_string(@scope_id)
        end
      end
      return s
    end
    ...
    def write_object(s)
      synchronized(self) do
        if (@scope_ifname_set)
          @ifname = RJava.cast_to_string(@scope_ifname.get_name)
        end
