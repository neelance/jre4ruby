...
module Java::Net
  ...
  class URL 
    ...
    def initialize(protocol, host, port, file, handler)
      ...
      if (!(host).nil?)
        ...
        @host = host
        if (port < -1)
          raise MalformedURLException.new("Invalid port number :" + RJava.cast_to_string(port))
        end
        @port = port
        @authority = RJava.cast_to_string(((port).equal?(-1)) ? host : host + ":" + RJava.cast_to_string(port))
      end
      parts = Parts.new(file)
      @path = RJava.cast_to_string(parts.get_path)
      @query = RJava.cast_to_string(parts.get_query)
      if (!(@query).nil?)
        @file = @path + "?" + @query
      else
        @file = @path
      end
      @ref = RJava.cast_to_string(parts.get_ref)
      ...
      begin
        ...
        if ((!(context).nil?) && (((new_protocol).nil?) || new_protocol.equals_ignore_case(context.attr_protocol)))
          ...
          if (!(context.attr_path).nil? && context.attr_path.starts_with("/"))
            new_protocol = RJava.cast_to_string(nil)
          end
          if ((new_protocol).nil?)
            @protocol = RJava.cast_to_string(context.attr_protocol)
            @authority = RJava.cast_to_string(context.attr_authority)
            @user_info = RJava.cast_to_string(context.attr_user_info)
            @host = RJava.cast_to_string(context.attr_host)
            @port = context.attr_port
            @file = RJava.cast_to_string(context.attr_file)
            @path = RJava.cast_to_string(context.attr_path)
            is_relative = true
          end
        end
        if ((@protocol).nil?)
        ...
        if (i >= 0)
          @ref = RJava.cast_to_string(spec.substring(i + 1, limit))
          limit = i
        end
        # Handle special case inheritance of query and fragment
        # implied by RFC2396 section 5.2.2.
        if (is_relative && (start).equal?(limit))
          @query = RJava.cast_to_string(context.attr_query)
          if ((@ref).nil?)
            @ref = RJava.cast_to_string(context.attr_ref)
          end
        end
      ...
      def get_urlstream_handler(protocol)
        ...
        if ((handler).nil?)
          ...
          if ((handler).nil?)
            ...
            package_prefix_list = RJava.cast_to_string(Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new(ProtocolPathProp, "")))
    ...
    def read_object(s)
      synchronized(self) do
        ...
        @path = RJava.cast_to_string(nil)
        @query = RJava.cast_to_string(nil)
        if (!(@file).nil?)
  ...
  class Parts 
    ...
    def initialize(file)
      ...
      @ref = RJava.cast_to_string(ind < 0 ? nil : file.substring(ind + 1))
      file = RJava.cast_to_string(ind < 0 ? file : file.substring(0, ind))
      q = file.last_index_of(Character.new(??.ord))
      if (!(q).equal?(-1))
        @query = RJava.cast_to_string(file.substring(q + 1))
        @path = RJava.cast_to_string(file.substring(0, q))
