...
# have any questions.
module Java::Net
  module SocketPermissionImports #:nodoc:
  ...
  class SocketPermission < SocketPermissionImports.const_get :Permission
          ...
          if (!(host.char_at(0)).equal?(Character.new(?[.ord)))
            if (!((ind = host.index_of(Character.new(?:.ord)))).equal?(host.last_index_of(Character.new(?:.ord))))
              ...
              if ((tokens).equal?(9))
                ...
                host = "[" + RJava.cast_to_string(host.substring(0, ind)) + "]" + RJava.cast_to_string(host.substring(ind))
              else
    ...
    def init(host, mask)
      ...
      if ((host.char_at(0)).equal?(Character.new(?[.ord)))
        ...
        if (!(rb).equal?(-1))
          host = RJava.cast_to_string(host.substring(start, rb))
        else
        ...
        end_ = sep
        if (!(sep).equal?(-1))
          host = RJava.cast_to_string(host.substring(start, end_))
        end
      end
      if (!(sep).equal?(-1))
      ...
      if (host.last_index_of(Character.new(?*.ord)) > 0)
        ...
        if (host.starts_with("*"))
          ...
          if ((host == "*"))
            ...
            if (host.starts_with("*."))
              @cname = RJava.cast_to_string(host.substring(1).to_lower_case)
            else
              raise IllegalArgumentException.new("invalid host wildcard specification")
            end
          end
    ...
    def get_canon_name
      ...
      begin
        ...
        if (@init_with_ip)
          @cname = RJava.cast_to_string(@addresses[0].get_host_name(false).to_lower_case)
        else
          @cname = RJava.cast_to_string(InetAddress.get_by_name(@addresses[0].get_host_address).get_host_name(false).to_lower_case)
            ...
            if (!match(@cname, @hostname) && (@default_deny || !(@cname == @addresses[0].get_host_address)))
              ...
              if (!authorized(@hostname, @addresses[0].get_address))
                ...
                if (!(debug).nil? && Debug.is_on("failure"))
                  debug.println("socket access restriction: proxied host " + "(" + RJava.cast_to_string(@addresses[0]) + ")" + " does not match " + @cname + " from reverse lookup")
                end
              end
            end
          end
    ...
    def fragment(cname)
      ...
      if ((dot).equal?(cname.length - 1))
        cname = RJava.cast_to_string(cname.substring(0, cname.length - 1))
        dot = cname.last_index_of(Character.new(?..ord))
      end
      if (dot < 1)
        return ""
    ...
    def authorized_ipv4(cname, addr)
      ...
      auth = nil
      begin
        auth_host = "auth." + RJava.cast_to_string((addr[3] & 0xff)) + "." + RJava.cast_to_string((addr[2] & 0xff)) + "." + RJava.cast_to_string((addr[1] & 0xff)) + "." + RJava.cast_to_string((addr[0] & 0xff)) + ".in-addr.arpa"
        # auth = InetAddress.getAllByName0(authHost, false)[0];
        auth_host = @hostname + RJava.cast_to_string(Character.new(?..ord)) + auth_host
        ...
        debug = get_debug
        if (!(debug).nil? && Debug.is_on("failure"))
          debug.println("socket access restriction: IP address of " + RJava.cast_to_string(auth) + " != " + RJava.cast_to_string(InetAddress.get_by_address(addr)))
        end
      rescue UnknownHostException => uhe
        debug_ = get_debug
    ...
    def authorized_ipv6(cname, addr)
      ...
      begin
        ...
        auth_host = "auth." + RJava.cast_to_string(sb.to_s) + "IP6.ARPA"
        # auth = InetAddress.getAllByName0(authHost, false)[0];
        auth_host = @hostname + RJava.cast_to_string(Character.new(?..ord)) + auth_host
        auth = InetAddress.get_all_by_name0(auth_host, false)[0]
        if ((auth == InetAddress.get_by_address(addr)))
    ...
    def get_ip
      ...
      begin
        ...
        if ((get_name.char_at(0)).equal?(Character.new(?[.ord)))
          ...
          host = RJava.cast_to_string(get_name.substring(1, get_name.index_of(Character.new(?].ord))))
        else
          i = get_name.index_of(":")
          if ((i).equal?(-1))
            host = RJava.cast_to_string(get_name)
          else
            host = RJava.cast_to_string(get_name.substring(0, i))
          end
        end
