...
# have any questions.
module Java::Net
  module URLStreamHandlerImports #:nodoc:
  ...
  class URLStreamHandler 
    ...
    def parse_url(u, spec, start, limit)
      ...
      if (start < limit)
        ...
        if ((!(query_start).equal?(-1)) && (query_start < limit))
          query = RJava.cast_to_string(spec.substring(query_start + 1, limit))
          if (limit > query_start)
            limit = query_start
          end
          spec = RJava.cast_to_string(spec.substring(0, query_start))
        end
      end
      i = 0
      ...
      if (!is_uncname && (start <= limit - 2) && ((spec.char_at(start)).equal?(Character.new(?/.ord))) && ((spec.char_at(start + 1)).equal?(Character.new(?/.ord))))
        ...
        host = RJava.cast_to_string(authority = RJava.cast_to_string(spec.substring(start, i)))
        ind = authority.index_of(Character.new(?@.ord))
        if (!(ind).equal?(-1))
          user_info = RJava.cast_to_string(authority.substring(0, ind))
          host = RJava.cast_to_string(authority.substring(ind + 1))
        else
          user_info = RJava.cast_to_string(nil)
        end
        if (!(host).nil?)
          ...
          if (host.length > 0 && ((host.char_at(0)).equal?(Character.new(?[.ord))))
            if ((ind = host.index_of(Character.new(?].ord))) > 2)
              nhost = host
              host = RJava.cast_to_string(nhost.substring(0, ind + 1))
            ...
            if (ind >= 0)
              ...
              host = RJava.cast_to_string(host.substring(0, ind))
            end
          end
        else
          host = ""
        ...
        if (port < -1)
          raise IllegalArgumentException.new("Invalid port number :" + RJava.cast_to_string(port))
        end
        start = i
      ...
      if (start < limit)
        if ((spec.char_at(start)).equal?(Character.new(?/.ord)))
          path = RJava.cast_to_string(spec.substring(start, limit))
        else
          if (!(path).nil? && path.length > 0)
            ...
            path = RJava.cast_to_string(path.substring(0, ind + 1)) + seperator + RJava.cast_to_string(spec.substring(start, limit))
          else
            seperator = (!(authority).nil?) ? "/" : ""
            path = seperator + RJava.cast_to_string(spec.substring(start, limit))
          end
        end
      else
        if (query_only && !(path).nil?)
          ...
          path = RJava.cast_to_string(path.substring(0, ind)) + "/"
        end
      end
      if ((path).nil?)
        path = ""
      ...
      if (is_rel_path)
        ...
        while ((i = path.index_of("/./")) >= 0)
          path = RJava.cast_to_string(path.substring(0, i) + path.substring(i + 2))
        end
        ...
        while ((i = path.index_of("/../", i)) >= 0)
          ...
          if (i > 0 && (limit = path.last_index_of(Character.new(?/.ord), i - 1)) >= 0 && (!(path.index_of("/../", limit)).equal?(0)))
            path = RJava.cast_to_string(path.substring(0, limit) + path.substring(i + 3))
            i = 0
          else
            i = i + 3
          end
        ...
        while (path.ends_with("/.."))
          ...
          if ((limit = path.last_index_of(Character.new(?/.ord), i - 1)) >= 0)
            path = RJava.cast_to_string(path.substring(0, limit + 1))
          else
            break
          end
        end
        ...
        if (path.starts_with("./") && path.length > 2)
          path = RJava.cast_to_string(path.substring(2))
        end
        # Remove trailing .
        if (path.ends_with("/."))
    ...
    def set_url(u, protocol, host, port, file, ref)
      ...
      if (!(host).nil? && !(host.length).equal?(0))
        authority = RJava.cast_to_string(((port).equal?(-1)) ? host : host + ":" + RJava.cast_to_string(port))
        at = host.last_index_of(Character.new(?@.ord))
        if (!(at).equal?(-1))
          user_info = RJava.cast_to_string(host.substring(0, at))
          host = RJava.cast_to_string(host.substring(at + 1))
        end
      end
      ...
      if (!(file).nil?)
        ...
        if (!(q).equal?(-1))
          query = RJava.cast_to_string(file.substring(q + 1))
          path = RJava.cast_to_string(file.substring(0, q))
        else
          path = file
        end
      end
