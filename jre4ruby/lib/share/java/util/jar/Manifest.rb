  ...
  class Manifest 
    ...
    def read(is)
      ...
      while (!((len = fis.read_line(lbuf))).equal?(-1))
        ...
        if ((name).nil?)
          name = RJava.cast_to_string(parse_name(lbuf, len))
          if ((name).nil?)
          ...
          name = RJava.cast_to_string(String.new(buf, 0, buf.attr_length, "UTF8"))
          lastline = nil
        end
