...
module Java::Util::Zip
  module ZipInputStreamImports #:nodoc:
  ...
  class ZipInputStream < ZipInputStreamImports.const_get :InflaterInputStream
    ...
    def read(b, off, len)
      ...
      when STORED
        ...
        if ((@remaining).equal?(0) && !(@entry.attr_crc).equal?(@crc.get_value))
          raise ZipException.new("invalid entry CRC (expected 0x" + RJava.cast_to_string(Long.to_hex_string(@entry.attr_crc)) + " but got 0x" + RJava.cast_to_string(Long.to_hex_string(@crc.get_value)) + ")")
        end
        return len
      else
    ...
    def read_end(e)
      ...
      if (!(e.attr_size).equal?(self.attr_inf.get_bytes_written))
        raise ZipException.new("invalid entry size (expected " + RJava.cast_to_string(e.attr_size) + " but got " + RJava.cast_to_string(self.attr_inf.get_bytes_written) + " bytes)")
      end
      if (!(e.attr_csize).equal?(self.attr_inf.get_bytes_read))
        raise ZipException.new("invalid entry compressed size (expected " + RJava.cast_to_string(e.attr_csize) + " but got " + RJava.cast_to_string(self.attr_inf.get_bytes_read) + " bytes)")
      end
      if (!(e.attr_crc).equal?(@crc.get_value))
        raise ZipException.new("invalid entry CRC (expected 0x" + RJava.cast_to_string(Long.to_hex_string(e.attr_crc)) + " but got 0x" + RJava.cast_to_string(Long.to_hex_string(@crc.get_value)) + ")")
      end
    end
