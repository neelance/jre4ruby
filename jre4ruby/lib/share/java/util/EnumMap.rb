  ...
  class EnumMap < EnumMapImports.const_get :AbstractMap
    ...
    def initialize(m)
      ...
      if (m.is_a?(EnumMap))
        ...
        @key_type = m.key_set.iterator.next_.get_declaring_class
        @key_universe = get_key_universe(@key_type)
        @vals = Array.typed(Object).new(@key_universe.attr_length) { nil }
        put_all(m)
      end
    ...
    def put_all(m)
      if (m.is_a?(EnumMap))
        ...
        if (!(em.attr_key_type).equal?(@key_type))
          ...
          raise ClassCastException.new(RJava.cast_to_string(em.attr_key_type) + " != " + RJava.cast_to_string(@key_type))
        end
        i = 0
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:EntryIterator) { Class.new(EnumMapIterator) do
        ...
        def to_s
          ...
          return RJava.cast_to_string(self.attr_key_universe[self.attr_last_returned_index]) + "=" + RJava.cast_to_string(unmask_null(self.attr_vals[self.attr_last_returned_index]))
        end
    ...
    def type_check(key)
      ...
      if (!(key_class).equal?(@key_type) && !(key_class.get_superclass).equal?(@key_type))
        raise ClassCastException.new(RJava.cast_to_string(key_class) + " != " + RJava.cast_to_string(@key_type))
      end
    end
