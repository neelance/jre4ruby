...
# have any questions.
module Java::Util
  module PropertiesImports #:nodoc:
  ...
  class Properties < PropertiesImports.const_get :Hashtable
    ...
    def store0(bw, comments, esc_unicode)
      ...
      synchronized((self)) do
        ...
        while e.has_more_elements
          key = e.next_element
          val = get(key)
          key = RJava.cast_to_string(save_convert(key, true, esc_unicode))
          # No need to escape embedded and trailing spaces for value, hence
          # pass false to flag.
          val = RJava.cast_to_string(save_convert(val, false, esc_unicode))
          bw.write(key + "=" + val)
          bw.new_line
        end
      end
    ...
    def list(out)
      ...
      while e.has_more_elements
        ...
        val = h.get(key)
        if (val.length > 40)
          val = RJava.cast_to_string(val.substring(0, 37)) + "..."
        end
        out.println(key + "=" + val)
      end
    end
