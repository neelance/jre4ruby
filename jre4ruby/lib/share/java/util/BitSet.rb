...
module Java::Util
  ...
  class BitSet 
    ...
    def initialize(nbits)
      ...
      if (nbits < 0)
        raise NegativeArraySizeException.new("nbits < 0: " + RJava.cast_to_string(nbits))
      end
      init_words(nbits)
      @size_is_sticky = true
    end
    ...
    class_module.module_eval {
      ...
      def check_range(from_index, to_index)
        if (from_index < 0)
          raise IndexOutOfBoundsException.new("fromIndex < 0: " + RJava.cast_to_string(from_index))
        end
        if (to_index < 0)
          raise IndexOutOfBoundsException.new("toIndex < 0: " + RJava.cast_to_string(to_index))
        end
        if (from_index > to_index)
          raise IndexOutOfBoundsException.new("fromIndex: " + RJava.cast_to_string(from_index) + " > toIndex: " + RJava.cast_to_string(to_index))
        end
      end
    }
    ...
    def flip(bit_index)
      if (bit_index < 0)
        raise IndexOutOfBoundsException.new("bitIndex < 0: " + RJava.cast_to_string(bit_index))
      end
