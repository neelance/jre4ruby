  ...
  class Matcher 
    ...
    def start(group)
      ...
      if (group > group_count)
        raise IndexOutOfBoundsException.new("No group " + RJava.cast_to_string(group))
      end
    ...
    def group(group_)
      ...
      if (group_ < 0 || group_ > group_count)
        raise IndexOutOfBoundsException.new("No group " + RJava.cast_to_string(group_))
      end
      ...
      while (cursor < replacement.length)
        ...
        if ((next_char).equal?(Character.new(?\\.ord)))
          ...
          if ((next_char).equal?(Character.new(?$.ord)))
            ...
            # Append group
            if (!(start(ref_num)).equal?(-1) && !(end_(ref_num)).equal?(-1))
              result.append(@text, start(ref_num), end_(ref_num))
            end
          else
            result.append(next_char)
            cursor += 1
    ...
    def region(start_, end__)
      ...
      if ((end__ < 0) || (end__ > get_text_length))
    ...
    def to_s
      ...
      sb.append("[pattern=" + RJava.cast_to_string(pattern))
      sb.append(" region=")
      sb.append(RJava.cast_to_string(region_start) + "," + RJava.cast_to_string(region_end))
      sb.append(" lastmatch=")
