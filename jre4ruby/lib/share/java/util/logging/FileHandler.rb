...
module Java::Util::Logging
  module FileHandlerImports #:nodoc:
  ...
  class FileHandler < FileHandlerImports.const_get :StreamHandler
    ...
    def configure
      ...
      @pattern = RJava.cast_to_string(manager.get_string_property(cname + ".pattern", "%h/java%u.log"))
    ...
    def open_files
      ...
      if (@count < 1)
        raise IllegalArgumentException.new("file count = " + RJava.cast_to_string(@count))
      end
      if (@limit < 0)
        @limit = 0
      end
      ...
      loop do
        ...
        @lock_file_name = RJava.cast_to_string(generate(@pattern, 0, unique).to_s) + ".lck"
      ...
      if (!(ex).nil?)
        if (ex.is_a?(IOException))
          ...
          if (ex.is_a?(SecurityException))
            ...
            raise IOException.new("Exception: " + RJava.cast_to_string(ex))
          end
        end
      end
    ...
    def generate(pattern, generation, unique)
      ...
      while (ix < pattern.length)
        ...
        if ((ch).equal?(Character.new(?/.ord)))
          ...
          if ((ch).equal?(Character.new(?%.ord)))
            if ((ch2).equal?(Character.new(?t.ord)))
              ...
              if ((tmp_dir).nil?)
                tmp_dir = RJava.cast_to_string(System.get_property("user.home"))
              end
              ...
              if ((ch2).equal?(Character.new(?h.ord)))
                ...
                if ((ch2).equal?(Character.new(?g.ord)))
                  word = word + RJava.cast_to_string(generation)
                  sawg = true
                  ix += 1
                  next
                else
                  if ((ch2).equal?(Character.new(?u.ord)))
                    word = word + RJava.cast_to_string(unique)
                    sawu = true
                    ix += 1
                    next
                  else
        ...
        word = word + RJava.cast_to_string(ch)
      end
      if (@count > 1 && !sawg)
        word = word + "." + RJava.cast_to_string(generation)
      end
    ...
    def close
      synchronized(self) do
        ...
        @lock_file_name = RJava.cast_to_string(nil)
        @lock_stream = nil
      end
    end
