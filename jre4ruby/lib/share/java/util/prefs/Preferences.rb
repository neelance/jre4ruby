...
module Java::Util::Prefs
  module PreferencesImports #:nodoc:
    class_module.module_eval {
      ...
      include_const ::Sun::Misc, :ServiceConfigurationError
  ...
  # @since   1.4
  class Preferences 
    include_class_members PreferencesImports
    
    class_module.module_eval {
      const_set_lazy(:Factory) { factory }
      const_attr_reader  :Factory
      ...
      def factory
        ...
        if (!(factory_name).nil?)
          ...
          begin
            return Class.for_name(factory_name, false, ClassLoader.get_system_class_loader).new_instance
            ...
            begin
              ...
              if (!(sm).nil?)
                sm.check_permission(Java::Security::AllPermission.new)
              end
              return Class.for_name(factory_name, false, JavaThread.current_thread.get_context_class_loader).new_instance
            rescue JavaException => e
              error = InternalError.new("Can't instantiate Preferences factory " + factory_name)
              error.init_cause(e)
              raise error
            end
          end
      ...
      def factory1
        ...
        i = Service.providers(PreferencesFactory, ClassLoader.get_system_class_loader)
        ...
        platform_factory = System.get_property("os.name").starts_with("Windows") ? "java.util.prefs.WindowsPreferencesFactory" : "java.util.prefs.FileSystemPreferencesFactory"
        begin
          return Class.for_name(platform_factory, false, nil).new_instance
        rescue JavaException => e
          error = InternalError.new("Can't instantiate platform default Preferences factory " + platform_factory)
          error.init_cause(e)
          raise error
        end
      end
      ...
      def node_name(c)
        ...
        pkg_end_index = class_name.last_index_of(Character.new(?..ord))
        if (pkg_end_index < 0)
          return "/<unnamed>"
        end
        package_name = class_name.substring(0, pkg_end_index)
        return "/" + RJava.cast_to_string(package_name.replace(Character.new(?..ord), Character.new(?/.ord)))
      end
      ...
      def prefs_perm
        defined?(@@prefs_perm) ? @@prefs_perm : @@prefs_perm= RuntimePermission.new("preferences")
      end
      alias_method :attr_prefs_perm, :prefs_perm
      
      def prefs_perm=(value)
        @@prefs_perm = value
      end
      alias_method :attr_prefs_perm=, :prefs_perm=
      ...
      def user_root
        ...
        if (!(security).nil?)
          security.check_permission(self.attr_prefs_perm)
        end
        return Factory.user_root
      end
    ...
    private
    alias_method :initialize__preferences, :initialize
  end
