...
module Java::Security
  module CodeSourceImports #:nodoc:
  ...
  class CodeSource 
    ...
    def match_location(that)
      ...
      if (@location.get_file.ends_with("/-"))
        ...
        if (@location.get_file.ends_with("/*"))
          ...
          if ((!(that.attr_location.get_file == @location.get_file)) && (!(that.attr_location.get_file == RJava.cast_to_string(@location.get_file) + "/")))
            return false
          end
        end
      end
    ...
    def to_s
      ...
      if (!(@certs).nil? && @certs.attr_length > 0)
        i = 0
        while i < @certs.attr_length
          sb.append(" " + RJava.cast_to_string(@certs[i]))
          i += 1
        end
      else
        if (!(@signers).nil? && @signers.attr_length > 0)
          i = 0
          while i < @signers.attr_length
            sb.append(" " + RJava.cast_to_string(@signers[i]))
            i += 1
          end
        else
