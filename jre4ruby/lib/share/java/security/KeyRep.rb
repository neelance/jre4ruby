  ...
  class KeyRep 
    ...
    def read_resolve
      begin
        if ((@type).equal?(Type::SECRET) && (RAW == @format))
          ...
          if ((@type).equal?(Type::PUBLIC) && (X509 == @format))
            ...
            if ((@type).equal?(Type::PRIVATE) && (PKCS8 == @format))
              ...
              raise NotSerializableException.new("unrecognized type/format combination: " + RJava.cast_to_string(@type) + "/" + @format)
            end
          end
        end
        ...
        nse = NotSerializableException.new("java.security.Key: " + "[" + RJava.cast_to_string(@type) + "] " + "[" + @algorithm + "] " + "[" + @format + "]")
        nse.init_cause(e)
        raise nse
      end
    end
