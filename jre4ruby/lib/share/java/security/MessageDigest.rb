...
module Java::Security
  module MessageDigestImports #:nodoc:
  ...
  class MessageDigest < MessageDigestImports.const_get :MessageDigestSpi
    ...
    def to_s
      ...
      p.print(@algorithm + " Message Digest from " + RJava.cast_to_string(@provider.get_name) + ", ")
      case (@state)
      when INITIAL
        p.print("<initialized>")
      when IN_PROGRESS
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:Delegate) { Class.new(MessageDigest) do
        ...
        typesig { [class_self::MessageDigestSpi, String] }
        # constructor
