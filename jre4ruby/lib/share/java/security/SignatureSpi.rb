...
module Java::Security
  module SignatureSpiImports #:nodoc:
  ...
  class SignatureSpi 
    ...
    def engine_sign(outbuf, offset, len)
      sig = engine_sign
      if (len < sig.attr_length)
        raise SignatureException.new("partial signatures not returned")
      end
      if (outbuf.attr_length - offset < sig.attr_length)
        raise SignatureException.new("insufficient space in the output buffer to store the " + "signature")
      end
      System.arraycopy(sig, 0, outbuf, offset, sig.attr_length)
      return sig.attr_length
    end
    ...
    def engine_verify(sig_bytes, offset, length)
      sig_bytes_copy = Array.typed(::Java::Byte).new(length) { 0 }
      System.arraycopy(sig_bytes, offset, sig_bytes_copy, 0, length)
      return engine_verify(sig_bytes_copy)
    end
    ...
    private
    alias_method :initialize__signature_spi, :initialize
  end
