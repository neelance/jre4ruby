...
module Java::Security::Cert
  module X509CRLSelectorImports #:nodoc:
  ...
  class X509CRLSelector 
    ...
    def to_s
      ...
      if (!(@min_crl).nil?)
        sb.append("  minCRLNumber: " + RJava.cast_to_string(@min_crl) + "\n")
      end
      if (!(@max_crl).nil?)
        sb.append("  maxCRLNumber: " + RJava.cast_to_string(@max_crl) + "\n")
      end
      if (!(@date_and_time).nil?)
        sb.append("  dateAndTime: " + RJava.cast_to_string(@date_and_time) + "\n")
      end
      if (!(@cert_checking).nil?)
        sb.append("  Certificate being checked: " + RJava.cast_to_string(@cert_checking) + "\n")
      end
      sb.append("]")
      return sb.to_s
    end
