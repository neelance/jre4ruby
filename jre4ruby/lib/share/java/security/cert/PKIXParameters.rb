...
module Java::Security::Cert
  module PKIXParametersImports #:nodoc:
  ...
  class PKIXParameters 
    ...
    def to_s
      ...
      if (!(@unmod_trust_anchors).nil?)
        sb.append("  Trust Anchors: " + RJava.cast_to_string(@unmod_trust_anchors.to_s) + "\n")
      end
      ...
      if (!(@unmod_initial_policies).nil?)
        if (@unmod_initial_policies.is_empty)
          ...
          sb.append("  Initial Policy OIDs: [" + RJava.cast_to_string(@unmod_initial_policies.to_s) + "]\n")
        end
      end
      # now, append constraints on all certificates in the path
      sb.append("  Validity Date: " + RJava.cast_to_string(String.value_of(@date)) + "\n")
      sb.append("  Signature Provider: " + RJava.cast_to_string(String.value_of(@sig_provider)) + "\n")
      sb.append("  Default Revocation Enabled: " + RJava.cast_to_string(@revocation_enabled) + "\n")
      sb.append("  Explicit Policy Required: " + RJava.cast_to_string(@explicit_policy_required) + "\n")
      sb.append("  Policy Mapping Inhibited: " + RJava.cast_to_string(@policy_mapping_inhibited) + "\n")
      sb.append("  Any Policy Inhibited: " + RJava.cast_to_string(@any_policy_inhibited) + "\n")
      sb.append("  Policy Qualifiers Rejected: " + RJava.cast_to_string(@policy_qualifiers_rejected) + "\n")
      # now, append target cert requirements
      sb.append("  Target Cert Constraints: " + RJava.cast_to_string(String.value_of(@cert_selector)) + "\n")
      # finally, append miscellaneous parameters
      if (!(@cert_path_checkers).nil?)
        sb.append("  Certification Path Checkers: [" + RJava.cast_to_string(@cert_path_checkers.to_s) + "]\n")
      end
      if (!(@cert_stores).nil?)
        sb.append("  CertStores: [" + RJava.cast_to_string(@cert_stores.to_s) + "]\n")
      end
      sb.append("]")
      return sb.to_s
    end
