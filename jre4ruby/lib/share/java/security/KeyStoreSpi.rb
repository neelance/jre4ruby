...
module Java::Security
  module KeyStoreSpiImports #:nodoc:
  ...
  class KeyStoreSpi 
    ...
    def engine_set_entry(alias_, entry, prot_param)
      ...
      raise KeyStoreException.new("unsupported entry type: " + RJava.cast_to_string(entry.get_class.get_name))
    end
    ...
    def engine_entry_instance_of(alias_, entry_class)
      if ((entry_class).equal?(KeyStore::TrustedCertificateEntry))
        return engine_is_certificate_entry(alias_)
      end
      if ((entry_class).equal?(KeyStore::PrivateKeyEntry))
        return engine_is_key_entry(alias_) && !(engine_get_certificate(alias_)).nil?
      end
      if ((entry_class).equal?(KeyStore::SecretKeyEntry))
        return engine_is_key_entry(alias_) && (engine_get_certificate(alias_)).nil?
      end
      return false
    end
