...
module Java::Lang
  ...
  class JavaThread 
    ...
    def initialize
      ...
      init(nil, nil, "Thread-" + RJava.cast_to_string(next_thread_num), 0)
    end
    ...
    def initialize(target)
      ...
      @throwable_from_stop = nil
      @uncaught_exception_handler = nil
      init(nil, target, "Thread-" + RJava.cast_to_string(next_thread_num), 0)
    end
    ...
    def initialize(group, target)
      ...
      @throwable_from_stop = nil
      @uncaught_exception_handler = nil
      init(group, target, "Thread-" + RJava.cast_to_string(next_thread_num), 0)
    end
    ...
    class_module.module_eval {
      ...
      def dump_stack
        JavaException.new("Stack trace").print_stack_trace
      end
    }
    ...
    def to_s
      ...
      if (!(group).nil?)
        return "Thread[" + RJava.cast_to_string(get_name) + "," + RJava.cast_to_string(get_priority) + "," + RJava.cast_to_string(group.get_name) + "]"
      else
        return "Thread[" + RJava.cast_to_string(get_name) + "," + RJava.cast_to_string(get_priority) + "," + "" + "]"
      end
    end
    ...
    def get_stack_trace
      if (!(self).equal?(JavaThread.current_thread))
        ...
        return (JavaException.new).get_stack_trace
      end
    end
