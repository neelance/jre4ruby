  ...
  class Package 
    ...
    def is_compatible_with(desired)
      ...
      while i < sa.attr_length
        ...
        if (si[i] < 0)
          raise NumberFormatException.for_input_string("" + RJava.cast_to_string(si[i]))
        end
        i += 1
      end
      ...
      while i_ < da.attr_length
        ...
        if (di[i_] < 0)
          raise NumberFormatException.for_input_string("" + RJava.cast_to_string(di[i_]))
        end
        i_ += 1
      end
    ...
    class_module.module_eval {
      ...
      def get_package(c)
        ...
        if (!(i).equal?(-1))
          name = RJava.cast_to_string(name.substring(0, i))
    ...
    class_module.module_eval {
      ...
      def get_system_package(name)
        synchronized((self.attr_pkgs)) do
          ...
          if ((pkg).nil?)
            name = RJava.cast_to_string(name.replace(Character.new(?..ord), Character.new(?/.ord)).concat("/"))
      ...
      def define_system_package(iname, fn)
        return AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          ...
          define_method :run do
            ...
            name = RJava.cast_to_string(name.substring(0, name.length - 1).replace(Character.new(?/.ord), Character.new(?..ord)))
            pkg = nil
