...
module Java::Lang::Management
  module MemoryUsageImports #:nodoc:
  ...
  class MemoryUsage 
    ...
    def initialize(init, used, committed, max)
      ...
      @max = 0
      if (init < -1)
        raise IllegalArgumentException.new("init parameter = " + RJava.cast_to_string(init) + " is negative but not -1.")
      end
      if (max < -1)
        raise IllegalArgumentException.new("max parameter = " + RJava.cast_to_string(max) + " is negative but not -1.")
      end
      if (used < 0)
        raise IllegalArgumentException.new("used parameter = " + RJava.cast_to_string(used) + " is negative.")
      end
      if (committed < 0)
        raise IllegalArgumentException.new("committed parameter = " + RJava.cast_to_string(committed) + " is negative.")
      end
      if (used > committed)
        raise IllegalArgumentException.new("used = " + RJava.cast_to_string(used) + " should be <= committed = " + RJava.cast_to_string(committed))
      end
      if (max >= 0 && committed > max)
        raise IllegalArgumentException.new("committed = " + RJava.cast_to_string(committed) + " should be < max = " + RJava.cast_to_string(max))
      end
      @init = init
      @used = used
      @committed = committed
    ...
    def to_s
      ...
      buf.append("init = " + RJava.cast_to_string(@init) + "(" + RJava.cast_to_string((@init >> 10)) + "K) ")
      buf.append("used = " + RJava.cast_to_string(@used) + "(" + RJava.cast_to_string((@used >> 10)) + "K) ")
      buf.append("committed = " + RJava.cast_to_string(@committed) + "(" + RJava.cast_to_string((@committed >> 10)) + "K) ")
      buf.append("max = " + RJava.cast_to_string(@max) + "(" + RJava.cast_to_string((@max >> 10)) + "K)")
      return buf.to_s
    end
