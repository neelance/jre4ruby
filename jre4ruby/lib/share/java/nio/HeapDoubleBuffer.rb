...
module Java::Nio
  module HeapDoubleBufferImports #:nodoc:
  ...
  class HeapDoubleBuffer < HeapDoubleBufferImports.const_get :DoubleBuffer
    ...
    def initialize(cap, lim)
      super(-1, 0, lim, cap, Array.typed(::Java::Double).new(cap) { 0.0 }, 0) # package-private
      # hb = new double[cap];
      # offset = 0;
    end
    ...
    typesig { [] }
    def slice
      return HeapDoubleBuffer.new(self.attr_hb, -1, 0, self.remaining, self.remaining, self.position + self.attr_offset)
    end
    
    typesig { [] }
    def duplicate
      return HeapDoubleBuffer.new(self.attr_hb, self.mark_value, self.position, self.limit, self.capacity, self.attr_offset)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return HeapDoubleBufferR.new(self.attr_hb, self.mark_value, self.position, self.limit, self.capacity, self.attr_offset)
    end
    ...
    private
    alias_method :initialize__heap_double_buffer, :initialize
  end
