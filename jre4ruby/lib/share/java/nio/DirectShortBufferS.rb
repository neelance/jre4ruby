...
module Java::Nio
  module DirectShortBufferSImports #:nodoc:
  ...
  class DirectShortBufferS < DirectShortBufferSImports.const_get :ShortBuffer
    include_class_members DirectShortBufferSImports
    overload_protected {
    ...
    def slice
      ...
      return DirectShortBufferS.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectShortBufferS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectShortBufferRS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [::Java::Int] }
    def ix(i)
      return self.attr_address + (i << 1)
    end
    
    typesig { [] }
    def get
      return (Bits.swap(UnsafeInstance.get_short(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return (Bits.swap(UnsafeInstance.get_short(ix(check_index(i)))))
    end
    ...
    def get(dst, offset, length)
      if ((length << 1) > Bits::JNI_COPY_TO_ARRAY_THRESHOLD)
        ...
        if (!(order).equal?(ByteOrder.native_order))
          Bits.copy_to_short_array(ix(pos), dst, offset << 1, length << 1)
        else
          Bits.copy_to_byte_array(ix(pos), dst, offset << 1, length << 1)
        end
        position(pos + length)
      else
    ...
    typesig { [::Java::Short] }
    def put(x)
      UnsafeInstance.put_short(ix(next_put_index), Bits.swap((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Short] }
    def put(i, x)
      UnsafeInstance.put_short(ix(check_index(i)), Bits.swap((x)))
      return self
    end
    
    typesig { [ShortBuffer] }
    def put(src)
      if (src.is_a?(DirectShortBufferS))
        if ((src).equal?(self))
    ...
    def put(src, offset, length)
      if ((length << 1) > Bits::JNI_COPY_FROM_ARRAY_THRESHOLD)
        ...
        if (!(order).equal?(ByteOrder.native_order))
          Bits.copy_from_short_array(src, offset << 1, ix(pos), length << 1)
        else
          Bits.copy_from_byte_array(src, offset << 1, ix(pos), length << 1)
        end
        position(pos + length)
      else
    ...
    private
    alias_method :initialize__direct_short_buffer_s, :initialize
  end
