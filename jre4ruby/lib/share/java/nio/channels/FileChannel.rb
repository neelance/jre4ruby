...
module Java::Nio::Channels
  module FileChannelImports #:nodoc:
  ...
  # @since 1.4
  class FileChannel < FileChannelImports.const_get :AbstractInterruptibleChannel
    include_class_members FileChannelImports
    overload_protected {
      include ByteChannel
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:MapMode) { Class.new do
        ...
        private
        alias_method :initialize__map_mode, :initialize
      end }
    }
    ...
    # @see     #lock(long,long,boolean)
    # @see     #tryLock()
    def try_lock(position, size, shared)
      raise NotImplementedError
    end
    ...
    # @see     #tryLock(long,long,boolean)
    def try_lock
      return try_lock(0, Long::MAX_VALUE, false)
    end
    
    private
    alias_method :initialize__file_channel, :initialize
  end
