...
module Java::Nio
  module DirectDoubleBufferSImports
  ...
  class DirectDoubleBufferS < DirectDoubleBufferSImports.const_get :DoubleBuffer
    include_class_members DirectDoubleBufferSImports
    overload_protected {
    ...
    def slice
      ...
      return DirectDoubleBufferS.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectDoubleBufferS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectDoubleBufferRS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return Double.long_bits_to_double(Bits.swap(UnsafeInstance.get_long(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return Double.long_bits_to_double(Bits.swap(UnsafeInstance.get_long(ix(check_index(i)))))
    end
    ...
    typesig { [::Java::Double] }
    def put(x)
      UnsafeInstance.put_long(ix(next_put_index), Bits.swap(Double.double_to_raw_long_bits(x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Double] }
    def put(i, x)
      UnsafeInstance.put_long(ix(check_index(i)), Bits.swap(Double.double_to_raw_long_bits(x)))
      return self
    end
    
    typesig { [DoubleBuffer] }
    def put(src)
      if (src.is_a?(DirectDoubleBufferS))
        if ((src).equal?(self))
    ...
    private
    alias_method :initialize__direct_double_buffer_s, :initialize
  end
