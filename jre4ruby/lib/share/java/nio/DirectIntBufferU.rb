...
module Java::Nio
  module DirectIntBufferUImports #:nodoc:
  ...
  class DirectIntBufferU < DirectIntBufferUImports.const_get :IntBuffer
    include_class_members DirectIntBufferUImports
    overload_protected {
    ...
    def slice
      ...
      return DirectIntBufferU.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectIntBufferU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectIntBufferRU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return ((UnsafeInstance.get_int(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return ((UnsafeInstance.get_int(ix(check_index(i)))))
    end
    ...
    typesig { [::Java::Int] }
    def put(x)
      UnsafeInstance.put_int(ix(next_put_index), ((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Int] }
    def put(i, x)
      UnsafeInstance.put_int(ix(check_index(i)), ((x)))
      return self
    end
    
    typesig { [IntBuffer] }
    def put(src)
      if (src.is_a?(DirectIntBufferU))
        if ((src).equal?(self))
    ...
    private
    alias_method :initialize__direct_int_buffer_u, :initialize
  end
