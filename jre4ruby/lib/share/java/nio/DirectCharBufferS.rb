...
module Java::Nio
  module DirectCharBufferSImports #:nodoc:
  ...
  class DirectCharBufferS < DirectCharBufferSImports.const_get :CharBuffer
    include_class_members DirectCharBufferSImports
    overload_protected {
    ...
    def slice
      ...
      return DirectCharBufferS.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectCharBufferS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    def sub_sequence(start, end_)
      ...
      off = (pos + start) << 1
      raise AssertError if not ((off >= 0))
      return DirectCharBufferS.new(self, -1, 0, sublen, sublen, off)
    end
    ...
    private
    alias_method :initialize__direct_char_buffer_s, :initialize
  end
