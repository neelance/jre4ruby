...
module Java::Nio
  module DirectCharBufferRUImports #:nodoc:
  ...
  class DirectCharBufferRU < DirectCharBufferRUImports.const_get :DirectCharBufferU
    include_class_members DirectCharBufferRUImports
    overload_protected {
    ...
    def slice
      ...
      return DirectCharBufferRU.new(self, -1, 0, rem, rem, off)
    end
    ...
    def sub_sequence(start, end_)
      ...
      off = (pos + start) << 1
      raise AssertError if not ((off >= 0))
      return DirectCharBufferRU.new(self, -1, 0, sublen, sublen, off)
    end
    ...
    private
    alias_method :initialize__direct_char_buffer_ru, :initialize
  end
