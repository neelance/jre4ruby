...
module Java::Nio
  module DirectLongBufferUImports
  ...
  class DirectLongBufferU < DirectLongBufferUImports.const_get :LongBuffer
    include_class_members DirectLongBufferUImports
    overload_protected {
    ...
    def slice
      ...
      return DirectLongBufferU.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectLongBufferU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectLongBufferRU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return ((Unsafe.get_long(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return ((Unsafe.get_long(ix(check_index(i)))))
    end
    ...
    typesig { [::Java::Long] }
    def put(x)
      Unsafe.put_long(ix(next_put_index), ((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Long] }
    def put(i, x)
      Unsafe.put_long(ix(check_index(i)), ((x)))
      return self
    end
    
    typesig { [LongBuffer] }
    def put(src)
      if (src.is_a?(DirectLongBufferU))
        if ((src).equal?(self))
    ...
    private
    alias_method :initialize__direct_long_buffer_u, :initialize
  end
