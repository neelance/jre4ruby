...
module Java::Nio
  module DirectShortBufferUImports #:nodoc:
  ...
  class DirectShortBufferU < DirectShortBufferUImports.const_get :ShortBuffer
    include_class_members DirectShortBufferUImports
    overload_protected {
    ...
    def slice
      ...
      return DirectShortBufferU.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectShortBufferU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return ((UnsafeInstance.get_short(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return ((UnsafeInstance.get_short(ix(check_index(i)))))
    end
    ...
    typesig { [::Java::Short] }
    def put(x)
      UnsafeInstance.put_short(ix(next_put_index), ((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Short] }
    def put(i, x)
      UnsafeInstance.put_short(ix(check_index(i)), ((x)))
      return self
    end
    
    typesig { [ShortBuffer] }
    def put(src)
      if (src.is_a?(DirectShortBufferU))
        if ((src).equal?(self))
    ...
    private
    alias_method :initialize__direct_short_buffer_u, :initialize
  end
