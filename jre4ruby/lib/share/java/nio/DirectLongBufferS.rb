...
module Java::Nio
  module DirectLongBufferSImports
  ...
  class DirectLongBufferS < DirectLongBufferSImports.const_get :LongBuffer
    include_class_members DirectLongBufferSImports
    overload_protected {
    ...
    def slice
      ...
      return DirectLongBufferS.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectLongBufferS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectLongBufferRS.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return (Bits.swap(Unsafe.get_long(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return (Bits.swap(Unsafe.get_long(ix(check_index(i)))))
    end
    ...
    typesig { [::Java::Long] }
    def put(x)
      Unsafe.put_long(ix(next_put_index), Bits.swap((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Long] }
    def put(i, x)
      Unsafe.put_long(ix(check_index(i)), Bits.swap((x)))
      return self
    end
    
    typesig { [LongBuffer] }
    def put(src)
      if (src.is_a?(DirectLongBufferS))
        if ((src).equal?(self))
        ...
        Unsafe.copy_memory(sb.ix(spos), ix(pos), srem << 3)
    ...
    def compact
      ...
      raise AssertError if not ((pos <= lim))
      rem = (pos <= lim ? lim - pos : 0)
      Unsafe.copy_memory(ix(pos), ix(0), rem << 3)
      position(rem)
      limit(capacity)
      discard_mark
      return self
    ...
    private
    alias_method :initialize__direct_long_buffer_s, :initialize
  end
