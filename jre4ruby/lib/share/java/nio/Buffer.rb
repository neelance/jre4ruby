...
module Java::Nio
  ...
  class Buffer 
    ...
    def initialize(mark, pos, lim, cap)
      ...
      if (cap < 0)
        raise IllegalArgumentException.new("Negative capacity: " + RJava.cast_to_string(cap))
      end
      @capacity = cap
      limit(lim)
      position(pos)
      if (mark >= 0)
        if (mark > pos)
          raise IllegalArgumentException.new("mark > position: (" + RJava.cast_to_string(mark) + " > " + RJava.cast_to_string(pos) + ")")
        end
        @mark = mark
      end
    end
