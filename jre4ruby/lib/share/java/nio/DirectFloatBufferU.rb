...
module Java::Nio
  module DirectFloatBufferUImports #:nodoc:
  ...
  class DirectFloatBufferU < DirectFloatBufferUImports.const_get :FloatBuffer
    include_class_members DirectFloatBufferUImports
    overload_protected {
    ...
    def slice
      ...
      return DirectFloatBufferU.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectFloatBufferU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectFloatBufferRU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return ((UnsafeInstance.get_float(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return ((UnsafeInstance.get_float(ix(check_index(i)))))
    end
    ...
    typesig { [::Java::Float] }
    def put(x)
      UnsafeInstance.put_float(ix(next_put_index), ((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Float] }
    def put(i, x)
      UnsafeInstance.put_float(ix(check_index(i)), ((x)))
      return self
    end
    
    typesig { [FloatBuffer] }
    def put(src)
      if (src.is_a?(DirectFloatBufferU))
        if ((src).equal?(self))
    ...
    private
    alias_method :initialize__direct_float_buffer_u, :initialize
  end
