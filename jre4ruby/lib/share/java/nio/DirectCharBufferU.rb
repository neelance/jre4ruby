...
module Java::Nio
  module DirectCharBufferUImports #:nodoc:
  ...
  class DirectCharBufferU < DirectCharBufferUImports.const_get :CharBuffer
    include_class_members DirectCharBufferUImports
    overload_protected {
    ...
    def slice
      ...
      return DirectCharBufferU.new(self, -1, 0, rem, rem, off)
    end
    
    typesig { [] }
    def duplicate
      return DirectCharBufferU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return DirectCharBufferRU.new(self, self.mark_value, self.position, self.limit, self.capacity, 0)
    end
    ...
    typesig { [] }
    def get
      return ((UnsafeInstance.get_char(ix(next_get_index))))
    end
    
    typesig { [::Java::Int] }
    def get(i)
      return ((UnsafeInstance.get_char(ix(check_index(i)))))
    end
    ...
    def get(dst, offset, length)
      if ((length << 1) > Bits::JNI_COPY_TO_ARRAY_THRESHOLD)
        ...
        if (!(order).equal?(ByteOrder.native_order))
          Bits.copy_to_char_array(ix(pos), dst, offset << 1, length << 1)
        else
    ...
    typesig { [::Java::Char] }
    def put(x)
      UnsafeInstance.put_char(ix(next_put_index), ((x)))
      return self
    end
    
    typesig { [::Java::Int, ::Java::Char] }
    def put(i, x)
      UnsafeInstance.put_char(ix(check_index(i)), ((x)))
      return self
    end
    
    typesig { [CharBuffer] }
    def put(src)
      if (src.is_a?(DirectCharBufferU))
        if ((src).equal?(self))
    ...
    def put(src, offset, length)
      if ((length << 1) > Bits::JNI_COPY_FROM_ARRAY_THRESHOLD)
        ...
        if (!(order).equal?(ByteOrder.native_order))
          Bits.copy_from_char_array(src, offset << 1, ix(pos), length << 1)
        else
    ...
    def sub_sequence(start, end_)
      ...
      off = (pos + start) << 1
      raise AssertError if not ((off >= 0))
      return DirectCharBufferU.new(self, -1, 0, sublen, sublen, off)
    end
    ...
    private
    alias_method :initialize__direct_char_buffer_u, :initialize
  end
