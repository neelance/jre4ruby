...
module Java::Nio
  module HeapIntBufferImports #:nodoc:
  ...
  class HeapIntBuffer < HeapIntBufferImports.const_get :IntBuffer
    include_class_members HeapIntBufferImports
    ...
    def initialize(cap, lim)
      super(-1, 0, lim, cap, Array.typed(::Java::Int).new(cap) { 0 }, 0) # package-private
      # hb = new int[cap];
      # offset = 0;
    end
    ...
    typesig { [] }
    def slice
      return HeapIntBuffer.new(self.attr_hb, -1, 0, self.remaining, self.remaining, self.position + self.attr_offset)
    end
    
    typesig { [] }
    def duplicate
      return HeapIntBuffer.new(self.attr_hb, self.mark_value, self.position, self.limit, self.capacity, self.attr_offset)
    end
    
    typesig { [] }
    def as_read_only_buffer
      return HeapIntBufferR.new(self.attr_hb, self.mark_value, self.position, self.limit, self.capacity, self.attr_offset)
    end
    ...
    private
    alias_method :initialize__heap_int_buffer, :initialize
  end
