...
# have any questions.
module Java::Io
  module RandomAccessFileImports #:nodoc:
  ...
  class RandomAccessFile 
    include_class_members RandomAccessFileImports
    include DataOutput
    include DataInput
    include Closeable
    ...
    class_module.module_eval {
      const_set_lazy(:O_RDONLY) { 1 }
      const_attr_reader  :O_RDONLY
      
      const_set_lazy(:O_RDWR) { 2 }
      const_attr_reader  :O_RDWR
      
      const_set_lazy(:O_SYNC) { 4 }
      const_attr_reader  :O_SYNC
      
      const_set_lazy(:O_DSYNC) { 8 }
      const_attr_reader  :O_DSYNC
    }
    ...
    def initialize(name, mode)
      initialize__random_access_file(!(name).nil? ? JavaFile.new(name) : nil, mode)
    end
    ...
    def initialize(file, mode)
      ...
      if (imode < 0)
        raise IllegalArgumentException.new("Illegal mode \"" + mode + "\" must be one of " + "\"r\", \"rw\", \"rws\"," + " or \"rwd\"")
      end
    ...
    def read_line
      ...
      while (!eol)
        ...
        when Character.new(?\r.ord)
          eol = true
          cur = get_file_pointer
          if (!((read)).equal?(Character.new(?\n.ord)))
            seek(cur)
          end
        else
          input.append(RJava.cast_to_char(c))
        end
      end
      if (((c).equal?(-1)) && ((input.length).equal?(0)))
        return nil
      end
      return input.to_s
    end
    ...
    def write_chars(s)
      ...
      c = CharArray.new(clen)
      s.get_chars(0, clen, c, 0)
      i = 0
      j = 0
      while i < clen
        b[((j += 1) - 1)] = (c[i] >> 8)
    ...
    # @exception  IOException  if an I/O error occurs.
    def write_utf(str)
      DataOutputStream.write_utf(str, self)
    end
    ...
    private
    alias_method :initialize__random_access_file, :initialize
  end
