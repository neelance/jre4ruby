...
# have any questions.
module Java::Io
  module ObjectOutputStreamImports #:nodoc:
  ...
  class ObjectOutputStream < ObjectOutputStreamImports.const_get :OutputStream
    ...
    def use_protocol_version(version)
      ...
      when PROTOCOL_VERSION_1, PROTOCOL_VERSION_2
        ...
        raise IllegalArgumentException.new("unknown version: " + RJava.cast_to_string(version))
      end
    end
    ...
    def verify_subclass
      cl = get_class
      if ((cl).equal?(ObjectOutputStream))
        return
      end
      sm = System.get_security_manager
      if ((sm).nil?)
          ...
          if (cl.is_array)
            ...
            if (obj.is_a?(Enum))
              ...
              if (obj.is_a?(Serializable))
                ...
                if (ExtendedDebugInfo)
                  raise NotSerializableException.new(RJava.cast_to_string(cl.get_name) + "\n" + RJava.cast_to_string(@debug_info_stack.to_s))
                else
    ...
    def write_array(array, desc, unshared)
      ...
      if (ccl.is_primitive)
        ...
        @bout.write_int(len)
        if (ExtendedDebugInfo)
          @debug_info_stack.push("array (class \"" + RJava.cast_to_string(array.get_class.get_name) + "\", size: " + RJava.cast_to_string(len) + ")")
        end
        begin
          i = 0
          while i < len
            if (ExtendedDebugInfo)
              @debug_info_stack.push("element of array (index: " + RJava.cast_to_string(i) + ")")
            end
            begin
    ...
    def write_enum(en, desc, unshared)
      ...
      write_class_desc(((sdesc.for_class).equal?(Enum)) ? desc : sdesc, false)
    ...
    def write_ordinary_object(obj, desc, unshared)
      if (ExtendedDebugInfo)
        @debug_info_stack.push(RJava.cast_to_string(((@depth).equal?(1) ? "root " : "")) + "object (class \"" + RJava.cast_to_string(obj.get_class.get_name) + "\", " + RJava.cast_to_string(obj.to_s) + ")")
      end
      begin
    ...
    def write_serial_data(obj, desc)
      ...
      while i < slots.attr_length
        ...
        if (slot_desc.has_write_object_method)
          ...
          @cur_put = nil
          if (ExtendedDebugInfo)
            @debug_info_stack.push("custom writeObject data (class \"" + RJava.cast_to_string(slot_desc.get_name) + "\")")
          end
          begin
            @bout.set_block_data_mode(true)
    ...
    def default_write_fields(obj, desc)
      ...
      while i < obj_vals.attr_length
        if (ExtendedDebugInfo)
          @debug_info_stack.push("field (class \"" + RJava.cast_to_string(desc.get_name) + "\", name: \"" + RJava.cast_to_string(fields[num_prim_fields + i].get_name) + "\", type: \"" + RJava.cast_to_string(fields[num_prim_fields + i].get_type) + "\")")
        end
        begin
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:PutFieldImpl) { Class.new(PutField) do
        ...
        def write_fields
          ...
          while i < @obj_vals.attr_length
            if (ExtendedDebugInfo)
              self.attr_debug_info_stack.push("field (class \"" + RJava.cast_to_string(@desc.get_name) + "\", name: \"" + RJava.cast_to_string(fields[num_prim_fields + i].get_name) + "\", type: \"" + RJava.cast_to_string(fields[num_prim_fields + i].get_type) + "\")")
            end
            begin
              write_object0(@obj_vals[i], fields[num_prim_fields + i].is_unshared)
            ensure
