...
# have any questions.
module Java::Io
  module StreamTokenizerImports #:nodoc:
  ...
  class StreamTokenizer 
    ...
    def next_token
      ...
      if (!((ctype & CT_ALPHA)).equal?(0))
        ...
        @peekc = c
        @sval = RJava.cast_to_string(String.copy_value_of(@buf, 0, i))
        if (@force_lower)
          @sval = RJava.cast_to_string(@sval.to_lower_case)
        end
    ...
    def to_s
      ret = nil
        ...
        when TT_WORD
          ret = @sval
        when TT_NUMBER
          ret = "n=" + RJava.cast_to_string(@nval)
        when TT_NOTHING
          ret = "NOTHING"
        else
          ...
          ret = RJava.cast_to_string(String.new(s))
        end
      end
      return "Token[" + ret + "], line " + RJava.cast_to_string(@lineno)
    end
