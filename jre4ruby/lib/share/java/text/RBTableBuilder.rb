...
module Java::Text
  ...
  class RBTableBuilder 
    ...
    def build(pattern, decmp)
      ...
      pattern = RJava.cast_to_string(NormalizerImpl.canonical_decompose_with_single_quotation(pattern))
      ...
      while i < @m_pattern.get_count
        ...
        if (!(entry).nil?)
          group_chars = RJava.cast_to_string(entry.get_chars)
          if (group_chars.length > 1)
            case (group_chars.char_at(group_chars.length - 1))
            when Character.new(?@.ord)
              @french_sec = true
              group_chars = RJava.cast_to_string(group_chars.substring(0, group_chars.length - 1))
            when Character.new(?!.ord)
          ...
          exp_chars = RJava.cast_to_string(entry.get_extension)
