  ...
  class MergeCollation 
    ...
    def fix_entry(new_entry)
      ...
      if (!(@last_entry).nil? && (new_entry.attr_chars == @last_entry.attr_chars) && (new_entry.attr_extension == @last_entry.attr_extension))
        if (!(new_entry.attr_strength).equal?(Collator::IDENTICAL) && !(new_entry.attr_strength).equal?(PatternEntry::RESET))
          raise ParseException.new("The entries " + RJava.cast_to_string(@last_entry) + " and " + RJava.cast_to_string(new_entry) + " are adjacent in the rules, but have conflicting " + "strengths: A character can't be unequal to itself.", -1)
        else
    ...
    def find_last_entry(entry, excess_chars)
      ...
      if (!(entry.attr_strength).equal?(PatternEntry::RESET))
        ...
        if (((old_index).equal?(-1)))
          raise ParseException.new("couldn't find last entry: " + RJava.cast_to_string(entry), old_index)
        end
        return old_index + 1
      else
        i = 0
        ...
        if ((i).equal?(-1))
          raise ParseException.new("couldn't find: " + RJava.cast_to_string(entry), i)
        end
        return i + 1
      end
    end
