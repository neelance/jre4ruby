  ...
  class SimpleDateFormat < SimpleDateFormatImports.const_get :DateFormat
    ...
    def initialize(time_style, date_style, loc)
      ...
      if ((date_time_patterns).nil?)
        ...
        if (!is_gregorian_calendar)
          begin
            date_time_patterns = r.get_string_array(RJava.cast_to_string(get_calendar_name) + ".DateTimePatterns")
      ...
      if ((time_style >= 0) && (date_style >= 0))
        ...
        @pattern = RJava.cast_to_string(MessageFormat.format(date_time_patterns[8], date_time_args))
      else
        if (time_style >= 0)
          @pattern = RJava.cast_to_string(date_time_patterns[time_style])
        else
          if (date_style >= 0)
            @pattern = RJava.cast_to_string(date_time_patterns[date_style + 4])
          else
    ...
    def compile(pattern)
      ...
      while i < length_
        ...
        if (((tag = DateFormatSymbols.attr_pattern_chars.index_of(c))).equal?(-1))
          raise IllegalArgumentException.new("Illegal pattern character " + "'" + RJava.cast_to_string(c) + "'")
        end
    ...
    def sub_format(pattern_char_index, count, delegate, buffer, use_date_format_symbols_)
      ...
      if (!use_date_format_symbols_)
        current = RJava.cast_to_string(self.attr_calendar.get_display_name(field, style, @locale))
      end
      ...
      when 0
        ...
        if (use_date_format_symbols_)
          ...
          if (value < eras.attr_length)
            current = RJava.cast_to_string(eras[value])
          end
        end
        if ((current).nil?)
          current = ""
      ...
      when 2
        ...
        if (use_date_format_symbols_)
          ...
          if (count >= 4)
            ...
            current = RJava.cast_to_string(months[value])
          else
            if ((count).equal?(3))
          ...
          if (count < 3)
            current = RJava.cast_to_string(nil)
          end
        end
        if ((current).nil?)
      ...
      when 9
        ...
        if (use_date_format_symbols_)
          ...
          if (count >= 4)
            ...
            current = RJava.cast_to_string(weekdays[value])
          else
      ...
      when 14
        ...
        if (use_date_format_symbols_)
          ...
          current = RJava.cast_to_string(ampm[value])
        end
      when 15
