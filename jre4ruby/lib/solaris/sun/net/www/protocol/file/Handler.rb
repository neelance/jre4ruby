    ...
    def open_connection(u, p)
      synchronized(self) do
        ...
        if ((host).nil? || (host == "") || (host == "~") || host.equals_ignore_case("localhost"))
          file = JavaFile.new(ParseUtil.decode(u.get_path))
          return create_file_urlconnection(u, file)
        end
        ...
        ru = nil
        begin
          ru = URL.new("ftp", host, u.get_file + ((u.get_ref).nil? ? "" : "#" + RJava.cast_to_string(u.get_ref)))
          if (!(p).nil?)
            uc = ru.open_connection(p)
          else
        ...
        if ((uc).nil?)
          raise IOException.new("Unable to connect to: " + RJava.cast_to_string(u.to_external_form))
        end
        return uc
      end
    end
