...
# have any questions.
module Sun::Nio::Ch
  module EPollSelectorImplImports #:nodoc:
  ...
  class EPollSelectorImpl < EPollSelectorImplImports.const_get :SelectorImpl
    include_class_members EPollSelectorImplImports
    ...
    def initialize(sp)
      ...
      @fd0 = fdes[0]
      @fd1 = fdes[1]
      @poll_wrapper = EPollArrayWrapper.new
    ...
    def do_select(timeout)
      ...
      if (@poll_wrapper.interrupted)
        ...
        @poll_wrapper.put_event_ops(@poll_wrapper.interrupted_index, 0)
    ...
    def update_selected_keys
      ...
      while i < entries
        ...
        if (!(ski).nil?)
          r_ops = @poll_wrapper.get_event_ops(i)
    ...
    typesig { [SelectionKeyImpl] }
    def impl_dereg(ski)
      raise AssertError if not ((ski.get_index >= 0))
    ...
    private
    alias_method :initialize__epoll_selector_impl, :initialize
  end
