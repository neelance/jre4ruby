    ...
    class_module.module_eval {
      ...
      def handle(sig, handler)
        synchronized(self) do
          ...
          if ((old_h).equal?(-1))
            raise IllegalArgumentException.new("Signal already used by VM or OS: " + (sig).to_s)
          end
      ...
      def raise(sig)
        if ((self.attr_handlers.get(sig)).nil?)
          raise IllegalArgumentException.new("Unhandled signal: " + (sig).to_s)
        end
      ...
      def dispatch(number)
        ...
        if (!(handler).nil?)
          JavaThread.new(runnable, (sig).to_s + " handler").start
        end
      end
