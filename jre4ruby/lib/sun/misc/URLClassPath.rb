  ...
  class URLClassPath 
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        const_set :JAVA_VERSION, (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("java.version"))).to_s
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:JarLoader) { Class.new(Loader) do
        ...
        def initialize(url, jar_handler, loader_map)
          ...
          @handler = nil
          @lmap = nil
          super(URL.new("jar", "", -1, (url).to_s + "!/", jar_handler))
          @csu = url
          @handler = jar_handler
          @lmap = loader_map
          if (!is_optimizable(url))
            ...
            if (!(file_name).nil?)
              file_name = (ParseUtil.decode(file_name)).to_s
          ...
          if ((@jar).nil?)
            begin
              Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedExceptionAction.class == Class ? Java::Security::PrivilegedExceptionAction : Object) do
                ...
                define_method :run do
                  if (DEBUG)
                    System.err.println("Opening " + (self.attr_csu).to_s)
        ...
        def valid_index(name)
          ...
          if (!((pos = name.last_index_of("/"))).equal?(-1))
            package_name = (name.substring(0, pos)).to_s
          end
          entry_name = nil
          entry = nil
          enum_ = @jar.entries
          while (enum_.has_more_elements)
            ...
            entry_name = (entry.get_name).to_s
            if (!((pos = entry_name.last_index_of("/"))).equal?(-1))
              entry_name = (entry_name.substring(0, pos)).to_s
            end
