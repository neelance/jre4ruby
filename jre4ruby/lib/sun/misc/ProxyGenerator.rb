  ...
  class ProxyGenerator 
    ...
    class_module.module_eval {
        ...
        if (SaveGeneratedFiles)
          Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
            ...
            define_method :run do
              begin
                file = FileOutputStream.new((dot_to_slash(name)).to_s + ".class")
                file.write(class_file)
                file.close
                return nil
              rescue IOException => e
                raise InternalError.new("I/O exception saving generated file: " + (e).to_s)
              end
            end
      ...
      when_class_loaded do
        begin
          self.attr_hash_code_method = Object.class.get_method("hashCode")
          self.attr_equals_method = Object.class.get_method("equals", Array.typed(Class).new([Object.class]))
          self.attr_to_string_method = Object.class.get_method("toString")
    ...
    def add_proxy_method(m, from_class)
      ...
      sig = name + (get_parameter_descriptors(parameter_types)).to_s
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:ProxyMethod) { Class.new do
        ...
        def initialize(method_name, parameter_types, return_type, exception_types, from_class)
          ...
          @method_field_name = "m" + (((self.attr_proxy_method_count += 1) - 1)).to_s
        end
    ...
    class_module.module_eval {
      ...
      def get_field_type(type)
        if (type.is_primitive)
          ...
          if (type.is_array)
            ...
            return "L" + (dot_to_slash(type.get_name)).to_s + ";"
          end
        end
      end
      ...
      def compute_unique_catch_list(exceptions)
        ...
        unique_list.add(JavaError.class) # always catch/rethrow these
        unique_list.add(RuntimeException.class)
        i = 0
      ...
      const_set_lazy(:PrimitiveTypeInfo) { Class.new do
        ...
        class_module.module_eval {
          ...
          when_class_loaded do
            add(Array, Byte.class)
            add(Array, Character.class)
            add(Array, Double.class)
            add(Array, Float.class)
            add(Array, JavaInteger.class)
            add(Array, Long.class)
            add(Array, Short.class)
            add(Array, Boolean.class)
          end
        ...
        def initialize(primitive_class, wrapper_class)
          ...
          @base_type_string = (Array.new_instance(primitive_class, 0).get_class.get_name.substring(1)).to_s
          @wrapper_class_name = (dot_to_slash(wrapper_class.get_name)).to_s
          @wrapper_value_of_desc = "(" + @base_type_string + ")L" + @wrapper_class_name + ";"
          @unwrap_method_name = (primitive_class.get_name).to_s + "Value"
      ...
      const_set_lazy(:ConstantPool) { Class.new do
        ...
        def get_value(key)
          ...
          if (!(index).nil?)
            ...
            if (@read_only)
              raise InternalError.new("late constant pool addition: " + (key).to_s)
            end
                  ...
                  if (@value.is_a?(Float))
                    ...
                    if (@value.is_a?(Long))
                      ...
                      if (@value.is_a?(Double))
                        ...
                        raise InternalError.new("bogus value entry: " + (@value).to_s)
                      end
                    end
                  end
                end
