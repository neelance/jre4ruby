...
# have any questions.
module Sun::Misc
  module ExtensionDependencyImports
  ...
  class ExtensionDependency 
      ...
      if (!(attr).nil?)
        ...
        if (!(value).nil?)
          ...
          while (st.has_more_tokens)
            ...
            debug("The file " + (jar.get_name).to_s + " appears to depend on " + extension_name)
            # Sanity Check
            ext_name = extension_name + "-" + (Name::EXTENSION_NAME.to_s).to_s
            if ((attr.get_value(ext_name)).nil?)
              debug("The jar file " + (jar.get_name).to_s + " appers to depend on " + extension_name + " but does not define the " + ext_name + " attribute in its manifest ")
            else
          ...
          debug("No dependencies for " + (jar.get_name).to_s)
        end
      end
      return result
    end
    ...
    def check_extension_against(extension_name, attr, file)
      debug("Checking extension " + extension_name + " against " + (file.get_name).to_s)
      # Load the jar file ...
      man = nil
      begin
      ...
      req_info = ExtensionInfo.new(extension_name, attr)
      debug("Requested Extension : " + (req_info).to_s)
      ...
      if (!(man).nil?)
        ...
        if (!(inst_attr).nil?)
          ...
          debug("Extension Installed " + (inst_info).to_s)
    ...
    def install_extension(req_info, inst_info)
      ...
      while e.has_more_elements
        ...
        if (!(eip).nil?)
          ...
          if (eip.install_extension(req_info, inst_info))
            debug((req_info.attr_name).to_s + " installation successful")
      ...
      debug((req_info.attr_name).to_s + " installation failed")
      return false
    end
            ...
            while i < dirs.attr_length
              ...
              while j < file_ext.attr_length
                if (ext_name.to_lower_case.ends_with(file_ext[j]))
                  ...
                  f_extension = JavaFile.new(dirs[i], ext_name + (file_ext[j]).to_s)
                end
                debug("checkExtensionExists:fileName " + (f_extension.get_name).to_s)
    ...
    class_module.module_eval {
      ...
      def get_ext_dirs
        ...
        if (!(s).nil?)
          ...
          debug("getExtDirs count " + (count).to_s)
          ...
          debug("getExtDirs dirs " + (dirs).to_s)
        end
        debug("getExtDirs dirs.length " + (dirs.attr_length).to_s)
        return dirs
      end
      ...
      def get_ext_files(dirs)
        ...
        while i < dirs.attr_length
          ...
          if (!(files).nil?)
            debug("getExtFiles files.length " + (files.attr_length).to_s)
            j = 0
        ...
        urls.copy_into(ua)
        debug("getExtFiles ua.length " + (ua.attr_length).to_s)
        return ua
      end
    }
        ...
        while i < installed_exts.attr_length
          ...
          if (!(inst_url).nil?)
            ...
            while j < urls.attr_length
              debug("URL[" + (j).to_s + "] is " + (urls[j]).to_s + " looking for " + (inst_url).to_s)
