...
# have any questions.
module Sun::Misc
  module PerformanceLoggerImports
      ...
      when_class_loaded do
        ...
        if (!(perf_logging_prop).nil?)
          ...
          if (perf_logging_prop.region_matches(true, 0, "file:", 0, 5))
            self.attr_log_file_name = (perf_logging_prop.substring(5)).to_s
          end
              ...
              Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
                ...
                define_method :run do
                  begin
                    ...
                    System.out.println((e).to_s + ": Creating logfile " + self.attr_log_file_name + ".  Log to console")
                  end
                  return nil
                end
        ...
        while i <= LAST_RESERVED
          self.attr_times.add(TimeData.new("Time " + (i).to_s + " not set", 0))
          (i += 1)
        end
      end
          ...
          begin
            synchronized((self.attr_times)) do
              ...
              while i < self.attr_times.size
                ...
                if (!(td).nil?)
                  writer.write((i).to_s + " " + (td.get_message).to_s + ": " + (td.get_time).to_s + "\n")
                end
                (i += 1)
              end
            end
            ...
            System.out.println((e).to_s + ": Writing performance log to " + (writer).to_s)
          end
        end
      end
