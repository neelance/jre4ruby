  ...
  class TimerThread < TimerImports.const_get :JavaThread
        ...
        while (true)
          ...
          if (!self.attr_notified)
            ...
            if (self.attr_debug)
              ...
              System.out.println("tick(" + (thr.get_name).to_s + "," + (timer.attr_interval).to_s + "," + (delta).to_s + ")")
              if (delta > 250)
    ...
    class_module.module_eval {
      ...
      def enqueue(timer)
        ...
        if (self.attr_debug)
          now = System.current_time_millis
          System.out.print((JavaThread.current_thread.get_name).to_s + ": enqueue " + (timer.attr_interval).to_s + ": ")
          cur = self.attr_timer_queue
          while (!(cur).nil?)
            delta = cur.attr_sleep_until - now
            System.out.print((cur.attr_interval).to_s + "(" + (delta).to_s + ") ")
            cur = cur.attr_next
          end
          System.out.println
        end
      ...
      def dequeue(timer)
        ...
        if ((cur).nil?)
          if (self.attr_debug)
            System.out.println((JavaThread.current_thread.get_name).to_s + ": dequeue " + (timer.attr_interval).to_s + ": no-op")
          end
          return false
        end
        if ((prev).nil?)
        ...
        if (self.attr_debug)
          now = System.current_time_millis
          System.out.print((JavaThread.current_thread.get_name).to_s + ": dequeue " + (timer.attr_interval).to_s + ": ")
          cur = self.attr_timer_queue
          while (!(cur).nil?)
            delta = cur.attr_sleep_until - now
            System.out.print((cur.attr_interval).to_s + "(" + (delta).to_s + ") ")
      ...
      def requeue(timer)
        if (!timer.attr_stopped)
          ...
          if (self.attr_debug)
            System.out.println((JavaThread.current_thread.get_name).to_s + ": requeue " + (timer.attr_interval).to_s + ": no-op")
          end
        end
      end
    }
