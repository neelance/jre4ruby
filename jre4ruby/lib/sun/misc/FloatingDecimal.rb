...
module Sun::Misc
  ...
  class FloatingDecimal 
    include_class_members FloatingDecimalImports
    ...
    typesig { [::Java::Boolean, ::Java::Int, Array.typed(::Java::Char), ::Java::Int, ::Java::Boolean] }
    # set by doubleValue
    def initialize(neg_sign, dec_exponent, digits, n, e)
    ...
    def get_chars(result)
      ...
      if (@is_exceptional)
        ...
        if (@dec_exponent > 0 && @dec_exponent < 8)
          # print digits.digits.
          char_length = Math.min(@n_digits, @dec_exponent)
          System.arraycopy(@digits, 0, result, i, char_length)
          i += char_length
          if (char_length < @dec_exponent)
            char_length = @dec_exponent - char_length
            System.arraycopy(Zero, 0, result, i, char_length)
            i += char_length
          ...
          if (@dec_exponent <= 0 && @dec_exponent > -3)
            result[((i += 1) - 1)] = Character.new(?0.ord)
            result[((i += 1) - 1)] = Character.new(?..ord)
            if (!(@dec_exponent).equal?(0))
              System.arraycopy(Zero, 0, result, i, -@dec_exponent)
              i -= @dec_exponent
            end
            System.arraycopy(@digits, 0, result, i, @n_digits)
            i += @n_digits
    ...
    class_module.module_eval {
      ...
      def per_thread_buffer
        ...
        Class.new(ThreadLocal.class == Class ? ThreadLocal : Object) do
          extend LocalClass
          include_class_members FloatingDecimal
        ...
        c = 0
          ...
          begin
            ...
            if ((c).equal?(Character.new(?N.ord)) || (c).equal?(Character.new(?I.ord)))
              ...
              target_chars = nil # char array of "NaN" or "Infinity"
              ...
              if (((j).equal?(target_chars.attr_length)) && ((i).equal?(l)))
                ...
                # NaN has no sign
                return (potential_na_n ? FloatingDecimal.new(Double::NaN) : FloatingDecimal.new(is_negative ? Double::NEGATIVE_INFINITY : Double::POSITIVE_INFINITY))
              else
            ...
            return FloatingDecimal.new(is_negative, dec_exp, digits, n_digits, false)
    ...
    class_module.module_eval {
      ...
      def parse_hex_string(s)
        ...
        if (!valid_input)
          ...
          if ((signif_length).equal?(0))
            # Only zeros in input
            return FloatingDecimal.new(sign * 0.0)
          end
          # Extract Exponent
          # 
          ...
          begin
            ...
            # -       +0.0            -0.0
            return FloatingDecimal.new(sign * (positive_exponent ? Double::POSITIVE_INFINITY : 0.0))
          end
          # exponent sign
            ...
            if (leading_digit <= 3)
              ...
              if (leading_digit <= 7)
                ...
                if (leading_digit <= 15)
                  ...
                  raise AssertionError.new("Result from digit conversion too large!")
                end
              end
            end
          end
          ...
          if (exponent > DoubleConsts::MAX_EXPONENT)
            ...
            return FloatingDecimal.new(sign * Double::POSITIVE_INFINITY)
          else
            # Finite return value
            ...
            fd = FloatingDecimal.new(FpUtils.raw_copy_sign(Double.long_bits_to_double(significand), sign))
            # 
      ...
      def get_hex_digit(s, position)
        ...
        if (value <= -1 || value >= 16)
          raise AssertionError.new("Unexpected failure of digit conversion of " + (s.char_at(position)).to_s)
        end
        return value
      end
    }
    ...
    private
    alias_method :initialize__floating_decimal, :initialize
  end
  ...
  class FDBigInt 
    ...
    attr_accessor :n_words
    alias_method :attr_n_words, :n_words
    undef_method :n_words
    alias_method :attr_n_words=, :n_words=
    undef_method :n_words=
    ...
    def initialize(v)
      ...
      @data[0] = RJava.cast_to_int(v)
      @data[1] = RJava.cast_to_int((v >> 32))
      @n_words = ((@data[1]).equal?(0)) ? 1 : 2
    end
    ...
    def initialize(other)
      @n_words = 0
      @data = nil
      @data = Array.typed(::Java::Int).new(@n_words = other.attr_n_words) { 0 }
      System.arraycopy(other.attr_data, 0, @data, 0, @n_words)
    end
    ...
    typesig { [::Java::Long, Array.typed(::Java::Char), ::Java::Int, ::Java::Int] }
    def initialize(seed, digit, nd0, nd)
      @n_words = 0
      @data = nil
      n = (nd + 8) / 9 # estimate size needed.
      if (n < 2)
        n = 2
      end
      @data = Array.typed(::Java::Int).new(n) { 0 } # allocate enough space
      @data[0] = RJava.cast_to_int(seed) # starting value
      @data[1] = RJava.cast_to_int((seed >> 32))
      @n_words = ((@data[1]).equal?(0)) ? 1 : 2
      i = nd0
      limit = nd - 5 # slurp digits 5 at a time.
      v = 0
      while (i < limit)
        ilim = i + 5
        v = RJava.cast_to_int(digit[((i += 1) - 1)]) - RJava.cast_to_int(Character.new(?0.ord))
        while (i < ilim)
          v = 10 * v + RJava.cast_to_int(digit[((i += 1) - 1)]) - RJava.cast_to_int(Character.new(?0.ord))
        end
        multadd_me(100000, v) # ... where 100000 is 10^5.
      end
      factor = 1
      v = 0
      while (i < nd)
    ...
    def lshift_me(c)
      if (c <= 0)
        if ((c).equal?(0))
          ...
          raise IllegalArgumentException.new("negative shift count")
        end
      end
      wordcount = c >> 5
      bitcount = c & 0x1f
      ...
      t = @data
      s = @data
      if (@n_words + wordcount + 1 > t.attr_length)
        # reallocate.
        t = Array.typed(::Java::Int).new(@n_words + wordcount + 1) { 0 }
      end
      target = @n_words + wordcount
      src = @n_words - 1
      if ((bitcount).equal?(0))
        # special hack, since an anticount of 32 won't go!
        System.arraycopy(s, 0, t, wordcount, @n_words)
        target = wordcount - 1
      else
        t[((target -= 1) + 1)] = s[src] >> anticount
        while (src >= 1)
          t[((target -= 1) + 1)] = (s[src] << bitcount) | (s[(src -= 1)] >> anticount)
        end
        t[((target -= 1) + 1)] = s[src] << bitcount
      end
      while (target >= 0)
        t[((target -= 1) + 1)] = 0
      end
      ...
      # if so, trim it
      while (@n_words > 1 && (@data[@n_words - 1]).equal?(0))
        ((@n_words -= 1) + 1)
      end
    end
    ...
    def normalize_me
      ...
      v = 0
      src = @n_words - 1
      while src >= 0 && ((v = @data[src])).equal?(0)
        wordcount += 1
        ((src -= 1) + 1)
      end
      if (src < 0)
        # oops. Value is zero. Cannot normalize it!
        raise IllegalArgumentException.new("zero value")
      end
      # 
      ...
      # do the work.
      if (!((v & -0x10000000)).equal?(0))
        # will have to shift up into the next word.
        # too bad.
        bitcount = 32
        while !((v & -0x10000000)).equal?(0)
          v >>= 1
          ((bitcount -= 1) + 1)
        end
      else
    ...
    def mult(iv)
      ...
      r = Array.typed(::Java::Int).new((v * (@data[@n_words - 1] & 0xffffffff) > 0xfffffff) ? @n_words + 1 : @n_words) { 0 }
      p = 0
      i = 0
      while i < @n_words
        p += v * (@data[i] & 0xffffffff)
        r[i] = RJava.cast_to_int(p)
        p >>= 32
        ((i += 1) - 1)
      end
      if ((p).equal?(0))
        return FDBigInt.new(r, @n_words)
      else
        r[@n_words] = RJava.cast_to_int(p)
        return FDBigInt.new(r, @n_words + 1)
      end
    end
    ...
    # Hope it fits!
    def multadd_me(iv, addend)
      v = iv
      p = 0
      # unroll 0th iteration, doing addition.
      p = v * (@data[0] & 0xffffffff) + (addend & 0xffffffff)
      @data[0] = RJava.cast_to_int(p)
      p >>= 32
      i = 1
      while i < @n_words
        p += v * (@data[i] & 0xffffffff)
        @data[i] = RJava.cast_to_int(p)
        p >>= 32
        ((i += 1) - 1)
      end
      if (!(p).equal?(0))
        @data[@n_words] = RJava.cast_to_int(p) # will fail noisily if illegal!
        ((@n_words += 1) - 1)
      end
    end
    ...
    def mult(other)
      ...
      r = Array.typed(::Java::Int).new(@n_words + other.attr_n_words) { 0 }
      i = 0
      # I think I am promised zeros...
      i = 0
      while i < @n_words
        v = @data[i] & 0xffffffff # UNSIGNED CONVERSION
        p = 0
        j = 0
        j = 0
        while j < other.attr_n_words
          p += (r[i + j] & 0xffffffff) + v * (other.attr_data[j] & 0xffffffff) # UNSIGNED CONVERSIONS ALL 'ROUND.
          r[i + j] = RJava.cast_to_int(p)
          p >>= 32
          ((j += 1) - 1)
        end
        r[i + j] = RJava.cast_to_int(p)
    ...
    def add(other)
      ...
      if (@n_words >= other.attr_n_words)
        a = @data
        n = @n_words
        b = other.attr_data
        m = other.attr_n_words
      else
        a = other.attr_data
        n = other.attr_n_words
        b = @data
        m = @n_words
      end
      r = Array.typed(::Java::Int).new(n) { 0 }
      i = 0
      while i < n
        c += a[i] & 0xffffffff
        if (i < m)
        ...
        r[i] = RJava.cast_to_int(c)
        c >>= 32 # signed shift.
        ((i += 1) - 1)
      end
      if (!(c).equal?(0))
        # oops -- carry out -- need longer result.
        s = Array.typed(::Java::Int).new(r.attr_length + 1) { 0 }
        System.arraycopy(r, 0, s, 0, r.attr_length)
        s[((i += 1) - 1)] = RJava.cast_to_int(c)
        return FDBigInt.new(s, i)
      end
      return FDBigInt.new(r, i)
    end
    ...
    def sub(other)
      r = Array.typed(::Java::Int).new(@n_words) { 0 }
      i = 0
      n = @n_words
      m = other.attr_n_words
      nzeros = 0
      ...
      while i < n
        c += @data[i] & 0xffffffff
        if (i < m)
          c -= other.attr_data[i] & 0xffffffff
        end
        if (((r[i] = RJava.cast_to_int(c))).equal?(0))
          ((nzeros += 1) - 1)
        else
          nzeros = 0
        end
      ...
      raise AssertError, (c).to_s if not ((c).equal?(0)) # borrow out of subtract
      raise AssertError if not (data_in_range_is_zero(i, m, other)) # negative result of subtract
      return FDBigInt.new(r, n - nzeros)
    end
    
    class_module.module_eval {
      typesig { [::Java::Int, ::Java::Int, FDBigInt] }
      def data_in_range_is_zero(i, m, other)
        while (i < m)
          if (!(other.attr_data[((i += 1) - 1)]).equal?(0))
            return false
          end
        end
        return true
    ...
    def cmp(other)
      ...
      if (@n_words > other.attr_n_words)
        ...
        if (@n_words < other.attr_n_words)
          ...
          # then the answer is evident
          j_ = @n_words - 1
          i = other.attr_n_words - 1
          while i > j_
            if (!(other.attr_data[i]).equal?(0))
              return -1
            end
            ((i -= 1) + 1)
          end
      ...
      while i > 0
        if (!(@data[i]).equal?(other.attr_data[i]))
          break
        end
        ((i -= 1) + 1)
      end
    ...
    def quo_rem_iteration(s)
      # ensure that this and S have the same number of
      # digits. If S is properly normalized and q < 10 then
      # this must be so.
      if (!(@n_words).equal?(s.attr_n_words))
        raise IllegalArgumentException.new("disparate values")
      end
      # estimate q the obvious way. We will usually be
      # right. If not, then we're only off by a little and
      # will re-add.
      n = @n_words - 1
      q = (@data[n] & 0xffffffff) / s.attr_data[n]
      diff = 0
      i = 0
      while i <= n
        diff += (@data[i] & 0xffffffff) - q * (s.attr_data[i] & 0xffffffff)
        @data[i] = RJava.cast_to_int(diff)
      ...
      if (!(diff).equal?(0))
        ...
        while ((sum).equal?(0))
          ...
          i_ = 0
          while i_ <= n
            sum += (@data[i_] & 0xffffffff) + (s.attr_data[i_] & 0xffffffff)
            @data[i_] = RJava.cast_to_int(sum)
          ...
          # +1.
          raise AssertError, (sum).to_s if not ((sum).equal?(0) || (sum).equal?(1)) # carry out of division correction
          q -= 1
        end
      end
      ...
      i__ = 0
      while i__ <= n
        p += 10 * (@data[i__] & 0xffffffff)
        @data[i__] = RJava.cast_to_int(p)
        p >>= 32 # SIGNED shift.
        ((i__ += 1) - 1)
      end
      raise AssertError, (p).to_s if not ((p).equal?(0)) # Carry out of *10
      return RJava.cast_to_int(q)
    end
    
    typesig { [] }
    def long_value
      # if this can be represented as a long, return the value
      raise AssertError, (@n_words).to_s if not (@n_words > 0) # longValue confused
      if ((@n_words).equal?(1))
        return (@data[0] & 0xffffffff)
      end
      raise AssertError if not (data_in_range_is_zero(2, @n_words, self)) # value too big
      raise AssertError if not (@data[1] >= 0) # value too big
      return ((@data[1]) << 32) | (@data[0] & 0xffffffff)
    end
    
    typesig { [] }
    def to_s
      r = StringBuffer.new(30)
      r.append(Character.new(?[.ord))
      i = Math.min(@n_words - 1, @data.attr_length - 1)
      if (@n_words > @data.attr_length)
        r.append("(" + (@data.attr_length).to_s + "<" + (@n_words).to_s + "!)")
      end
      while i > 0
        r.append(JavaInteger.to_hex_string(@data[i]))
        r.append(Character.new(?\s.ord))
        ((i -= 1) + 1)
      end
      r.append(JavaInteger.to_hex_string(@data[0]))
      r.append(Character.new(?].ord))
      return String.new(r)
    end
    
    private
    alias_method :initialize__fdbig_int, :initialize
  end
