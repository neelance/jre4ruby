...
module Sun::Misc
  ...
  class Queue 
    ...
    def dump(msg)
      synchronized(self) do
        ...
        System.err.println("[" + (@length).to_s + " elt(s); head = " + (((@head).nil? ? "null" : ((@head.attr_obj)).to_s + "")).to_s + " tail = " + (((@tail).nil? ? "null" : ((@tail.attr_obj)).to_s + "")).to_s)
        cursor = @head
        last = nil
        while (!(cursor).nil?)
          System.err.println("  " + (cursor).to_s)
          last = cursor
          cursor = cursor.attr_next
        end
        if (!(last).equal?(@tail))
          System.err.println("  tail != last: " + (@tail).to_s + ", " + (last).to_s)
        end
        System.err.println("]")
      end
    end
  ...
  class QueueElement 
    ...
    typesig { [] }
    def to_s
      return "QueueElement[obj=" + (@obj).to_s + (((@prev).nil? ? " null" : " prev")).to_s + (((@next).nil? ? " null" : " next")).to_s + "]"
    end
