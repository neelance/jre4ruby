  ...
  class StreamEncoder < StreamEncoderImports.const_get :Writer
    ...
    class_module.module_eval {
      ...
      def for_output_stream_writer(out, lock, charset_name)
        csn = charset_name
        if ((csn).nil?)
          csn = (Charset.default_charset.name).to_s
        end
        begin
      ...
      if (rem > 0)
        if (!(@ch).nil?)
          if (!(@ch.write(@bb)).equal?(rem))
            raise AssertError, (rem).to_s if not (false)
          end
        else
    ...
    def impl_write(cbuf, off, len)
      ...
      while (cb.has_remaining)
        ...
        if (cr.is_underflow)
          raise AssertError, (cb.remaining).to_s if not ((cb.remaining <= 1))
