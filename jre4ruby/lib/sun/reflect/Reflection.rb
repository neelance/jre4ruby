  ...
  class Reflection 
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        map = HashMap.new
        map.put(Reflection.class, Array.typed(String).new(["fieldFilterMap", "methodFilterMap"]))
        map.put(System.class, Array.typed(String).new(["security"]))
      ...
      def ensure_member_access(current_class, member_class, target, modifiers)
        ...
        if (!verify_member_access(current_class, member_class, target, modifiers))
          raise IllegalAccessException.new("Class " + (current_class.get_name).to_s + " can not access a member of class " + (member_class.get_name).to_s + " with modifiers \"" + (Modifier.to_s(modifiers)).to_s + "\"")
        end
      end
      ...
      def register_filter(map, containing_class, *names)
        if (!(map.get(containing_class)).nil?)
          raise IllegalArgumentException.new("Filter already registered: " + (containing_class).to_s)
        end
        map = HashMap.new(map)
