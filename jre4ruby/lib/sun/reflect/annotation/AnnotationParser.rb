...
module Sun::Reflect::Annotation
  module AnnotationParserImports
  ...
  class AnnotationParser 
    ...
    class_module.module_eval {
          ...
          if (!(a).nil?)
            ...
            if ((type.retention).equal?(RetentionPolicy::RUNTIME))
              if (!(result.put(klass, a)).nil?)
                raise AnnotationFormatError.new("Duplicate annotation for class: " + (klass).to_s + ": " + (a).to_s)
              end
            end
          end
          i += 1
      ...
      def parse_annotation(buf, const_pool, container, exception_on_missing_annotation_class)
        ...
        begin
          begin
            sig = (const_pool.get_utf8at(type_index)).to_s
      ...
      def parse_member_value(member_type, buf, const_pool, container)
        ...
        if (!(result.is_a?(ExceptionProxy)) && !member_type.is_instance(result))
          result = AnnotationTypeMismatchExceptionProxy.new((result.get_class).to_s + "[" + (result).to_s + "]")
        end
        return result
      end
      ...
      def parse_const(tag, buf, const_pool)
        ...
        when Character.new(?s.ord)
          ...
          raise AnnotationFormatError.new("Invalid member-value tag in annotation: " + (tag).to_s)
        end
      end
                  ...
                  if ((component_type).equal?(Array))
                    ...
                    if ((component_type).equal?(Array))
                      ...
                      if ((component_type).equal?(Array))
                        ...
                        if ((component_type).equal?(String.class))
                          return parse_string_array(length, buf, const_pool)
                        else
                          if ((component_type).equal?(Class.class))
      ...
      def exception_proxy(tag)
        return AnnotationTypeMismatchExceptionProxy.new("Array with component tag: " + (tag).to_s)
      end
