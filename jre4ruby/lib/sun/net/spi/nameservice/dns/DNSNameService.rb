  ...
  class DNSNameService 
    ...
    def get_temporary_context
      ...
      if ((thr_ctxt).nil?)
        ...
        if ((prov_url).nil?)
          prov_url = (create_provider_url(ns_list)).to_s
    ...
    def initialize
      ...
      if (!(nameservers_).nil? && nameservers_.length > 0)
        @name_provider_url = (create_provider_url(nameservers_)).to_s
        ...
        while (i.has_next)
          ...
          while (!((start = parent_domain.index_of("."))).equal?(-1) && start < parent_domain.length - 1)
            begin
              ...
              # devolve
              parent_domain = (parent_domain.substring(start + 1)).to_s
            end
          end
          if (!(results).nil?)
            break
    ...
    def get_host_by_addr(addr)
      ...
      # remove trailing dot
      if (host.ends_with("."))
        host = (host.substring(0, host.length - 1)).to_s
      end
      return host
    end
    ...
    class_module.module_eval {
      ...
      def create_provider_url(ns_list)
        ...
        sb = StringBuffer.new
        while (i.has_next)
          append_if_literal_address(i.next, sb)
        end
        return sb.to_s
      end
