...
module Sun::Net::Httpserver
  ...
  class ServerImpl 
    ...
    def initialize(wrapper, protocol, addr, backlog)
      ...
      @logger.config("HttpServer created " + protocol + " " + (addr).to_s)
    end
    ...
    def remove_context(context)
      synchronized(self) do
        ...
        @logger.config("context removed: " + (context.get_path).to_s)
      end
    end
      ...
      const_set_lazy(:Exchange) { Class.new do
        ...
        def run
          ...
          begin
            ...
            request_line = (req.request_line).to_s
            if ((request_line).nil?)
              # connection closed
              @connection.close
              return
            ...
            if (!(s).nil? && s.equals_ignore_case("chunked"))
              ...
              s = (headers_.get_first("Content-Length")).to_s
              if (!(s).nil?)
            ...
            if (version.equals_ignore_case("http/1.0"))
              ...
              if ((chdr).nil?)
                ...
                if (chdr.equals_ignore_case("keep-alive"))
                  ...
                  val = "timeout=" + (idle).to_s + ", max=" + (max).to_s
        ...
        def reject(code, request_str, message)
          ...
          send_reply(code, true, "<h1>" + (code).to_s + (Code.msg(code)).to_s + "</h1>" + message)
        end
        
        typesig { [::Java::Int, ::Java::Boolean, String] }
        def send_reply(code, close_now, text)
          begin
            s = "HTTP/1.1 " + (code).to_s + (Code.msg(code)).to_s + "\r\n"
            if (!(text).nil? && !(text.length).equal?(0))
              s = s + "Content-Length: " + (text.length).to_s + "\r\n"
    ...
    def log_reply(code, request_str, text)
      ...
      message = request_str + " [" + (code).to_s + " " + (Code.msg(code)).to_s + "] (" + text + ")"
      @logger.fine(message)
    end
