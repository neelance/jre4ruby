...
module Sun::Net::Smtp
  ...
  class SmtpClient < SmtpClientImports.const_get :TransferProtocolClient
    ...
    def open_server(host)
      ...
      issue_command("helo " + (InetAddress.get_local_host.get_host_name).to_s + "\r\n", 250)
    end
    ...
    def initialize(host)
      ...
      begin
        s = nil
        @mailhost = (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("mail.host"))).to_s
        if (!(@mailhost).nil?)
