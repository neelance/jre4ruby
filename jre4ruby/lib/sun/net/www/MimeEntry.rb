...
module Sun::Net::Www
  ...
  class MimeEntry 
    ...
    def initialize(type, image_file_name, extension_string)
      ...
      @file_extensions = nil
      @starred = false
      @type_name = (type.to_lower_case).to_s
      @action = UNKNOWN
      @command = (nil).to_s
    ...
    def set_image_file_name(filename)
      synchronized(self) do
        ...
        if ((file.get_parent).nil?)
          @image_file_name = (System.get_property("java.net.ftp.imagepath." + filename)).to_s
        else
      ...
      synchronized(self) do
        ...
        if (!(@file_extensions).nil?)
          ...
          while i < @file_extensions.attr_length
            extensions_as_string += (@file_extensions[i]).to_s
    ...
    def to_property
      synchronized(self) do
        ...
        if (!(action).equal?(MimeEntry::UNKNOWN))
          buf.append("action=" + (ActionKeywords[action]).to_s)
          need_separator = true
        end
        ...
        if (!(get_image_file_name).nil?)
          ...
          buf.append("icon=" + (get_image_file_name).to_s)
          need_separator = true
        end
    ...
    typesig { [] }
    def to_s
      return "MimeEntry[contentType=" + @type_name + ", image=" + @image_file_name + ", action=" + (@action).to_s + ", command=" + @command + ", extensions=" + (get_extensions_as_list).to_s + "]"
    end
