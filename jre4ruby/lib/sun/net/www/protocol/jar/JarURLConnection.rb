...
module Sun::Net::Www::Protocol::Jar
  ...
  class JarURLConnection < Java::Net::JarURLConnection
    ...
    def initialize(url, handler)
      ...
      @entry_name = (get_entry_name).to_s
    end
      ...
      if (!self.attr_connected)
        ...
        if ((!(@entry_name).nil?))
          ...
          if ((@jar_entry).nil?)
            ...
            raise FileNotFoundException.new("JAR entry " + @entry_name + " not found in " + (@jar_file.get_name).to_s)
          end
        end
      ...
      if ((@content_type).nil?)
        if ((@entry_name).nil?)
          ...
          begin
            ...
            @content_type = (guess_content_type_from_stream(BufferedInputStream.new(in_))).to_s
            in_.close
          rescue IOException => e
            # don't do anything
          end
        ...
        if ((@content_type).nil?)
          @content_type = (guess_content_type_from_name(@entry_name)).to_s
        end
        if ((@content_type).nil?)
