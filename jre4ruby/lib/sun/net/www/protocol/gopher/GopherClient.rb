  ...
  class GopherClient < GopherClientImports.const_get :NetworkClient
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        ...
        self.attr_gopher_proxy_host = (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("gopherProxyHost"))).to_s
    ...
    def open_stream(u)
      ...
      if (!(@gtype).equal?(Character.new(?7.ord)))
        self.attr_server_output.print((decode_percent(@gkey)).to_s + "\r\n")
        self.attr_server_output.flush
      else
        if ((i = @gkey.index_of(Character.new(??.ord))) >= 0)
          self.attr_server_output.print(decode_percent((@gkey.substring(0, i)).to_s + "\t" + (@gkey.substring(i + 1)).to_s + "\r\n"))
          self.attr_server_output.flush
          msgh.add("content-type", "text/html")
        else
          msgh.add("content-type", "text/html")
    ...
    def run
      ...
      begin
        ...
        raise InternalError.new((self.attr_encoding).to_s + " encoding not found")
