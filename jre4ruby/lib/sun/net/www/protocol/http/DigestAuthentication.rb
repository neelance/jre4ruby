  ...
  class DigestAuthentication < DigestAuthenticationImports.const_get :AuthenticationInfo
    ...
    def set_headers(conn, p, raw)
      ...
      if (@auth_method.length >= 1)
        ...
        # here.
        @auth_method = (Character.to_upper_case(@auth_method.char_at(0)) + @auth_method.substring(1).to_lower_case).to_s
      end
    ...
    def get_header_value_impl(uri, method)
      ...
      ncstring = nil
      if (!(nccount).equal?(-1))
        ncstring = (JavaInteger.to_hex_string(nccount).to_lower_case).to_s
        len = ncstring.length
        if (len < 8)
          ncstring = (ZeroPad[len]).to_s + ncstring
        end
      end
      begin
        response = (compute_digest(true, self.attr_pw.get_user_name, passwd, self.attr_realm, method, uri, nonce, cnonce, ncstring)).to_s
      ...
      value = @auth_method + " username=\"" + (self.attr_pw.get_user_name).to_s + "\", realm=\"" + (self.attr_realm).to_s + "\", nonce=\"" + nonce + ncfield + ", uri=\"" + uri + "\", response=\"" + response + "\", algorithm=\"" + algorithm
      if (!(opaque).nil?)
    ...
    def check_response(header, method, url)
      ...
      if (!(nccount).equal?(-1))
        ncstring = (JavaInteger.to_hex_string(nccount).to_upper_case).to_s
        len = ncstring.length
        if (len < 8)
          ncstring = (ZeroPad[len]).to_s + ncstring
        end
    ...
    def compute_digest(is_request, user_name, password, realm, conn_method, request_uri, nonce_string, cnonce, nc_value)
      ...
      if (md5sess)
        if (((hash_a1 = (@params.get_cached_ha1).to_s)).nil?)
          s = user_name + ":" + realm + ":"
          s1 = encode(s, password, md)
          a1 = s1 + ":" + nonce_string + ":" + cnonce
          hash_a1 = (encode(a1, nil, md)).to_s
          @params.set_cached_ha1(hash_a1)
        end
      else
        a1 = user_name + ":" + realm + ":"
        hash_a1 = (encode(a1, password, md)).to_s
      end
      a2 = nil
      if (is_request)
      ...
      final_hash = (encode(combo, nil, md)).to_s
      return final_hash
    end
