...
module Sun::Net::Www::Protocol::Http
  module AuthenticationInfoImports
  ...
  class AuthenticationInfo < AuthenticationInfoImports.const_get :AuthCacheValue
    ...
    class_module.module_eval {
      ...
      def get_server_auth(url)
        ...
        key = (SERVER_AUTHENTICATION).to_s + ":" + (url.get_protocol.to_lower_case).to_s + ":" + (url.get_host.to_lower_case).to_s + ":" + (port).to_s
      ...
      def get_server_auth(url, realm, atype)
        ...
        key = (SERVER_AUTHENTICATION).to_s + ":" + (atype).to_s + ":" + (url.get_protocol.to_lower_case).to_s + ":" + (url.get_host.to_lower_case).to_s + ":" + (port).to_s + ":" + realm
      ...
      def get_proxy_auth(host, port)
        key = (PROXY_AUTHENTICATION).to_s + "::" + (host.to_lower_case).to_s + ":" + (port).to_s
      ...
      def get_proxy_auth(host, port, realm, atype)
        key = (PROXY_AUTHENTICATION).to_s + ":" + (atype).to_s + "::" + (host.to_lower_case).to_s + ":" + (port).to_s + ":" + realm
    ...
    def cache_key(include_realm)
      ...
      # class.
      if (include_realm)
        return (@type).to_s + ":" + (@auth_type).to_s + ":" + @protocol + ":" + @host + ":" + (@port).to_s + ":" + @realm
      else
        return (@type).to_s + ":" + @protocol + ":" + @host + ":" + (@port).to_s
      end
    end
    ...
    def write_object(s)
      synchronized(self) do
        @s1 = (@pw.get_user_name).to_s
        @s2 = (String.new(@pw.get_password)).to_s
