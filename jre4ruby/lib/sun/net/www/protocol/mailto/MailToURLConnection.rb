...
module Sun::Net::Www::Protocol::Mailto
  module MailToURLConnectionImports
  ...
  class MailToURLConnection < MailToURLConnectionImports.const_get :URLConnection
    ...
    def get_from_address
      ...
      if ((str).nil?)
        str = (System.get_property("user.name")).to_s
        if (!(str).nil?)
          host = System.get_property("mail.host")
          if ((host).nil?)
            begin
              host = (InetAddress.get_local_host.get_host_name).to_s
    ...
    typesig { [] }
    def connect
      System.err.println("connect. Timeout = " + (@connect_timeout).to_s)
    ...
    def get_permission
      if ((@permission).nil?)
        connect
        host = (@client.get_mail_host).to_s + ":" + (25).to_s
