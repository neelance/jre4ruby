...
module Sun::Net::Www
  ...
  class URLConnection < Java::Net::URLConnection
    ...
    def get_content_type
      if ((@content_type).nil?)
        @content_type = (get_header_field("content-type")).to_s
      end
      if ((@content_type).nil?)
        ct = nil
        begin
          ct = (guess_content_type_from_stream(get_input_stream)).to_s
        rescue Java::Io::IOException => e
        end
        ce = @properties.find_value("content-encoding")
        if ((ct).nil?)
          ct = (@properties.find_value("content-type")).to_s
          if ((ct).nil?)
            if (self.attr_url.get_file.ends_with("/"))
              ct = "text/html"
            else
              ct = (guess_content_type_from_name(self.attr_url.get_file)).to_s
            end
          end
        end
