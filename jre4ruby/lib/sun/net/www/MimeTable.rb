...
module Sun::Net::Www
  ...
  class MimeTable 
      ...
      when_class_loaded do
        Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
          ...
          define_method :run do
            self.attr_temp_file_template = (System.get_property("content.types.temp.file.template", "/tmp/%s")).to_s
            self.attr_mailcap_locations = Array.typed(String).new([System.get_property("user.mailcap"), (System.get_property("user.home")).to_s + "/.mailcap", "/etc/mailcap", "/usr/etc/mailcap", "/usr/local/etc/mailcap", (System.get_property("hotjava.home", "/usr/local/hotjava")).to_s + "/lib/mailcap", ])
            return nil
          end
      ...
      def get_default_table
        if ((self.attr_default_instance).nil?)
          Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
            ...
            typesig { [] }
            define_method :run do
              self.attr_default_instance = MimeTable.new
    ...
    def find_by_file_name(fname)
      ...
      if (i > 0)
        fname = (fname.substring(0, i - 1)).to_s
      end
      i = fname.last_index_of(Character.new(?..ord))
      ...
      if (!(i).equal?(-1) && (fname.char_at(i)).equal?(Character.new(?..ord)))
        ext = (fname.substring(i).to_lower_case).to_s
      end
        ...
        begin
          ...
          if (!(user_table_path).nil?)
            ...
            if (!file.exists)
              ...
              file = JavaFile.new((System.get_property("java.home") + JavaFile.attr_separator).to_s + "lib" + (JavaFile.attr_separator).to_s + "content-types.properties")
            end
          else
            # No user table, try to load the default built-in table.
            file = JavaFile.new((System.get_property("java.home") + JavaFile.attr_separator).to_s + "lib" + (JavaFile.attr_separator).to_s + "content-types.properties")
          ...
          System.err.println("Warning: default mime table not found: " + (file.get_path).to_s)
          return
        end
        parse(entries)
      end
    ...
    def parse(pair, entry)
      ...
      while (tokenizer.has_more_tokens)
        if (got_name)
          value = (tokenizer.next_token.trim).to_s
        else
          name = (tokenizer.next_token.trim).to_s
          got_name = true
        end
      end
    ...
    def save(filename)
      synchronized(self) do
        if ((filename).nil?)
          filename = (System.get_property("user.home" + (JavaFile.attr_separator).to_s + "lib" + (JavaFile.attr_separator).to_s + "content-types.properties")).to_s
        end
