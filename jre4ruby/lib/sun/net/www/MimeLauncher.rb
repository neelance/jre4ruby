  ...
  class MimeLauncher < MimeLauncherImports.const_get :JavaThread
    ...
    def initialize(m, uc, is, temp_file_template, thread_name)
      ...
      if (!find_executable_path(launch_string))
        ...
        if (!(index).equal?(-1))
          app_name = (launch_string.substring(0, index)).to_s
        else
    ...
    def get_temp_file_name(url, template)
      ...
      if (wildcard < temp_filename.length - 2)
        suffix = (temp_filename.substring(wildcard + 2)).to_s
      end
      timestamp = System.current_time_millis / 1000
      arg_index = 0
      while ((arg_index = prefix.index_of("%s")) >= 0)
        prefix = (prefix.substring(0, arg_index) + timestamp + prefix.substring(arg_index + 2)).to_s
      end
      ...
      if (dot >= 0 && dot > filename.last_index_of(Character.new(?/.ord)))
        extension = (filename.substring(dot)).to_s
      end
      filename = "HJ" + (url.hash_code).to_s
      temp_filename = prefix + filename + (timestamp).to_s + extension + suffix
    ...
    def run
      begin
        ...
        ofn = (get_temp_file_name(@uc.get_url, ofn)).to_s
        begin
        ...
        while ((inx = c.index_of("%t")) >= 0)
          c = (c.substring(0, inx) + @uc.get_content_type + c.substring(inx + 2)).to_s
        end
        substituted = false
        while ((inx = c.index_of("%s")) >= 0)
          c = (c.substring(0, inx)).to_s + ofn + (c.substring(inx + 2)).to_s
          substituted = true
        end
        if (!substituted)
          c = c + " <" + ofn
    ...
    def find_executable_path(str)
      ...
      if (!(index).equal?(-1))
        command = (str.substring(0, index)).to_s
      else
        command = str
      end
      ...
      exec_path_list = nil
      exec_path_list = (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("exec.path"))).to_s
      ...
      while (iter.has_more_elements)
        ...
        full_cmd = prefix + (JavaFile.attr_separator).to_s + command
        f = JavaFile.new(full_cmd)
        if (f.is_file)
          @exec_path = prefix + (JavaFile.attr_separator).to_s + str
          return true
        end
      end
