...
module Sun::Net::Www::Http
  ...
  class HttpClient < HttpClientImports.const_get :NetworkClient
    ...
    def parse_httpheader(responses, pi, httpuc)
      ...
      begin
        ...
        if (ret)
          ...
          if (@using_proxy)
            ...
            keep = (responses.find_value("Proxy-Connection")).to_s
          end
          if ((keep).nil?)
            keep = (responses.find_value("Connection")).to_s
          end
      ...
      begin
        resp = nil
        resp = (responses.get_value(0)).to_s
      ...
      te = nil
      begin
        te = (responses.find_value("Transfer-Encoding")).to_s
      rescue Exception => e
      end
    ...
    typesig { [] }
    def to_s
      return (get_class.get_name).to_s + "(" + (@url).to_s + ")"
    end
