  ...
  class HttpTimestamper 
    ...
    def generate_timestamp(ts_query)
      ...
      if (DEBUG)
        ...
        System.out.println((connection.get_request_method).to_s + " " + @tsa_url + " HTTP/1.1")
        i = headers.iterator
        while i.has_next
          System.out.println("  " + (i.next).to_s)
        end
        System.out.println
      end
      ...
      begin
        ...
        output.flush
        if (DEBUG)
          System.out.println("sent timestamp query (length=" + (request.attr_length).to_s + ")")
        end
      ensure
        if (!(output).nil?)
          output.close
      ...
      begin
        ...
        if (DEBUG)
          System.out.println("received timestamp response (length=" + (reply_buffer.attr_length).to_s + ")")
        end
      ensure
        if (!(input).nil?)
          input.close
