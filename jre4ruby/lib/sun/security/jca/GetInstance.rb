  ...
  class GetInstance 
    ...
    class_module.module_eval {
      ...
      def get_service(type, algorithm, provider)
        ...
        if ((s).nil?)
          raise NoSuchAlgorithmException.new("no such algorithm: " + algorithm + " for provider " + (provider.get_name).to_s)
        end
        return s
      end
      ...
      def check_super_class(s, sub_class, super_class)
        ...
        if ((super_class.is_assignable_from(sub_class)).equal?(false))
          raise NoSuchAlgorithmException.new("class configured for " + (s.get_type).to_s + ": " + (s.get_class_name).to_s + " not a " + (s.get_type).to_s)
        end
      end
    }
