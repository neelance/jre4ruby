...
module Sun::Security::Jca
  ...
  class ProviderList 
    ...
    def initialize
      ...
      i = 1
      while true
        entry = Security.get_property("security.provider." + (i).to_s)
        if ((entry).nil?)
          break
        end
        entry = (entry.trim).to_s
        if ((entry.length).equal?(0))
          System.err.println("invalid entry for " + "security.provider." + (i).to_s)
          break
        end
      ...
      if (!(Debug).nil?)
        Debug.println("provider configuration: " + (config_list).to_s)
      end
    end
