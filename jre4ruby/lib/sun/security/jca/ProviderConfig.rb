...
module Sun::Security::Jca
  ...
  class ProviderConfig 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:CL_STRING) { Array.typed(Class).new([String.class]) }
    ...
    def get_provider
      ...
      synchronized((get_lock)) do
        ...
        if (@is_loading)
          ...
          if (!(Debug).nil?)
            Debug.println("Recursion loading provider: " + (self).to_s)
    ...
    def do_load_provider
      return AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
        ...
        define_method :run do
          if (!(Debug).nil?)
            Debug.println("Loading provider: " + (@local_class_parent).to_s)
          end
          begin
            ...
            if (obj.is_a?(Provider))
              if (!(Debug).nil?)
                Debug.println("Loaded provider " + (obj).to_s)
              end
              return obj
            else
              if (!(Debug).nil?)
                Debug.println((self.attr_class_name).to_s + " is not a provider")
              end
              disable_load
              return nil
            end
            ...
            if (!(Debug).nil?)
              Debug.println("Error loading provider " + (@local_class_parent).to_s)
              t.print_stack_trace
            end
