...
module Sun::Security::Smartcardio
  ...
  class PCSCTerminals < PCSCTerminalsImports.const_get :CardTerminals
        ...
        begin
          ...
          reader_names.each do |readerName|
            ...
            when CARD_REMOVAL
              ...
              raise CardException.new("Unknown state: " + (state).to_s)
            end
          end
    ...
    def wait_for_change(timeout)
      synchronized(self) do
        if (timeout < 0)
          raise IllegalArgumentException.new("Timeout must not be negative: " + (timeout).to_s)
        end
        if ((@state_map).nil?)
      ...
      def wait_for_cards(terminals, timeout, want_present)
        ...
        terminals.each do |terminal|
          if ((terminal.is_a?(TerminalImpl)).equal?(false))
            raise IllegalArgumentException.new("Invalid terminal type: " + (terminal.get_class.get_name).to_s)
          end
          impl = terminal
