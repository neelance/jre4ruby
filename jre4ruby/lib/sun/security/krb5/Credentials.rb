  ...
  class Credentials 
    ...
    class_module.module_eval {
      ...
      def acquire_tgtfrom_cache(princ, ticket_cache)
          ...
          if (os.to_upper_case.starts_with("WINDOWS"))
            ...
            if (!(princ).nil?)
              if ((creds.get_client == princ))
                if (self.attr_debug)
                  System.out.println(">>> Obtained TGT from LSA: " + (creds).to_s)
                end
                return creds
              else
                if (self.attr_debug)
                  System.out.println(">>> LSA contains TGT for " + (creds.get_client).to_s + " not " + (princ).to_s)
                end
                return nil
              end
            else
        ...
        if (EType.is_supported(tgt_cred.get_etype))
          ...
          if (self.attr_debug)
            System.out.println(">>> unsupported key type found the default TGT: " + (tgt_cred.get_etype).to_s)
          end
          return nil
        end
      end
        ...
        synchronized(self) do
          ...
          if (!(self.attr_cache).nil?)
            ...
            if (EType.is_supported(temp.get_etype))
              ...
              if (self.attr_debug)
                System.out.println(">>> unsupported key type found the default TGT: " + (temp.get_etype).to_s)
              end
            end
          end
          if ((result).nil?)
    ...
    class_module.module_eval {
      ...
      def print_debug(c)
        ...
        System.out.println("\tclient: " + (c.attr_client.to_s).to_s)
        System.out.println("\tserver: " + (c.attr_server.to_s).to_s)
        System.out.println("\tticket: realm: " + (c.attr_ticket.attr_realm.to_s).to_s)
        System.out.println("\t        sname: " + (c.attr_ticket.attr_sname.to_s).to_s)
        if (!(c.attr_start_time).nil?)
          System.out.println("\tstartTime: " + (c.attr_start_time.get_time).to_s)
        end
        System.out.println("\tendTime: " + (c.attr_end_time.get_time).to_s)
