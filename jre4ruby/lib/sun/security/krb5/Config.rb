...
module Sun::Security::Krb5
  ...
  class Config 
    ...
    def initialize
      ...
      @default_realm = (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("java.security.krb5.realm"))).to_s
      ...
      if (!(kdchost).nil?)
        ...
        kdchost = (kdchost.replace(Character.new(?:.ord), Character.new(?\s.ord))).to_s
        ...
        begin
          ...
          ke = KrbException.new("Could not load " + "configuration file " + (ioe.get_message).to_s)
          ke.init_cause(ioe)
          raise (ke)
        end
      end
    ...
    def get_default_int_value(name)
      ...
      result = (get_default(name)).to_s
      if (!(result).nil?)
        begin
          value = parse_int_value(result)
        rescue NumberFormatException => e
          if (self.attr_debug)
            System.out.println("Exception in getting value of " + name + " " + (e.get_message).to_s)
    ...
    def get_default_int_value(name, section)
      result = nil
      value = JavaInteger::MIN_VALUE
      result = (get_default(name, section)).to_s
      if (!(result).nil?)
        begin
          value = parse_int_value(result)
        rescue NumberFormatException => e
          if (self.attr_debug)
            System.out.println("Exception in getting value of " + name + " in section " + section + " " + (e.get_message).to_s)
            System.out.println("Setting " + name + " to minimum value")
          end
          value = JavaInteger::MIN_VALUE
        end
    ...
    def get_default(k, t)
      ...
      if (!(@stanza_table).nil?)
        ...
        while e.has_more_elements
          key = (e.next_element).to_s
          ob = t.get(key)
          if (ob.is_a?(Hashtable))
            result = (get_default(k, ob)).to_s
            if (!(result).nil?)
              return result
            end
          else
    ...
    def get_default(name, section)
      ...
      if (name.equals_ignore_case("kdc") && (!section.equals_ignore_case("libdefaults")) && (!(Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("java.security.krb5.kdc"))).nil?))
        result = (get_default("kdc", "libdefaults")).to_s
        return result
      end
      if (!(@stanza_table).nil?)
        e = @stanza_table.keys
        while e.has_more_elements
          stanza_name = (e.next_element).to_s
    ...
    def get_default_boolean_value(name)
      ...
      if ((@stanza_table).nil?)
        ...
        val = (get_default(name, @stanza_table)).to_s
      end
    ...
    def find(key1, key2)
      result = nil
      if ((!(@stanza_table).nil?) && (!((result = ((((@stanza_table.get(key1))).get(key2))).to_s)).nil?))
        return result
      else
        return ""
      end
    ...
    def load_config_file
      begin
        ...
        if (!(file_name == ""))
          ...
          v = Vector.new
          previous = nil
          while (!((line = (br.read_line).to_s)).nil?)
        ...
        if (temp)
          ...
          if (osname.starts_with("Windows"))
            ...
            if (Credentials.attr_already_loaded)
              if (((name = (get_windows_directory).to_s)).nil?)
    ...
    def parse_realm_field(v, start, end_)
      ...
      i = start
      while i < end_
        line = (v.element_at(i).trim).to_s
        if (line.ends_with("{"))
          key = ""
          j = 0
          while j < line.length
            if ((line.char_at(j)).equal?(Character.new(?=.ord)))
              key = (line.substring(0, j).trim).to_s
              # get the key
              break
            end
            j += 1
    ...
    def parse_realm_field_ex(v, start, end_)
      ...
      while i < end_
        ...
        while j < line.length
          if ((line.char_at(j)).equal?(Character.new(?=.ord)))
            index = 0
            key = (line.substring(0, j - 1).trim).to_s
        ...
        while e.has_more_elements
          ...
          if (object.is_a?(Hashtable))
            ...
            if (object.is_a?(String))
              System.out.println("\t" + key + " = " + (table.get(key)).to_s)
            else
              if (object.is_a?(Vector))
                v = object
                i = 0
    ...
    def default_etype(enctypes)
      ...
      default_enctypes = (get_default(enctypes, "libdefaults")).to_s
      delim = " "
      st = nil
      etype = nil
    ...
    def use_addresses
      ...
      if ((use_addr).equal?(false))
        ...
        value = (get_default("noaddresses", "libdefaults")).to_s
        use_addr = (!(value).nil? && value.equals_ignore_case("false"))
      end
      return use_addr
    end
