    ...
    class_module.module_eval {
      ...
      def printcrc32_table
        ...
        while i < 256
          ...
          temp = (JavaInteger.to_hex_string(int2crc32(i))).to_s
          System.out.print("0x" + (zerofill.substring(temp.length)).to_s + temp)
          if (!(i).equal?(255))
            System.out.print(", ")
          end
          ((i += 1) - 1)
      ...
      def byte2crc32sum_bytes(data, size)
        ...
        if (self.attr_debug)
          System.out.println(">>>crc32: " + (JavaInteger.to_hex_string(temp)).to_s)
          System.out.println(">>>crc32: " + (JavaInteger.to_binary_string(temp)).to_s)
        end
        return int2quad(temp)
      end
