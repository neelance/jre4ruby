    ...
    class_module.module_eval {
      ...
      def get_instance(e_type_const)
        ...
        when EncryptedData::ETYPE_ARCFOUR_HMAC
          ...
          msg = "encryption type = " + (to_s(e_type_const)).to_s + " (" + (e_type_const).to_s + ")"
    ...
    class_module.module_eval {
      ...
      def get_defaults(config_name)
        begin
          ...
          if (DEBUG)
            System.out.println("Exception while getting " + config_name + (exc.get_message).to_s)
