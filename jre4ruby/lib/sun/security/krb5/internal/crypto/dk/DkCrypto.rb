  ...
  class DkCrypto 
    ...
    def encrypt(base_key, usage, ivec, new_ivec, plaintext, start, len)
      if (!KeyUsage.is_valid(usage))
        raise GeneralSecurityException.new("Invalid key usage number: " + (usage).to_s)
      end
      ke = nil
      ki = nil
      begin
        ...
        if (Debug)
          System.err.println("usage: " + (usage).to_s)
          if (!(ivec).nil?)
        ...
        if (Debug)
          System.err.println("confounder = " + (confounder.attr_length).to_s + "; plaintext = " + (len).to_s + "; padding = " + ((plain_size - confounder.attr_length - len)).to_s + "; total = " + (plain_size).to_s)
    ...
    def encrypt_raw(base_key, usage, ivec, plaintext, start, len)
      ...
      if (!((len % block_size)).equal?(0))
        raise GeneralSecurityException.new("length of data to be encrypted (" + (len).to_s + ") is not a multiple of the blocksize (" + (block_size).to_s + ")")
      end
    ...
    def decrypt_raw(base_key, usage, ivec, ciphertext, start, len)
      ...
      if (!((len % block_size)).equal?(0))
        raise GeneralSecurityException.new("length of data to be decrypted (" + (len).to_s + ") is not a multiple of the blocksize (" + (block_size).to_s + ")")
      end
      ...
      begin
        ...
        if ((hmac.attr_length).equal?(get_checksum_length))
          ...
          if (hmac.attr_length > get_checksum_length)
            ...
            raise GeneralSecurityException.new("checksum size too short: " + (hmac.attr_length).to_s + "; expecting : " + (get_checksum_length).to_s)
          end
        end
      ensure
    ...
    def dr(key, constant)
      ...
      while (n < keybytes)
        if (Debug)
          System.err.println("Encrypting: " + (bytes_to_string(to_be_encrypted)).to_s)
        end
        cipher_block = enc_cipher.do_final(to_be_encrypted)
        if (Debug)
          System.err.println("K: " + ((posn += 1)).to_s + " = " + (bytes_to_string(cipher_block)).to_s)
        end
        len = (keybytes - n <= cipher_block.attr_length ? (keybytes - n) : cipher_block.attr_length)
        if (Debug)
          System.err.println("copying " + (len).to_s + " key bytes")
        end
    ...
    class_module.module_eval {
      ...
      def nfold(in_, outbits)
        ...
        if (Debug)
          System.err.println("k: " + (inbits).to_s)
          System.err.println("n: " + (outbits).to_s)
          System.err.println("lcm: " + (lcm).to_s)
        end
        ...
        while i >= 0
          ...
          if (Debug)
            System.err.println("msbit[" + (i).to_s + "] = " + (msbit).to_s + "\tbval=" + (JavaInteger.to_hex_string(bval)).to_s + "\toval=" + (JavaInteger.to_hex_string(oval)).to_s + "\tsum = " + (JavaInteger.to_hex_string(thisbyte)).to_s)
          end
      ...
      def bytes_to_string(digest)
        ...
        while i < digest.attr_length
          if ((digest[i] & 0xff) < 0x10)
            digest_string.append("0" + (JavaInteger.to_hex_string(digest[i] & 0xff)).to_s)
          else
      ...
      def trace_output(trace_tag, output, offset, len)
        begin
          ...
          System.err.println(trace_tag + ":" + (out.to_s).to_s)
        rescue Exception => e
        end
      end
