    ...
    class_module.module_eval {
      ...
      def des3_expand(input, start, end_)
        if (!((end_ - start)).equal?(7))
          raise IllegalArgumentException.new("Invalid length of DES Key Value:" + (start).to_s + "," + (end_).to_s)
        end
        ...
        while i < end_
          bit = (input[i] & 0x1)
          if (self.attr_debug)
            System.out.println((i).to_s + ": " + (JavaInteger.to_hex_string(input[i])).to_s + " bit= " + (JavaInteger.to_hex_string(bit)).to_s)
          end
          (posn += 1)
          if (!(bit).equal?(0))
            last |= (bit << posn)
