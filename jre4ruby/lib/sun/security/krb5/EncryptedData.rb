  ...
  class EncryptedData 
    ...
    def decrypt(key, usage)
      if (!(@e_type).equal?(key.get_etype))
        raise KrbCryptoException.new("EncryptedData is encrypted using keytype " + (EType.to_s(@e_type)).to_s + " but decryption key is of type " + (EType.to_s(key.get_etype)).to_s)
      end
