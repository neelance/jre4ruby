...
module Sun::Security::Krb5
  ...
  class KrbApReq 
    ...
    def initialize(tgs_cred, mutual_required, use_sub_key, use_seq_number, cksum)
      ...
      if (self.attr_debug)
        System.out.println(">>> KrbApReq: APOptions are " + (ap_options).to_s)
      end
    ...
    def decode(encoding)
      ...
      begin
        ...
        if ((err_str.char_at(err_str.length - 1)).equal?(0))
          e_text = (err_str.substring(0, err_str.length - 1)).to_s
        else
          e_text = err_str
        end
    ...
    def authenticate(keys, initiator)
      ...
      if ((dkey).nil?)
        raise KrbException.new(Krb5::API_INVALID_ARG, "Cannot find key of appropriate type to decrypt AP REP - " + (EType.to_s(enc_part_key_type)).to_s)
      end
    ...
    class_module.module_eval {
      ...
      def check_permitted_etype(target)
        ...
        if (!EType.is_supported(target, etypes))
          raise KrbException.new((EType.to_s(target)).to_s + " encryption type not in permitted_enctypes list")
        end
      end
    }
