...
module Sun::Security::Krb5
  ...
  class KrbAsRep < KrbAsRepImports.const_get :KrbKdcRep
    ...
    def initialize(ibuf, keys, as_req)
      ...
      if ((dkey).nil?)
        raise KrbException.new(Krb5::API_INVALID_ARG, "Cannot find key of appropriate type to decrypt AS REP - " + (EType.to_s(enc_part_key_type)).to_s)
      end
      ...
      if (@debug)
        System.out.println(">>> KrbAsRep cons in KrbAsReq.getReply " + (req.attr_req_body.attr_cname.get_name_string).to_s)
      end
      @rep = rep
      @creds = @creds
    end
