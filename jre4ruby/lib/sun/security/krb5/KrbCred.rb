...
module Sun::Security::Krb5
  ...
  class KrbCred 
    ...
    def initialize(asn1message, key)
      ...
      if (self.attr_debug)
        System.out.println(">>>Delegated Creds have pname=" + (pname).to_s + " sname=" + (sname).to_s + " authtime=" + (authtime).to_s + " starttime=" + (starttime).to_s + " endtime=" + (endtime).to_s + "renewTill=" + (renew_till).to_s)
      end
