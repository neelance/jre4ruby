...
module Sun::Security::Tools
  ...
  class KeyTool 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:PARAM_STRING) { Array.typed(Class).new([String.class]) }
                                                                                              ...
                                                                                              if ((Collator.compare(flags, "-trustcacerts")).equal?(0))
                                                                                                ...
                                                                                                if ((Collator.compare(flags, "-protected")).equal?(0) || (Collator.compare(flags, "-destprotected")).equal?(0))
                                                                                                  ...
                                                                                                  if ((Collator.compare(flags, "-srcprotected")).equal?(0))
                                                                                                    ...
                                                                                                    System.err.println((Rb.get_string("Illegal option:  ")).to_s + flags)
                                                                                                    tiny_help
                                                                                                  end
                                                                                                end
                                                                                              end
    ...
    def do_commands(out)
      if ((@storetype).nil?)
        @storetype = (KeyStore.get_default_type).to_s
      end
      @storetype = (KeyStoreUtil.nice_store_type_name(@storetype)).to_s
      if ((@srcstoretype).nil?)
        @srcstoretype = (KeyStore.get_default_type).to_s
      end
      @srcstoretype = (KeyStoreUtil.nice_store_type_name(@srcstoretype)).to_s
      ...
      if (@validity <= 0)
        raise Exception.new(Rb.get_string("Validity must be greater than zero"))
      end
      ...
      if (!(@providers).nil?)
        ...
        if (!(@pathlist).nil?)
          path = nil
          path = (PathList.append_path(path, System.get_property("java.class.path"))).to_s
          path = (PathList.append_path(path, System.get_property("env.class.path"))).to_s
          path = (PathList.append_path(path, @pathlist)).to_s
      ...
      if ((@command).equal?(GENKEYPAIR) && !(@key_pass).nil? && @key_pass.attr_length < 6)
        raise Exception.new(Rb.get_string("Key password must be at least 6 characters"))
      end
      if (!(@new_pass).nil? && @new_pass.attr_length < 6)
        raise Exception.new(Rb.get_string("New password must be at least 6 characters"))
      end
      ...
      if (!(@command).equal?(PRINTCERT))
        if ((@ksfname).nil?)
          @ksfname = (System.get_property("user.home") + JavaFile.attr_separator).to_s + ".keystore"
        end
        if (!@null_stream)
          begin
            ...
            if (@ksfile.exists && (@ksfile.length).equal?(0))
              raise Exception.new((Rb.get_string("Keystore file exists, but is empty: ")).to_s + @ksfname)
            end
            ...
            if (!(@command).equal?(GENKEYPAIR) && !(@command).equal?(GENSECKEY) && !(@command).equal?(IDENTITYDB) && !(@command).equal?(IMPORTCERT) && !(@command).equal?(IMPORTKEYSTORE))
              raise Exception.new((Rb.get_string("Keystore file does not exist: ")).to_s + @ksfname)
            end
          end
        end
      end
      if (((@command).equal?(KEYCLONE) || (@command).equal?(CHANGEALIAS)) && (@dest).nil?)
        @dest = (get_alias("destination")).to_s
        if (("" == @dest))
          raise Exception.new(Rb.get_string("Must specify destination alias"))
        end
      end
      if ((@command).equal?(DELETE) && (@alias).nil?)
        @alias = (get_alias(nil)).to_s
        if (("" == @alias))
          raise Exception.new(Rb.get_string("Must specify alias"))
        end
      end
      # Create new keystore
      ...
      if (@null_stream && !(@store_pass).nil?)
        ...
        if (!@null_stream && !(@store_pass).nil?)
          ...
          if ((@ks_stream).nil? && @store_pass.attr_length < 6)
            raise Exception.new(Rb.get_string("Keystore password must be at least 6 characters"))
          end
        else
          if ((@store_pass).nil?)
      ...
      if ((sig_alg_name).nil?)
        ...
        if ("DSA".equals_ignore_case(key_alg_name) || "DSS".equals_ignore_case(key_alg_name))
          ...
          if ("RSA".equals_ignore_case(key_alg_name))
            ...
            raise Exception.new(Rb.get_string("Cannot derive signature algorithm"))
          end
        end
      end
    ...
    def prompt_for_key_pass(alias_, orig, orig_pass)
      if (P12KEYSTORE.equals_ignore_case(@storetype))
        ...
        if (!@token)
          ...
          if ((count).equal?(3))
            if ((@command).equal?(KEYCLONE))
              raise Exception.new(Rb.get_string("Too many failures. Key entry not cloned"))
            else
              raise Exception.new(Rb.get_string("Too many failures - key not added to keystore"))
            end
          end
        end
      end
      ...
      if (!(keysize).equal?(-1))
        ...
        if ("DES".equals_ignore_case(key_alg_name))
          ...
          if ("DESede".equals_ignore_case(key_alg_name))
            ...
            raise Exception.new(Rb.get_string("Please provide -keysize for secret key generation"))
          end
        end
      end
    ...
    def do_print_entry(alias_, out, print_warning_)
      ...
      if (@key_store.entry_instance_of(alias_, KeyStore::SecretKeyEntry.class))
        ...
        if (@key_store.entry_instance_of(alias_, KeyStore::PrivateKeyEntry.class))
          ...
          if (@key_store.entry_instance_of(alias_, KeyStore::TrustedCertificateEntry.class))
      ...
      if (P11KEYSTORE.equals_ignore_case(@srcstoretype) || KeyStoreUtil.is_windows_key_store(@srcstoretype))
        ...
        if (!(@srcksfname).nil?)
          ...
          if (srcksfile.exists && (srcksfile.length).equal?(0))
            raise Exception.new((Rb.get_string("Source keystore file exists, but is empty: ")).to_s + @srcksfname)
          end
          is = FileInputStream.new(srcksfile)
        else
          raise Exception.new(Rb.get_string("Please specify -srckeystore"))
        end
      end
      store_ = nil
      begin
    ...
    def do_import_key_store
      if (!(@alias).nil?)
        ...
        if (!(@dest).nil? || !(@srckey_pass).nil? || !(@dest_key_pass).nil?)
          raise Exception.new(Rb.get_string("if alias not specified, destalias, srckeypass, " + "and destkeypass must not be specified"))
        end
    ...
    def do_import_key_store_single(srckeystore, alias_)
      ...
      if (@key_store.contains_alias(new_alias))
        ...
        if (@noprompt)
          ...
          if (("NO" == reply))
            new_alias = (input_string_from_stdin(Rb.get_string("Enter new alias name\t(RETURN to cancel import for this entry):  "))).to_s
            if (("" == new_alias))
    ...
    def do_print_cert(in_, out)
      ...
      begin
        ...
        raise Exception.new(Rb.get_string("Failed to parse input"), ce)
      end
      if (c.is_empty)
        raise Exception.new(Rb.get_string("Empty input"))
      end
      ...
      while i < certs.attr_length
        ...
        begin
          ...
          raise Exception.new(Rb.get_string("Not X.509 certificate"))
        end
    ...
    def do_self_cert(alias_, dname, sig_alg_name)
      ...
      cert_info = cert_impl.get((X509CertImpl::NAME).to_s + "." + (X509CertImpl::INFO).to_s)
      # Extend its validity
      ...
      if ((dname).nil?)
        ...
        owner = cert_info.get((X509CertInfo::SUBJECT).to_s + "." + (CertificateSubjectName::DN_NAME).to_s)
      else
        # Use the owner name specified at the command line
        owner = X500Name.new(dname)
        cert_info.set((X509CertInfo::SUBJECT).to_s + "." + (CertificateSubjectName::DN_NAME).to_s, owner)
      end
      # Make issuer same as owner (self-signed!)
      cert_info.set((X509CertInfo::ISSUER).to_s + "." + (CertificateIssuerName::DN_NAME).to_s, owner)
      ...
      cert_info.set((CertificateAlgorithmId::NAME).to_s + "." + (CertificateAlgorithmId::ALGORITHM).to_s, sig_algid)
    ...
    def install_reply(alias_, in_)
      ...
      c = @cf.generate_certificates(in_)
      if (c.is_empty)
        raise Exception.new(Rb.get_string("Reply has no certificates"))
      end
    ...
    def add_trusted_cert(alias_, in_)
      ...
      begin
        ...
        raise Exception.new(Rb.get_string("Input not an X.509 certificate"))
      rescue CertificateException => ce
        raise Exception.new(Rb.get_string("Input not an X.509 certificate"))
      end
      # if certificate is self-signed, make sure it verifies
      ...
      if (!(trustalias).nil?)
        ...
        if (self_signed)
          ...
          if ((trustalias).nil?)
            ...
            reply = (get_yes_no_reply(Rb.get_string("Trust this certificate? [no]:  "))).to_s
          end
        end
      end
      if (!(reply).nil?)
      ...
      if (cert.is_a?(X509CertImpl))
        ...
        if (!(cert.get_critical_extension_oids).nil?)
          cert.get_critical_extension_oids.each do |extOID|
            ...
            out.println("#" + (((extnum += 1))).to_s + ": " + (impl.get_extension(ObjectIdentifier.new(ext_oid))).to_s)
          end
        end
    ...
    def get_x500name
      ...
      begin
        ...
        common_name = (input_string(in_, Rb.get_string("What is your first and last name?"), common_name)).to_s
        organizational_unit = (input_string(in_, Rb.get_string("What is the name of your organizational unit?"), organizational_unit)).to_s
        organization = (input_string(in_, Rb.get_string("What is the name of your organization?"), organization)).to_s
        city = (input_string(in_, Rb.get_string("What is the name of your City or Locality?"), city)).to_s
        state = (input_string(in_, Rb.get_string("What is the name of your State or Province?"), state)).to_s
        country = (input_string(in_, Rb.get_string("What is the two-letter country code for this unit?"), country)).to_s
        ...
        user_input = (input_string(in_, form.format(source), Rb.get_string("no"))).to_s
    ...
    def establish_cert_chain(user_cert, cert_to_verify)
      if (!(user_cert).nil?)
        ...
        if (!(orig_pub_key == reply_pub_key))
          raise Exception.new(Rb.get_string("Public keys in reply and keystore don't match"))
        end
        # If the two certs are identical, we're done: no need to import
        # anything
        if ((cert_to_verify == user_cert))
          raise Exception.new(Rb.get_string("Certificate reply and certificate in keystore are identical"))
        end
      end
      ...
      if (build_chain(cert_to_verify, chain, certs))
        ...
        raise Exception.new(Rb.get_string("Failed to establish chain from reply"))
      end
    end
    ...
    def get_yes_no_reply(prompt)
      ...
      begin
        ...
        System.err.print(prompt)
        System.err.flush
        reply = ((BufferedReader.new(InputStreamReader.new(System.in))).read_line).to_s
    ...
    def get_cacerts_key_store
      ...
      file = JavaFile.new((System.get_property("java.home")).to_s + sep + "lib" + sep + "security" + sep + "cacerts")
      if (!file.exists)
        return nil
      end
      fis = nil
      ...
      def get_start_date(s)
        ...
        if (!(s).nil?)
          ...
          if ((s.char_at(0)).equal?(Character.new(?-.ord)) || (s.char_at(0)).equal?(Character.new(?+.ord)))
            ...
            time = nil
            if ((len).equal?(19))
              date = (s.substring(0, 10)).to_s
              time = (s.substring(11)).to_s
  ...
  class Pair 
    ...
    typesig { [Object] }
    def equals(other)
      return other.is_a?(Pair) && equals(@fst, (other).attr_fst) && equals(@snd, (other).attr_snd)
    end
