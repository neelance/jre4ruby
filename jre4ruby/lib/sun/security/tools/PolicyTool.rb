...
module Sun::Security::Tools
  ...
  class PolicyTool 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:TWOPARAMS) { Array.typed(Class).new([String.class, String.class]) }
      const_attr_reader  :TWOPARAMS
      
      const_set_lazy(:ONEPARAMS) { Array.typed(Class).new([String.class]) }
    ...
    def open_key_store(name, type, provider, pwd_url)
      ...
      if (!(self.attr_policy_file_name).nil?)
        ...
        policy_url = URL.new("file:" + (pfile.get_canonical_path).to_s)
      end
      ...
      if (!(name).nil? && name.length > 0)
        name = (PropertyExpander.expand(name).replace(JavaFile.attr_separator_char, Character.new(?/.ord))).to_s
      end
      if ((type).nil? || (type.length).equal?(0))
        type = (KeyStore.get_default_type).to_s
      end
      if (!(pwd_url).nil? && pwd_url.length > 0)
        pwd_url = (PropertyExpander.expand(pwd_url).replace(JavaFile.attr_separator_char, Character.new(?/.ord))).to_s
      end
      begin
      ...
      def parse_args(args)
        ...
        while (n < args.attr_length) && args[n].starts_with("-")
          ...
          if ((Collator.compare(flags, "-file")).equal?(0))
            ...
            self.attr_policy_file_name = (args[n]).to_s
          else
  ...
  class PolicyEntry 
    ...
    def initialize(tool, ge)
      ...
      if (@testing)
        ...
        System.out.println("    CodeBase = " + (location).to_s)
        System.out.println("    Signers = " + (ge.attr_signed_by).to_s)
        System.out.println("    with " + (ge.attr_principals.size).to_s + " Principals")
      end
      @grant_entry = ge
    end
    ...
    def header_to_string
      ...
      if ((p_string.length).equal?(0))
        ...
        return (codebase_to_string).to_s + ", " + p_string
      end
    end
    ...
    def codebase_to_string
      ...
      if (!(@grant_entry.attr_code_base).nil? && ((@grant_entry.attr_code_base == "")).equal?(false))
        string_entry = (string_entry.concat("CodeBase \"" + (@grant_entry.attr_code_base).to_s + "\"")).to_s
      end
      if (!(@grant_entry.attr_signed_by).nil? && ((@grant_entry.attr_signed_by == "")).equal?(false))
        string_entry = (((string_entry.length > 0) ? string_entry.concat(", SignedBy \"" + (@grant_entry.attr_signed_by).to_s + "\"") : string_entry.concat("SignedBy \"" + (@grant_entry.attr_signed_by).to_s + "\""))).to_s
      end
    ...
    def principals_to_string
      ...
      if ((!(@grant_entry.attr_principals).nil?) && (!@grant_entry.attr_principals.is_empty))
        ...
        while (list.has_next)
          pppe = list.next
          buffer.append(" Principal " + (pppe.get_display_class).to_s + " " + (pppe.get_display_name(true)).to_s)
          if (list.has_next)
            buffer.append(", ")
          end
        end
        result = (buffer.to_s).to_s
      end
      return result
    end
  ...
  class ToolWindow < PolicyToolImports.const_get :Frame
    ...
    def init_window
      ...
      if ((policy_file).nil?)
        user_home = nil
        user_home = (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("user.home"))).to_s
        policy_file = user_home + (JavaFile.attr_separator_char).to_s + ".java.policy"
      end
      begin
  ...
  class ToolDialog < PolicyToolImports.const_get :Dialog
    ...
    def get_policy_entry_from_dialog
      ...
      if (((tf.get_text.trim == "")).equal?(false))
        codebase = (String.new(tf.get_text.trim)).to_s
      end
      # get the SignedBy
      tf = get_component(PE_SIGNEDBY_TEXTFIELD)
      signedby = nil
      if (((tf.get_text.trim == "")).equal?(false))
        signedby = (String.new(tf.get_text.trim)).to_s
      end
    ...
    def get_perm_from_dialog
      ...
      name = nil
      if (((tf.get_text.trim == "")).equal?(false))
        name = (String.new(tf.get_text.trim)).to_s
      end
      ...
      actions = nil
      if (((tf.get_text.trim == "")).equal?(false))
        actions = (String.new(tf.get_text.trim)).to_s
      end
      # get the Signed By
      tf = get_component(PD_SIGNEDBY_TEXTFIELD)
      signed_by = nil
      if (((tf.get_text.trim == "")).equal?(false))
        signed_by = (String.new(tf.get_text.trim)).to_s
      end
    ...
    class_module.module_eval {
      ...
      def _permission_entry_to_user_friendly_string(pppe)
        ...
        if (!(pppe.attr_name).nil?)
          result += " " + (pppe.attr_name).to_s
        end
        if (!(pppe.attr_action).nil?)
          result += ", \"" + (pppe.attr_action).to_s + "\""
        end
        if (!(pppe.attr_signed_by).nil?)
          result += ", signedBy " + (pppe.attr_signed_by).to_s
        end
        return result
      end
  ...
  class RemovePrinButtonListener 
    ...
    def action_performed(e)
      ...
      prin_index = prin_list.get_selected_index
      if (prin_index < 0)
        @tw.display_error_dialog(@td, Exception.new(PolicyTool.attr_rb.get_string("No principal selected")))
        return
      end
  ...
  class RemovePermButtonListener 
    ...
    def action_performed(e)
      ...
      perm_index = perm_list.get_selected_index
      if (perm_index < 0)
        @tw.display_error_dialog(@td, Exception.new(PolicyTool.attr_rb.get_string("No permission selected")))
        return
      end
    ...
    def item_state_changed(e)
      ...
      if ((tf.get_text).nil? || (tf.get_text == ""))
        ...
        if ((tf.get_text.index_of(e.get_item)).equal?(-1))
          tf.set_text((tf.get_text).to_s + ", " + (e.get_item).to_s)
        end
      end
    end
  ...
  class RuntimePerm < PolicyToolImports.const_get :Perm
    ...
    def initialize
      # "inheritedChannel"
      super("RuntimePermission", "java.lang.RuntimePermission", Array.typed(String).new(["createClassLoader", "getClassLoader", "setContextClassLoader", "enableContextClassLoaderOverride", "setSecurityManage", "createSecurityManager", "getenv.<" + (PolicyTool.attr_rb.get_string("environment variable name")).to_s + ">", "exitVM", "shutdownHooks", "setFactory", "setIO", "modifyThread", "stopThread", "modifyThreadGroup", "getProtectionDomain", "readFileDescriptor", "writeFileDescriptor", "loadLibrary.<" + (PolicyTool.attr_rb.get_string("library name")).to_s + ">", "accessClassInPackage.<" + (PolicyTool.attr_rb.get_string("package name")).to_s + ">", "defineClassInPackage.<" + (PolicyTool.attr_rb.get_string("package name")).to_s + ">", "accessDeclaredMembers", "queuePrintJob", "getStackTrace", "setDefaultUncaughtExceptionHandler", "preferences", "usePolicy", ]), nil)
    end
  ...
  class SecurityPerm < PolicyToolImports.const_get :Perm
    ...
    def initialize
      ...
      # "getSignerPrivateKey",
      # "setSignerKeyPair"
      super("SecurityPermission", "java.security.SecurityPermission", Array.typed(String).new(["createAccessControlContext", "getDomainCombiner", "getPolicy", "setPolicy", "createPolicy.<" + (PolicyTool.attr_rb.get_string("policy type")).to_s + ">", "getProperty.<" + (PolicyTool.attr_rb.get_string("property name")).to_s + ">", "setProperty.<" + (PolicyTool.attr_rb.get_string("property name")).to_s + ">", "insertProvider.<" + (PolicyTool.attr_rb.get_string("provider name")).to_s + ">", "removeProvider.<" + (PolicyTool.attr_rb.get_string("provider name")).to_s + ">", "clearProviderProperties.<" + (PolicyTool.attr_rb.get_string("provider name")).to_s + ">", "putProviderProperty.<" + (PolicyTool.attr_rb.get_string("provider name")).to_s + ">", "removeProviderProperty.<" + (PolicyTool.attr_rb.get_string("provider name")).to_s + ">", ]), nil)
    end
