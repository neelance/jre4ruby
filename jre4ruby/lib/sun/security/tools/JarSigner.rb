...
module Sun::Security::Tools
  ...
  class JarSigner 
    ...
    def parse_args(args)
        ...
        if ((Collator.compare(flags, "-keystore")).equal?(0))
          ...
          if ((Collator.compare(flags, "-storepass")).equal?(0))
            ...
            if ((Collator.compare(flags, "-storetype")).equal?(0))
              ...
              @storetype = (args[n]).to_s
            else
              if ((Collator.compare(flags, "-providerName")).equal?(0))
                if (((n += 1)).equal?(args.attr_length))
                  usage
                end
                @provider_name = (args[n]).to_s
              else
      ...
      @jarfile = (args[((n += 1) - 1)]).to_s
      if (!@verify)
        if ((n).equal?(args.attr_length))
          usage
        end
        @alias = (args[((n += 1) - 1)]).to_s
      end
      if ((@storetype).nil?)
      ...
      while i < signers.attr_length
        ...
        certs.each do |c|
          ...
          if (!(alias_).nil?)
            ...
            if (!(@store).nil?)
              begin
                alias_ = (@store.get_certificate_alias(c)).to_s
            ...
            if (!found && (!(@scope).nil?))
              ...
              if (!(id).nil?)
                result |= IN_SCOPE
                @store_hash.put(c, "[" + (id.get_name).to_s + "]")
              end
            end
          end
        end
    ...
    def sign_jar(jar_name, alias_, args)
      ...
      if (@sigfile.length > 8)
        @sigfile = (@sigfile.substring(0, 8).to_upper_case).to_s
      else
        @sigfile = (@sigfile.to_upper_case).to_s
      end
      ...
      begin
        ...
        error((Rb.get_string("unable to open jar file: ")).to_s + jar_name, ioe)
      end
      fos = nil
      begin
      ...
      begin
        ...
        begin
          ...
          error((Rb.get_string("unable to sign jar: ") + Rb.get_string("no response from the Timestamping Authority. ") + Rb.get_string("When connecting from behind a firewall then an HTTP proxy may need to be specified. ") + Rb.get_string("Supply the following options to jarsigner: ")).to_s + "\n  -J-Dhttp.proxyHost=<hostname> " + "\n  -J-Dhttp.proxyPort=<portnumber> ", e)
        end
        sf_filename = (sf.get_meta_name).to_s
        bk_filename = (block.get_meta_name).to_s
        ...
        if (@verbose)
          if (!(@zip_file.get_entry(sf_filename)).nil?)
            System.out.println((Rb.get_string(" updating: ")).to_s + sf_filename)
          else
            System.out.println((Rb.get_string("   adding: ")).to_s + sf_filename)
          end
        end
        if (@verbose)
          ...
          if (!(@tsa_url).nil?)
            System.out.println((Rb.get_string("TSA location: ")).to_s + @tsa_url)
          end
          if (!(tsa_cert).nil?)
            cert_url = TimestampedSigner.get_timestamping_url(tsa_cert)
            if (!(cert_url).nil?)
              System.out.println((Rb.get_string("TSA location: ")).to_s + cert_url)
            end
        ...
        if (@verbose)
          if (!(@zip_file.get_entry(bk_filename)).nil?)
            System.out.println((Rb.get_string(" updating: ")).to_s + bk_filename)
          else
            System.out.println((Rb.get_string("   adding: ")).to_s + bk_filename)
          end
        end
    ...
    def load_key_store(key_store_name, prompt)
      if (!@null_stream && (key_store_name).nil?)
        key_store_name = (System.get_property("user.home") + JavaFile.attr_separator).to_s + ".keystore"
      end
      begin
        ...
        if (@null_stream)
          ...
          key_store_name = (key_store_name.replace(JavaFile.attr_separator_char, Character.new(?/.ord))).to_s
          url = nil
          begin
    ...
    typesig { [String] }
    def error(message)
      System.out.println((Rb.get_string("jarsigner: ")).to_s + message)
      System.exit(1)
    end
    ...
    def load_signing_mechanism(signer_class_name, signer_class_path)
      ...
      cp_string = (PathList.append_path(System.get_property("env.class.path"), cp_string)).to_s
      cp_string = (PathList.append_path(System.get_property("java.class.path"), cp_string)).to_s
      cp_string = (PathList.append_path(signer_class_path, cp_string)).to_s
    ...
    class_module.module_eval {
      const_set_lazy(:Block) { Class.new do
        ...
        def initialize(sfg, private_key, sigalg, cert_chain, external_sf, tsa_url, tsa_cert, signing_mechanism, args, zip_file)
          ...
          @block_file_name = "META-INF/" + (sfg.get_base_name).to_s + "." + key_algorithm
