  ...
  class DerValue 
    ...
    def get_boolean
      if (!(@tag).equal?(Tag_Boolean))
        raise IOException.new("DerValue.getBoolean, not a BOOLEAN " + (@tag).to_s)
      end
      if (!(@length).equal?(1))
        raise IOException.new("DerValue.getBoolean, invalid length " + (@length).to_s)
      end
    ...
    def get_oid
      if (!(@tag).equal?(Tag_ObjectId))
        raise IOException.new("DerValue.getOID, not an OID " + (@tag).to_s)
      end
    ...
    def get_octet_string
      ...
      if (!(@tag).equal?(Tag_OctetString) && !is_constructed(Tag_OctetString))
        raise IOException.new("DerValue.getOctetString, not an Octet String: " + (@tag).to_s)
      end
      bytes = Array.typed(::Java::Byte).new(@length) { 0 }
    ...
    def get_integer
      if (!(@tag).equal?(Tag_Integer))
        raise IOException.new("DerValue.getInteger, not an int " + (@tag).to_s)
      end
    ...
    def get_big_integer
      if (!(@tag).equal?(Tag_Integer))
        raise IOException.new("DerValue.getBigInteger, not an int " + (@tag).to_s)
      end
    ...
    def get_enumerated
      if (!(@tag).equal?(Tag_Enumerated))
        raise IOException.new("DerValue.getEnumerated, incorrect tag: " + (@tag).to_s)
      end
      return @buffer.get_integer(@data.available)
    end
    ...
    def get_bit_string
      if (!(@tag).equal?(Tag_BitString))
        raise IOException.new("DerValue.getBitString, not a bit string " + (@tag).to_s)
      end
    ...
    def get_printable_string
      if (!(@tag).equal?(Tag_PrintableString))
        raise IOException.new("DerValue.getPrintableString, not a string " + (@tag).to_s)
      end
    ...
    def get_t61string
      if (!(@tag).equal?(Tag_T61String))
        raise IOException.new("DerValue.getT61String, not T61 " + (@tag).to_s)
      end
    ...
    def get_ia5string
      if (!(@tag).equal?(Tag_IA5String))
        raise IOException.new("DerValue.getIA5String, not IA5 " + (@tag).to_s)
      end
      return String.new(get_data_bytes, "ASCII")
    end
    ...
    def get_bmpstring
      if (!(@tag).equal?(Tag_BMPString))
        raise IOException.new("DerValue.getBMPString, not BMP " + (@tag).to_s)
      end
    ...
    def get_utf8string
      if (!(@tag).equal?(Tag_UTF8String))
        raise IOException.new("DerValue.getUTF8String, not UTF-8 " + (@tag).to_s)
      end
    ...
    def get_general_string
      if (!(@tag).equal?(Tag_GeneralString))
        raise IOException.new("DerValue.getGeneralString, not GeneralString " + (@tag).to_s)
      end
      return String.new(get_data_bytes, "ASCII")
    end
    ...
    def get_utctime
      if (!(@tag).equal?(Tag_UtcTime))
        raise IOException.new("DerValue.getUTCTime, not a UtcTime: " + (@tag).to_s)
      end
    ...
    def get_generalized_time
      if (!(@tag).equal?(Tag_GeneralizedTime))
        raise IOException.new("DerValue.getGeneralizedTime, not a GeneralizedTime: " + (@tag).to_s)
      end
    ...
    def to_s
      begin
        ...
        if ((@tag).equal?(Tag_ObjectId))
          return "OID." + (get_oid).to_s
        # integers
        else
          return "[DerValue, tag = " + (@tag).to_s + ", length = " + (@length).to_s + "]"
        end
    ...
    def to_der_input_stream
      ...
      raise IOException.new("toDerInputStream rejects tag type " + (@tag).to_s)
    end
