...
module Sun::Security::Util
  ...
  class MemoryCache < CacheImports.const_get :Cache
    ...
    def empty_queue
      ...
      if (DEBUG)
        ...
        if (!(start_size).equal?(end_size))
          System.out.println("*** Expunged " + ((start_size - end_size)).to_s + " entries, " + (end_size).to_s + " entries left")
        end
      end
    end
    ...
    def expunge_expired_entries
      ...
      if (DEBUG)
        if (!(cnt).equal?(0))
          System.out.println("Removed " + (cnt).to_s + " expired entries, remaining " + (@cache_map.size).to_s)
        end
      end
    end
      ...
      synchronized(self) do
        ...
        if (@cache_map.size > @max_size)
          ...
          if (@cache_map.size > @max_size)
            ...
            lru_entry = t.next
            if (DEBUG)
              System.out.println("** Overflow removal " + (lru_entry.get_key).to_s + " | " + (lru_entry.get_value).to_s)
            end
            t.remove
