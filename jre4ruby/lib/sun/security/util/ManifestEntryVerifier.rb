  ...
  class ManifestEntryVerifier 
    ...
    def verify(verified_signers, sig_file_signers)
      ...
      while i < @digests.size
        ...
        if (!(Debug).nil?)
          Debug.println("Manifest Entry: " + @name + " digest=" + (digest.get_algorithm).to_s)
          Debug.println("  manifest " + (to_hex(man_hash)).to_s)
          Debug.println("  computed " + (to_hex(the_hash)).to_s)
          Debug.println
        end
