  ...
  class GSSCredentialImpl 
      ...
      while i < mechs.attr_length
        begin
          ...
          if (default_list)
            ...
            GSSUtil.debug("Ignore " + (e).to_s + " while acquring cred for " + (mechs[i]).to_s)
            # e.printStackTrace();
          else
            raise e
    ...
    def add(name, init_lifetime, accept_lifetime, mech, usage)
      ...
      if (@hashtable.contains_key(key))
        raise GSSExceptionImpl.new(GSSException::DUPLICATE_ELEMENT, "Duplicate element found: " + (get_element_str(mech, usage)).to_s)
      end
    ...
    def get_element(mech_oid, initiate)
      ...
      if ((element).nil?)
        raise GSSExceptionImpl.new(GSSException::NO_CRED, "No credential found for: " + (mech_oid).to_s + (get_element_str(mech_oid, initiate ? INITIATE_ONLY : ACCEPT_ONLY)).to_s)
      end
      return element
    end
    ...
    class_module.module_eval {
      ...
      def get_element_str(mech_oid, usage)
        ...
        if ((usage).equal?(GSSCredential::INITIATE_ONLY))
          display_string = (display_string.concat(" usage: Initiate")).to_s
        else
          if ((usage).equal?(GSSCredential::ACCEPT_ONLY))
            display_string = (display_string.concat(" usage: Accept")).to_s
          else
            display_string = (display_string.concat(" usage: Initiate and Accept")).to_s
          end
        end
