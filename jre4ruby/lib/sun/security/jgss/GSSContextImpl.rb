        ...
        if ((@mech_ctxt).nil?)
          ...
          if ((@mech_ctxt.get_provider.get_name == "SunNativeGSS") || GSSUtil.is_sp_nego_mech(@mech_oid))
            ...
            if (!(gss_header.get_oid == @obj_id))
              raise GSSExceptionImpl.new(GSSException::DEFECTIVE_TOKEN, "Mechanism not equal to " + (@mech_oid.to_s).to_s + " in initSecContext token")
            end
      ...
      begin
        if ((@mech_ctxt).nil?)
          ...
          if ((@mech_ctxt.get_provider.get_name == "SunNativeGSS") || (GSSUtil.is_sp_nego_mech(@mech_oid)))
            ...
            gss_header = GSSHeader.new(in_stream)
            if (!(gss_header.get_oid == @obj_id))
              raise GSSExceptionImpl.new(GSSException::DEFECTIVE_TOKEN, "Mechanism not equal to " + (@mech_oid.to_s).to_s + " in acceptSecContext token")
            end
            in_token_len = gss_header.get_mech_token_length
          end
        end
