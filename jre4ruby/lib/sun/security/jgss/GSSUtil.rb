  ...
  class GSSUtil 
    ...
    class_module.module_eval {
      ...
      def get_subject(name, creds)
        ...
        if (name.is_a?(GSSNameImpl))
          begin
            ...
            if (ne.is_a?(Krb5NameElement))
              krb_name = ((ne).get_krb5principal_name.get_name).to_s
            end
            krb_princ = KerberosPrincipal.new(krb_name)
            krb5principals.add(krb_princ)
          rescue GSSException => ge
            debug("Skipped name " + (name).to_s + " due to " + (ge).to_s)
          end
        end
        ...
        debug("principals=" + (krb5principals).to_s)
        debug("public creds=" + (pub_credentials).to_s)
        debug("private creds=" + (priv_credentials).to_s)
        ...
        while (elements.has_next)
          ...
          if (cred.is_a?(KerberosTicket))
            ...
            if (cred.is_a?(KerberosKey))
              ...
              debug("Skipped cred element: " + (cred).to_s)
            end
          end
        end
      end
      ...
      def search_subject(name, mech, initiate, cred_cls)
        debug("Search Subject for " + (get_mech_str(mech)).to_s + ((initiate ? " INIT" : " ACCEPT")).to_s + " cred (" + (((name).nil? ? "<<DEF>>" : name.to_s)).to_s + ", " + (cred_cls.get_name).to_s + ")")
