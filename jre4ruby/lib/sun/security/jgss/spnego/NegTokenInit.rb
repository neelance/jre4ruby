  ...
  class NegTokenInit < NegTokenInitImports.const_get :SpNegoToken
    ...
    def encode
      begin
        ...
        raise GSSException.new(GSSException::DEFECTIVE_TOKEN, -1, "Invalid SPNEGO NegTokenInit token : " + (e.get_message).to_s)
      end
    end
    ...
    def parse_token(in_)
      begin
        ...
        if (!GSSUtil.use_msinterop && (tmp1.attr_data.available > 0))
          ...
          if (DEBUG)
            System.out.println("SpNegoToken NegTokenInit: " + "MechListMIC Token = " + (get_hex_bytes(@mech_list_mic)).to_s)
          end
        else
          if (DEBUG)
