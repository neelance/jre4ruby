  ...
  class Krb5Util 
    ...
    class_module.module_eval {
      ...
      def get_ticket_from_subject_and_tgs(caller, client_principal, server_principal, tgs_principal, acc)
        ...
        ticket = SubjectComber.find(acc_subj, server_principal, client_principal, KerberosTicket.class)
        if (!(ticket).nil?)
        ...
        if (!GSSUtil.use_subject_creds_only(caller))
          ...
          begin
            ...
            ticket = SubjectComber.find(login_subj, server_principal, client_principal, KerberosTicket.class)
            if (!(ticket).nil?)
              return ticket # found it
            end
        ...
        tgt = SubjectComber.find(acc_subj, tgs_principal, client_principal, KerberosTicket.class)
        from_acc = false
        if ((tgt).nil? && !(login_subj).nil?)
          # 4. Try to get TGT from login subject
          tgt = SubjectComber.find(login_subj, tgs_principal, client_principal, KerberosTicket.class)
          from_acc = false
        else
          from_acc = true
        end
      ...
      def get_ticket(caller, client_principal, server_principal, acc)
        ...
        if ((ticket).nil? && !GSSUtil.use_subject_creds_only(caller))
          ...
          ticket = SubjectComber.find(subject, server_principal, client_principal, KerberosTicket.class)
        end
        return ticket
      end
      ...
      def get_keys(caller, server_principal, acc)
        acc_subj = Subject.get_subject(acc)
        kkeys = SubjectComber.find_many(acc_subj, server_principal, nil, KerberosKey.class)
        if ((kkeys).nil? && !GSSUtil.use_subject_creds_only(caller))
          subject = GSSUtil.login(caller, GSSUtil::GSS_KRB5_MECH_OID)
          kkeys = SubjectComber.find_many(subject, server_principal, nil, KerberosKey.class)
        end
        len = 0
