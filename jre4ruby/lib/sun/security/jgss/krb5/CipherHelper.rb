  ...
  class CipherHelper 
    ...
    def calculate_checksum(alg, header, trailer, data, start, len, token_id)
      ...
      when MessageToken::SGN_ALG_DES_MAC_MD5
        ...
        begin
          ...
          ge = GSSException.new(GSSException::FAILURE, -1, "Could not get MD5 Message Digest - " + (e.get_message).to_s)
          ge.init_cause(e)
          raise ge
        end
    ...
    def calculate_checksum(header, data, start, len, key_usage)
      ...
      when EncryptedData::ETYPE_AES128_CTS_HMAC_SHA1_96
        begin
          ...
          ge = GSSException.new(GSSException::FAILURE, -1, "Could not use AES128 signing algorithm - " + (e.get_message).to_s)
          ge.init_cause(e)
          raise ge
        end
        begin
    ...
    def encrypt_seq(ivec, plaintext, start, len)
      ...
      when MessageToken::SGN_ALG_DES_MAC_MD5, MessageToken::SGN_ALG_DES_MAC
        begin
          ...
          ge = GSSException.new(GSSException::FAILURE, -1, "Could not encrypt sequence number using DES - " + (e.get_message).to_s)
          ge.init_cause(e)
          raise ge
        end
        iv = nil
    ...
    def decrypt_seq(ivec, ciphertext, start, len)
      ...
      when MessageToken::SGN_ALG_DES_MAC_MD5, MessageToken::SGN_ALG_DES_MAC
        begin
          ...
          ge = GSSException.new(GSSException::FAILURE, -1, "Could not decrypt sequence number using DES - " + (e.get_message).to_s)
          ge.init_cause(e)
          raise ge
        end
        iv = nil
    ...
    def decrypt_data(token, ciphertext, c_start, c_len, plaintext, p_start)
      ...
      when MessageToken::SEAL_ALG_ARCFOUR_HMAC
        ...
        raise GSSException.new(GSSException::FAILURE, -1, "Unsupported seal algorithm: " + (@seal_alg).to_s)
      end
    end
    ...
    def decrypt_data(token, ciphertext, c_start, c_len, plaintext, p_start, key_usage)
      ...
      when EncryptedData::ETYPE_AES256_CTS_HMAC_SHA1_96
        ...
        raise GSSException.new(GSSException::FAILURE, -1, "Unsupported etype: " + (@etype).to_s)
      end
    end
    ...
    def encrypt_data(token, confounder, plaintext, p_start, p_len, padding, ciphertext, c_start)
      ...
      when MessageToken::SEAL_ALG_DES
        ...
        begin
          ...
          ge = GSSException.new(GSSException::FAILURE, -1, "Could not use DES Cipher - " + (e.get_message).to_s)
          ge.init_cause(e)
          raise ge
        end
      when MessageToken::SEAL_ALG_DES3_KD
    ...
    def des3_kd_decrypt(token, ciphertext, c_start, c_len, plaintext, p_start)
      ...
      begin
        ...
        ge = GSSException.new(GSSException::FAILURE, -1, "Could not use DES3-KD Cipher - " + (e.get_message).to_s)
        ge.init_cause(e)
        raise ge
      end
      # 
    ...
    def arc_four_decrypt(token, ciphertext, c_start, c_len, plaintext, p_start)
      ...
      begin
        ...
        ge = GSSException.new(GSSException::FAILURE, -1, "Could not use ArcFour Cipher - " + (e.get_message).to_s)
        ge.init_cause(e)
        raise ge
      end
      # 
    ...
    def aes128_encrypt(confounder, token_header, plaintext, start, len, key_usage)
      ...
      begin
        ...
        # GeneralSecurityException, KrbCryptoException
        ge = GSSException.new(GSSException::FAILURE, -1, "Could not use AES128 Cipher - " + (e.get_message).to_s)
        ge.init_cause(e)
        raise ge
      end
    end
    ...
    def aes256_encrypt(confounder, token_header, plaintext, start, len, key_usage)
      ...
      begin
        ...
        # GeneralSecurityException, KrbCryptoException
        ge = GSSException.new(GSSException::FAILURE, -1, "Could not use AES256 Cipher - " + (e.get_message).to_s)
        ge.init_cause(e)
        raise ge
      end
    end
