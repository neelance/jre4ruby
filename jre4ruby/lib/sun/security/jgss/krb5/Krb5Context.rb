...
module Sun::Security::Jgss::Krb5
  ...
  class Krb5Context 
    ...
    def reset_my_sequence_number(seq_number)
      if (DEBUG)
        System.out.println("Krb5Context setting mySeqNumber to: " + (seq_number).to_s)
      end
      synchronized((@my_seq_number_lock)) do
    ...
    def reset_peer_sequence_number(seq_number)
      if (DEBUG)
        System.out.println("Krb5Context setting peerSeqNumber to: " + (seq_number).to_s)
      end
    ...
    def init_sec_context(is, mech_token_size)
      ...
      if (DEBUG)
        System.out.println("Entered Krb5Context.initSecContext with " + "state=" + (print_state(@state)).to_s)
      end
      if (!is_initiator)
      ...
      begin
        if ((@state).equal?(STATE_NEW))
          ...
          if (GSSUtil.use_subject_creds_only(@caller))
            ...
            if (!(kerb_ticket).nil?)
              if (DEBUG)
                System.out.println("Found service ticket in " + "the subject" + (kerb_ticket).to_s)
              end
    ...
    def accept_sec_context(is, mech_token_size)
      ret_val = nil
      if (DEBUG)
        System.out.println("Entered Krb5Context.acceptSecContext with " + "state=" + (print_state(@state)).to_s)
      end
      if (is_initiator)
    ...
    def wrap(in_buf, offset, len, msg_prop)
      if (DEBUG)
        System.out.println("Krb5Context.wrap: data=[" + (get_hex_bytes(in_buf, offset, len)).to_s + "]")
      end
      ...
      begin
        ...
        if (DEBUG)
          System.out.println("Krb5Context.wrap: token=[" + (get_hex_bytes(enc_token, 0, enc_token.attr_length)).to_s + "]")
        end
        return enc_token
      rescue IOException => e
        enc_token = nil
    ...
    def wrap(in_buf, in_offset, len, out_buf, out_offset, msg_prop)
      ...
      begin
        ...
        if (DEBUG)
          System.out.println("Krb5Context.wrap: token=[" + (get_hex_bytes(out_buf, out_offset, ret_val)).to_s + "]")
        end
        return ret_val
      rescue IOException => e
        ret_val = 0
    ...
    def unwrap(in_buf, offset, len, msg_prop)
      if (DEBUG)
        System.out.println("Krb5Context.unwrap: token=[" + (get_hex_bytes(in_buf, offset, len)).to_s + "]")
      end
      if (!(@state).equal?(STATE_DONE))
      ...
      if (DEBUG)
        System.out.println("Krb5Context.unwrap: data=[" + (get_hex_bytes(data, 0, data.attr_length)).to_s + "]")
      end
      return data
    end
