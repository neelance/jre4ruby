      ...
      begin
        ...
        if (!prop.get_privacy && ((token_id).equal?(Krb5Token::WRAP_ID_v2)))
          if (!(@checksum.attr_length).equal?(@ec))
            raise GSSException.new(GSSException::DEFECTIVE_TOKEN, -1, (get_token_name(token_id)).to_s + ":" + "EC incorrect!")
          end
        end
      rescue IOException => e
        raise GSSException.new(GSSException::DEFECTIVE_TOKEN, -1, (get_token_name(token_id)).to_s + ":" + (e.get_message).to_s)
      end
    end
      ...
      const_set_lazy(:MessageTokenHeader) { Class.new do
        ...
        def initialize(is, prop, tok_id)
          ...
          if (!((flag).equal?(acceptor_flag)))
            raise GSSException.new(GSSException::DEFECTIVE_TOKEN, -1, (get_token_name(@token_id)).to_s + ":" + "Acceptor Flag Missing!")
          end
          ...
          if (!(@token_id).equal?(tok_id))
            raise GSSException.new(GSSException::DEFECTIVE_TOKEN, -1, (get_token_name(@token_id)).to_s + ":" + "Defective Token ID!")
          end
          # validate filler
