...
module Sun::Security::Provider
  ...
  class SunEntries 
    include_class_members SunEntriesImports
    ...
    class_module.module_eval {
      ...
      def put_entries(map)
        ...
        # otherwise after it
        native_available = NativePRNG.is_available
        use_urandom = (SeedSource == URL_DEV_URANDOM)
        ...
        dsa_key_classes = "java.security.interfaces.DSAPublicKey" + "|java.security.interfaces.DSAPrivateKey"
        map.put("Signature.SHA1withDSA SupportedKeyClasses", dsa_key_classes)
        map.put("Signature.NONEwithDSA SupportedKeyClasses", dsa_key_classes)
      ...
      const_set_lazy(:PROP_EGD) { "java.security.egd" }
      const_attr_reader  :PROP_EGD
      
      # name of the *Security* property
      const_set_lazy(:PROP_RNDSOURCE) { "securerandom.source" }
      const_attr_reader  :PROP_RNDSOURCE
      
      const_set_lazy(:URL_DEV_RANDOM) { "file:/dev/random" }
      const_attr_reader  :URL_DEV_RANDOM
      
      const_set_lazy(:URL_DEV_URANDOM) { "file:/dev/urandom" }
      const_attr_reader  :URL_DEV_URANDOM
      
      when_class_loaded do
        const_set :SeedSource, (AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          extend LocalClass
          ...
          define_method :run do
            egd_source = System.get_property(PROP_EGD, "")
            if (!(egd_source.length).equal?(0))
              return egd_source
            end
            egd_source = (Security.get_property(PROP_RNDSOURCE)).to_s
            if ((egd_source).nil?)
              return ""
            end
            return egd_source
    ...
    private
    alias_method :initialize__sun_entries, :initialize
  end
