...
module Sun::Security::Provider
  module IdentityDatabaseImports
  ...
  class IdentityDatabase < IdentityDatabaseImports.const_get :IdentityScope
    ...
    def remove_identity(identity)
      ...
      if ((@identities.get(name)).nil?)
        raise KeyManagementException.new("there is no identity named " + name + " in " + (self).to_s)
      end
    ...
    class_module.module_eval {
      ...
      def system_database_file
        ...
        if ((db_path).nil?)
          db_path = (System.get_property("user.home") + JavaFile.attr_separator_char).to_s + "identitydb.obj"
        end
      ...
      def initialize_system
        ...
        begin
          if (db_file.exists)
            debug("loading system database from file: " + (db_file).to_s)
          ...
          debug("System database initialized: " + (system_database).to_s)
    ...
    # }
    def to_s
      return "sun.security.provider.IdentityDatabase, source file: " + (@source_file).to_s
    end
    ...
    def local_check(directive)
      ...
      if (!(security).nil?)
        directive = (self.get_class.get_name).to_s + "." + directive + "." + (local_full_name).to_s
    ...
    def local_full_name
      parsable = get_name
      if (!(get_scope).nil?)
        parsable += "." + (get_scope.get_name).to_s
      end
      return parsable
    end
