  ...
  class DSAKeyFactory < DSAKeyFactoryImports.const_get :KeyFactorySpi
    ...
    def engine_generate_public(key_spec)
      begin
        ...
        raise InvalidKeySpecException.new("Inappropriate key specification: " + (e.get_message).to_s)
      end
    end
    ...
    def engine_get_key_spec(key, key_spec)
      ...
      begin
        ...
        raise InvalidKeySpecException.new("Unsupported key specification: " + (e.get_message).to_s)
      end
    end
    ...
    def engine_translate_key(key)
      begin
        if (key.is_a?(Java::Security::Interfaces::DSAPublicKey))
          ...
          # Convert key to spec
          dsa_pub_key_spec = engine_get_key_spec(key, DSAPublicKeySpec.class)
          ...
          if (key.is_a?(Java::Security::Interfaces::DSAPrivateKey))
            ...
            # Convert key to spec
            dsa_priv_key_spec = engine_get_key_spec(key, DSAPrivateKeySpec.class)
            # Create key from spec, and return it
        ...
        raise InvalidKeyException.new("Cannot translate key: " + (e.get_message).to_s)
      end
    end
