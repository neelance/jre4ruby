  ...
  class DSAPrivateKey < DSAPrivateKeyImports.const_get :PKCS8Key
    ...
    def initialize(x, p, q, g)
      ...
      begin
        ...
        ike = InvalidKeyException.new("could not DER encode x: " + (e.get_message).to_s)
        ike.init_cause(e)
        raise ike
      end
    end
    ...
    def get_params
      begin
        if (self.attr_algid.is_a?(DSAParams))
          ...
          param_spec = alg_params.get_parameter_spec(DSAParameterSpec.class)
          return param_spec
        end
    ...
    typesig { [] }
    def to_s
      return "Sun DSA Private Key \nparameters:" + (self.attr_algid).to_s + "\nx: " + (Debug.to_hex_string(@x)).to_s + "\n"
    end
