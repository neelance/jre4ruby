  ...
  class DistributionPointFetcher 
    ...
    def get_crls(selector, sign_flag, prev_key, provider, cert_stores, reasons_mask, anchor)
      ...
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("DistributionPointFetcher.getCRLs: Checking " + "CRLDPs for " + (cert_impl.get_subject_x500principal).to_s)
        end
        ...
        if (!(Debug).nil?)
          Debug.println("Returning " + (results.size).to_s + " CRLs")
        end
        return results
    ...
    def get_crls(selector, cert_impl, point, reasons_mask, sign_flag, prev_key, provider, cert_stores, anchor)
      ...
      possible_crls.each do |crl|
        begin
          ...
          # don't add the CRL
          if (!(Debug).nil?)
            Debug.println("Exception verifying CRL: " + (e.get_message).to_s)
            e.print_stack_trace
          end
        end
      end
    ...
    def get_crl(name)
      uri = name.get_uri
      if (!(Debug).nil?)
        Debug.println("Trying to fetch CRL from DP " + (uri).to_s)
      end
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("Exception getting CRL from CertStore: " + (e).to_s)
          e.print_stack_trace
        end
      end
      return nil
    ...
    def get_crls(name, cert_issuer, cert_stores)
      if (!(Debug).nil?)
        Debug.println("Trying to fetch CRL from DP " + (name).to_s)
      end
      ...
      cert_stores.each do |store|
        begin
          ...
          # don't add the CRL
          if (!(Debug).nil?)
            Debug.println("Non-fatal exception while retrieving " + "CRLs: " + (cse).to_s)
      ...
      if (!(idp_ext).nil?)
        ...
        if (!(idp_point).nil?)
          ...
          if ((idp_names).nil?)
            ...
            if (!(Debug).nil?)
              Debug.println("IDP relativeName:" + (relative_name).to_s)
            end
