...
module Sun::Security::Provider::Certpath
  ...
  class ReverseBuilder < ReverseBuilderImports.const_get :Builder
    ...
    def get_matching_eecerts(current_state, cert_stores)
      ...
      if (!(@debug).nil?)
        @debug.println("ReverseBuilder.getMatchingEECerts got " + (ee_certs.size).to_s + " certs.")
      end
      return ee_certs
    end
    ...
    def get_matching_cacerts(current_state, cert_stores)
      ...
      if (!(@debug).nil?)
        @debug.println("ReverseBuilder.getMatchingCACerts got " + (reverse_certs.size).to_s + " certs.")
      end
    ...
    def verify_cert(cert, curr_state, cert_path_list)
      if (!(@debug).nil?)
        @debug.println("ReverseBuilder.verifyCert(SN: " + (Debug.to_hex_string(cert.get_serial_number)).to_s + "\n  Subject: " + (cert.get_subject_x500principal).to_s + ")")
      end
      current_state = curr_state
