...
module Sun::Security::Provider::Certpath
  ...
  class SunCertPathBuilder < SunCertPathBuilderImports.const_get :CertPathBuilderSpi
    ...
    def engine_build(params)
      if (!(Debug).nil?)
        Debug.println("SunCertPathBuilder.engineBuild(" + (params).to_s + ")")
      end
    ...
    def build_reverse(adjacency_list, cert_path_list)
      if (!(Debug).nil?)
        ...
        Debug.println("SunCertPathBuilder.buildReverse() InitialPolicies: " + (@build_params.get_initial_policies).to_s)
      end
      ...
      if (!(Debug).nil?)
        ...
        Debug.println("SunCertPathBuilder.buildReverse() " + "certPathList.size: " + (cert_path_list.size).to_s)
      end
    end
    ...
    def depth_first_search_forward(d_n, current_state, builder, adj_list, cert_path_list)
      ...
      vertices.each do |vertex|
          ...
          begin
            ...
            if (!(Debug).nil?)
              Debug.println("SunCertPathBuilder.depthFirstSearchForward()" + ": validation failed: " + (gse).to_s)
          ...
          if (builder.is_path_completed(cert))
            ...
            if (next_state.key_params_needed)
              ...
              if ((builder.attr_trust_anchor.get_trusted_cert).nil?)
                ...
                if (!(Debug).nil?)
                  Debug.println("SunCertPathBuilder.depthFirstSearchForward" + " using buildParams public key: " + (root_key.to_s).to_s)
                end
              end
            ...
            while i < appended_certs.size
              ...
              if (!(Debug).nil?)
                Debug.println("current subject = " + (curr_cert.get_subject_x500principal).to_s)
              end
              ...
              while j < user_checkers.size
                ...
                if (j < must_check || !curr_checker.is_forward_checking_supported)
                  ...
                  begin
                    ...
                    if (!(Debug).nil?)
                      Debug.println("SunCertPathBuilder.depthFirstSearchForward(): " + "final verification failed: " + (cpve).to_s)
                    end
    ...
    def depth_first_search_reverse(d_n, current_state, builder, adj_list, cert_path_list)
      ...
      vertices.each do |vertex|
        ...
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("SunCertPathBuilder.depthFirstSearchReverse()" + ": validation failed: " + (gse).to_s)
          end
          vertex.set_throwable(gse)
          next
        end
      ...
      stores.each do |store|
        begin
          ...
          if (!target_certs.is_empty)
            target_cert = target_certs.iterator.next
            return target_cert.get_subject_x500principal
          end
        rescue CertStoreException => e
          # ignore but log it
          if (!(Debug).nil?)
            Debug.println("SunCertPathBuilder.getTargetSubjectDN: " + "non-fatal exception retrieving certs: " + (e).to_s)
            e.print_stack_trace
          end
        end
      end
