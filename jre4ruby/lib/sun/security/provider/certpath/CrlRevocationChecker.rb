...
module Sun::Security::Provider::Certpath
  ...
  class CrlRevocationChecker < CrlRevocationCheckerImports.const_get :PKIXCertPathChecker
    ...
    def initialize(anchor, params, certs)
      ...
      @m_sig_provider = (params.get_sig_provider).to_s
      if (!(certs).nil?)
        begin
          ...
          # ignore and continue
          if (!(Debug).nil?)
            Debug.println("CrlRevocationChecker: " + "error creating Collection CertStore: " + (e).to_s)
          end
        end
      end
    ...
    def verify_revocation_status(curr_cert, prev_key, sign_flag, allow_separate_key, stacked_certs)
      ...
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("CrlRevocationChecker.verifyRevocationStatus() " + "unexpected exception: " + (e.get_message).to_s)
        end
        raise CertPathValidatorException.new(e)
      end
      if (!(Debug).nil?)
        Debug.println("CrlRevocationChecker.verifyRevocationStatus() " + "crls.size() = " + (@m_possible_crls.size).to_s)
      end
      ...
      if (!(Debug).nil?)
        Debug.println("CrlRevocationChecker.verifyRevocationStatus() " + "approved crls.size() = " + (@m_approved_crls.size).to_s)
      end
      ...
      if (!(Debug).nil?)
        ...
        Debug.println("CrlRevocationChecker.verifyRevocationStatus" + " cert SN: " + (sn.to_s).to_s)
      end
      reason_code = 0
      entry = nil
        ...
        if (!(e_).nil?)
          ...
          if (!(unres_crit_exts).nil? && !unres_crit_exts.is_empty)
            ...
            if (!unres_crit_exts.is_empty)
              if (!(Debug).nil?)
                Debug.println("Unrecognized " + "critical extension(s) in revoked CRL entry: " + (unres_crit_exts).to_s)
              end
              raise CertPathValidatorException.new("Could not determine revocation status")
            end
          end
          raise CertificateRevokedException.new("Certificate has been" + " revoked, reason: " + (reason_to_string(reason_code)).to_s)
        end
      end
    end
    ...
    def build_to_new_key(curr_cert, prev_key, stacked_certs)
      ...
      if ((Builder::USE_AIA).equal?(true))
        ...
        begin
          ...
          # ignore but log it
          if (!(Debug).nil?)
            Debug.println("CrlRevocationChecker.buildToNewKey: " + "error decoding cert: " + (ce).to_s)
          end
        end
        aia_ext = nil
        ...
        begin
          ...
          begin
            ...
            while i >= 0
              cert = cp_list.get(i)
              if (!(Debug).nil?)
                Debug.println("CrlRevocationChecker.buildToNewKey()" + " index " + (i).to_s + " checking " + (cert).to_s)
              end
          ...
          if (!(Debug).nil?)
            Debug.println("CrlRevocationChecker.buildToNewKey()" + " got key " + (cpbr.get_public_key).to_s)
          end
    ...
    def verify_possible_crls(crls, cert, sign_flag, prev_key, reasons_mask)
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("CRLRevocationChecker.verifyPossibleCRLs: " + "Checking CRLDPs for " + (cert_impl.get_subject_x500principal).to_s)
        end
        ...
        if (!(Debug).nil?)
          Debug.println("Exception while verifying CRL: " + (e.get_message).to_s)
          e.print_stack_trace
        end
