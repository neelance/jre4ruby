...
module Sun::Security::Provider::Certpath
  module ConstraintsCheckerImports
  ...
  class ConstraintsChecker < ConstraintsCheckerImports.const_get :PKIXCertPathChecker
    ...
    def verify_name_constraints(curr_cert)
      ...
      if (!(@prev_nc).nil? && (((@i).equal?(@cert_path_length)) || !X509CertImpl.is_self_issued(curr_cert)))
        if (!(Debug).nil?)
          Debug.println("prevNC = " + (@prev_nc).to_s)
          Debug.println("currDN = " + (curr_cert.get_subject_x500principal).to_s)
        end
        begin
    ...
    class_module.module_eval {
      ...
      def merge_name_constraints(curr_cert, prev_nc)
        ...
        if (!(Debug).nil?)
          Debug.println("prevNC = " + (prev_nc).to_s)
          Debug.println("newNC = " + (String.value_of(new_constraints)).to_s)
        end
        # if there are no previous name constraints, we just return the
        # new name constraints.
        if ((prev_nc).nil?)
          if (!(Debug).nil?)
            Debug.println("mergedNC = " + (String.value_of(new_constraints)).to_s)
          end
          ...
          if (!(Debug).nil?)
            Debug.println("mergedNC = " + (prev_nc).to_s)
          end
          return prev_nc
        end
      end
    ...
    def check_basic_constraints(curr_cert)
      ...
      if (!(Debug).nil?)
        Debug.println("---checking " + msg + "...")
        Debug.println("i = " + (@i).to_s)
        Debug.println("maxPathLength = " + (@max_path_length).to_s)
      end
      ...
      if (!(Debug).nil?)
        Debug.println("after processing, maxPathLength = " + (@max_path_length).to_s)
        Debug.println(msg + " verified.")
      end
    end
