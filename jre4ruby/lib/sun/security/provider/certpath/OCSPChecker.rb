      ...
      begin
        ...
        if (seek_issuer_cert || seek_responder_cert)
          ...
          while (anchors.has_next && (seek_issuer_cert || seek_responder_cert))
            ...
            if (Dump)
              System.out.println("Issuer DN is " + (cert_issuer_name).to_s)
              System.out.println("Subject DN is " + (anchor_subject_name).to_s)
            end
          ...
          if ((issuer_cert_impl).nil?)
            raise CertPathValidatorException.new("No trusted certificate for " + (curr_cert_impl.get_issuer_dn).to_s)
          end
        ...
        if (!(DEBUG).nil?)
          DEBUG.println("connecting to OCSP service at: " + (url).to_s)
        end
        ...
        if (!(DEBUG).nil? && !(con.get_response_code).equal?(HttpURLConnection::HTTP_OK))
          DEBUG.println("Received HTTP error: " + (con.get_response_code).to_s + " - " + (con.get_response_message).to_s)
        end
        ...
        if (!(DEBUG).nil?)
          DEBUG.println("Status of certificate (with serial number " + (serial_number.get_number).to_s + ") is: " + (OCSPResponse.cert_status_to_text(cert_ocspstatus)).to_s)
        end
