  ...
  class PolicyChecker < PolicyCheckerImports.const_get :PKIXCertPathChecker
    ...
    def check_policy(curr_cert)
      ...
      if (!(Debug).nil?)
        ...
        Debug.println("PolicyChecker.checkPolicy() certIndex = " + (@cert_index).to_s)
        Debug.println("PolicyChecker.checkPolicy() BEFORE PROCESSING: " + "explicitPolicy = " + (@explicit_policy).to_s)
        Debug.println("PolicyChecker.checkPolicy() BEFORE PROCESSING: " + "policyMapping = " + (@policy_mapping).to_s)
        Debug.println("PolicyChecker.checkPolicy() BEFORE PROCESSING: " + "inhibitAnyPolicy = " + (@inhibit_any_policy).to_s)
        Debug.println("PolicyChecker.checkPolicy() BEFORE PROCESSING: " + "policyTree = " + (@root_node).to_s)
      end
      curr_cert_impl = nil
      begin
      ...
      if (!(Debug).nil?)
        ...
        Debug.println("PolicyChecker.checkPolicy() AFTER PROCESSING: " + "inhibitAnyPolicy = " + (@inhibit_any_policy).to_s)
        Debug.println("PolicyChecker.checkPolicy() AFTER PROCESSING: " + "policyTree = " + (@root_node).to_s)
    ...
    class_module.module_eval {
      ...
      def merge_explicit_policy(explicit_policy, curr_cert, final_cert)
        ...
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("PolicyChecker.mergeExplicitPolicy() " + "require Index from cert = " + (require).to_s)
          end
          if (!final_cert)
            if (!(require).equal?(-1))
      ...
      def merge_policy_mapping(policy_mapping, curr_cert)
        ...
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("PolicyChecker.mergePolicyMapping() " + "inhibit Index from cert = " + (inhibit).to_s)
          end
          if (!(inhibit).equal?(-1))
      ...
      def merge_inhibit_any_policy(inhibit_any_policy, curr_cert)
        ...
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("PolicyChecker.mergeInhibitAnyPolicy() " + "skipCerts Index from cert = " + (skip_certs).to_s)
          end
      ...
      def process_policies(cert_index, init_policies, explicit_policy, policy_mapping, inhibit_any_policy, reject_policy_qualifiers, orig_root_node, curr_cert, final_cert)
        ...
        if ((!(curr_cert_policies).nil?) && (!(root_node).nil?))
          ...
          if (!(Debug).nil?)
            Debug.println("PolicyChecker.processPolicies() " + "policiesCritical = " + (policies_critical).to_s)
          end
          begin
          ...
          if (!(Debug).nil?)
            Debug.println("PolicyChecker.processPolicies() " + "rejectPolicyQualifiers = " + (reject_policy_qualifiers).to_s)
          end
      ...
      def rewrite_leaf_nodes(cert_index, init_policies, root_node)
        ...
        any_node = any_nodes.iterator.next
      ...
      def process_parents(cert_index, policies_critical, reject_policy_qualifiers, root_node, cur_policy, p_quals, match_any)
        found_match = false
        if (!(Debug).nil?)
          Debug.println("PolicyChecker.processParents(): matchAny = " + (match_any).to_s)
        end
            ...
            valid_nodes.each do |curNode|
              if ((policy_mapping > 0) || ((policy_mapping).equal?(-1)))
                ...
                if ((policy_mapping).equal?(0))
                  ...
                  if (!(Debug).nil?)
                    Debug.println("PolicyChecker.processPolicyMappings" + "() before deleting: policy tree = " + (root_node).to_s)
                  end
                  parent_node.delete_child(cur_node)
                  child_deleted = true
                  if (!(Debug).nil?)
                    Debug.println("PolicyChecker.processPolicyMappings" + "() after deleting: policy tree = " + (root_node).to_s)
                  end
                end
              end
            end
          ...
          valid_nodes.each do |curNode|
            ...
            if ((parent_node.get_valid_policy == ANY_POLICY))
              if ((!init_policies.contains(cur_policy)) && (!(cur_policy == ANY_POLICY)))
                if (!(Debug).nil?)
                  Debug.println("PolicyChecker.processPolicies() " + "before deleting: policy tree = " + (root_node).to_s)
                end
                parent_node.delete_child(cur_node)
                child_deleted = true
                if (!(Debug).nil?)
                  Debug.println("PolicyChecker.processPolicies() " + "after deleting: policy tree = " + (root_node).to_s)
                end
              end
            end
          end
