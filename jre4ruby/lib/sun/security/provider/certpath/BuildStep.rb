...
module Sun::Security::Provider::Certpath
  ...
  class BuildStep 
    ...
    def to_s
      ...
      case (@result)
      when BACK, FAIL
        out = (result_to_string(@result)).to_s
        out = out + (@vertex.throwable_to_string).to_s
    ...
    def verbose_to_string
      ...
      when BACK, FAIL
        out = out + (@vertex.throwable_to_string).to_s
      when FOLLOW, SUCCEED
        out = out + (@vertex.more_to_string).to_s
      when POSSIBLE
      else
      end
      out = out + "Certificate contains:\n" + (@vertex.cert_to_string).to_s
      return out
    end
