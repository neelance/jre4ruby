...
module Sun::Security::Provider::Certpath
  ...
  class Vertex 
    ...
    def cert_to_string
      ...
      out = "Issuer:     " + (x509cert.get_issuer_x500principal).to_s + "\n"
      out = out + "Subject:    " + (x509cert.get_subject_x500principal).to_s + "\n"
      out = out + "SerialNum:  " + ((x509cert.get_serial_number).to_s(16)).to_s + "\n"
      out = out + "Expires:    " + (x509cert.get_not_after.to_s).to_s + "\n"
      ...
      begin
        ...
        if (!(s_key_id).nil?)
          ...
          out = out + "SubjKeyID:  " + (key_id.to_s).to_s
        end
      rescue Exception => e
        if (!(Debug).nil?)
          Debug.println("Vertex.certToString() unexpected exception")
    ...
    def more_to_string
      out = "Last cert?  "
      out = out + ((((@index).equal?(-1)) ? "Yes" : "No")).to_s
      out = out + "\n"
      return out
    end
