  ...
  class DSA < DSAImports.const_get :SignatureSpi
    ...
    def engine_init_sign(private_key)
      if (!(private_key.is_a?(Java::Security::Interfaces::DSAPrivateKey)))
        raise InvalidKeyException.new("not a DSA private key: " + (private_key).to_s)
      end
      priv = private_key
      @preset_x = priv.get_x
      @preset_y = nil
    ...
    def engine_init_verify(public_key)
      if (!(public_key.is_a?(Java::Security::Interfaces::DSAPublicKey)))
        raise InvalidKeyException.new("not a DSA public key: " + (public_key).to_s)
      end
      pub = public_key
      @preset_y = pub.get_y
      @preset_x = nil
    ...
    def to_s
      ...
      if (!(@preset_p).nil? && !(@preset_q).nil? && !(@preset_g).nil?)
        printable += "\n\tp: " + (Debug.to_hex_string(@preset_p)).to_s
        printable += "\n\tq: " + (Debug.to_hex_string(@preset_q)).to_s
        printable += "\n\tg: " + (Debug.to_hex_string(@preset_g)).to_s
      else
        printable += "\n\t P, Q or G not initialized."
      end
      if (!(@preset_y).nil?)
        printable += "\n\ty: " + (Debug.to_hex_string(@preset_y)).to_s
      end
