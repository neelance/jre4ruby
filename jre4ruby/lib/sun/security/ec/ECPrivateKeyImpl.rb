...
module Sun::Security::Ec
  module ECPrivateKeyImplImports
  ...
  class ECPrivateKeyImpl < ECPrivateKeyImplImports.const_get :PKCS8Key
    ...
    def parse_key_bits
      begin
        ...
        while (!(data.available).equal?(0))
          ...
          if (value.is_context_specific(0))
            ...
            if (value.is_context_specific(1))
              ...
              raise InvalidKeyException.new("Unexpected value: " + (value).to_s)
            end
          end
        end
        ...
        @params = alg_params.get_parameter_spec(ECParameterSpec.class)
    ...
    def to_s
      return "Sun EC private key, " + (@params.get_curve.get_field.get_field_size).to_s + " bits\n  private value:  " + (@s).to_s + "\n  parameters: " + (@params).to_s
    end
