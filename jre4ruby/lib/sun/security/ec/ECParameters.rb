  ...
  class ECParameters < ECParametersImports.const_get :AlgorithmParametersSpi
    ...
    class_module.module_eval {
      ...
      def encode_parameters(params)
        curve = get_named_curve(params)
        if ((curve).nil?)
          raise RuntimeException.new("Not a known named curve: " + (params).to_s)
        end
      ...
      def decode_parameters(params)
        ...
        if ((encoded_params.attr_tag).equal?(DerValue.attr_tag_object_id))
          ...
          if ((spec).nil?)
            raise IOException.new("Unknown named curve: " + (oid).to_s)
          end
          return spec
        end
    ...
    def engine_init(param_spec)
      if (param_spec.is_a?(ECParameterSpec))
        ...
        if ((@param_spec).nil?)
          raise InvalidParameterSpecException.new("Not a supported named curve: " + (param_spec).to_s)
        end
      else
    ...
    def engine_get_parameter_spec(spec)
      if (spec.is_assignable_from(ECParameterSpec.class))
        return @param_spec
      else
        if (spec.is_assignable_from(ECGenParameterSpec.class))
