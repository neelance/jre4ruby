...
module Sun::Security::Pkcs11
  ...
  class SunPKCS11 < SunPKCS11Imports.const_get :AuthProvider
    ...
    def initialize(config_name, config_stream)
      ...
      @token = nil
      @poller = nil
      super("SunPKCS11-" + (Config.get_config(config_name, config_stream).get_name).to_s, 1.7, Config.get_config(config_name, config_stream).get_description)
      @lock_handler = Object.new
      ...
      if (use_secmod)
        ...
        if (@config.get_show_info)
          System.out.println("NSS modules: " + (modules).to_s)
        end
        ...
        if ((module_name).nil?)
          ...
          if (!(nss_module).nil?)
            ...
            module_name = (((nss_db_mode).equal?(DbMode::NO_DB)) ? "crypto" : "keystore").to_s
          end
        end
        if ((module_name == "fips"))
          nss_module = secmod.get_module(ModuleType::FIPS)
            ...
            if ((module_name == "crypto"))
              ...
              if ((module_name == "trustanchors"))
                ...
                if (module_name.starts_with("external-"))
                  ...
                  if ((nss_module).nil?)
                    raise ProviderException.new("Invalid module " + module_name + ": only " + (k).to_s + " external NSS modules available")
                  end
                else
        ...
        library = (nss_module.attr_library_name).to_s
      ...
      begin
        ...
        if (p11info.attr_cryptoki_version.attr_major < 2)
          raise ProviderException.new("Only PKCS#11 v2.0 and later " + "supported, library version is v" + (p11info.attr_cryptoki_version).to_s)
        end
        show_info = @config.get_show_info
        if (show_info)
          System.out.println("Information for provider " + (get_name).to_s)
        ...
        if ((slot_id < 0) || show_info)
          ...
          if (show_info)
            System.out.println("All slots: " + (to_s(slots)).to_s)
            slots = @p11._c_get_slot_list(true)
            System.out.println("Slots with tokens: " + (to_s(slots)).to_s)
          end
          if (slot_id < 0)
            if ((slot_list_index < 0) || (slot_list_index >= slots.attr_length))
              raise ProviderException.new("slotListIndex is " + (slot_list_index).to_s + " but token only has " + (slots.attr_length).to_s + " slots")
            end
    ...
    def create_poller
      ...
      t = JavaThread.new(poller, "Poller " + (get_name).to_s)
      t.set_daemon(true)
    ...
    def init_token(slot_info)
      ...
      show_info = @config.get_show_info
      if (show_info)
        System.out.println("Slot info for slot " + (@slot_id).to_s + ":")
        System.out.println(slot_info)
      end
      token = Token.new(self)
      if (show_info)
        System.out.println("Token info for token in slot " + (@slot_id).to_s + ":")
      ...
      while i < supported_mechanisms.attr_length
        ...
        if (show_info)
          ...
          System.out.println("Mechanism " + (Functions.get_mechanism_name(long_mech)).to_s + ":")
      ...
      Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
        ...
        define_method :run do
          ...
          if (self.attr_config.is_enabled(PCKM_KEYSTORE))
            put_service(P11Service.new(token, KS, "PKCS11", "sun.security.pkcs11.P11KeyStore", s("PKCS11-" + (self.attr_config.get_name).to_s), PCKM_KEYSTORE))
          end
          return nil
        end
    ...
    class_module.module_eval {
      const_set_lazy(:P11Service) { Class.new(Service) do
        ...
        typesig { [] }
        def to_s
          return (super).to_s + " (" + (Functions.get_mechanism_name(@mechanism)).to_s + ")"
        end
    ...
    def login(subject, handler)
      ...
      if (!(sm).nil?)
        ...
        sm.check_permission(SecurityPermission.new("authProvider." + (self.get_name).to_s))
      end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:SunPKCS11Rep) { Class.new do
        ...
        def initialize(provider)
          @provider_name = nil
          @config_name = nil
          @provider_name = (provider.get_name).to_s
          @config_name = (provider.attr_config_name).to_s
