  ...
  class P11KeyStore < P11KeyStoreImports.const_get :KeyStoreSpi
      ...
      const_set_lazy(:AliasInfo) { Class.new do
        ...
        def to_s
          ...
          if ((@id).nil?)
            ...
            sb.append("\tid=" + (P11KeyStore.get_id(@id)).to_s + "\n")
          end
          sb.append("\ttrusted=[" + (@trusted).to_s + "]\n")
          sb.append("\tmatched=[" + (@matched).to_s + "]\n")
          if ((@cert).nil?)
            sb.append("\tcert=[null]\n")
          else
            sb.append("\tcert=[\tsubject: " + (@cert.get_subject_x500principal).to_s + "\n\t\tissuer: " + (@cert.get_issuer_x500principal).to_s + "\n\t\tserialNum: " + (@cert.get_serial_number.to_s).to_s + "]")
          end
          return sb.to_s
        end
        ...
        if (entry.is_a?(KeyStore::TrustedCertificateEntry))
          ...
          if (entry.is_a?(KeyStore::PrivateKeyEntry))
            ...
            if (!(key.is_a?(P11Key)) && !(key.is_a?(RSAPrivateKey)) && !(key.is_a?(DSAPrivateKey)) && !(key.is_a?(DHPrivateKey)) && !(key.is_a?(ECPrivateKey)))
              raise KeyStoreException.new("unsupported key type: " + (key.get_class.get_name).to_s)
            end
            # only support X509Certificate chains
            chain = (entry).get_certificate_chain
            if (!(chain.is_a?(Array.typed(X509Certificate))))
              raise KeyStoreException.new(UnsupportedOperationException.new("unsupported certificate array type: " + (chain.get_class.get_name).to_s))
            end
            begin
              updated_alias = false
      ...
      while (true)
        ...
        if ((ch).nil? || (ch.attr_length).equal?(0))
          ...
          if (!(Debug).nil? && ch.attr_length > 1)
            Debug.println("engineGetEntry found " + (ch.attr_length).to_s + " certificate entries for subject [" + (next_.get_issuer_x500principal.to_s).to_s + "] in token - using first entry")
          end
        ...
        if ((k_type).equal?(CKK_AES))
          ...
          if ((k_type).equal?(CKK_BLOWFISH))
            ...
            if ((k_type).equal?(CKK_RC4))
              ...
              if (!(Debug).nil?)
                Debug.println("unknown key type [" + (k_type).to_s + "] - using 'Generic Secret'")
              end
        ...
        if ((k_type).equal?(CKK_DSA))
          ...
          if ((k_type).equal?(CKK_DH))
            ...
            if ((k_type).equal?(CKK_EC))
              ...
              if (!(Debug).nil?)
                Debug.println("unknown key type [" + (k_type).to_s + "]")
              end
    ...
    def update_pkey(alias_, cka_id, chain, replace_cert)
      ...
      begin
        ...
        if ((h.attr_type).equal?(ATTR_CLASS_PKEY))
          ...
          raise KeyStoreException.new("expected but could not find private key " + "with CKA_ID " + (get_id(cka_id)).to_s)
        end
        ...
        if (!(h.attr_type).equal?(ATTR_CLASS_CERT))
          raise KeyStoreException.new("expected but could not find certificate " + "with CKA_ID " + (get_id(cka_id)).to_s)
        else
          if (replace_cert)
          ...
          if (key.is_a?(DHPrivateKey))
            ...
            if (key.is_a?(ECPrivateKey))
              ...
              if (key.is_a?(P11Key))
                ...
                raise KeyStoreException.new("unsupported key type: " + (key).to_s)
              end
            end
          end
        end
    ...
    def destroy_cert(cka_id)
      ...
      begin
        ...
        if (!(Debug).nil?)
          Debug.println("destroyCert destroyed cert with CKA_ID [" + (get_id(cka_id)).to_s + "]")
        end
        return true
      ensure
        @token.release_session(session)
    ...
    def destroy_chain(cka_id)
      ...
      begin
        ...
        if (!(h.attr_type).equal?(ATTR_CLASS_CERT))
          if (!(Debug).nil?)
            Debug.println("destroyChain could not find " + "end entity cert with CKA_ID [0x" + (Functions.to_hex_string(cka_id)).to_s + "]")
          end
          return false
        end
        ...
        @token.attr_p11._c_destroy_object(session.id, h.attr_handle)
        if (!(Debug).nil?)
          Debug.println("destroyChain destroyed end entity cert " + "with CKA_ID [" + (get_id(cka_id)).to_s + "]")
        end
        ...
        while (true)
          ...
          if ((ch).nil? || (ch.attr_length).equal?(0))
            ...
            # if more than one found, use first
            if (!(Debug).nil? && ch.attr_length > 1)
              Debug.println("destroyChain found " + (ch.attr_length).to_s + " certificate entries for subject [" + (next_.get_issuer_x500principal).to_s + "] in token - using first entry")
            end
            next_ = load_cert(session, ch[0])
            ...
            if (destroy_it)
              ...
              if (!(Debug).nil?)
                Debug.println("destroyChain destroyed cert in chain " + "with subject [" + (next_.get_subject_x500principal).to_s + "]")
              end
            else
              if (!(Debug).nil?)
                Debug.println("destroyChain did not destroy " + "shared cert in chain with subject [" + (next_.get_subject_x500principal).to_s + "]")
              end
            end
          end
        end
    ...
    def destroy_pkey(cka_id)
      ...
      begin
        ...
        if (!(h.attr_type).equal?(ATTR_CLASS_PKEY))
          if (!(Debug).nil?)
            Debug.println("destroyPkey did not find private key with CKA_ID [" + (get_id(cka_id)).to_s + "]")
          end
          return false
        end
        @token.attr_p11._c_destroy_object(session.id, h.attr_handle)
    ...
    def get_id(alias_, cert)
      ...
      return alias_ + ALIAS_SEP + (issuer.get_name(X500Principal::CANONICAL)).to_s + ALIAS_SEP + (serial_num.to_s).to_s
    end
      ...
      if ((h.attr_length).equal?(0))
        if (!(Debug).nil?)
          if ((type).equal?(ATTR_CLASS_SKEY))
            ...
            if ((type).equal?(ATTR_CLASS_CERT))
              Debug.println("getTokenObject did not find cert with CKA_ID [" + (get_id(cka_id)).to_s + "]")
            else
              Debug.println("getTokenObject did not find private key " + "with CKA_ID [" + (get_id(cka_id)).to_s + "]")
            end
          end
        end
      else
        if ((h.attr_length).equal?(1))
          ...
          if ((type).equal?(ATTR_CLASS_SKEY))
            ...
            if ((list.size).equal?(1))
              ...
              raise KeyStoreException.new("invalid KeyStore state: " + "found " + (list.size).to_s + " secret keys sharing CKA_LABEL [" + cka_label + "]")
            end
          else
            if ((type).equal?(ATTR_CLASS_CERT))
              raise KeyStoreException.new("invalid KeyStore state: " + "found " + (h.attr_length).to_s + " certificates sharing CKA_ID " + (get_id(cka_id)).to_s)
            else
              raise KeyStoreException.new("invalid KeyStore state: " + "found " + (h.attr_length).to_s + " private keys sharing CKA_ID " + (get_id(cka_id)).to_s)
            end
          end
        end
      end
        ...
        handles.each do |handle|
          ...
          if ((attrs[0].attr_p_value).nil?)
            ...
            if ((cka_label).nil?)
              ...
              cka_label = (get_id(attrs[0].get_byte_array)).to_s
            end
    ...
    def map_private_keys(pkey_ids, cert_map)
      ...
      pkey_ids.each do |pkeyID|
        ...
        if (!found_match)
          if (!(Debug).nil?)
            Debug.println("did not find match for private key with CKA_ID [" + (get_id(pkey_id)).to_s + "] (ignoring entry)")
          end
        end
      end
    ...
    def dump_token_map
      ...
      if ((aliases.size).equal?(0))
        ...
        aliases.each do |s|
          System.out.println("  " + s + (@alias_map.get(s)).to_s)
        end
      end
    end
