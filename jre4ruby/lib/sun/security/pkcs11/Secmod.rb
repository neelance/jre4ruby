    ...
    def initialize_(db_mode, config_dir, nss_lib_dir)
      synchronized(self) do
        ...
        if ((nss_lib_dir).nil?)
          ...
          platform_path = (platform_file.get_path).to_s
        end
        if (!(config_dir).nil?)
        ...
        if (DEBUG)
          System.out.println("handle: " + (@nss_handle).to_s)
        end
        fetch_versions
        if ((@supported).equal?(false))
        ...
        if (DEBUG)
          System.out.println("init: " + (initok).to_s)
        end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:Module) { Class.new do
        ...
        def initialize(library_name, common_name, fips, slot)
          ...
          if (((library_name).nil?) || ((library_name.length).equal?(0)))
            # must be softtoken
            library_name = (System.map_library_name(SOFTTOKEN_LIB_NAME)).to_s
            if ((fips).equal?(false))
        ...
        def init_configuration
          case (@type)
          when EXTERNAL
            @config = (String.format(TEMPLATE_EXTERNAL, @library_name, @common_name + " " + (@slot).to_s, @slot)).to_s
          when CRYPTO
            @config = (String.format(TEMPLATE_CRYPTO, @library_name)).to_s
          when KEYSTORE
            @config = (String.format(TEMPLATE_KEYSTORE, @library_name)).to_s
          when FIPS
            @config = (String.format(TEMPLATE_FIPS, @library_name)).to_s
          when TRUSTANCHOR
            @config = (String.format(TEMPLATE_TRUSTANCHOR, @library_name)).to_s
          else
            raise RuntimeException.new("Unknown module type: " + (@type).to_s)
          end
        end
        ...
        typesig { [] }
        def to_s
          return @common_name + " (" + (@type).to_s + ", " + @library_name + ", slot " + (@slot).to_s + ")"
        end
      ...
      def get_trust(provider)
        ...
        begin
          ...
          if (DEBUG)
            System.out.println("handles: " + (handles.attr_length).to_s)
          end
