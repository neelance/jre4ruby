...
module Sun::Security::Pkcs11
  ...
  class P11Key 
    ...
    def to_s
      @token.ensure_valid
      s1 = (@token.attr_provider.get_name).to_s + " " + @algorithm + " " + @type + " key, " + (@key_length).to_s + " bits"
      s1 += " (id " + (@key_id).to_s + ", " + ((@token_object ? "token" : "session")).to_s + " object"
      if (is_public)
        s1 += ")"
      else
        s1 += ", " + ((@sensitive ? "" : "not ")).to_s + "sensitive"
        s1 += ", " + ((@extractable ? "" : "un")).to_s + "extractable)"
      end
      return s1
    end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:P11ECPublicKey) { Class.new(P11Key) do
        ...
        def to_s
          fetch_values
          return (super).to_s + "\n  public x coord: " + (@w.get_affine_x).to_s + "\n  public y coord: " + (@w.get_affine_y).to_s + "\n  parameters: " + (@params).to_s
        end
