...
module Sun::Security::Pkcs11
  module P11SecretKeyFactoryImports
  ...
  class P11SecretKeyFactory < P11SecretKeyFactoryImports.const_get :SecretKeyFactorySpi
    ...
    class_module.module_eval {
      ...
      def get_key_type(algorithm)
        ...
        if ((l).nil?)
          algorithm = (algorithm.to_upper_case(Locale::ENGLISH)).to_s
          l = KeyTypes.get(algorithm)
          if ((l).nil?)
    ...
    def engine_generate_secret(key_spec)
      ...
      raise InvalidKeySpecException.new("Unsupported spec: " + (key_spec.get_class.get_name).to_s)
    end
    ...
    def engine_get_key_spec(key, key_spec)
      ...
      if (SecretKeySpec.class.is_assignable_from(key_spec))
        return SecretKeySpec.new(get_key_bytes(key), @algorithm)
      else
        if (@algorithm.equals_ignore_case("DES"))
          begin
            if (DESKeySpec.class.is_assignable_from(key_spec))
          ...
          if (@algorithm.equals_ignore_case("DESede"))
            begin
              if (DESedeKeySpec.class.is_assignable_from(key_spec))
      ...
      raise InvalidKeySpecException.new("Unsupported spec: " + (key_spec.get_name).to_s)
    end
