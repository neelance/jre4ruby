  ...
  class P11KeyPairGenerator < P11KeyPairGeneratorImports.const_get :KeyPairGeneratorSpi
    ...
    def initialize_(key_size, random)
      ...
      if ((@algorithm == "EC"))
        ...
        if ((@params).nil?)
          raise InvalidParameterException.new("No EC parameters available for key size " + (key_size).to_s + " bits")
        end
      end
    end
          ...
          if ((@algorithm == "DSA"))
            ...
            if ((@algorithm == "EC"))
              ...
              if (params.is_a?(ECParameterSpec))
                ...
                if ((ec_params).nil?)
                  raise InvalidAlgorithmParameterException.new("Unsupported curve: " + (params).to_s)
                end
              else
