  ...
  class P11RSACipher < P11RSACipherImports.const_get :CipherSpi
    ...
    def impl_init(opmode, key)
        ...
        if ((opmode).equal?(Cipher::DECRYPT_MODE))
          ...
          if ((opmode).equal?(Cipher::WRAP_MODE))
            ...
            if ((opmode).equal?(Cipher::UNWRAP_MODE))
              ...
              raise InvalidKeyException.new("Unsupported mode: " + (opmode).to_s)
            end
          end
        end
      end
      if (@p11key.is_public)
        ...
        if (@p11key.is_private)
          ...
          raise InvalidKeyException.new("Unknown key type: " + (@p11key).to_s)
        end
      end
    ...
    def impl_do_final(out, out_ofs, out_len)
      if (@buf_ofs > @max_input_size)
        raise IllegalBlockSizeException.new("Data must not be longer " + "than " + (@max_input_size).to_s + " bytes")
      end
      begin
        ensure_initialized
        p11 = @token.attr_p11
    ...
    class_module.module_eval {
      ...
      def construct_key(encoding, key_algorithm, key_type)
        ...
        when Cipher::PUBLIC_KEY
          ...
          raise InvalidKeyException.new("Unknown keytype " + (key_type).to_s)
        end
      end
    }
