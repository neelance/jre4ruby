...
module Sun::Security::Pkcs11
  ...
  class Config 
    ...
    typesig { [String] }
    def exc_token(msg)
      return ConfigurationException.new(msg + " " + (@st).to_s)
    end
    
    typesig { [String] }
    def exc_line(msg)
      return ConfigurationException.new(msg + ", line " + (@st.lineno).to_s)
    end
    ...
    def parse
      while (true)
        ...
        word = @st.attr_sval
        if ((word == "name"))
          @name = (parse_string_entry(word)).to_s
        else
          if ((word == "library"))
            @library = (parse_library(word)).to_s
          else
            if ((word == "description"))
                                  ...
                                  if ((word == "omitInitialize"))
                                    ...
                                    if ((word == "allowSingleThreadedModules"))
                                      ...
                                      if ((word == "functionList"))
                                        @function_list = (parse_string_entry(word)).to_s
                                      else
                                        if ((word == "nssUseSecmod"))
                                          @nss_use_secmod = parse_boolean_entry(word)
                                        else
                                          if ((word == "nssLibraryDirectory"))
                                            @nss_library_directory = (parse_library(word)).to_s
                                            @nss_use_secmod = true
                                          else
                                            if ((word == "nssSecmodDirectory"))
                                              @nss_secmod_directory = (expand(parse_string_entry(word))).to_s
                                              @nss_use_secmod = true
                                            else
                                              if ((word == "nssModule"))
                                                @nss_module = (parse_string_entry(word)).to_s
                                                @nss_use_secmod = true
                                              else
                                                if ((word == "nssDbMode"))
                                                  ...
                                                  if ((word == "nssNetscapeDbWorkaround"))
                                                    ...
                                                    if ((word == "nssArgs"))
                                                      ...
                                                      if ((word == "nssUseSecmodTrust"))
                                                        ...
                                                        raise ConfigurationException.new("Unknown keyword '" + word + "', line " + (@st.lineno).to_s)
                                                      end
                                                    end
                                                  end
                                                end
    ...
    def parse_slot_list_index(keyword)
      ...
      debug("slotListIndex: " + (@slot_list_index).to_s)
    end
    ...
    def parse_nssargs(keyword)
      ...
      @nss_args = (expand(@st.attr_sval)).to_s
    ...
    def parse_handle_startup_errors(keyword)
      ...
      debug("handleStartupErrors: " + (@handle_startup_errors).to_s)
    end
