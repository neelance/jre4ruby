...
module Sun::Security::Pkcs
  ...
  class PKCS7 
      ...
      if ((@content_type == ContentInfo::SIGNED_DATA_OID))
        ...
        if ((@content_type == ContentInfo::OLD_SIGNED_DATA_OID))
          ...
          if ((@content_type == ContentInfo::NETSCAPE_CERT_SEQUENCE_OID))
            ...
            raise ParsingException.new("content type " + (@content_type).to_s + " not supported.")
          end
        end
      end
    end
      ...
      while i < @certificates.attr_length
        ...
        if (!(cert_issuer_name.is_a?(X500Name)))
          ...
          begin
            ...
            cert_issuer_name = tbs_cert.get((CertificateIssuerName::NAME).to_s + "." + (CertificateIssuerName::DN_NAME).to_s)
          rescue Exception => e
    ...
    def to_s
      out = ""
      out += (@content_info).to_s + "\n"
      if (!(@version).nil?)
        out += "PKCS7 :: version: " + (Debug.to_hex_string(@version)).to_s + "\n"
      end
