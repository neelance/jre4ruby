    ...
    class_module.module_eval {
      ...
      def parse_key(in_)
        ...
        if (!(Version == parsed_version))
          raise IOException.new("version mismatch: (supported: " + (Debug.to_hex_string(Version)).to_s + ", parsed: " + (Debug.to_hex_string(parsed_version)).to_s)
        end
    ...
    class_module.module_eval {
      ...
      def build_pkcs8key(algid, key)
        ...
        begin
          ...
          classname = (sun_provider.get_property("PrivateKey.PKCS#8." + (algid.get_name).to_s)).to_s
          if ((classname).nil?)
            raise InstantiationException.new
          end
          key_class = nil
    ...
    def decode(in_)
      ...
      begin
        ...
        if (!(version == self.attr_version))
          raise IOException.new("version mismatch: (supported: " + (Debug.to_hex_string(self.attr_version)).to_s + ", parsed: " + (Debug.to_hex_string(version)).to_s)
        end
