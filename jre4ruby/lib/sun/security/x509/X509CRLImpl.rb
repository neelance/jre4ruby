  ...
  class X509CRLImpl < X509CRLImplImports.const_get :X509CRL
    ...
    def initialize(crl_data)
      ...
      begin
        ...
        @signed_crl = nil
        raise CRLException.new("Parsing error: " + (e.get_message).to_s)
      end
    end
    ...
    def encode_info(out)
      begin
        ...
        raise CRLException.new("Encoding error: " + (e.get_message).to_s)
      end
    end
    ...
    def sign(key, algorithm, provider)
      begin
        ...
        raise CRLException.new("Error while encoding data: " + (e.get_message).to_s)
      end
    end
    ...
    def to_s
      sb = StringBuffer.new
      sb.append("X.509 CRL v" + ((@version + 1)).to_s + "\n")
      if (!(@sig_alg_id).nil?)
        sb.append("Signature Algorithm: " + (@sig_alg_id.to_s).to_s + ", OID=" + ((@sig_alg_id.get_oid).to_s).to_s + "\n")
      end
      if (!(@issuer).nil?)
        sb.append("Issuer: " + (@issuer.to_s).to_s + "\n")
      end
      if (!(@this_update).nil?)
        sb.append("\nThis Update: " + (@this_update.to_s).to_s + "\n")
      end
      if (!(@next_update).nil?)
        sb.append("Next Update: " + (@next_update.to_s).to_s + "\n")
      end
      if (@revoked_certs.is_empty)
        sb.append("\nNO certificates have been revoked\n")
      else
        sb.append("\nRevoked Certificates: " + (@revoked_certs.size).to_s)
        i = 1
          ...
          begin
            if ((OIDMap.get_class(ext.get_extension_id)).nil?)
              ...
              if (!(ext_value).nil?)
                ...
                sb.append("Extension unknown: " + "DER encoded OCTET string =\n" + (enc.encode_buffer(ext_value)).to_s + "\n")
              end
            else
              sb.append(ext.to_s)
    ...
    def parse(val)
      ...
      if (!(val.attr_data.available).equal?(0))
        raise CRLException.new("signed overrun, bytes = " + (val.attr_data.available).to_s)
      end
      ...
      if ((next_byte).equal?(DerValue.attr_tag_utc_time))
        ...
        if ((next_byte).equal?(DerValue.attr_tag_generalized_time))
          ...
          raise CRLException.new("Invalid encoding for thisUpdate" + " (tag=" + (next_byte).to_s + ")")
        end
      end
