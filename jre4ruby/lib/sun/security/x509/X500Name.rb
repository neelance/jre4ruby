...
module Sun::Security::X509
  ...
  class X500Name 
    ...
    def get_string(attribute)
      ...
      if ((value).nil?)
        raise IOException.new("not a DER string encoding, " + (attribute.attr_tag).to_s)
      else
        return value
      end
    end
    ...
    def get_rfc1779name(oid_map)
      if (oid_map.is_empty)
        ...
        if (!(@rfc1779dn).nil?)
          ...
          @rfc1779dn = (generate_rfc1779dn(oid_map)).to_s
          return @rfc1779dn
        end
      end
    ...
    def get_rfc2253name(oid_map)
      ...
      if (oid_map.is_empty)
        if (!(@rfc2253dn).nil?)
          ...
          @rfc2253dn = (generate_rfc2253dn(oid_map)).to_s
          return @rfc2253dn
        end
      end
      ...
      when_class_loaded do
        pa = Class.new(PrivilegedExceptionAction.class == Class ? PrivilegedExceptionAction : Object) do
          ...
          define_method :run do
            p_class = X500Principal.class
            args = Array.typed(Class).new([X500Name.class])
