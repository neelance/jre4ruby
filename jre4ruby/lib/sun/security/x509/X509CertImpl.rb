...
module Sun::Security::X509
  ...
  class X509CertImpl < X509CertImplImports.const_get :X509Certificate
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:SUBJECT_DN) { NAME + DOT + INFO + DOT + (X509CertInfo::SUBJECT).to_s + DOT + (CertificateSubjectName::DN_NAME).to_s }
      const_attr_reader  :SUBJECT_DN
      
      # x509.info.issuer.dname
      const_set_lazy(:ISSUER_DN) { NAME + DOT + INFO + DOT + (X509CertInfo::ISSUER).to_s + DOT + (CertificateIssuerName::DN_NAME).to_s }
      const_attr_reader  :ISSUER_DN
      
      # x509.info.serialNumber.number
      const_set_lazy(:SERIAL_ID) { NAME + DOT + INFO + DOT + (X509CertInfo::SERIAL_NUMBER).to_s + DOT + (CertificateSerialNumber::NUMBER).to_s }
      const_attr_reader  :SERIAL_ID
      
      # x509.info.key.value
      const_set_lazy(:PUBLIC_KEY) { NAME + DOT + INFO + DOT + (X509CertInfo::KEY).to_s + DOT + (CertificateX509Key::KEY).to_s }
      const_attr_reader  :PUBLIC_KEY
      
      # x509.info.version.value
      const_set_lazy(:VERSION) { NAME + DOT + INFO + DOT + (X509CertInfo::VERSION).to_s + DOT + (CertificateVersion::VERSION).to_s }
    ...
    def initialize(cert_data)
      ...
      begin
        ...
        @signed_cert = nil
        ce = CertificateException.new("Unable to initialize, " + (e).to_s)
        ce.init_cause(e)
        raise ce
      end
    end
    ...
    def initialize(in_)
      ...
      begin
        ...
        begin
          ...
          ce = CertificateException.new("Input stream must be " + "either DER-encoded bytes " + "or RFC1421 hex-encoded " + "DER-encoded bytes: " + (ioe1.get_message).to_s)
          ce.init_cause(ioe1)
          raise ce
        end
      end
    ...
    def read_rfc1421cert(in_)
      ...
      begin
        line = (cert_buffered_reader.read_line).to_s
      rescue IOException => ioe1
        raise IOException.new("Unable to read InputStream: " + (ioe1.get_message).to_s)
      end
      if ((line == X509Factory::BEGIN_CERT))
        ...
        begin
          while (!((line = (cert_buffered_reader.read_line).to_s)).nil?)
          ...
          raise IOException.new("Unable to read InputStream: " + (ioe2.get_message).to_s)
        end
      else
    ...
    def to_s
      ...
      sb.append("[\n")
      sb.append((@info.to_s).to_s + "\n")
      sb.append("  Algorithm: [" + (@alg_id.to_s).to_s + "]\n")
      encoder = HexDumpEncoder.new
      sb.append("  Signature:\n" + (encoder.encode_buffer(@signature)).to_s)
      sb.append("\n]")
      return sb.to_s
    end
    ...
    def get_public_key
      ...
      begin
        key = @info.get((CertificateX509Key::NAME).to_s + DOT + (CertificateX509Key::KEY).to_s)
        return key
      rescue Exception => e
        return nil
      end
    ...
    def get_version
      ...
      begin
        vers = (@info.get((CertificateVersion::NAME).to_s + DOT + (CertificateVersion::VERSION).to_s)).int_value
        return vers + 1
      rescue Exception => e
        return -1
      end
    ...
    def get_serial_number_object
      ...
      begin
        ser = @info.get((CertificateSerialNumber::NAME).to_s + DOT + (CertificateSerialNumber::NUMBER).to_s)
        return ser
      rescue Exception => e
        return nil
      end
    ...
    def get_subject_dn
      ...
      begin
        subject = @info.get((CertificateSubjectName::NAME).to_s + DOT + (CertificateSubjectName::DN_NAME).to_s)
        return subject
      rescue Exception => e
        return nil
      end
    ...
    def get_subject_x500principal
      ...
      begin
        subject = @info.get((CertificateSubjectName::NAME).to_s + DOT + (CertificateSubjectName::DN_PRINCIPAL).to_s)
        return subject
      rescue Exception => e
        return nil
      end
    ...
    def get_issuer_dn
      ...
      begin
        issuer = @info.get((CertificateIssuerName::NAME).to_s + DOT + (CertificateIssuerName::DN_NAME).to_s)
        return issuer
      rescue Exception => e
        return nil
      end
    ...
    def get_issuer_x500principal
      ...
      begin
        issuer = @info.get((CertificateIssuerName::NAME).to_s + DOT + (CertificateIssuerName::DN_PRINCIPAL).to_s)
        return issuer
      rescue Exception => e
        return nil
      end
    ...
    def get_not_before
      ...
      begin
        d = @info.get((CertificateValidity::NAME).to_s + DOT + (CertificateValidity::NOT_BEFORE).to_s)
        return d
      rescue Exception => e
        return nil
      end
    ...
    def get_not_after
      ...
      begin
        d = @info.get((CertificateValidity::NAME).to_s + DOT + (CertificateValidity::NOT_AFTER).to_s)
        return d
      rescue Exception => e
        return nil
      end
    ...
    def get_issuer_unique_id
      ...
      begin
        id = @info.get((CertificateIssuerUniqueIdentity::NAME).to_s + DOT + (CertificateIssuerUniqueIdentity::ID).to_s)
        if ((id).nil?)
          return nil
        else
          return (id.get_id)
    ...
    def get_subject_unique_id
      ...
      begin
        id = @info.get((CertificateSubjectUniqueIdentity::NAME).to_s + DOT + (CertificateSubjectUniqueIdentity::ID).to_s)
        if ((id).nil?)
          return nil
        else
          return (id.get_id)
    ...
    def parse(val)
      ...
      if (!(val.attr_data.available).equal?(0))
        raise CertificateParsingException.new("signed overrun, bytes = " + (val.attr_data.available).to_s)
      end
      ...
      info_sig_alg = @info.get((CertificateAlgorithmId::NAME).to_s + DOT + (CertificateAlgorithmId::ALGORITHM).to_s)
