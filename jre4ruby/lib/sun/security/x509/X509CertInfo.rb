...
module Sun::Security::X509
  ...
  class X509CertInfo 
    ...
    def to_s
      ...
      sb.append("[\n")
      sb.append("  " + (@version.to_s).to_s + "\n")
      sb.append("  Subject: " + (@subject.to_s).to_s + "\n")
      sb.append("  Signature Algorithm: " + (@alg_id.to_s).to_s + "\n")
      sb.append("  Key:  " + (@pub_key.to_s).to_s + "\n")
      sb.append("  " + (@interval.to_s).to_s + "\n")
      sb.append("  Issuer: " + (@issuer.to_s).to_s + "\n")
      sb.append("  " + (@serial_num.to_s).to_s + "\n")
      # optional v2, v3 extras
      if (!(@issuer_unique_id).nil?)
        sb.append("  Issuer Id:\n" + (@issuer_unique_id.to_s).to_s + "\n")
      end
      if (!(@subject_unique_id).nil?)
        sb.append("  Subject Id:\n" + (@subject_unique_id.to_s).to_s + "\n")
      end
      if (!(@extensions).nil?)
        all_exts = @extensions.get_all_extensions
        objs = all_exts.to_array
        sb.append("\nCertificate Extensions: " + (objs.attr_length).to_s)
        i = 0
        ...
        if ((invalid.is_empty).equal?(false))
          sb.append("\nUnparseable certificate extensions: " + (invalid.size).to_s)
          i_ = 1
