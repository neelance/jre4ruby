...
module Sun::Security::Ssl
  ...
  class X509KeyManagerImpl < X509KeyManagerImplImports.const_get :X509ExtendedKeyManager
    ...
    def make_alias(entry)
      return (@uid_counter.increment_and_get).to_s + "." + (entry.attr_builder_index).to_s + "." + (entry.attr_alias).to_s
    end
      ...
      const_set_lazy(:KeyType) { Class.new do
        ...
        def initialize(algorithm)
          ...
          if ((k).equal?(-1))
            ...
            @sig_key_algorithm = (nil).to_s
          else
            @key_algorithm = (algorithm.substring(0, k)).to_s
            @sig_key_algorithm = (algorithm.substring(k + 1)).to_s
          end
        end
        ...
        def matches(chain)
          ...
          if (chain.attr_length > 1)
            ...
            pattern = "WITH" + (@sig_key_algorithm.to_upper_case(ENGLISH)).to_s
        ...
        begin
          ...
          if (!(results).nil?)
            ...
            if ((status.attr_check_result).equal?(CheckResult::OK))
              if (UseDebug)
                Debug.println("KeyMgr: choosing key: " + (status).to_s)
              end
    ...
    def get_aliases(key_type, issuers, check_type)
      ...
      Collections.sort(all_results)
      if (UseDebug)
        Debug.println("KeyMgr: getting aliases: " + (all_results).to_s)
      end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:EntryStatus) { Class.new do
        ...
        typesig { [] }
        def to_s
          s = @alias + " (verified: " + (@check_result).to_s + ")"
          if ((@builder_index).equal?(0))
            return s
          else
            return "Builder #" + (@builder_index).to_s + ", alias: " + s
          end
        end
