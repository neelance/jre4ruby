...
module Sun::Security::Ssl
  module X509TrustManagerImplImports
    ...
    def show_trusted_certs
      if (!(Debug).nil? && Debug.is_on("trustmanager"))
        @trusted_certs.each do |cert|
          ...
          System.out.println("  Subject: " + (cert.get_subject_x500principal).to_s)
          System.out.println("  Issuer:  " + (cert.get_issuer_x500principal).to_s)
          System.out.println("  Algorithm: " + (cert.get_public_key.get_algorithm).to_s + "; Serial number: 0x" + (cert.get_serial_number.to_s(16)).to_s)
          System.out.println("  Valid from " + (cert.get_not_before).to_s + " until " + (cert.get_not_after).to_s)
          System.out.println
        end
      end
    end
    ...
    def check_identity(hostname, cert, algorithm)
      if (!(algorithm).nil? && !(algorithm.length).equal?(0))
        ...
        if (!(hostname).nil? && hostname.starts_with("[") && hostname.ends_with("]"))
          hostname = (hostname.substring(1, hostname.length - 1)).to_s
        end
