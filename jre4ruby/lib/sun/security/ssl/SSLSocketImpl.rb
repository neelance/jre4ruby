  ...
  class SSLSocketImpl < SSLSocketImplImports.const_get :BaseSSLSocketImpl
            ...
            if (@write_lock.try_lock(get_so_linger, TimeUnit::SECONDS))
              ...
              if (!(self.attr_self).equal?(self) && !@auto_close)
                ...
                if ((!(Debug).nil?) && Debug.is_on("ssl"))
                  System.out.println((thread_name).to_s + ", received Exception: " + (ssle).to_s)
                end
              end
      ...
      synchronized((@read_lock)) do
        ...
        while ((!((state = get_connection_state)).equal?(Cs_CLOSED)) && (!(state).equal?(Cs_ERROR)) && (!(state).equal?(Cs_APP_CLOSED)))
          ...
          begin
            ...
            if ((!(Debug).nil?) && Debug.is_on("ssl"))
              System.out.println((thread_name).to_s + ", received EOFException: " + ((rethrow ? "error" : "ignored")).to_s)
            end
            if (rethrow)
              e_ = nil
              if (handshaking)
                e_ = SSLHandshakeException.new("Remote host closed connection during handshake")
              else
                e_ = SSLProtocolException.new("Remote host closed connection incorrectly")
              end
              e_.init_cause(eof)
              raise e_
            else
          ...
          begin
            ...
            fatal(alert_type, "Invalid padding", e__)
          end
          ...
          synchronized((self)) do
            ...
            when Record.attr_ct_alert
              ...
              if (!(Debug).nil? && Debug.is_on("ssl"))
                System.out.println((thread_name).to_s + ", Received record type: " + (r.content_type).to_s)
              end
              next
    ...
    def check_eof
      ...
      when Cs_ERROR, Cs_CLOSED
        ...
        e = SSLException.new("Connection has been shutdown: " + (@close_reason).to_s)
        ...
        e_ = SSLException.new("Connection has been shutdown: " + (@close_reason).to_s)
        e_.init_cause(@close_reason)
        raise e_
      end
    end
    ...
    def close_socket
      if ((!(Debug).nil?) && Debug.is_on("ssl"))
        System.out.println((thread_name).to_s + ", called closeSocket()")
      end
      if ((self.attr_self).equal?(self))
    ...
    def close
      if ((!(Debug).nil?) && Debug.is_on("ssl"))
        System.out.println((thread_name).to_s + ", called close()")
      end
    ...
    def close_internal(self_initiated)
      if ((!(Debug).nil?) && Debug.is_on("ssl"))
        System.out.println((thread_name).to_s + ", called closeInternal(" + (self_initiated).to_s + ")")
      end
      ...
      begin
        ...
        when Cs_CLOSED, Cs_APP_CLOSED
          ...
          if ((state).equal?(Cs_SENT_CLOSE))
            if (!(Debug).nil? && Debug.is_on("ssl"))
              System.out.println((thread_name).to_s + ", close invoked again; state = " + (get_connection_state).to_s)
            end
            ...
            if ((!(Debug).nil?) && Debug.is_on("ssl"))
              System.out.println((thread_name).to_s + ", after primary close; state = " + (get_connection_state).to_s)
            end
            return
          end
          if ((self.attr_self).equal?(self))
    ...
    def wait_for_close(rethrow)
      if (!(Debug).nil? && Debug.is_on("ssl"))
        System.out.println((thread_name).to_s + ", waiting for close_notify or alert: state " + (get_connection_state).to_s)
      end
      begin
        state = 0
        while ((!((state = get_connection_state)).equal?(Cs_CLOSED)) && (!(state).equal?(Cs_ERROR)) && (!(state).equal?(Cs_APP_CLOSED)))
        ...
        if (!(Debug).nil? && Debug.is_on("ssl"))
          System.out.println((thread_name).to_s + ", Exception while waiting for close " + (e_).to_s)
        end
        if (rethrow)
          raise e_ # pass exception up
        end
      end
    end
    ...
    def handle_exception(e, resumable)
      synchronized(self) do
        if ((!(Debug).nil?) && Debug.is_on("ssl"))
          System.out.println((thread_name).to_s + ", handling exception: " + (e.to_s).to_s)
        end
    ...
    def get_host
      synchronized(self) do
        if ((@host).nil?)
          @host = (get_inet_address.get_host_name).to_s
        end
        return @host
      end
    end
      ...
      if ((get_connection_state).equal?(Cs_HANDSHAKE))
        begin
          ...
          if (!(Debug).nil? && Debug.is_on("handshake"))
            System.out.println((thread_name).to_s + ", IOException in getSession():  " + (e).to_s)
          end
        end
      end
      synchronized((self)) do
    ...
    def set_so_timeout(timeout)
      if ((!(Debug).nil?) && Debug.is_on("ssl"))
        System.out.println((thread_name).to_s + ", setSoTimeout(" + (timeout).to_s + ") called")
      end
      if ((self.attr_self).equal?(self))
        super(timeout)
      else
