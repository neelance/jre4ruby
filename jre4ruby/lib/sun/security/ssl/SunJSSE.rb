...
module Sun::Security::Ssl
  ...
  class SunJSSE < Java::Security::Provider
        ...
        synchronized(self) do
          if ((self.attr_fips).nil?)
            ...
            if (!(self.attr_crypto_provider).equal?(p))
              raise ProviderException.new("SunJSSE already initialized with FIPS crypto provider " + (self.attr_crypto_provider).to_s)
            end
          end
        end
      end
    ...
    typesig { [] }
    def subclass_check
      if (!(get_class).equal?(Com::Sun::Net::Ssl::Internal::Ssl::Provider.class))
        raise AssertionError.new("Illegal subclass: " + (get_class).to_s)
      end
    end
