  ...
  class KerberosClientKeyExchange < KerberosClientKeyExchangeImports.const_get :HandshakeMessage
    ...
    def initialize(protocol_version, client_version, rand, input, server_keys)
      ...
      begin
        ...
        ticket_princ = ticket_sname.to_s.concat("@" + (ticket_realm.to_s).to_s)
        ...
        if ((dkey).nil?)
          ...
          raise IOException.new("Cannot find key of appropriate type to decrypt ticket - need etype " + (enc_part_key_type).to_s)
        end
        ...
        if (!(self.attr_debug).nil? && Debug.is_on("handshake"))
          ...
          System.out.println("realm: " + (enc_ticket_part.attr_crealm.to_s).to_s)
          System.out.println("cname: " + (enc_ticket_part.attr_cname.to_s).to_s)
        end
        ...
        if (is_loopback)
          local_host = Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedAction.class == Class ? Java::Security::PrivilegedAction : Object) do
            ...
            define_method :run do
              hostname = nil
              begin
                hostname = (InetAddress.get_local_host.get_host_name).to_s
