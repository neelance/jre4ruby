...
module Sun::Security::Ssl
  ...
  class ExtensionType 
    ...
    class_module.module_eval {
      ...
      def get(id)
        ...
        return ExtensionType.new(id, "type_" + (id).to_s)
      end
  ...
  class UnknownExtension < HelloExtensionsImports.const_get :HelloExtension
    ...
    typesig { [] }
    def to_s
      return "Unsupported extension " + (self.attr_type).to_s + ", data: " + (Debug.to_s(@data)).to_s
    end
  ...
  class ServerNameExtension < HelloExtensionsImports.const_get :HelloExtension
      ...
      const_set_lazy(:ServerName) { Class.new do
        ...
        def initialize(s)
          ...
          if ((@type).equal?(NAME_HOST_NAME))
            @hostname = (String.new(@data, "UTF8")).to_s
          else
            @hostname = (nil).to_s
          end
        end
        ...
        def to_s
          if ((@type).equal?(NAME_HOST_NAME))
            ...
            return "unknown-" + (@type).to_s + ": " + (Debug.to_s(@data)).to_s
          end
        end
    ...
    typesig { [] }
    def to_s
      return "Unsupported extension " + (self.attr_type).to_s + ", " + (@names.to_s).to_s
    end
  ...
  class SupportedEllipticCurvesExtension < HelloExtensionsImports.const_get :HelloExtension
    ...
    def initialize(s, len)
      ...
      if ((!((len & 1)).equal?(0)) || (!(k + 2).equal?(len)))
        raise SSLProtocolException.new("Invalid " + (self.attr_type).to_s + " extension")
      end
    ...
    def to_s
      ...
      sb.append("Extension " + (self.attr_type).to_s + ", curve names: {")
      first = true
      @curve_ids.each do |curveId|
        if (first)
          first = false
        ...
        if (!(oid).nil?)
          ...
          if ((curve_id).equal?(ARBITRARY_PRIME))
            ...
            if ((curve_id).equal?(ARBITRARY_CHAR2))
              ...
              sb.append("unknown curve " + (curve_id).to_s)
            end
          end
        end
      end
  ...
  class SupportedEllipticPointFormatsExtension < HelloExtensionsImports.const_get :HelloExtension
    ...
    def to_s
      ...
      return "Extension " + (self.attr_type).to_s + ", formats: " + (list).to_s
    end
