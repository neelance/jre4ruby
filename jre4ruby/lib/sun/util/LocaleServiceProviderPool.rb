          ...
          if (!(lsp).nil?)
            ...
            if (!(providers_obj).nil?)
              ...
              if (is_object_provider)
                Logger.get_logger("sun.util.LocaleServiceProviderPool").config("A locale sensitive service provider returned null for a localized objects,  which should not happen.  provider: " + (lsp).to_s + " locale: " + (requested).to_s)
              end
            end
          end
