  ...
  class LocaleData 
    ...
    class_module.module_eval {
      ...
      def is_non_euro_lang_supported
        ...
        locale_data_jar = (Java::Security::AccessController.do_privileged(Sun::Security::Action::GetPropertyAction.new("java.home"))).to_s + sep + "lib" + sep + "ext" + sep + LocaleDataJarName
      ...
      def create_locale_list
        ...
        while i < locales.attr_length
          ...
          if ((p1).equal?(-1))
            ...
            language = (current_token.substring(0, p1)).to_s
            p2 = current_token.index_of(Character.new(?_.ord), p1 + 1)
            if ((p2).equal?(-1))
              country = (current_token.substring(p1 + 1)).to_s
            else
              country = (current_token.substring(p1 + 1, p2)).to_s
              if (p2 < current_token.length)
                variant = (current_token.substring(p2 + 1)).to_s
              end
            end
          end
