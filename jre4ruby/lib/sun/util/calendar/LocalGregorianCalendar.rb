...
module Sun::Util::Calendar
  module LocalGregorianCalendarImports
  ...
  class LocalGregorianCalendar < LocalGregorianCalendarImports.const_get :BaseCalendar
    ...
    class_module.module_eval {
      const_set_lazy(:Date) { Class.new(BaseCalendar::Date) do
        ...
        def to_s
          time = super
          time = (time.substring(time.index_of(Character.new(?T.ord)))).to_s
          sb = StringBuffer.new
          era = get_era
          if (!(era).nil?)
      ...
      def get_local_gregorian_calendar(name)
        ...
        begin
          ...
          fname = home_dir + (JavaFile.attr_separator).to_s + "lib" + (JavaFile.attr_separator).to_s + "calendars.properties"
