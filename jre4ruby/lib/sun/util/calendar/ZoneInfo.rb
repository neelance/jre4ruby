...
module Sun::Util::Calendar
  ...
  class ZoneInfo < ZoneInfoImports.const_get :TimeZone
    ...
    # @return the string
    def to_s
      return (get_class.get_name).to_s + "[id=\"" + (get_id).to_s + "\"" + ",offset=" + (get_last_raw_offset).to_s + ",dstSavings=" + (@dst_savings).to_s + ",useDaylight=" + (use_daylight_time).to_s + ",transitions=" + (((!(@transitions).nil?) ? @transitions.attr_length : 0)).to_s + ",lastRule=" + (((@last_rule).nil? ? get_last_rule_instance : @last_rule)).to_s + "]"
    end
      ...
      def get_time_zone(id)
        ...
        if ((zi).nil?)
          ...
          begin
            map = get_alias_table
            alias_ = id
            while (!((alias_ = (map.get(alias_)).to_s)).nil?)
