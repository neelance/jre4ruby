...
module Java::Nio
  ...
  class HeapCharBufferR < HeapCharBufferRImports.const_get :HeapCharBuffer
    include_class_members HeapCharBufferRImports
    ...
    typesig { [] }
    def slice
      return HeapCharBufferR.new(self.attr_hb, -1, 0, self.remaining, self.remaining, self.position + self.attr_offset)
    end
    ...
    def sub_sequence(start, end_)
      ...
      len = end_ - start
      return HeapCharBufferR.new(self.attr_hb, -1, 0, len, len, self.attr_offset + position + start)
    end
    ...
    private
    alias_method :initialize__heap_char_buffer_r, :initialize
  end
