    ...
    class_module.module_eval {
      ...
      def at_bug_level(bl)
        ...
        if ((self.attr_bug_level).nil?)
          ...
          self.attr_bug_level = (AccessController.do_privileged(GetPropertyAction.new("sun.nio.cs.bugLevel"))).to_s
          if ((self.attr_bug_level).nil?)
            self.attr_bug_level = ""
          end
        end
      ...
      def providers
        return Class.new(Iterator.class == Class ? Iterator : Object) do
          ...
          define_method :initialize do
            ...
            @sl = ServiceLoader.load(CharsetProvider.class, @cl)
            @i = @sl.iterator
            @next = nil
          end
