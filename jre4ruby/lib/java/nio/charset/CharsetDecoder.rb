...
module Java::Nio::Charset
  ...
  class CharsetDecoder 
      ...
      loop do
        ...
        if (cr.is_malformed)
          ...
          if (cr.is_unmappable)
            ...
            raise AssertError, (cr.to_s).to_s if not (false)
          end
        end
    ...
    def throw_illegal_state_exception(from, to)
      raise IllegalStateException.new("Current state = " + (self.attr_state_names[from]).to_s + ", new state = " + (self.attr_state_names[to]).to_s)
    end
