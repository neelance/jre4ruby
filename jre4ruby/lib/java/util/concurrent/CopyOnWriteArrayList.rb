...
module Java::Util::Concurrent
  ...
  class CopyOnWriteArrayList 
    ...
    def add(index, element)
      ...
      begin
        ...
        if (index > len || index < 0)
          raise IndexOutOfBoundsException.new("Index: " + (index).to_s + ", Size: " + (len).to_s)
        end
        new_elements = nil
        num_moved = len - index
    ...
    def equals(o)
      ...
      i = 0
      while i < len
        if (!it.has_next || !eq(elements[i], it.next))
          return false
        end
        (i += 1)
      end
    ...
    class_module.module_eval {
      ...
      when_class_loaded do
        begin
          const_set :LockOffset, UnsafeInstance.object_field_offset(CopyOnWriteArrayList.class.get_declared_field("lock"))
        rescue Exception => ex
