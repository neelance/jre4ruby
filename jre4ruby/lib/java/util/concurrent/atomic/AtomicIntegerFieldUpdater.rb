...
module Java::Util::Concurrent::Atomic
  module AtomicIntegerFieldUpdaterImports
  ...
  class AtomicIntegerFieldUpdater 
    include_class_members AtomicIntegerFieldUpdaterImports
    ...
    class_module.module_eval {
      ...
      def new_updater(tclass, field_name)
        return AtomicIntegerFieldUpdaterImpl.new(tclass, field_name)
      end
    }
      ...
      const_set_lazy(:AtomicIntegerFieldUpdaterImpl) { Class.new(AtomicIntegerFieldUpdater) do
        ...
        def initialize(tclass, field_name)
          ...
          if (!(fieldt).equal?(Array))
            raise IllegalArgumentException.new("Must be integer type")
          end
        ...
        def ensure_protected_access(obj)
          ...
          raise RuntimeException.new(IllegalAccessException.new("Class " + (@cclass.get_name).to_s + " can not access a protected member of class " + (@tclass.get_name).to_s + " using an instance of " + (obj.get_class.get_name).to_s))
        end
        
        private
        alias_method :initialize__atomic_integer_field_updater_impl, :initialize
      end }
    }
    
    private
    alias_method :initialize__atomic_integer_field_updater, :initialize
  end
