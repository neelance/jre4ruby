    ...
    class_module.module_eval {
      ...
      const_set_lazy(:DefaultThreadFactory) { Class.new do
        ...
        def initialize
          ...
          @name_prefix = "pool-" + (self.class::PoolNumber.get_and_increment).to_s + "-thread-"
        end
        
        typesig { [Runnable] }
        def new_thread(r)
          t = JavaThread.new(@group, r, @name_prefix + (@thread_number.get_and_increment).to_s, 0)
          if (t.is_daemon)
            t.set_daemon(false)
          end
