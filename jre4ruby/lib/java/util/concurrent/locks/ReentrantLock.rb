...
module Java::Util::Concurrent::Locks
  ...
  # @author Doug Lea
  class ReentrantLock 
    include_class_members ReentrantLockImports
    include Lock
    include Java::Io::Serializable
    
    class_module.module_eval {
      const_set_lazy(:SerialVersionUID) { 7373984872572414699 }
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:Sync) { Class.new(AbstractQueuedSynchronizer) do
        ...
        class_module.module_eval {
          const_set_lazy(:SerialVersionUID) { -5179523762034025860 }
          const_attr_reader  :SerialVersionUID
        }
        ...
        def nonfair_try_acquire(acquires)
        ...
        def get_owner
          return (get_state).equal?(0) ? nil : get_exclusive_owner_thread
        end
        
        typesig { [] }
        def get_hold_count
          return is_held_exclusively ? get_state : 0
        end
      ...
      const_set_lazy(:NonfairSync) { Class.new(Sync) do
        ...
        class_module.module_eval {
          const_set_lazy(:SerialVersionUID) { 7316153563782823691 }
          const_attr_reader  :SerialVersionUID
        }
        ...
        def try_acquire(acquires)
          return nonfair_try_acquire(acquires)
        end
      ...
      const_set_lazy(:FairSync) { Class.new(Sync) do
        ...
        class_module.module_eval {
          const_set_lazy(:SerialVersionUID) { -3000897897090466540 }
          const_attr_reader  :SerialVersionUID
        }
        ...
        def try_acquire(acquires)
          ...
          c = get_state
          if ((c).equal?(0))
            if (!has_queued_predecessors && compare_and_set_state(0, acquires))
              set_exclusive_owner_thread(current)
              return true
            end
          else
    ...
    def to_s
      o = @sync.get_owner
      return super + (((o).nil?) ? "[Unlocked]" : "[Locked by thread " + (o.get_name).to_s + "]")
    end
    
    private
    alias_method :initialize__reentrant_lock, :initialize
  end
