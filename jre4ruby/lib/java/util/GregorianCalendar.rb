  ...
  class GregorianCalendar < GregorianCalendarImports.const_get :Calendar
    ...
    def compute_fields(field_mask, tz_mask)
      ...
      if (fixed_date >= @gregorian_cutover_date)
        ...
        raise AssertError, "cache control: inconsictency" + ", cachedFixedDate=" + (@cached_fixed_date).to_s + ", computed=" + (Gcal.get_fixed_date(@gdate.get_normalized_year, @gdate.get_month, @gdate.get_day_of_month, @gdate)).to_s + ", date=" + (@gdate).to_s if not ((@cached_fixed_date).equal?(Long::MIN_VALUE) || (Gcal.get_fixed_date(@gdate.get_normalized_year, @gdate.get_month, @gdate.get_day_of_month, @gdate)).equal?(@cached_fixed_date))
        ...
        @calsys = Gcal
        @cdate = @gdate
        raise AssertError, "dow=" + (@cdate.get_day_of_week).to_s + ", date=" + (@cdate).to_s if not (@cdate.get_day_of_week > 0)
      else
