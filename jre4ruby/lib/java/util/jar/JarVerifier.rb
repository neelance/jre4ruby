    ...
    def process_entry(mev)
      if (!@parsing_block_or_sf)
        ...
        begin
          ...
          if (!(Debug).nil?)
            Debug.println("processEntry caught: " + (ioe).to_s)
          end
          # ignore and treat as unsigned
        rescue SignatureException => se
          if (!(Debug).nil?)
            Debug.println("processEntry caught: " + (se).to_s)
          end
          # ignore and treat as unsigned
        rescue NoSuchAlgorithmException => nsae
          if (!(Debug).nil?)
            Debug.println("processEntry caught: " + (nsae).to_s)
          end
          # ignore and treat as unsigned
        rescue CertificateException => ce
          if (!(Debug).nil?)
            Debug.println("processEntry caught: " + (ce).to_s)
          end
          # ignore and treat as unsigned
        end
      end
