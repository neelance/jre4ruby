  ...
  class ResourceBundle 
    ...
    def get_object(key)
      ...
      if ((obj).nil?)
        ...
        if ((obj).nil?)
          raise MissingResourceException.new("Can't find resource for bundle " + (self.get_class.get_name).to_s + ", key " + key, self.get_class.get_name, key)
        end
      end
      return obj
    end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:CacheKey) { Class.new do
        ...
        def to_s
          ...
          return "CacheKey[" + @name + ", lc=" + l + ", ldr=" + (get_loader).to_s + "(format=" + @format + ")]"
        end
      ...
      def throw_missing_resource_exception(base_name, locale, cause)
        ...
        # className
        # key
        raise MissingResourceException.new("Can't find bundle for base name " + base_name + ", locale " + (locale).to_s, base_name + "_" + (locale).to_s, "", cause)
      end
      ...
      def set_expiration_time(cache_key, control)
        ...
        if (ttl >= 0)
          ...
          if (ttl >= Control::TTL_NO_EXPIRATION_CONTROL)
            ...
            raise IllegalArgumentException.new("Invalid Control: TTL=" + (ttl).to_s)
          end
        end
      end
      ...
      const_set_lazy(:Control) { Class.new do
        ...
        def new_bundle(base_name, locale, format, loader, reload)
          ...
          if ((format == "java.class"))
            begin
              ...
              # ClassCastException.
              if (ResourceBundle.class.is_assignable_from(bundle_class))
                bundle = bundle_class.new_instance
              else
                raise ClassCastException.new((bundle_class.get_name).to_s + " cannot be cast to ResourceBundle")
              end
        ...
        def needs_reload(base_name, locale, format, loader, bundle, load_time)
          ...
          if ((format == "java.class") || (format == "java.properties"))
            format = (format.substring(5)).to_s
          end
          result = false
          begin
