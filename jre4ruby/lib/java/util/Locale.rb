...
module Java::Util
  ...
  class Locale 
    ...
    class_module.module_eval {
      ...
      def get_default
        ...
        if ((self.attr_default_locale).nil?)
          ...
          country = nil
          variant = nil
          language = (AccessController.do_privileged(GetPropertyAction.new("user.language", "en"))).to_s
          # for compatibility, check for old user.region property
          region = (AccessController.do_privileged(GetPropertyAction.new("user.region"))).to_s
          if (!(region).nil?)
            # region can be of form country, country_variant, or _variant
            i = region.index_of(Character.new(?_.ord))
            if (i >= 0)
              country = (region.substring(0, i)).to_s
              variant = (region.substring(i + 1)).to_s
            else
              country = region
              variant = ""
            end
          else
            country = (AccessController.do_privileged(GetPropertyAction.new("user.country", ""))).to_s
            variant = (AccessController.do_privileged(GetPropertyAction.new("user.variant", ""))).to_s
          end
    ...
    def get_iso3language
      ...
      if ((language3).nil?)
        raise MissingResourceException.new("Couldn't find 3-letter language code for " + @language, "FormatData_" + (to_s).to_s, "ShortLanguage")
      end
      return language3
    end
    ...
    def get_iso3country
      ...
      if ((country3).nil?)
        raise MissingResourceException.new("Couldn't find 3-letter country code for " + @country, "FormatData_" + (to_s).to_s, "ShortCountry")
      end
      return country3
    end
    ...
    def get_display_string(code, in_locale, type)
      ...
      begin
        ...
        pool = LocaleServiceProviderPool.get_pool(LocaleNameProvider.class)
        if (pool.has_providers)
          result = (pool.get_localized_object(LocaleNameGetter::INSTANCE, in_locale, bundle, key, type, code)).to_s
        end
        if ((result).nil?)
          result = (bundle.get_string(key)).to_s
        end
        if (!(result).nil?)
          return result
        end
    ...
    def get_display_variant(in_locale)
      ...
      begin
        list_pattern = (bundle.get_string("ListPattern")).to_s
        list_composition_pattern = (bundle.get_string("ListCompositionPattern")).to_s
    ...
    def get_display_name(in_locale)
      ...
      list_composition_pattern = nil
      begin
        display_name_pattern = (bundle.get_string("DisplayNamePattern")).to_s
        list_pattern = (bundle.get_string("ListPattern")).to_s
        list_composition_pattern = (bundle.get_string("ListCompositionPattern")).to_s
      rescue MissingResourceException => e
      end
    ...
    def convert_old_isocodes(language)
      ...
      language = (to_lower_case(language).intern).to_s
