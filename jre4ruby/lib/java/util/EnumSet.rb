  ...
  class EnumSet < EnumSetImports.const_get :AbstractSet
    ...
    class_module.module_eval {
      ...
      def none_of(element_type)
        ...
        if ((universe).nil?)
          raise ClassCastException.new((element_type).to_s + " not an enum")
        end
    ...
    class_module.module_eval {
      ...
      def range(from, to)
        if ((from <=> to) > 0)
          raise IllegalArgumentException.new((from).to_s + " > " + (to).to_s)
        end
    ...
    def type_check(e)
      ...
      if (!(e_class).equal?(@element_type) && !(e_class.get_superclass).equal?(@element_type))
        raise ClassCastException.new((e_class).to_s + " != " + (@element_type).to_s)
      end
    end
