      ...
      const_set_lazy(:CalendarAccessControlContext) { Class.new do
        ...
        class_module.module_eval {
          when_class_loaded do
            perm = RuntimePermission.new("accessClassInPackage.sun.util.calendar")
            perms = perm.new_permission_collection
            perms.add(perm)
            const_set :INSTANCE, AccessControlContext.new(Array.typed(ProtectionDomain).new([ProtectionDomain.new(nil, perms)]))
          end
        }
