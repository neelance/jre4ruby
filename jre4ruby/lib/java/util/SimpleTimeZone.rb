...
module Java::Util
  ...
  class SimpleTimeZone < SimpleTimeZoneImports.const_get :TimeZone
    ...
    def initialize(raw_offset, id, start_month, start_day, start_day_of_week, start_time, start_time_mode, end_month, end_day, end_day_of_week, end_time, end_time_mode, dst_savings)
      ...
      decode_rules
      if (dst_savings <= 0)
        raise IllegalArgumentException.new("Illegal daylight saving value: " + (dst_savings).to_s)
      end
    end
    ...
    def get_offset(era, year, month, day, day_of_week, millis)
      if (!(era).equal?(GregorianCalendar::AD) && !(era).equal?(GregorianCalendar::BC))
        raise IllegalArgumentException.new("Illegal era " + (era).to_s)
      end
      y = year
    ...
    def set_dstsavings(millis_saved_during_dst)
      if (millis_saved_during_dst <= 0)
        raise IllegalArgumentException.new("Illegal daylight saving value: " + (millis_saved_during_dst).to_s)
      end
    ...
    def to_s
      return (get_class.get_name).to_s + "[id=" + (get_id).to_s + ",offset=" + (@raw_offset).to_s + ",dstSavings=" + (@dst_savings).to_s + ",useDaylight=" + (@use_daylight).to_s + ",startYear=" + (@start_year).to_s + ",startMode=" + (@start_mode).to_s + ",startMonth=" + (@start_month).to_s + ",startDay=" + (@start_day).to_s + ",startDayOfWeek=" + (@start_day_of_week).to_s + ",startTime=" + (@start_time).to_s + ",startTimeMode=" + (@start_time_mode).to_s + ",endMode=" + (@end_mode).to_s + ",endMonth=" + (@end_month).to_s + ",endDay=" + (@end_day).to_s + ",endDayOfWeek=" + (@end_day_of_week).to_s + ",endTime=" + (@end_time).to_s + ",endTimeMode=" + (@end_time_mode).to_s + (Character.new(?].ord)).to_s
    end
    ...
    def decode_start_rule
      ...
      if (!(@start_day).equal?(0))
        if (@start_month < Calendar::JANUARY || @start_month > Calendar::DECEMBER)
          raise IllegalArgumentException.new("Illegal start month " + (@start_month).to_s)
        end
        if (@start_time < 0 || @start_time >= MillisPerDay)
          raise IllegalArgumentException.new("Illegal start time " + (@start_time).to_s)
        end
        if ((@start_day_of_week).equal?(0))
          ...
          if (@start_day_of_week > Calendar::SATURDAY)
            raise IllegalArgumentException.new("Illegal start day of week " + (@start_day_of_week).to_s)
          end
        end
        if ((@start_mode).equal?(DOW_IN_MONTH_MODE))
          if (@start_day < -5 || @start_day > 5)
            raise IllegalArgumentException.new("Illegal start day of week in month " + (@start_day).to_s)
          end
        else
          if (@start_day < 1 || @start_day > StaticMonthLength[@start_month])
            raise IllegalArgumentException.new("Illegal start day " + (@start_day).to_s)
          end
        end
      end
    end
    ...
    def decode_end_rule
      ...
      if (!(@end_day).equal?(0))
        if (@end_month < Calendar::JANUARY || @end_month > Calendar::DECEMBER)
          raise IllegalArgumentException.new("Illegal end month " + (@end_month).to_s)
        end
        if (@end_time < 0 || @end_time >= MillisPerDay)
          raise IllegalArgumentException.new("Illegal end time " + (@end_time).to_s)
        end
        if ((@end_day_of_week).equal?(0))
          ...
          if (@end_day_of_week > Calendar::SATURDAY)
            raise IllegalArgumentException.new("Illegal end day of week " + (@end_day_of_week).to_s)
          end
        end
        if ((@end_mode).equal?(DOW_IN_MONTH_MODE))
          if (@end_day < -5 || @end_day > 5)
            raise IllegalArgumentException.new("Illegal end day of week in month " + (@end_day).to_s)
          end
        else
          if (@end_day < 1 || @end_day > StaticMonthLength[@end_month])
            raise IllegalArgumentException.new("Illegal end day " + (@end_day).to_s)
          end
        end
      end
    end
