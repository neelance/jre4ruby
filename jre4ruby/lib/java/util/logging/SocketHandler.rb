...
module Java::Util::Logging
  ...
  class SocketHandler < SocketHandlerImports.const_get :StreamHandler
    ...
    def configure
      ...
      @host = (manager.get_string_property(cname + ".host", nil)).to_s
    end
    ...
    def initialize
      ...
      begin
        ...
        System.err.println("SocketHandler: connect failed to " + @host + ":" + (@port).to_s)
        raise ix
      end
    ...
    def connect
      ...
      if ((@port).equal?(0))
        raise IllegalArgumentException.new("Bad port: " + (@port).to_s)
      end
      if ((@host).nil?)
