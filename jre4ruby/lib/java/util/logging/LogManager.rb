...
module Java::Util::Logging
  ...
  class LogManager 
      ...
      when_class_loaded do
        AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          ...
          define_method :run do
            cname = nil
            begin
              cname = (System.get_property("java.util.logging.manager")).to_s
              if (!(cname).nil?)
                begin
    ...
    def initialize
      ...
      @changes = PropertyChangeSupport.new(LogManager.class)
    ...
    def find_node(name)
      ...
      while (name.length > 0)
        ...
        head = nil
        if (ix > 0)
          head = (name.substring(0, ix)).to_s
          name = (name.substring(ix + 1)).to_s
        else
          head = name
          name = ""
        end
