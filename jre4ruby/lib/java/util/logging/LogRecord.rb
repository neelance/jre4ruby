...
module Java::Util::Logging
  ...
  class LogRecord 
    ...
    def initialize(level, msg)
      ...
      synchronized((LogRecord.class)) do
    ...
    def read_object(in_)
      ...
      minor = in_.read_byte
      if (!(major).equal?(1))
        raise IOException.new("LogRecord: bad version: " + (major).to_s + "." + (minor).to_s)
      end
      len = in_.read_int
      if ((len).equal?(-1))
        @parameters = nil
    ...
    def infer_caller
      @need_to_infer_caller = false
      # Get the stack trace.
      stack = (Exception.new).get_stack_trace
