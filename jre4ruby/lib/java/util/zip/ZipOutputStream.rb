  ...
  class ZipOutputStream < ZipOutputStreamImports.const_get :DeflaterOutputStream
    ...
    def put_next_entry(e)
      ...
      if (!@names.add(e.attr_name))
        raise ZipException.new("duplicate entry: " + (e.attr_name).to_s)
      end
      ...
      if (!(@current).nil?)
        ...
        when DEFLATED
          ...
          if (((@current.attr_flag & 8)).equal?(0))
            ...
            if (!(e.attr_size).equal?(self.attr_def.get_bytes_read))
              raise ZipException.new("invalid entry size (expected " + (e.attr_size).to_s + " but got " + (self.attr_def.get_bytes_read).to_s + " bytes)")
            end
            if (!(e.attr_csize).equal?(self.attr_def.get_bytes_written))
              raise ZipException.new("invalid entry compressed size (expected " + (e.attr_csize).to_s + " but got " + (self.attr_def.get_bytes_written).to_s + " bytes)")
            end
            if (!(e.attr_crc).equal?(@crc.get_value))
              raise ZipException.new("invalid entry CRC-32 (expected 0x" + (Long.to_hex_string(e.attr_crc)).to_s + " but got 0x" + (Long.to_hex_string(@crc.get_value)).to_s + ")")
            end
          else
        ...
        when STORED
          ...
          if (!(e.attr_size).equal?(@written - @locoff))
            raise ZipException.new("invalid entry size (expected " + (e.attr_size).to_s + " but got " + ((@written - @locoff)).to_s + " bytes)")
          end
          if (!(e.attr_crc).equal?(@crc.get_value))
            raise ZipException.new("invalid entry crc-32 (expected 0x" + (Long.to_hex_string(e.attr_crc)).to_s + " but got 0x" + (Long.to_hex_string(@crc.get_value)).to_s + ")")
          end
