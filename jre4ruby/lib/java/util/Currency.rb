  ...
  class Currency 
      ...
      when_class_loaded do
        AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          ...
          define_method :run do
            ...
            begin
              data_file = home_dir + (JavaFile.attr_separator).to_s + "lib" + (JavaFile.attr_separator).to_s + "currency.data"
              ...
              self.attr_other_currencies = (dis.read_utf).to_s
            ...
            if (false)
              ...
              begin
                prop_file = JavaFile.new(home_dir + (JavaFile.attr_separator).to_s + "lib" + (JavaFile.attr_separator).to_s + "currency.properties")
                if (prop_file.exists)
    ...
    class_module.module_eval {
      ...
      def get_available_currencies
        synchronized((Currency.class)) do
    ...
    def get_symbol(locale)
      begin
        ...
        pool = LocaleServiceProviderPool.get_pool(CurrencyNameProvider.class)
    ...
    def get_display_name(locale)
      begin
        ...
        pool = LocaleServiceProviderPool.get_pool(CurrencyNameProvider.class)
        if (pool.has_providers)
          result = (pool.get_localized_object(CurrencyNameGetter::INSTANCE, locale, bundle_key, bundle, @currency_code, DISPLAYNAME)).to_s
        end
        if ((result).nil?)
          result = (bundle.get_string(bundle_key)).to_s
        end
        if (!(result).nil?)
          return result
        end
