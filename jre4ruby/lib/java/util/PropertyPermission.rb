...
# have any questions.
module Java::Util
  module PropertyPermissionImports
  ...
  class PropertyPermissionCollection < PropertyPermissionImports.const_get :PermissionCollection
    ...
    def implies(permission)
      ...
      while (!((last = name.last_index_of(".", offset))).equal?(-1))
        name = (name.substring(0, last + 1)).to_s + "*"
        # System.out.println("check "+name);
        synchronized((self)) do
          x = @perms.get(name)
        end
    ...
    class_module.module_eval {
      ...
      # boolean saying if "*" is in the collection.
      const_set_lazy(:SerialPersistentFields) { Array.typed(ObjectStreamField).new([ObjectStreamField.new("permissions", Hashtable.class), ObjectStreamField.new("all_allowed", Boolean::TYPE), ]) }
