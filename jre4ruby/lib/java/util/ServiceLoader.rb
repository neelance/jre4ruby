...
module Java::Util
  ...
  class ServiceLoader 
    ...
    class_module.module_eval {
      typesig { [Class, String, Exception] }
      def fail(service, msg, cause)
        raise ServiceConfigurationError.new((service.get_name).to_s + ": " + msg, cause)
      end
      
      typesig { [Class, String] }
      def fail(service, msg)
        raise ServiceConfigurationError.new((service.get_name).to_s + ": " + msg)
      end
      
      typesig { [Class, URL, ::Java::Int, String] }
      def fail(service, u, line, msg)
        fail(service, (u).to_s + ":" + (line).to_s + ": " + msg)
      end
    }
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:LazyIterator) { Class.new do
        ...
        def has_next
          ...
          if ((@configs).nil?)
            begin
              full_name = PREFIX + (@service.get_name).to_s
              if ((@loader).nil?)
    ...
    def iterator
      return Class.new(Iterator.class == Class ? Iterator : Object) do
        ...
        define_method :next do
          if (@known_providers.has_next)
            return @known_providers.next.get_value
          end
          return self.attr_lookup_iterator.next
        end
    ...
    def to_s
      return "java.util.ServiceLoader[" + (@service.get_name).to_s + "]"
    end
