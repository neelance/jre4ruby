...
module Java::Util
  ...
  class Scanner 
    ...
    typesig { [] }
    def cache_result
      @has_next_result = (@matcher.group).to_s
      @has_next_position = @matcher.end
    ...
    def get_complete_token_in_buffer(pattern_)
      ...
      if (found_next_delim && ((@matcher.end).equal?(@position)))
    ...
    def use_locale(locale)
      ...
      @group_separator = "\\" + (dfs.get_grouping_separator).to_s
      @decimal_separator = "\\" + (dfs.get_decimal_separator).to_s
      # Quoting the nonzero length locale-specific things
      # to avoid potential conflict with metacharacters
      @nan_string = "\\Q" + (dfs.get_na_n).to_s + "\\E"
      @infinity_string = "\\Q" + (dfs.get_infinity).to_s + "\\E"
      @positive_prefix = (df.get_positive_prefix).to_s
      if (@positive_prefix.length > 0)
        @positive_prefix = "\\Q" + @positive_prefix + "\\E"
      end
      @negative_prefix = (df.get_negative_prefix).to_s
      if (@negative_prefix.length > 0)
        @negative_prefix = "\\Q" + @negative_prefix + "\\E"
      end
      @positive_suffix = (df.get_positive_suffix).to_s
      if (@positive_suffix.length > 0)
        @positive_suffix = "\\Q" + @positive_suffix + "\\E"
      end
      @negative_suffix = (df.get_negative_suffix).to_s
    ...
    def use_radix(radix)
      if ((radix < Character::MIN_RADIX) || (radix > Character::MAX_RADIX))
        raise IllegalArgumentException.new("radix:" + (radix).to_s)
      end
    ...
    def to_s
      ...
      sb.append("[delimiters=" + (@delim_pattern).to_s + "]")
      sb.append("[position=" + (@position).to_s + "]")
      sb.append("[match valid=" + (@match_valid).to_s + "]")
      sb.append("[need input=" + (@need_input).to_s + "]")
      sb.append("[source closed=" + (@source_closed).to_s + "]")
      sb.append("[skipped=" + (@skipped).to_s + "]")
    ...
    # @throws IllegalStateException if this scanner is closed
    def next(pattern_)
      return next(@pattern_cache.for_name(pattern_))
    end
    ...
    def has_next_line
      ...
      if (!(result).nil?)
        ...
        line_sep = mr.group(1)
        if (!(line_sep).nil?)
          result = (result.substring(0, result.length - line_sep.length)).to_s
          cache_result(result)
        else
          cache_result
        end
    ...
    def next_boolean
      clear_caches
      return Boolean.parse_boolean(next(bool_pattern))
    end
    ...
    def next_byte(radix)
      ...
      begin
        s = next(integer_pattern)
        if ((@matcher.group(@simple_group_index)).nil?)
          s = (process_integer_token(s)).to_s
        end
    ...
    def process_integer_token(token)
      ...
      if ((pre_len > 0) && result.starts_with(@negative_prefix))
        is_negative = true
        result = (result.substring(pre_len)).to_s
      end
      suf_len = @negative_suffix.length
      if ((suf_len > 0) && result.ends_with(@negative_suffix))
        is_negative = true
        result = (result.substring(result.length - suf_len, result.length)).to_s
      end
      if (is_negative)
        result = "-" + result
      end
    ...
    def process_float_token(token)
      ...
      if (!(@decimal_separator == "\\."))
        result = (result.replace_all(@decimal_separator, ".")).to_s
      end
      is_negative = false
      pre_len = @negative_prefix.length
      if ((pre_len > 0) && result.starts_with(@negative_prefix))
    ...
    def next_float
      ...
      clear_caches
      begin
        return Float.parse_float(process_float_token(next(float_pattern)))
      rescue NumberFormatException => nfe
        @position = @matcher.start # don't skip bad token
        raise InputMismatchException.new(nfe.get_message)
      end
    ...
    def next_double
      ...
      begin
        return Double.parse_double(process_float_token(next(float_pattern)))
      rescue NumberFormatException => nfe
        @position = @matcher.start # don't skip bad token
        raise InputMismatchException.new(nfe.get_message)
      end
    ...
    def next_big_decimal
      ...
      # Search for next float
      begin
        s = process_float_token(next(decimal_pattern))
