  ...
  class Bidi 
    ...
    def initialize(text, text_start, embeddings, emb_start, paragraph_length, flags)
      ...
      if (paragraph_length < 0)
        raise IllegalArgumentException.new("bad length: " + (paragraph_length).to_s)
      end
      if (text_start < 0 || paragraph_length > text.attr_length - text_start)
        raise IllegalArgumentException.new("bad range: " + (text_start).to_s + " length: " + (paragraph_length).to_s + " for text of length: " + (text.attr_length).to_s)
      end
      if (!(embeddings).nil? && (emb_start < 0 || paragraph_length > embeddings.attr_length - emb_start))
        raise IllegalArgumentException.new("bad range: " + (emb_start).to_s + " length: " + (paragraph_length).to_s + " for embeddings of length: " + (text.attr_length).to_s)
      end
      if (!(embeddings).nil?)
    ...
    def create_line_bidi(line_start, line_limit)
      ...
      if (line_start < 0 || line_limit < line_start || line_limit > @length)
        raise IllegalArgumentException.new("range " + (line_start).to_s + " to " + (line_limit).to_s + " is invalid for paragraph of length " + (@length).to_s)
      end
      if ((@runs).nil?)
    ...
    class_module.module_eval {
      ...
      def reorder_visually(levels, level_start, objects, object_start, count)
        if (count < 0)
          raise IllegalArgumentException.new("count " + (count).to_s + " must be >= 0")
        end
        if (level_start < 0 || level_start + count > levels.attr_length)
          raise IllegalArgumentException.new("levelStart " + (level_start).to_s + " and count " + (count).to_s + " out of range [0, " + (levels.attr_length).to_s + "]")
        end
        if (object_start < 0 || object_start + count > objects.attr_length)
          raise IllegalArgumentException.new("objectStart " + (object_start).to_s + " and count " + (count).to_s + " out of range [0, " + (objects.attr_length).to_s + "]")
        end
    ...
    def to_s
      ...
      buf.append("[dir: " + (@dir).to_s)
      buf.append(" baselevel: " + (@baselevel).to_s)
      buf.append(" length: " + (@length).to_s)
      if ((@runs).nil?)
