...
module Java::Text
  module RuleBasedBreakIteratorImports
  ...
  class RuleBasedBreakIterator < RuleBasedBreakIteratorImports.const_get :BreakIterator
    ...
    def read_file(datafile)
      ...
      if (!(buf[offset]).equal?(SupportedVersion))
        raise MissingResourceException.new("Unsupported version(" + (buf[offset]).to_s + ")", datafile, "")
      end
    ...
    def to_s
      ...
      sb.append("checksum=0x" + (Long.to_hex_string(@checksum)).to_s)
      sb.append(Character.new(?].ord))
      return sb.to_s
    end
      ...
      const_set_lazy(:SafeCharIterator) { Class.new do
        ...
        def clone
          ...
          begin
            ...
            raise JavaError.new("Clone not supported: " + (e).to_s)
          end
