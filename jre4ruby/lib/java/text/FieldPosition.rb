...
module Java::Text
  ...
  class FieldPosition 
    ...
    def to_s
      return (get_class.get_name).to_s + "[field=" + (@field).to_s + ",attribute=" + (@attribute).to_s + ",beginIndex=" + (@begin_index).to_s + ",endIndex=" + (@end_index).to_s + (Character.new(?].ord)).to_s
    end
