...
module Java::Text
  ...
  class DecimalFormat < DecimalFormatImports.const_get :NumberFormat
    ...
    def expand_affixes
      ...
      if (!(@pos_prefix_pattern).nil?)
        @positive_prefix = (expand_affix(@pos_prefix_pattern, buffer)).to_s
        @positive_prefix_field_positions = nil
      end
      if (!(@pos_suffix_pattern).nil?)
        @positive_suffix = (expand_affix(@pos_suffix_pattern, buffer)).to_s
        @positive_suffix_field_positions = nil
      end
      if (!(@neg_prefix_pattern).nil?)
        @negative_prefix = (expand_affix(@neg_prefix_pattern, buffer)).to_s
        @negative_prefix_field_positions = nil
      end
      if (!(@neg_suffix_pattern).nil?)
        @negative_suffix = (expand_affix(@neg_suffix_pattern, buffer)).to_s
        @negative_suffix_field_positions = nil
      end
    end
      ...
      while i < pattern.length
        ...
        if ((c).equal?(QUOTE))
          ...
          when CURRENCY_SIGN
            ...
            if (i < pattern.length && (pattern.char_at(i)).equal?(CURRENCY_SIGN))
              (i += 1)
              string = (@symbols.get_international_currency_symbol).to_s
            else
              string = (@symbols.get_currency_symbol).to_s
            end
            if (string.length > 0)
              if ((positions).nil?)
      ...
      while j >= 0
        ...
        if ((j).equal?(1))
          ...
          if (((@neg_suffix_pattern).equal?(@pos_suffix_pattern) && (@negative_suffix == @positive_suffix)) || (!(@neg_suffix_pattern).nil? && (@neg_suffix_pattern == @pos_suffix_pattern)))
            # n == p == null
            if ((!(@neg_prefix_pattern).nil? && !(@pos_prefix_pattern).nil? && (@neg_prefix_pattern == "'-" + @pos_prefix_pattern)) || ((@neg_prefix_pattern).equal?(@pos_prefix_pattern) && (@negative_prefix == (@symbols.get_minus_sign).to_s + @positive_prefix)))
              break
            end
          end
    ...
    def apply_pattern(pattern, localized)
      ...
      if (localized)
        ...
        exponent = (@symbols.get_exponent_separator).to_s
      ...
      while j >= 0 && start < pattern.length
                  ...
                  if ((ch).equal?(QUOTE))
                    ...
                    if ((ch).equal?(separator))
                      ...
                      if ((phase).equal?(0) || (j).equal?(0))
                        raise IllegalArgumentException.new("Unquoted special character '" + (ch).to_s + "' in pattern \"" + pattern + (Character.new(?".ord)).to_s)
                      end
                      start = pos + 1
                      pos = pattern.length
                      (pos += 1)
                    ...
                    # Next handle characters which are appended directly.
                      ...
                      if ((ch).equal?(percent))
                        if (!(multiplier).equal?(1))
                          raise IllegalArgumentException.new("Too many percent/per mille characters in pattern \"" + pattern + (Character.new(?".ord)).to_s)
                        end
                        multiplier = 100
                        affix.append("'%")
                        (pos += 1)
            ...
            if ((ch).equal?(digit_))
              ...
              if ((ch).equal?(zero_digit))
                if (digit_right_count > 0)
                  raise IllegalArgumentException.new("Unexpected '0' in pattern \"" + pattern + (Character.new(?".ord)).to_s)
                end
                (zero_digit_count += 1)
                if (grouping_count >= 0 && decimal_pos < 0)
                  (grouping_count += 1)
                ...
                if ((ch).equal?(grouping_separator))
                  ...
                  if ((ch).equal?(decimal_separator))
                    if (decimal_pos >= 0)
                      raise IllegalArgumentException.new("Multiple decimal separators in pattern \"" + pattern + (Character.new(?".ord)).to_s)
                    end
                    decimal_pos = digit_left_count + zero_digit_count + digit_right_count
                  else
                    if (pattern.region_matches(pos, exponent, 0, exponent.length))
                      if (@use_exponential_notation)
                        raise IllegalArgumentException.new("Multiple exponential " + "symbols in pattern \"" + pattern + (Character.new(?".ord)).to_s)
                      end
                      ...
                      if ((digit_left_count + zero_digit_count) < 1 || @min_exponent_digits < 1)
                        raise IllegalArgumentException.new("Malformed exponential " + "pattern \"" + pattern + (Character.new(?".ord)).to_s)
                      end
        ...
        if ((decimal_pos < 0 && digit_right_count > 0) || (decimal_pos >= 0 && (decimal_pos < digit_left_count || decimal_pos > (digit_left_count + zero_digit_count))) || (grouping_count).equal?(0) || in_quote)
          raise IllegalArgumentException.new("Malformed pattern \"" + pattern + (Character.new(?".ord)).to_s)
        end
        if ((j).equal?(1))
          @pos_prefix_pattern = (prefix.to_s).to_s
          @pos_suffix_pattern = (suffix.to_s).to_s
          ...
          @neg_prefix_pattern = (prefix.to_s).to_s
          @neg_suffix_pattern = (suffix.to_s).to_s
          got_negative = true
        end
        (j -= 1)
      end
      if ((pattern.length).equal?(0))
        @pos_prefix_pattern = (@pos_suffix_pattern = "").to_s
