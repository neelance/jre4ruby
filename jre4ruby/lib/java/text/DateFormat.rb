  ...
  class DateFormat < DateFormatImports.const_get :Format
    ...
    class_module.module_eval {
      ...
      def get_available_locales
        pool = LocaleServiceProviderPool.get_pool(DateFormatProvider.class)
      ...
      def get(time_style, date_style, flags, loc)
        if (!((flags & 1)).equal?(0))
          if (time_style < 0 || time_style > 3)
            raise IllegalArgumentException.new("Illegal time style " + (time_style).to_s)
          end
        else
          time_style = -1
        end
        if (!((flags & 2)).equal?(0))
          if (date_style < 0 || date_style > 3)
            raise IllegalArgumentException.new("Illegal date style " + (date_style).to_s)
          end
        else
          date_style = -1
        end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:Field) { Class.new(Format::Field) do
        ...
        class_module.module_eval {
          ...
          const_set_lazy(:InstanceMap) { HashMap.new(18) }
          const_attr_reader  :InstanceMap
          
          # Maps from Calendar constant (such as Calendar.ERA) to Field
          # constant (such as Field.ERA).
          const_set_lazy(:CalendarToFieldMapping) { Array.typed(Field).new(Calendar::FIELD_COUNT) { nil } }
        ...
        class_module.module_eval {
          ...
          def of_calendar_field(calendar_field)
            if (calendar_field < 0 || calendar_field >= self.class::CalendarToFieldMapping.attr_length)
              raise IllegalArgumentException.new("Unknown Calendar constant " + (calendar_field).to_s)
            end
        ...
        def initialize(name, calendar_field)
          ...
          if ((self.get_class).equal?(DateFormat::Field.class))
      ...
      const_set_lazy(:DateFormatGetter) { Class.new do
        ...
        class_module.module_eval {
          const_set_lazy(:INSTANCE) { DateFormatGetter.new }
          const_attr_reader  :INSTANCE
        }
        
        typesig { [DateFormatProvider, Locale, String, Object] }
