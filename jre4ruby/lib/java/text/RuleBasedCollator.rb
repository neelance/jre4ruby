  ...
  class RuleBasedCollator < RuleBasedCollatorImports.const_get :Collator
    ...
    def compare(source, target)
      synchronized(self) do
        ...
        while (true)
          ...
          if (gets)
            s_order = @source_cursor.next
          else
            gets = true
          end
          if (gett)
            t_order = @target_cursor.next
          else
            gett = true
          end
        ...
        if (!(s_order).equal?(CollationElementIterator::NULLORDER))
          ...
          end while (!((s_order = @source_cursor.next)).equal?(CollationElementIterator::NULLORDER))
        else
          if (!(t_order).equal?(CollationElementIterator::NULLORDER))
            ...
            end while (!((t_order = @target_cursor.next)).equal?(CollationElementIterator::NULLORDER))
          end
        end
    ...
    def get_collation_key(source)
      synchronized(self) do
        ...
        while (!((order = @source_cursor.next)).equal?(CollationElementIterator::NULLORDER))
    ...
    def clone
      ...
      if ((get_class).equal?(RuleBasedCollator.class))
