...
module Java::Lang
  ...
  class SecurityManager 
    ...
    def initialize
      @in_check = false
      @initialized = false
      synchronized((SecurityManager.class)) do
    ...
    def check_exit(status)
      check_permission(RuntimePermission.new("exitVM." + (status).to_s))
    end
    ...
    def check_connect(host, port)
      ...
      if ((port).equal?(-1))
        ...
        check_permission(SocketPermission.new(host + ":" + (port).to_s, SecurityConstants::SOCKET_CONNECT_ACTION))
      end
    end
    ...
    def check_connect(host, port, context)
      ...
      if ((port).equal?(-1))
        ...
        check_permission(SocketPermission.new(host + ":" + (port).to_s, SecurityConstants::SOCKET_CONNECT_ACTION), context)
      end
    end
    ...
    def check_listen(port)
      if ((port).equal?(0))
        ...
        check_permission(SocketPermission.new("localhost:" + (port).to_s, SecurityConstants::SOCKET_LISTEN_ACTION))
      end
    end
    ...
    def check_accept(host, port)
      ...
      check_permission(SocketPermission.new(host + ":" + (port).to_s, SecurityConstants::SOCKET_ACCEPT_ACTION))
    end
