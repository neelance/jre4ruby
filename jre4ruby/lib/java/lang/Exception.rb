...
module Java::Lang
  ...
  class Exception < Exception
    ...
    def initialize(cause)
      ...
      @cause = self
      fill_in_stack_trace
      @detail_message = (((cause).nil? ? nil : cause.to_s)).to_s
      @cause = cause
    end
    ...
    def print_stack_trace_as_cause(s, caused_trace)
      ...
      if (!(frames_in_common).equal?(0))
        s.println("\t... " + (frames_in_common).to_s + " more")
      end
    ...
    def set_stack_trace(stack_trace)
      ...
      while i < defensive_copy.attr_length
        if ((defensive_copy[i]).nil?)
          raise NullPointerException.new("stackTrace[" + (i).to_s + "]")
        end
        ((i += 1) - 1)
      end
