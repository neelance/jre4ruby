...
module Java::Lang::Reflect
  ...
  class Field < FieldImports.const_get :AccessibleObject
    ...
    def to_s
      mod = get_modifiers
      return (((((mod).equal?(0)) ? "" : ((Modifier.to_s(mod)).to_s + " ")) + get_type_name(get_type)).to_s + " " + (get_type_name(get_declaring_class)).to_s + "." + (get_name).to_s)
    end
    ...
    def to_generic_string
      ...
      return (((((mod).equal?(0)) ? "" : ((Modifier.to_s(mod)).to_s + " ")) + ((field_type.is_a?(Class)) ? get_type_name(field_type) : field_type.to_s)).to_s + " " + (get_type_name(get_declaring_class)).to_s + "." + (get_name).to_s)
    end
