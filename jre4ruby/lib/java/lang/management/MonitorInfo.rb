  ...
  class MonitorInfo < MonitorInfoImports.const_get :LockInfo
    ...
    def initialize(class_name, identity_hash_code, stack_depth, stack_frame)
      ...
      if (stack_depth >= 0 && (stack_frame).nil?)
        raise IllegalArgumentException.new("Parameter stackDepth is " + (stack_depth).to_s + " but stackFrame is null")
      end
      if (stack_depth < 0 && !(stack_frame).nil?)
        raise IllegalArgumentException.new("Parameter stackDepth is " + (stack_depth).to_s + " but stackFrame is not null")
      end
