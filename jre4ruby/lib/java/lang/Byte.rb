  ...
  class Byte < ByteImports.const_get :Numeric
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:ByteCache) { Class.new do
        ...
        class_module.module_eval {
          const_set_lazy(:Cache) { Array.typed(Byte).new(-(-128) + 127 + 1) { nil } }
          const_attr_reader  :Cache
          
          when_class_loaded do
            i = 0
            while i < self.class::Cache.attr_length
              self.class::Cache[i] = Byte.new((i - 128))
              i += 1
            end
          end
        }
      ...
      def parse_byte(s, radix)
        ...
        if (i < MIN_VALUE || i > MAX_VALUE)
          raise NumberFormatException.new("Value out of range. Value:\"" + s + "\" Radix:" + (radix).to_s)
        end
        return i
      end
      ...
      def decode(nm)
        ...
        if (i < MIN_VALUE || i > MAX_VALUE)
          raise NumberFormatException.new("Value " + (i).to_s + " out of range from input " + nm)
        end
        return i
      end
    }
