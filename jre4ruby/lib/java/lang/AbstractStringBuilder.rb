  ...
  class AbstractStringBuilder 
    ...
    def append(s, start, end_)
      ...
      if ((start < 0) || (end_ < 0) || (start > end_) || (end_ > s.length))
        raise IndexOutOfBoundsException.new("start " + (start).to_s + ", end " + (end_).to_s + ", s.length() " + (s.length).to_s)
      end
      len = end_ - start
      if ((len).equal?(0))
        return self
    ...
    def insert(index, str, offset, len)
      ...
      if ((offset < 0) || (len < 0) || (offset > str.attr_length - len))
        raise StringIndexOutOfBoundsException.new("offset " + (offset).to_s + ", len " + (len).to_s + ", str.length " + (str.attr_length).to_s)
      end
      new_count = @count + len
      if (new_count > @value.attr_length)
        expand_capacity(new_count)
    ...
    def insert(dst_offset, s, start, end_)
      ...
      if ((dst_offset < 0) || (dst_offset > self.length))
        raise IndexOutOfBoundsException.new("dstOffset " + (dst_offset).to_s)
      end
      if ((start < 0) || (end_ < 0) || (start > end_) || (end_ > s.length))
        raise IndexOutOfBoundsException.new("start " + (start).to_s + ", end " + (end_).to_s + ", s.length() " + (s.length).to_s)
      end
