...
module Java::Lang
  ...
  class ClassLoader 
    ...
    def pre_define_class(name, protection_domain)
      ...
      if ((!(name).nil?) && name.starts_with("java."))
        raise SecurityException.new("Prohibited package name: " + (name.substring(0, name.last_index_of(Character.new(?..ord)))).to_s)
      end
    ...
    def define_class_source_location(protection_domain)
      ...
      if (!(cs).nil? && !(cs.get_location).nil?)
        source = (cs.get_location.to_s).to_s
      end
      return source
    end
    ...
    def desired_assertion_status(class_name)
      synchronized(self) do
        ...
        while (dot_index > 0)
          class_name = (class_name.substring(0, dot_index)).to_s
  ...
  class SystemClassLoaderAction 
    ...
    def run
      ...
      ctor = nil
      c = nil
      cp = Array.typed(Class).new([ClassLoader.class])
