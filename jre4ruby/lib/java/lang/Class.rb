...
module Java::Lang
  ...
  class Class 
      ...
      const_set_lazy(:EnclosingMethodInfo) { Class.new do
        ...
        def initialize(enclosing_info)
          ...
          begin
            ...
            # name (can be null).
            @name = (enclosing_info[1]).to_s
            # the immediately enclosing method or constructor's
            # descriptor (null iff name is).
            @descriptor = (enclosing_info[2]).to_s
    ...
    def get_simple_name
      if (is_array)
        return (get_component_type.get_simple_name).to_s + "[]"
      end
      simple_name = get_simple_binary_name
      if ((simple_name).nil?)
        # top level class
        simple_name = (get_name).to_s
    ...
    def get_canonical_name
      ...
      if ((enclosing_class).nil?)
        ...
        return enclosing_name + "." + (get_simple_name).to_s
      end
    end
    ...
    def get_method(name, *parameter_types)
      ...
      if ((method).nil?)
        raise NoSuchMethodException.new((get_name).to_s + "." + name + (argument_types_to_string(parameter_types)).to_s)
      end
      return method
    end
    ...
    def resolve_name(name)
      ...
      if (!name.starts_with("/"))
        ...
        if (!(index).equal?(-1))
          name = (base_name.substring(0, index).replace(Character.new(?..ord), Character.new(?/.ord))).to_s + "/" + name
        end
      else
    ...
    def is_enum
      ...
      # don't do the former.
      return !((self.get_modifiers & ENUM)).equal?(0) && (self.get_superclass).equal?(Java::Lang::Enum.class)
    end
    ...
    def enum_constant_directory
      if ((@enum_constant_directory).nil?)
        ...
        if ((universe).nil?)
          raise IllegalArgumentException.new((get_name).to_s + " is not an enum type")
        end
    ...
    def cannot_cast_msg(obj)
      return "Cannot cast " + (obj.get_class.get_name).to_s + " to " + (get_name).to_s
    end
