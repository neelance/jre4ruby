...
module Java::Lang
  ...
  class StackTraceElement 
    ...
    def to_s
      return (get_class_name).to_s + "." + @method_name + ((is_native_method ? "(Native Method)" : (!(@file_name).nil? && @line_number >= 0 ? "(" + @file_name + ":" + (@line_number).to_s + ")" : (!(@file_name).nil? ? "(" + @file_name + ")" : "(Unknown Source)")))).to_s
    end
