...
module Java::Math
  ...
  class BigDecimal < BigDecimalImports.const_get :Numeric
    ...
    def to_s
      if ((@string_cache).nil?)
        @string_cache = (layout_chars(true)).to_s
      end
      return @string_cache
    end
    ...
    def get_value_string(signum_, int_string, scale_)
      ...
      if ((insertion_point).equal?(0))
        ...
        return ((signum_ < 0 ? "-0." : "0.")).to_s + int_string
      else
        ...
        if (sci)
          ...
          if ((signum).equal?(0))
            ...
            when 3
              ...
              raise AssertionError.new("Unexpected sig value " + (sig).to_s)
            end
          else
      ...
      if ((@int_compact).equal?(INFLATED))
        ...
        if (!(@int_val).nil?)
          ...
          if (!(val).equal?(@int_compact))
            print("audit", self)
            raise AssertionError.new("Inconsistent state, intCompact=" + (@int_compact).to_s + "\t intVal=" + (val).to_s)
          end
        end
      end
      return self
