...
module Java::Security
  ...
  class Provider < ProviderImports.const_get :Properties
    ...
    # for this provider.
    def to_s
      return @name + " version " + (@version).to_s
    end
    ...
    def put(key, value)
      synchronized(self) do
        check("putProviderProperty." + @name)
        if (!(Debug).nil?)
          Debug.println("Set " + @name + " provider property [" + (key).to_s + "/" + (value).to_s + "]")
        end
    ...
    def remove(key)
      synchronized(self) do
        ...
        if (!(Debug).nil?)
          Debug.println("Remove " + @name + " provider property " + (key).to_s)
        end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:ServiceKey) { Class.new do
        ...
        def initialize(type, algorithm, intern)
          ...
          algorithm = (algorithm.to_upper_case(ENGLISH)).to_s
    ...
    def put_service(s)
      synchronized(self) do
        check("putProviderProperty." + @name)
        if (!(Debug).nil?)
          Debug.println(@name + ".putService(): " + (s).to_s)
        end
        if ((s).nil?)
    ...
    def put_property_strings(s)
      ...
      s.attr_attributes.entry_set.each do |entry|
        key = type + "." + algorithm + " " + (entry.get_key).to_s
    ...
    def remove_service(s)
      synchronized(self) do
        check("removeProviderProperty." + @name)
        if (!(Debug).nil?)
          Debug.println(@name + ".removeService(): " + (s).to_s)
        end
        if ((s).nil?)
          raise NullPointerException.new
        end
      ...
      const_set_lazy(:Service) { Class.new do
        ...
        def new_instance(constructor_parameter)
          if ((@registered).equal?(false))
            if (!(@provider.get_service(@type, @algorithm)).equal?(self))
              raise NoSuchAlgorithmException.new("Service not registered with Provider " + (@provider.get_name).to_s + ": " + (self).to_s)
            end
            @registered = true
          end
          begin
        ...
        def get_impl_class
          begin
            ...
            raise NoSuchAlgorithmException.new("class configured for " + @type + "(provider: " + (@provider.get_name).to_s + ")" + "cannot be found.", e)
          end
        end
        ...
        def has_key_attributes
          ...
          if ((b).nil?)
            synchronized((self)) do
              s = nil
              s = (get_attribute("SupportedKeyFormats")).to_s
              if (!(s).nil?)
                @supported_formats = s.split(Regexp.new("\\|"))
              end
              s = (get_attribute("SupportedKeyClasses")).to_s
              if (!(s).nil?)
        ...
        # @return a String representation of this service.
        def to_s
          a_string = @aliases.is_empty ? "" : "\r\n  aliases: " + (@aliases.to_s).to_s
          attrs = @attributes.is_empty ? "" : "\r\n  attributes: " + (@attributes.to_s).to_s
          return (@provider.get_name).to_s + ": " + @type + "." + @algorithm + " -> " + @class_name + a_string + attrs + "\r\n"
        end
