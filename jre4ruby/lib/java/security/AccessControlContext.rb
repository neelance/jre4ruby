...
module Java::Security
  module AccessControlContextImports
  ...
  class AccessControlContext 
    ...
    def check_permission(perm)
      ...
      if (!(get_debug).nil?)
        ...
        if (!dump_debug)
          ...
          while !(@context).nil? && i < @context.attr_length
            if (!(@context[i].get_code_source).nil? && !(@context[i].get_code_source.get_location).nil? && Debug.is_on("codebase=" + (@context[i].get_code_source.get_location.to_s).to_s))
              dump_debug = true
              break
            end
            i += 1
          ...
          if (Debug.is_on("failure"))
            ...
            AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
              ...
              define_method :run do
                db.println("domain that failed " + (pd).to_s)
                return nil
              end
      ...
      if (dump_debug)
        self.attr_debug.println("access allowed " + (perm).to_s)
      end
      return
    end
