  ...
  class KeyStore 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:PrivateKeyEntry) { Class.new do
        ...
        def to_s
          ...
          sb.append("Private key entry and certificate chain with " + (@chain.attr_length).to_s + " elements:\r\n")
          @chain.each do |cert|
            sb.append(cert)
            sb.append("\r\n")
          end
      ...
      const_set_lazy(:SecretKeyEntry) { Class.new do
        ...
        def to_s
          return "Secret key entry with algorithm " + (@s_key.get_algorithm).to_s
        end
    ...
    class_module.module_eval {
      ...
      def get_default_type
        kstype = nil
        kstype = (AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          extend LocalClass
          include_class_members KeyStore
        ...
        class_module.module_eval {
          ...
          def new_instance(type, provider, file, protection)
            ...
            if ((file.is_file).equal?(false))
              raise IllegalArgumentException.new("File does not exist or it does not refer " + "to a normal file: " + (file).to_s)
            end
                  ...
                  define_method :run0 do
                    ...
                    begin
                      ...
                      if (self.attr_protection.is_a?(PasswordProtection))
                        ...
                        callback = PasswordCallback.new("Password for keystore " + (self.attr_file.get_name).to_s, false)
