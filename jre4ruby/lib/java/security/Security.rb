  ...
  class Security 
      ...
      def initialize_
        ...
        if (prop_file.exists)
          ...
          begin
            ...
            loaded_props = true
            if (!(Sdebug).nil?)
              Sdebug.println("reading security properties file: " + (prop_file).to_s)
            end
          rescue IOException => e
            if (!(Sdebug).nil?)
              Sdebug.println("unable to load security properties from " + (prop_file).to_s)
              e.print_stack_trace
            end
          ensure
            if (!(is).nil?)
        ...
        if ("true".equals_ignore_case(self.attr_props.get_property("security.overridePropertiesFile")))
          ...
          if (!(extra_prop_file).nil? && extra_prop_file.starts_with("="))
            override_all = true
            extra_prop_file = (extra_prop_file.substring(1)).to_s
          end
          if (override_all)
            self.attr_props = Properties.new
            if (!(Sdebug).nil?)
          ...
          if (!(extra_prop_file).nil?)
            ...
            begin
              prop_url = nil
              extra_prop_file = (PropertyExpander.expand(extra_prop_file)).to_s
              prop_file = JavaFile.new(extra_prop_file)
              if (prop_file.exists)
                prop_url = URL.new("file:" + (prop_file.get_canonical_path).to_s)
              else
              ...
              loaded_props = true
              if (!(Sdebug).nil?)
                Sdebug.println("reading security properties file: " + (prop_url).to_s)
                if (override_all)
                  Sdebug.println("overriding other security properties files!")
                end
              end
    ...
    class_module.module_eval {
      ...
      def security_prop_file(filename)
        ...
        return JavaFile.new((System.get_property("java.home")).to_s + sep + "lib" + sep + "security" + sep + filename)
      end
        ...
        while i < providers.size
          ...
          if ((prop).nil?)
            ...
            while e.has_more_elements && (prop).nil?
              match_key = (e.next_element).to_s
              if (key.equals_ignore_case(match_key))
                prop = (prov.get_property(match_key)).to_s
                break
              end
            end
          end
      ...
      def get_provider_property(key, provider)
        ...
        if ((prop).nil?)
          ...
          while e.has_more_elements && (prop).nil?
            ...
            if (key.equals_ignore_case(match_key))
              prop = (provider.get_property(match_key)).to_s
              break
            end
          end
        end
      ...
      def get_providers(filter)
        ...
        if ((index).equal?(-1))
          ...
          key = (filter.substring(0, index)).to_s
          value = (filter.substring(index + 1)).to_s
        end
      ...
      def is_criterion_satisfied(prov, service_name, alg_name, attr_name, filter_value)
        key = service_name + (Character.new(?..ord)).to_s + alg_name
        if (!(attr_name).nil?)
          key += (Character.new(?\s.ord)).to_s + attr_name
        end
        ...
        if ((prop_value).nil?)
          ...
          if (!(standard_name).nil?)
            ...
            prop_value = (get_provider_property(key, prov)).to_s
          end
          if ((prop_value).nil?)
      ...
      def get_filter_components(filter_key, filter_value)
        ...
        if ((filter_value.length).equal?(0))
          ...
          alg_name = (filter_key.substring(alg_index + 1).trim).to_s
          ...
          if ((attr_index).equal?(-1))
            ...
            attr_name = (filter_key.substring(attr_index + 1).trim).to_s
          ...
          if ((attr_index < alg_index) || ((alg_index).equal?(attr_index - 1)))
            ...
            alg_name = (filter_key.substring(alg_index + 1, attr_index)).to_s
          end
        end
