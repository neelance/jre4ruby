    ...
    def print_certificates
      ...
      if ((@certificates).nil?)
        ...
        @certificates.each do |cert|
          out += "\tcertificate " + (((i += 1) - 1)).to_s + "\tfor  : " + (cert.get_principal).to_s + "\n"
          out += "\t\t\tfrom : " + (cert.get_guarantor).to_s + "\n"
        end
      end
      return out
    end
