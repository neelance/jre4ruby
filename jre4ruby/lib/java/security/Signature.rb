  ...
  class Signature < SignatureImports.const_get :SignatureSpi
    ...
    class_module.module_eval {
      ...
      def get_instance(algorithm)
        ...
        begin
          ...
          if (is_spi(s))
            ...
            begin
              instance = GetInstance.get_instance(s, SignatureSpi.class)
        ...
        if ((result).nil?)
          begin
            ...
            if ((!(Debug).nil?) && ((r).equal?(false)))
              ...
              Debug.println("Delayed provider selection may not be " + "available for algorithm " + (s.get_algorithm).to_s)
            end
      ...
      def get_instance(algorithm, provider)
        ...
        instance = GetInstance.get_instance("Signature", SignatureSpi.class, algorithm, provider)
        return get_instance(instance, algorithm)
      end
      ...
      def get_instance_rsa(p)
        ...
        begin
          ...
          raise NoSuchAlgorithmException.new("no such algorithm: " + RSA_SIGNATURE + " for provider " + (p.get_name).to_s, e)
        end
      end
    }
    ...
    def to_s
      ...
      return "Signature object: " + (get_algorithm).to_s + init_state
    end
      ...
      const_set_lazy(:Delegate) { Class.new(Signature) do
          ...
          def new_instance(s)
            if ((s.get_type == "Cipher"))
              ...
              if ((o.is_a?(SignatureSpi)).equal?(false))
                raise NoSuchAlgorithmException.new("Not a SignatureSpi: " + (o.get_class.get_name).to_s)
              end
              return o
            end
          end
          ...
          synchronized((@lock)) do
            ...
            if (!(Debug).nil?)
              ...
              if (w >= 0)
                ...
                Exception.new("Call trace").print_stack_trace
              end
            end
            last_exception = nil
        ...
        def init(spi, type, key, random)
          ...
          when self.class::I_PRIV_SR
            ...
            raise AssertionError.new("Internal error: " + (type).to_s)
          end
        end
