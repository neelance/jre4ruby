  ...
  class ServerSocket 
    ...
    def initialize(port, backlog, bind_addr)
      ...
      if (port < 0 || port > 0xffff)
        raise IllegalArgumentException.new("Port value out of range: " + (port).to_s)
      end
      if (backlog < 1)
        backlog = 50
      end
    ...
    def to_s
      ...
      return "ServerSocket[addr=" + (@impl.get_inet_address).to_s + ",port=" + (@impl.get_port).to_s + ",localport=" + (@impl.get_local_port).to_s + "]"
    end
