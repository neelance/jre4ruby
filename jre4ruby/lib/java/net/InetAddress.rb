...
module Java::Net
  ...
  class InetAddress 
    ...
    def get_host_name(check)
      if ((@host_name).nil?)
        @host_name = (InetAddress.get_host_from_name_service(self, check)).to_s
      end
      return @host_name
    end
    ...
    def get_canonical_host_name
      if ((@canonical_host_name).nil?)
        @canonical_host_name = (InetAddress.get_host_from_name_service(self, true)).to_s
      end
      ...
      def get_host_from_name_service(addr, check)
        ...
        self.attr_name_services.each do |nameService|
          begin
            ...
            host = (name_service.get_host_by_addr(addr.get_address)).to_s
            ...
            if (!ok)
              host = (addr.get_host_address).to_s
              return host
            end
            break
    ...
    def to_s
      return (((!(@host_name).nil?) ? @host_name : "")).to_s + "/" + (get_host_address).to_s
    end
          ...
          begin
            name_service = Java::Security::AccessController.do_privileged(Class.new(Java::Security::PrivilegedExceptionAction.class == Class ? Java::Security::PrivilegedExceptionAction : Object) do
              ...
              define_method :run do
                itr = Service.providers(NameServiceDescriptor.class)
                while (itr.has_next)
                  nsd = itr.next
                  if (provider_name.equals_ignore_case((nsd.get_type).to_s + "," + (nsd.get_provider_name).to_s))
                    begin
                      return nsd.create_name_service
                    rescue Exception => e
                      e.print_stack_trace
                      System.err.println("Cannot create name service:" + provider_name + ": " + (e).to_s)
                    end
                  end
                end
                return nil
          ...
          if ((addr).nil?)
            ...
            if (!((pos = host.index_of("%"))).equal?(-1))
              ...
              if ((numeric_zone).equal?(-1))
                ...
                ifname = (host.substring(pos + 1)).to_s
              end
            end
          ...
          synchronized((CacheLock)) do
            ...
            if ((ret).nil?)
              ...
              begin
                ...
                raise UnknownHostException.new(local + ": " + (uhe.get_message).to_s)
              end
