...
module Java::Net
  ...
  class HttpCookie 
    ...
    class_module.module_eval {
      ...
      def parse(header)
        ...
        if (starts_with_ignore_case(header, SET_COOKIE2))
          header = (header.substring(SET_COOKIE2.length)).to_s
        else
          if (starts_with_ignore_case(header, SET_COOKIE))
            header = (header.substring(SET_COOKIE.length)).to_s
          end
        end
    ...
    def set_domain(pattern)
      if (!(pattern).nil?)
        @domain = (pattern.to_lower_case).to_s
      else
        @domain = pattern
      end
    end
    ...
    def equals(obj)
      ...
      return equals_ignore_case(get_name, other.get_name) && equals_ignore_case(get_domain, other.get_domain) && equals(get_path, other.get_path)
    end
    ...
    class_module.module_eval {
      ...
      def assign_attribute(cookie, attr_name, attr_value)
        ...
        attr_value = (strip_off_surrounding_quote(attr_value)).to_s
    ...
    def to_netscape_header_string
      ...
      sb.append((get_name).to_s + "=" + (get_value).to_s)
      return sb.to_s
    end
    ...
    class_module.module_eval {
      ...
      def guess_cookie_version(header)
        version = 0
        header = (header.to_lower_case).to_s
