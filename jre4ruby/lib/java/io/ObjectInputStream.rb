...
# have any questions.
module Java::Io
  module ObjectInputStreamImports
  ...
  class ObjectInputStream < ObjectInputStreamImports.const_get :InputStream
    ...
    def verify_subclass
      cl = get_class
      if ((cl).equal?(ObjectInputStream.class))
        return
      end
      sm = System.get_security_manager
      if ((sm).nil?)
      ...
      def audit_subclass(subcl)
        result = AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          ...
          define_method :run do
            cl = subcl
            while !(cl).equal?(ObjectInputStream.class)
              begin
    ...
    def read_string(unshared)
      ...
      case (tc)
      when TC_STRING
        str = (@bin.read_utf).to_s
      when TC_LONGSTRING
        str = (@bin.read_long_utf).to_s
      else
        raise StreamCorruptedException.new(String.format("invalid type code: %02X", tc))
      end
    ...
    def read_enum(unshared)
      ...
      desc = read_class_desc(false)
      if (!desc.is_enum)
        raise InvalidClassException.new("non-enum class: " + (desc).to_s)
      end
      ...
      if (!(cl).nil?)
        begin
          ...
          raise InvalidObjectException.new("enum constant " + name + " does not exist in " + (cl).to_s).init_cause(ex)
        end
        if (!unshared)
    ...
    def handle_reset
      if (@depth > 0)
        raise StreamCorruptedException.new("unexpected reset; recursion depth: " + (@depth).to_s)
      end
      clear
    end
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:GetFieldImpl) { Class.new(GetField) do
        ...
        typesig { [String, Object] }
        def get(name, val)
          off = get_field_offset(name, Object.class)
          if (off >= 0)
          ...
          begin
            loop do
              ...
              when TC_BLOCKDATALONG
                ...
                if (len < 0)
                  raise StreamCorruptedException.new("illegal block data header length: " + (len).to_s)
                end
                return len
              when TC_RESET
                @in.read
