...
module Java::Io
  ...
  class ObjectStreamClass 
    ...
    class_module.module_eval {
      ...
      def lookup(cl, all)
        if (!(all || Serializable.class.is_assignable_from(cl)))
          return nil
        end
        ...
        if (entry.is_a?(ObjectStreamClass))
          ...
          if (entry.is_a?(RuntimeException))
            ...
            if (entry.is_a?(JavaError))
              ...
              raise InternalError.new("unexpected entry: " + (entry).to_s)
            end
          end
        end
      end
    ...
    def initialize(cl)
      ...
      @super_desc = nil
      @cl = cl
      @name = (cl.get_name).to_s
      @is_proxy = Proxy.is_proxy_class(cl)
      @is_enum = Enum.class.is_assignable_from(cl)
      @serializable = Serializable.class.is_assignable_from(cl)
      @externalizable = Externalizable.class.is_assignable_from(cl)
        ...
        AccessController.do_privileged(Class.new(PrivilegedAction.class == Class ? PrivilegedAction : Object) do
          ...
          define_method :run do
            ...
            if (self.attr_externalizable)
              ...
              self.attr_write_object_method = get_private_method(cl, "writeObject", Array.typed(Class).new([ObjectOutputStream.class]), Void::TYPE)
              self.attr_read_object_method = get_private_method(cl, "readObject", Array.typed(Class).new([ObjectInputStream.class]), Void::TYPE)
            ...
            self.attr_write_replace_method = get_inheritable_method(cl, "writeReplace", nil, Object.class)
            self.attr_read_resolve_method = get_inheritable_method(cl, "readResolve", nil, Object.class)
            return nil
          end
    ...
    def init_proxy(cl, resolve_ex, super_desc)
      ...
      if (!(cl).nil?)
        ...
        @name = (@local_desc.attr_name).to_s
    ...
    def init_non_proxy(model, cl, resolve_ex, super_desc)
      ...
      if (!(cl).nil?)
        ...
        if ((@serializable).equal?(@local_desc.attr_serializable) && !cl.is_array && !(@suid.long_value).equal?(@local_desc.get_serial_version_uid))
          raise InvalidClassException.new(@local_desc.attr_name, "local class incompatible: " + "stream classdesc serialVersionUID = " + (@suid).to_s + ", local class serialVersionUID = " + (@local_desc.get_serial_version_uid).to_s)
        end
    ...
    def read_non_proxy(in_)
      ...
      if (@is_enum && !(@suid.long_value).equal?(0))
        raise InvalidClassException.new(@name, "enum descriptor has non-zero serialVersionUID: " + (@suid).to_s)
      end
      num_fields = in_.read_short
      if (@is_enum && !(num_fields).equal?(0))
        raise InvalidClassException.new(@name, "enum descriptor has non-zero field count: " + (num_fields).to_s)
      end
    ...
    def get_field(name, type)
      ...
      while i < @fields.attr_length
        f = @fields[i]
        if ((f.get_name == name))
          if ((type).nil? || ((type).equal?(Object.class) && !f.is_primitive))
            return f
          end
          ftype = f.get_type
    ...
    def get_class_data_layout0
      ...
      while (!(end_).nil? && Serializable.class.is_assignable_from(end_))
    ...
    class_module.module_eval {
      ...
      def get_serializable_constructor(cl)
        init_cl = cl
        while (Serializable.class.is_assignable_from(init_cl))
      ...
      def class_names_equal(name1, name2)
        name1 = (name1.substring(name1.last_index_of(Character.new(?..ord)) + 1)).to_s
        name2 = (name2.substring(name2.last_index_of(Character.new(?..ord)) + 1)).to_s
        return (name1 == name2)
      end
      ...
      def get_serial_fields(cl)
        fields = nil
        if (Serializable.class.is_assignable_from(cl) && !Externalizable.class.is_assignable_from(cl) && !Proxy.is_proxy_class(cl) && !cl.is_interface)
      ...
      const_set_lazy(:MemberSignature) { Class.new do
        ...
        def initialize(field)
          ...
          @signature = nil
          @member = field
          @name = (field.get_name).to_s
          @signature = (get_class_signature(field.get_type)).to_s
        end
        ...
        def initialize(cons)
          ...
          @signature = nil
          @member = cons
          @name = (cons.get_name).to_s
          @signature = (get_method_signature(cons.get_parameter_types, Void::TYPE)).to_s
        end
        ...
        def initialize(meth)
          ...
          @signature = nil
          @member = meth
          @name = (meth.get_name).to_s
          @signature = (get_method_signature(meth.get_parameter_types, meth.get_return_type)).to_s
        end
          ...
          while i < @fields.attr_length
            ...
            when Character.new(?L.ord), Character.new(?[.ord)
              ...
              if (!(val).nil? && !@types[i - @num_prim_fields].is_instance(val))
                ...
                raise ClassCastException.new("cannot assign instance of " + (val.get_class.get_name).to_s + " to field " + (f.get_declaring_class.get_name).to_s + "." + (f.get_name).to_s + " of type " + (f.get_type.get_name).to_s + " in instance of " + (obj.get_class.get_name).to_s)
              end
          ...
          while j < local_fields.attr_length
            ...
            if ((f.get_name == lf.get_name))
              if ((f.is_primitive || lf.is_primitive) && !(f.get_type_code).equal?(lf.get_type_code))
                raise InvalidClassException.new(local_desc.attr_name, "incompatible types for field " + (f.get_name).to_s)
              end
              if (!(lf.get_field).nil?)
