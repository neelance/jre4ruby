...
module Java::Io
  ...
  class FilePermission < FilePermissionImports.const_get :Permission
    ...
    def init(mask)
      ...
      @cpath = (AccessController.do_privileged(# store only the canonical cpath if possible
      ...
      if ((last).equal?(RECURSIVE_CHAR) && (@cpath.char_at(len - 2)).equal?(JavaFile.attr_separator_char))
        @directory = true
        @recursive = true
        @cpath = (@cpath.substring(0, (len -= 1))).to_s
      else
