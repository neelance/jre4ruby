        ...
        while (count < utflen)
          ...
          when 12, 13
            ...
            if (!((char2 & 0xc0)).equal?(0x80))
              raise UTFDataFormatException.new("malformed input around byte " + (count).to_s)
            end
          ...
          when 14
            ...
            if ((!((char2 & 0xc0)).equal?(0x80)) || (!((char3 & 0xc0)).equal?(0x80)))
              raise UTFDataFormatException.new("malformed input around byte " + ((count - 1)).to_s)
            end
