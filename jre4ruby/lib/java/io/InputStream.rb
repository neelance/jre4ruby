...
module Java::Io
  ...
  class InputStream 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:SKIP_BUFFER_SIZE) { 2048 }
      const_attr_reader  :SKIP_BUFFER_SIZE
      
      # skipBuffer is initialized in skip(long), if needed.
      
      def skip_buffer
        defined?(@@skip_buffer) ? @@skip_buffer : @@skip_buffer= nil
      end
      alias_method :attr_skip_buffer, :skip_buffer
      
      def skip_buffer=(value)
        @@skip_buffer = value
      end
      alias_method :attr_skip_buffer=, :skip_buffer=
    }
    ...
    def skip(n)
      remaining = n
      nr = 0
      if ((self.attr_skip_buffer).nil?)
        self.attr_skip_buffer = Array.typed(::Java::Byte).new(SKIP_BUFFER_SIZE) { 0 }
      end
      local_skip_buffer = self.attr_skip_buffer
      if (n <= 0)
        return 0
      end
      while (remaining > 0)
